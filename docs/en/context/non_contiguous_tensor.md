...
Currently most operator APIs' input aclTensor supports "**non-contiguous Tensor**", i.e., a Tensor can be represented by \(shape, strides, offset\).

Note: Creating aclTensor can be implemented through "Public Interface > aclCreateTensor" in [Operator Library Interface](https://hiascend.com/document/redirect/CannCommunityOplist).
...
That is, this Tensor is laid out at the dark positions in the above figure. This complete Tensor is non-contiguous in memory layout. strides describes the interval between two adjacent elements on Tensor dimension. If stride on dimension 1 is 1, this dimension is contiguous; if stride on dimension 0 is 10, then adjacent elements are separated by 10 elements, i.e., non-contiguous. offset represents the offset of this Tensor's first element relative to addr.
...
That is, this Tensor is laid out at the dark positions in the above figure. This complete Tensor is non-contiguous in memory layout. strides describes the interval between two adjacent elements on Tensor dimension. If stride on dimension 1 is 2, elements on this dimension are separated by 1 element; if stride on dimension 0 is 20, then adjacent elements are separated by 20 elements, i.e., non-contiguous. offset represents the offset of this Tensor's first element relative to addr.
