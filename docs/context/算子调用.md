...
`AddExample`算子以aclnn接口形式调用的示例代码如下（详细代码参见[test_aclnn_add_example.cpp](../../example/add_example/examples/test_aclnn_add_example.cpp)），**仅为参考**，其他算子的aclnn接口调用过程类似，请替换为实际aclnn接口。
...
>说明：对于项目内已实现的算子（非自定义算子），可通过根目录下[build.sh](../../build.sh)直接运行算子，操作请参考[快速入门 > 本地验证](./QuickStart.md#本地验证)。

1. 前提条件。
   请参考本项目[快速入门 > 编译执行](./QuickStart.md#编译执行)完成目标算子的编译部署。

2. 创建CMakelist文件。

   在`AddExample`算子调用样例目录中创建用于构建的CMakelist文件，路径为本项目`example/add_example/examples/CMakeLists.txt` ，内容示例如下，请根据实际情况自行修改。
...
3. 创建编译执行脚本run.sh。
    ...
    在`AddExample`算子调用样例目录中创建用于编译执行的run.sh文件，路径为本项目`example/add_example/examples/run.sh` ，内容示例如下，请根据实际情况自行修改。
...
4. 运行run.sh。
    在`AddExample`算子的`example/add_example/examples/`目录下，执行如下命令：

   ```bash
    bash run.sh
   ```
    默认在当前执行路径 `/build/bin`下生成可执行文件test\_aclnn\_add\_example，运行后的结果打印如下：
