...
2. **安装驱动与固件（运行态依赖）**
   ...
   根据实际产品型号和环境架构，下载对应`Ascend-hdk-<chip_type>-npu-driver_<version>_linux-<arch>.run`、`Ascend-hdk-<chip_type>-npu-firmware_<version>.run`包，[下载链接](https://www.hiascend.com/hardware/firmware-drivers/community)。
...
1. **安装社区CANN toolkit包**
 ...
 2. **安装社区版CANN ops包（运行态依赖）**
    ...
    根据产品型号和环境架构，下载对应`Ascend-cann-${soc_name}-ops_${cann_version}_linux-${arch}.run`包，[下载链接](https://ascend.devcloud.huaweicloud.com/cann/run/software/8.5.0-beta.1)。
