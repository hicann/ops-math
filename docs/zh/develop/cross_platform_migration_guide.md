...
这套信号量的本质是保证多线程/多流水阶段间一致的同步序列，防止因资源未就绪或依赖没完成而发生数据竞争或死锁等硬件异常。关于CrossCoreSetFlag与CrossCoreWaitFlag核间同步接口的详解请参考[《Ascend C API》](https://www.hiascend.com/document/redirect/CannCommunityAscendCApi)。
