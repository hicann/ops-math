...
Decompress multiple cann-ops-math-static tar.gz packages and merge them.
...
merge_packages() {
    ...
    shopt -s nullglob
    PKG_DIRS=()
    for dir in "$TMPDIR"/cann-ops-math-static-*/; do
        [[ -d "$dir" ]] && PKG_DIRS+=("$dir")
    done
    shopt -u nullglob

    if [[ ${#PKG_DIRS[@]} -eq 0 ]]; then
        log_error "no cann-ops-math-static-* directories found after extraction"
        exit 0
    fi

    local VERSION
    VERSION=$(grep 'set_cann_package' "$(dirname "$0")/../../version.cmake" | sed 's/.*set_cann_package([^ ]* [^ ]* "\([^"]*\)".*/\1/')
    local base_name="cann-${SOC}-ops-math-static-${VERSION}_linux-${ARCH_INFO}"
    ...
    printf '%s\0' "${PKG_DIRS[@]}" | xargs -0 -r -n 1 -P "$njobs" bash -c '
        lib="$1/lib64/libcann_math_static.a"
        [[ -f "$lib" ]] || exit 0
    ...
    find "$base_dir/include" -maxdepth 2 -name 'aclnn_ops_math_*.h' -delete 2>/dev/null || true
...
generate_merged_header() {
    ...
    local output="$include_dir/aclnn_ops_math.h"

    log_info "Generating aclnn_ops_math.h"
    {
        echo "#ifndef ACLNN_OPS_MATH_H_"
        echo "#define ACLNN_OPS_MATH_H_"
        find "$include_dir" -maxdepth 1 \( -name 'aclnn_*.h' -o -name 'acl_*.h' \) \
            ! -name 'aclnn_ops_math_*.h' ! -name 'aclnn_ops_math.h' \
            -printf '#include "%f"\n' | sort
        echo '#endif // ACLNN_OPS_MATH_H_'
    } > "$output"
...
package_result() {
    ...
    if [[ ${#o_files[@]} -gt 0 ]]; then
        ar rcs libcann_math_static.a "${o_files[@]}"
        rm -f "${o_files[@]}"
    else
        ar rcs libcann_math_static.a
    fi
    popd > /dev/null
