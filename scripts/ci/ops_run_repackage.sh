...
TARGET_CONFIG_DIR="$HOST_EXTRACT_DIR/${OPP_PREFIX}/built-in/op_impl/ai_core/tbe/kernel/config/$SOC/ops_math"
...
if [[ ${#kernel_run_files[@]} -gt 0 ]]; then
    ...
    for runfile in "${kernel_run_files[@]}"; do
        read -u8
        {
            ...
            full_path=$(find "${extract_dir}/packages/vendors" -maxdepth 1 -type d -name "custom_*_math" 2>/dev/null | head -n 1)
