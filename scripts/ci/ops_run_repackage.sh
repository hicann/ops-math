...
counter=1
find "$RUN_PACKAGE_SAVE_AB_PATH" -name "cann-ops-math-custom_operator_group*.run" -type f | while read -r runfile; do
...
for runfile in kernel_*.run; do
    ...
    if [ -n "$full_path" ]; then
    # 4. 提取目录名 (这就是你要的 custom_operator_group_3_math)
    ...
    if $first_run; then
        ...
        dest_conf_ascend_first="$HOST_EXTRACT_DIR/${PKG_NAME}/built-in/op_impl/ai_core/tbe/kernel/config/$SOC/ops_math"
        ...
        dest_conf_ascend="$HOST_EXTRACT_DIR/${PKG_NAME}/built-in/op_impl/ai_core/tbe/kernel/config/$SOC/ops_math"
