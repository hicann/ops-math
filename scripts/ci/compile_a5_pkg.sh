...
if [ ${#builtin_ops_name[@]} -gt 0 ]; then
    ...
    build_cmd="bash build.sh --pkg --ops=$builtin_ops_str --soc=ascend950 -j16 --cann_3rd_lib_path=${CANN_3RD_LIB_PATH}"
...
if [ ${#experimental_ops_name[@]} -gt 0 ]; then
    ...
    build_cmd="bash build.sh --pkg --experimental --ops=$experimental_ops_str --soc=ascend950 -j16 --cann_3rd_lib_path=${CANN_3RD_LIB_PATH}"
    run_build_command "$build_cmd"
    execute_run_file "custom"
fi

if [ ${build_all} -eq 1 ]; then
    build_cmd="bash build.sh --pkg --jit --soc=ascend950 -j16 --cann_3rd_lib_path=${CANN_3RD_LIB_PATH}"
    run_build_command "$build_cmd"
