...
/**
 * @file test_geir_asinh_grad.cpp
 * @brief GE IR 图模式调用示例 - AsinhGrad
 *
 * 数学公式: z = dy / cosh(y)
 *
 * 通过 GE Session 构建计算图，编译并执行 AsinhGrad 算子。
 */
...
#include "../../op_graph/asinh_grad_proto.h"
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape)                                                \
    ...
    asinhGradOp.set_input_##inputName(placeholder##inputIndex);                                                  \
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                            \
    ...
    asinhGradOp.update_output_desc_##outputName(outputName##outputIndex##_desc);
...
{
    Status ret = SUCCESS;
    auto asinhGradOp = op::AsinhGrad("asinh_grad_1");
    ...
    outputs.push_back(asinhGradOp);
    return SUCCESS;
}

int main(int argc, char* argv[])
{
    const char* graph_name = "asinh_grad_ge_ir_test";
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        if (output[i].GetTensorDesc().GetDataType() == DT_FLOAT && output_shape > 0) {
            ...
            for (int64_t j = 0; j < printNum; j++) {
                ...
                float expected = 1.0f / std::cosh(y);
