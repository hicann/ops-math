...
const aclTensor* Lerp(const aclTensor* self, const aclTensor* other, const aclTensor* weight, aclOpExecutor* executor)
{
    ...
    if (!BroadcastInferShape(weight->GetViewShape(), broadcastShape, outBroadcastShape)) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Broadcast %s and %s failed.",
                op::ToString(weight->GetViewShape()).GetString(), op::ToString(broadcastShape).GetString());
        return nullptr;
    }
    ...
    OP_CHECK(ret == ACL_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "Lerp ADD_TO_LAUNCHER_LIST_AICORE failed."),
             return nullptr);
    return out;
}
} // namespace l0op
