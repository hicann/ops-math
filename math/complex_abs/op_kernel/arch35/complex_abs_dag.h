        ...
        __VEC_SCOPE__
        {
            ...
            for (uint16_t i = 0; i < repeatTime; i++) {
                ...
                if constexpr (std::is_same<U, complex32>::value) {
                    ...
                    // 0的处理
                    Reg::Compare<uint32_t, CMPMODE::EQ>(preg4, (AscendC::MicroAPI::RegTensor<uint32_t>&)vB, vZero,
                                                        preg);

                    // subnormal处理
                    Reg::Compare<uint32_t, CMPMODE::LT>(preg3, (AscendC::MicroAPI::RegTensor<uint32_t>&)vA,
                                                        vMinNormalFloat, preg);
