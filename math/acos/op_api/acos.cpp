...
static inline const aclTensor* AcosAiCore(const aclTensor* input, aclTensor* output, aclOpExecutor* executor)
{
    ...
    OP_CHECK(ret == ACLNN_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "AcosAiCore ADD_TO_LAUNCHER_LIST_AICORE failed."),
             return nullptr);
    return output;
}
...
static inline const aclTensor* AcosAiCpu(const aclTensor* input, aclTensor* output, aclOpExecutor* executor)
{
    ...
    OP_CHECK(ret == ACL_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "AcosAiCpu ADD_TO_LAUNCHER_LIST_AICPU failed."),
             return nullptr);
    return output;
}
