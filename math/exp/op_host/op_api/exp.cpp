...
static inline const aclTensor *ExpAiCore(const aclTensor *self, aclTensor *out, aclOpExecutor *executor) {
  ...
  OP_CHECK(retAicore == ACLNN_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "Exp ADD_TO_LAUNCHER_LIST_AICORE failed."), return nullptr);
  return out;
}
...
static inline const aclTensor *ExpAiCpu(const aclTensor *self, aclTensor *out, aclOpExecutor *executor) {
  ...
  OP_CHECK(ret ==  ACL_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "ExpAiCpu ADD_TO_LAUNCHER_LIST_AICPU failed."), return nullptr);
  return out;
}
