...
int main()
{
    ...
    // 调用aclnnExp第一段接口
    ret = aclnnExpGetWorkspaceSize(self.get(), out.get(), &workspaceSize, &executor);
    ...
    // 调用aclnnExp第二段接口
    ret = aclnnExp(workspaceAddr.get(), workspaceSize, executor, stream.get());
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnExp failed. ERROR: %d\n", ret); return ret);

    uint64_t inplaceWorkspaceSize = 0;
    aclOpExecutor* inplaceExecutor;
    // 调用aclnnInplaceExp第一段接口
    ret = aclnnInplaceExpGetWorkspaceSize(self.get(), &inplaceWorkspaceSize, &inplaceExecutor);
    ...
    ret = aclnnInplaceExp(inplaceWorkspaceAddr.get(), inplaceWorkspaceSize, inplaceExecutor, stream.get());
