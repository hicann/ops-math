...
namespace LogicalOrOp {
    ...
    struct LogicalOrFloatCustom : public Vec::ElemwiseBinaryOP<uint8_t, T, T> {
        __aicore__ inline LogicalOrFloatCustom(
            LocalTensor<uint8_t>& dst, LocalTensor<T>& src1, LocalTensor<T>& src2, const uint32_t& count)
        {
#ifdef __CCE_AICORE__
            ...
            __VEC_SCOPE__
            {
                ...
                MicroAPI::RegTensor<T> orRes;
                MicroAPI::RegTensor<uint8_t> dstReg;
                MicroAPI::RegTensor<uint8_t> oneReg;
                MicroAPI::RegTensor<uint8_t> zeroReg;
                ...
                for (uint16_t j = 0; j < loopTimes; j++) {
                    ...
                    // 以浮点类型 T 加载输入
                    MicroAPI::DataCopy<T, MicroAPI::LoadDist::DIST_NORM>(src1Value, src1Addr + VL_T * j);
                    MicroAPI::DataCopy<T, MicroAPI::LoadDist::DIST_NORM>(src2Value, src2Addr + VL_T * j);

                    // 位OR操作：当作对应位宽的整形处理
                    MicroAPI::Or((MicroAPI::RegTensor<IntT>&)orRes, (MicroAPI::RegTensor<IntT>&)src1Value, (MicroAPI::RegTensor<IntT>&)src2Value, preg);

                    // Compare: OR结果 != 0 (NE模式)
                    MicroAPI::Compares<T, CMPMODE::NE>(cmpMask, orRes, static_cast<T>(0), preg);

                    // Select: 根据cmpMask选择1或0 (uint8_t)
                    MicroAPI::Duplicate(oneReg, static_cast<uint8_t>(1));
                    MicroAPI::Duplicate(zeroReg, static_cast<uint8_t>(0));
                    MicroAPI::Select<uint8_t>(dstReg, oneReg, zeroReg, cmpMask);

                    // 存储uint8 BOOL结果
                    if constexpr (sizeof(T) == sizeof(half)) {
                        MicroAPI::DataCopy<uint8_t, MicroAPI::StoreDist::DIST_PACK_B16>(
                        dstAddr + VL_T * j, dstReg, preg);
                    } else {
