...
int main()
{
    ...
    ret = CreateAclTensor(otherHostData, otherShape, aclDataType::ACL_INT32, otherDeviceAddr, other);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ret = CreateAclTensor(outHostData, outShape, aclDataType::ACL_INT32, outDeviceAddr, out);
    CHECK_RET(ret == ACL_SUCCESS, return ret);

    uint64_t workspaceSize = 0;
    aclOpExecutor* executor;
    ret = aclnnGcdGetWorkspaceSize(self.get(), other.get(), out.get(), &workspaceSize, &executor);
    ...
    ret = aclnnGcd(workspaceAddr.get(), workspaceSize, executor, stream.get());
