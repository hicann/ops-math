...
#include "reduce_min.cpp"

using namespace std;

class reduce_min_test : public testing::Test
{
protected:
    static void SetUpTestCase()
    {
        cout << "reduce_min_test SetUp\n" << endl;
    }
    static void TearDownTestCase()
    {
        cout << "reduce_min_test TearDown\n" << endl;
    }
};

static bool CompareData()
{
    std::string cmd = "cd ./reduce_min_data/ && python3 verify.py y.bin golden.bin ";
    return system(cmd.c_str()) == 0;
}

static void InitEnv()
{
    system("cp -r ../../../../../../../ops/built-in/tests/ut/fast_op_test/reduce_min/reduce_min_data ./");
    system("chmod -R 755 ./reduce_min_data/");
    system("cd ./reduce_min_data/ && rm -rf ./*bin");
    system("cd ./reduce_min_data/ && python3 gen_data.py");
}
...
TEST_F(reduce_min_test, test_case_0)
{
    ...
    string path(cpath);
    free(cpath);
    ReadFile(path + "/reduce_min_data/x.bin", xSize, x, xSize);
