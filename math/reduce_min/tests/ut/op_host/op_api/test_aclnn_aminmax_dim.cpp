...
protected:
    ...
    {
        std::cout << "aminmax_dim_test SetUp" << std::endl;
    }

    static void TearDownTestCase()
    {
        std::cout << "aminmax_dim_test TearDown" << std::endl;
    }
};
...
TEST_F(l2_aminmax_dim_test, case_nullptr)
{
    ...
    auto ut = OP_API_UT(aclnnAminmaxDim, INPUT(nullptr, dim, true), OUTPUT(minOut, maxOut));
    uint64_t workspace_size = 0;
    aclnnStatus aclRet = ut.TestGetWorkspaceSize(&workspace_size);
    EXPECT_EQ(aclRet, ACLNN_ERR_PARAM_NULLPTR);

    auto ut1 = OP_API_UT(aclnnAminmaxDim, INPUT(tensor_desc, dim, true), OUTPUT(nullptr, maxOut));
    aclRet = ut1.TestGetWorkspaceSize(&workspace_size);
    EXPECT_EQ(aclRet, ACLNN_ERR_PARAM_NULLPTR);

    auto ut2 = OP_API_UT(aclnnAminmaxDim, INPUT(tensor_desc, dim, true), OUTPUT(minOut, nullptr));
