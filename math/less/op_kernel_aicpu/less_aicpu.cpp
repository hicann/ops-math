...
{
    // check params
    KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum), "Less check input and output number failed.");
...
uint32_t LessCpuKernel::LessParamCheck(const CpuKernelContext& ctx) const
{
    ...
    KERNEL_LOG_DEBUG("LessCpuKernel[%s], input0: size[%lu];"
...
OPS_MATH_REGISTER_CPU_KERNELV2(kLess, LessCpuKernel);
} // namespace aicpu
