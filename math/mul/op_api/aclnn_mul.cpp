                                      ...
                                      uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    bool canUseMuls = IsRegBase() &&
                      (self->GetDataType() == DataType::DT_BF16 || self->GetDataType() == DataType::DT_FLOAT16) &&
                      GetScalarDefaultDtype(other->GetDataType()) == DataType::DT_FLOAT;
    canUseMuls = canUseMuls || (!IsRegBase() && self->GetDataType() == DataType::DT_BF16 &&
...
static void MulCheckFormat(const aclTensor* self, const aclTensor* other)
{
    ...
    if (selfStorageFormat != ge::Format::FORMAT_ND || otherStorageFormat != ge::Format::FORMAT_ND) {
        OP_LOGW("aclnnMul only supports format ND.");
    }
}
...
static MulRunGather MulInnerRunAndGather(const aclTensor* x)
{
    ...
    for (size_t d = 0; d < n && m < MUL_NC_MAX_DIM; ++d) {
        ...
        if (sz <= 1 || st == 0) {
            continue;
        } // size-1 与 stride-0(广播)维不参与连续块/gather 计算
        strides[m] = st;
        sizes[m] = sz;
        ++m;
    }
    ...
    for (size_t i = 0; i < m; ++i) {
        if (strides[i] != run) {
            return {run, strides[i]};
        } // contiguity breaks here; this stride is the gather span
        if (run >= MUL_NC_CONTIG_RUN_MIN || sizes[i] >= MUL_NC_CONTIG_RUN_MIN) {
            return {MUL_NC_CONTIG_RUN_MIN, 0};
        }
...
static int64_t MulBroadcastFactor(const aclTensor* x, const aclTensor* y)
{
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (xd == 1 && yd > 1) {
            if (factor >= MUL_NC_BCAST_FACTOR_MIN || yd >= MUL_NC_BCAST_FACTOR_MIN) {
                return MUL_NC_BCAST_FACTOR_MIN;
            }
...
static int64_t MulViewNumel(const aclTensor* x)
{
    ...
    for (size_t i = 0; i < shape.GetDimNum(); ++i) {
        ...
        if (numel >= MUL_NC_PAYLOAD_MIN || d >= MUL_NC_PAYLOAD_MIN) {
            return MUL_NC_PAYLOAD_MIN;
        }
...
static bool MulIsSparseBroadcastOperand(const aclTensor* x, const aclTensor* y)
{
    ...
    return MulBroadcastFactor(x, y) >= MUL_NC_BCAST_FACTOR_MIN && MulViewNumel(x) >= MUL_NC_PAYLOAD_MIN &&
           rg.run < MUL_NC_CONTIG_RUN_MIN && rg.gather >= MUL_NC_GATHER_STRIDE_MIN;
}
...
{
    const bool prefer = MulIsSparseBroadcastOperand(self, other) || MulIsSparseBroadcastOperand(other, self);
    if (prefer) {
        OP_LOGI(
            "aclnnMul: sparse-broadcast operand detected, route to Contiguous + Mul instead of non-contiguous kernel.");
    }
    return prefer;
}
                                             ...
                                             aclOpExecutor** executor)
{
    ...
    bool canUseMuls = IsRegBase() &&
                      ...
                      GetScalarDefaultDtype(other->GetDataType()) == DataType::DT_FLOAT;
    canUseMuls = canUseMuls || (!IsRegBase() && selfRefContiguous->GetDataType() == DataType::DT_BF16 &&
                                other->GetDataType() == DataType::DT_DOUBLE);
    if (canUseMuls) {
        // BF16的tensor与DOUBLE类型的scalar需调用Muls，确保不降低精度的同时输出BF16
