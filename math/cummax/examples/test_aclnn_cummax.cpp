...
int main()
{
    ...
    // 调用aclnnCummax第一段接口
    ret = aclnnCummaxGetWorkspaceSize(self.get(), dim, valuesOut.get(), indicesOut.get(), &workspaceSize, &executor);
    ...
    // 调用aclnnCummax第二段接口
    ret = aclnnCummax(workspaceAddr.get(), workspaceSize, executor, stream.get());
