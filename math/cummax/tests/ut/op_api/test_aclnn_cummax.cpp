...
protected:
    static void SetUpTestCase() { cout << "cummax_test SetUp" << endl; }

    static void TearDownTestCase() { cout << "cummax_test TearDown" << endl; }
};
...
TEST_F(l2_cummax_test, l2_cummax_out_int64_self_discontinues)
{
    int64_t dim = 0;
    aclDataType dtype = ACL_FLOAT;
    auto self_tensor = TensorDesc({2, 2, 5, 4}, ACL_FLOAT, ACL_FORMAT_NCHW, {40, 20, 1, 5}, 0, {2, 2, 4, 5})
                           .ValueRange(-50, 50);
...
TEST_F(l2_cummax_test, l2_cummax_out_int32_self_discontinues)
{
    int64_t dim = 0;
    aclDataType dtype = ACL_FLOAT;
    auto self_tensor = TensorDesc({2, 2, 5, 4}, dtype, ACL_FORMAT_ND, {40, 20, 1, 5}, 0, {2, 2, 4, 5})
                           .ValueRange(-50, 50);
...
TEST_F(l2_cummax_test, l2_cummax_out_int64_out_discontinues)
{
    ...
    auto values_tensor = TensorDesc({2, 2, 5, 4}, dtype, ACL_FORMAT_ND, {40, 20, 1, 5}, 0, {2, 2, 4, 5})
                             .ValueRange(-50, 50);
    auto indices_tensor = TensorDesc({2, 2, 5, 4}, ACL_INT64, ACL_FORMAT_ND, {40, 20, 1, 5}, 0, {2, 2, 4, 5})
                              .ValueRange(-50, 50);
