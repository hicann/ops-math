...
static const std::initializer_list<op::DataType> AICORE_DTYPE_SUPPORT_LIST_310P = {op::DataType::DT_FLOAT,
...
static const aclTensor* ArgMaxV2AiCpu(const aclTensor* x, const aclTensor* dim, aclTensor* out, aclOpExecutor* executor)
{
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICPU(ArgMaxV2, OP_ATTR_NAMES({"Tidx", "output_type"}), OP_INPUT(x, dim),
                                          OP_OUTPUT(out), OP_ATTR(dim->GetDataType(), out->GetDataType()));
    OP_CHECK(ret == ACL_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "ArgMaxV2AiCpu ADD_TO_LAUNCHER_LIST_AICPU failed."),
             return nullptr);
    return out;
}
                          ...
                          aclOpExecutor* executor)
{
    ...
    if (IsRegBase()) {
        out = executor->AllocTensor(outDataType, op::Format::FORMAT_ND, op::Format::FORMAT_ND);
    } else {
