...
int main()
{
    ...
    // 调用aclnnArgMax第一段接口
    ret = aclnnArgMaxGetWorkspaceSize(self.get(), dim, keepdim, out.get(), &workspaceSize, &executor);
    ...
    // 调用aclnnArgMax第二段接口
    ret = aclnnArgMax(workspaceAddr.get(), workspaceSize, executor, stream.get());
