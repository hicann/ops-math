...
#include "nn_other.h"
#include "../op_graph/prod_virial_se_a_proto.h"
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::ProdVirialSeA("add1");

    // 属性设置: n_a_sel=3, n_r_sel=0 => nnei=3, ndescrpt=12
    add1.set_attr_n_a_sel(3);
    add1.set_attr_n_r_sel(0);

    // nframes=2, nloc=2, nall=2, nnei=3, ndescrpt=12
    std::vector<int64_t> netDerivShape = {2, 24}; // (nframes, nloc * ndescrpt)
    std::vector<int64_t> inDerivShape = {2, 72};  // (nframes, nloc * ndescrpt * 3)
    std::vector<int64_t> rijShape = {2, 18};      // (nframes, nloc * nnei * 3)
    std::vector<int64_t> nlistShape = {2, 6};     // (nframes, nloc * nnei)
    std::vector<int64_t> natomsShape = {3};       // (2 + ntypes,) natoms=[2,2,2]

    ADD_INPUT(0, net_deriv, inDtype, netDerivShape);
    ADD_INPUT(1, in_deriv, inDtype, inDerivShape);
    ADD_INPUT(2, rij, inDtype, rijShape);
    ADD_INPUT(3, nlist, ge::DT_INT32, nlistShape);
    ADD_CONST_INPUT(4, natoms, ge::DT_INT32, natomsShape);

    std::vector<int64_t> virialShape = {2, 9};      // (nframes, 9)
    std::vector<int64_t> atomVirialShape = {2, 18}; // (nframes, nall * 9)
    ADD_OUTPUT(1, virial, inDtype, virialShape);
    ADD_OUTPUT(2, atom_virial, inDtype, atomVirialShape);
