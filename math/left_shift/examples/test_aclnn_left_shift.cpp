...
int main()
{
    ...
    DeviceMemPtr shiftBitsDeviceAddr(nullptr, &aclrtFree);
    DeviceMemPtr outDeviceAddr(nullptr, &aclrtFree);
    TensorPtr self(nullptr, &aclDestroyTensor);
    TensorPtr shiftBits(nullptr, &aclDestroyTensor);
    ...
    ret = CreateAclTensor(shiftBitsHostData, shiftBitsShape, aclDataType::ACL_INT64, shiftBitsDeviceAddr, shiftBits);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建out aclTensor
    ret = CreateAclTensor(outHostData, outShape, aclDataType::ACL_INT64, outDeviceAddr, out);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ...
    ret = aclnnLeftShiftGetWorkspaceSize(self.get(), shiftBits.get(), out.get(), &workspaceSize, &executor);
    ...
    ret = aclnnLeftShift(workspaceAddr.get(), workspaceSize, executor, stream.get());
