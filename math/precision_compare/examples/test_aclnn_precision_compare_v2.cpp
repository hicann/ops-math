...
#include <limits>
#include "acl/acl.h"
#include "aclnnop/aclnn_precision_compare_v2.h"
...
int main() {
  ...
  // 调用aclnnPrecisionCompareV2第一段接口
  ret = aclnnPrecisionCompareV2GetWorkspaceSize(golden, realdata, detect_type, out, &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnPrecisionCompareV2GetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  // 调用aclnnPrecisionCompareV2第二段接口
  ret = aclnnPrecisionCompareV2(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnPrecisionCompareV2 failed. ERROR: %d\n", ret); return ret);
  // 4. （固定写法）同步等待任务执行结束
