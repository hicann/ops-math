...
#include "precision_compare_math.h"
                                              ...
                                              const uint32_t detect_type, aclOpExecutor* executor)
{
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICPU(PrecisionCompare, OP_ATTR_NAMES({"detect_type"}),
                                          OP_INPUT(benchMark, realData), OP_OUTPUT(out), OP_ATTR(detect_type));
