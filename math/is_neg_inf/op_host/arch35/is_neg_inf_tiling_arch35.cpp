...
ge::graphStatus IsNegInfRegbaseTiling::CheckShape() const
{
...
ge::graphStatus IsNegInfRegbaseTiling::RunTiling()
{
    ...
    if (this->inputDtype == ge::DT_FLOAT16) {
        ...
        dType = TPL_BF16;
        baseTilingResult = elewiseBaseTiling.DoTiling<IsNegInfOp::IsNegInfDAG<bfloat16_t>::OpDag>(*tiling,
                                                                                                  ASCEND_API_BUFFER);
    } else if (this->inputDtype == ge::DT_FLOAT) {
        dType = TPL_FP32;
        baseTilingResult = elewiseBaseTiling.DoTiling<IsNegInfOp::IsNegInfDAG<float>::OpDag>(*tiling,
                                                                                             ASCEND_API_BUFFER);
    } else {
        OP_LOGE_FOR_INVALID_DTYPE(tilingContext->GetNodeName(), "x",
                                  ge::TypeUtils::DataTypeToSerialString(this->inputDtype), "FLOAT16, BF16, FLOAT");
