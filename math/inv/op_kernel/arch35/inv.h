...
private:
    ...
    __aicore__ inline void ComputeInt32(LocalTensor<int32_t>& xLocal, LocalTensor<int32_t>& yLocal, int64_t currentNum);
...
__aicore__ inline void Inv<T>::Init(GM_ADDR self, GM_ADDR out, const InvTilingData* tilingData)
{
    ...
    if constexpr (std::is_floating_point_v<T> || std::is_same_v<T, half> || std::is_same_v<T, bfloat16_t>) {
...
template <typename T>
__aicore__ inline void Inv<T>::ComputeFloat32(LocalTensor<float>& xLocal, LocalTensor<float>& yLocal,
                                              int64_t alignedNum)
{
    LocalTensor<float> ones = tmpBuf2_.template Get<float>();
...
template <typename T>
template <typename SrcT>
__aicore__ inline void Inv<T>::ComputeWithCast(LocalTensor<SrcT>& xLocal, LocalTensor<SrcT>& yLocal, int64_t currentNum,
                                               int64_t alignedNum)
{
...
// 纯整型 Compare/Select（Reg，arch35 实证：adjacent_difference / floor_mod），
// 不经 fp32 中转，不对 INT_MIN 取负 —— x=0/|x|>=2/INT_MIN/INT_MAX 均落入 else->0。
//
// 迭代二 A1-Main 边界/对齐落实（结构对齐同仓已上库 adjacent_difference 的 Reg 尾块模式）：
...
template <typename T>
__aicore__ inline void Inv<T>::ComputeInt32(LocalTensor<int32_t>& xLocal, LocalTensor<int32_t>& yLocal,
                                            int64_t currentNum)
{
    if (currentNum <= 0) {
        return;
