...
static bool CheckBetaAndAlphaDtyeValid(const aclScalar* beta, const aclScalar* alpha,
                                       DataType highDtype, bool allIntInputs) {
  ...
  if (allIntInputs && beta && !IsIntegralType(beta->GetDataType(), true)) {
    OP_LOGE(ACLNN_ERR_PARAM_INVALID, "For integral input tensors, argument beta must not be a floating point number");
    return false;
  }

  if (allIntInputs && alpha && !IsIntegralType(alpha->GetDataType(), true)) {
    OP_LOGE(ACLNN_ERR_PARAM_INVALID, "For integral input tensors, argument alpha must not be a floating point number");
    return false;
  }
...
static bool CheckDtypeValid(const aclTensor* self, const aclTensor* vec1, const aclTensor* vec2,
...
static aclnnStatus addrOutHandle(const aclTensor* outCalcuResult, aclTensor* out, aclOpExecutor* executor, DataType hightDtype) {
...
static aclnnStatus addrStub(const aclTensor* selfCast, const aclTensor* vec1Cast, const aclTensor* vec2Cast, const aclScalar* beta,
...
aclnnStatus aclnnInplaceAddrGetWorkspaceSize(aclTensor* selfRef, const aclTensor* vec1, const aclTensor* vec2,
                                             ...
                                             uint64_t* workspaceSize, aclOpExecutor** executor) {
  return aclnnAddrGetWorkspaceSize(selfRef, vec1, vec2, betaOptional, alphaOptional, selfRef, workspaceSize, executor);
}

aclnnStatus aclnnInplaceAddr(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
