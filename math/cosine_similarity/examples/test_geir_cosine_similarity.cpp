...
#include "nn_other.h"
#include "../op_graph/cosine_similarity_proto.h"
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::CosineSimilarity("add1");
    std::vector<int64_t> xShape = {4, 8};
    // output shape: reduce along dim=1, so output shape is {4}
    std::vector<int64_t> yShape = {4};
    ADD_INPUT(1, input_x1, inDtype, xShape);
    ADD_INPUT(2, input_x2, inDtype, xShape);

    ADD_OUTPUT(1, output_y, inDtype, yShape);
