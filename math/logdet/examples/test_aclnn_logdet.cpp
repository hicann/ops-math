...
aclError CreateInputs(std::vector<int64_t>& selfShape, std::vector<int64_t>& outShape, DeviceMemPtr& selfDeviceAddr,
                      DeviceMemPtr& outDeviceAddr, TensorPtr& self, TensorPtr& out)
{
    ...
    auto ret = CreateAclTensor(selfHostData, selfShape, aclDataType::ACL_DOUBLE, selfDeviceAddr, self);
    CHECK_RET(ret == ACL_SUCCESS, return ret);

    ret = CreateAclTensor(outHostData, outShape, aclDataType::ACL_DOUBLE, outDeviceAddr, out);
    CHECK_RET(ret == ACL_SUCCESS, return ret);

    return ACL_SUCCESS;
}

aclError ExecOpApi(TensorPtr& self, TensorPtr& out, uint64_t& workspaceSize, DeviceMemPtr& outDeviceAddr,
                   std::vector<int64_t>& outShape, StreamPtr& stream)
{
    aclOpExecutor* executor;

    auto ret = aclnnLogdetGetWorkspaceSize(self.get(), out.get(), &workspaceSize, &executor);
    ...
    ret = aclnnLogdet(workspaceAddr.get(), workspaceSize, executor, stream.get());
...
int main()
{
    ...
    ret = ExecOpApi(self, out, workspaceSize, outDeviceAddr, outShape, stream);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
