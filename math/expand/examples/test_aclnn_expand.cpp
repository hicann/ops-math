...
int main()
{
    ...
    // 调用aclnnExpand第一段接口
    ret = aclnnExpandGetWorkspaceSize(self.get(), size, out.get(), &workspaceSize, &executor);
    ...
    // 调用aclnnExpand第二段接口
    ret = aclnnExpand(workspaceAddr.get(), workspaceSize, executor, stream.get());
    ...
    ret = aclrtMemcpy(resultData.data(), resultData.size() * sizeof(resultData[0]), outDeviceAddr.get(),
                      resultSize * sizeof(resultData[0]), ACL_MEMCPY_DEVICE_TO_HOST);
