...
int main()
{
    ...
    DeviceMemPtr vec2DeviceAddr(nullptr, &aclrtFree);
    DeviceMemPtr outDeviceAddr(nullptr, &aclrtFree);
    TensorPtr self(nullptr, &aclDestroyTensor);
    TensorPtr vec2(nullptr, &aclDestroyTensor);
    ...
    // 创建vec2 aclTensor
    ret = CreateAclTensor(vec2HostData, vec2Shape, aclDataType::ACL_FLOAT, vec2DeviceAddr, vec2);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建out aclTensor
    ...
    // 调用aclnnGer第一段接口
    ret = aclnnGerGetWorkspaceSize(self.get(), vec2.get(), out.get(), &workspaceSize, &executor);
    ...
    // 调用aclnnGer第二段接口
    ret = aclnnGer(workspaceAddr.get(), workspaceSize, executor, stream.get());
