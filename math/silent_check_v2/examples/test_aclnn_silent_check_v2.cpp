...
int main()
{
    ...
    ret = CreateAclTensor(inputGradRefHostData, inputGradRefShape, &inputGradRefDeviceAddr, aclDataType::ACL_FLOAT,
                          &inputGradRef);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建stepRef aclTensor
    ...
    ret = aclnnSilentCheckV2GetWorkspaceSize(val, max, avgRef, inputGradRef, stepRef, dstSize, dstStride, dstOffset,
                                             cThreshL1, cThreshL2, beta1, npuAsdDetect, result, &workspaceSize,
                                             &executor);
