...
#include "../../op_graph/prod_force_se_a_proto.h"
                    ...
                    int value)
{
    ...
    if (pData == nullptr) {
        printf("ERROR: Failed to allocate memory for GenOnesData\n");
        return FAILED;
    }
    ...
    delete[] pData; // Tensor 拷贝数据后释放原始内存
    return SUCCESS;
}
...
{
    Status ret = SUCCESS;
    // ProdForceSeA: 4 inputs, 1 output, 2 required attrs
    auto add1 = op::ProdForceSeA("prod_force_se_a_1");

    // net_deriv: [1, 36] float32, nloc=3, nnei=3, ndescrpt=12
    std::vector<int64_t> netDerivShape = {1, 36};
    ADD_INPUT(1, net_deriv, inDtype, netDerivShape);

    // in_deriv: [1, 108] float32
    std::vector<int64_t> inDerivShape = {1, 108};
    ADD_INPUT(2, in_deriv, inDtype, inDerivShape);

    // nlist: [1, 9] int32
    std::vector<int64_t> nlistShape = {1, 9};
    ADD_CONST_INPUT(3, nlist, ge::DT_INT32, nlistShape);

    // natoms: [3] int32, natoms[0]=nloc=3, natoms[1]=nall=5
    std::vector<int64_t> natomsShape = {3};
    ADD_CONST_INPUT(4, natoms, ge::DT_INT32, natomsShape);

    // atom_force: [1, 5, 3] float32
    std::vector<int64_t> forceShape = {1, 5, 3};
    ADD_OUTPUT(1, atom_force, inDtype, forceShape);

    // set required attrs
    add1.set_attr_n_a_sel(2);
    add1.set_attr_n_r_sel(1);
