...
static const std::initializer_list<op::DataType> ANCIENT_DTYPE_SUPPORT_LIST = {op::DataType::DT_FLOAT16,
...
std::tuple<aclTensor*, aclTensor*> RadixTopK(const aclTensor* self, const aclTensor* k, int64_t dim, bool largest,
                                             bool sorted, aclTensor* values, aclTensor* indices,
                                             op::DataType indicesDType, aclOpExecutor* executor)
{
    L0_DFX(RadixTopK, self, k, dim, largest, sorted, values, indices, indicesDType);
    ADD_TO_LAUNCHER_LIST_AICORE(RadixTopK, OP_INPUT(self, k), OP_OUTPUT(values, indices),
                                OP_ATTR(sorted, dim, largest, indicesDType));
    return std::tuple<aclTensor*, aclTensor*>(values, indices);
}

std::tuple<aclTensor*, aclTensor*> TopkV3(const aclTensor* self, const aclTensor* k, int64_t dim, bool largest,
                                          bool sorted, aclTensor* values, aclTensor* indices, aclOpExecutor* executor)
{
...
// AICPU算子kernel
std::tuple<aclTensor*, aclTensor*> TopkAiCpu(const aclTensor* self, const aclTensor* k, int64_t dim, bool largest,
                                             bool sorted, aclTensor* values, aclTensor* indices,
                                             aclOpExecutor* executor)
{
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICPU(TopK, OP_ATTR_NAMES({"sorted", "largest", "dim"}), OP_INPUT(self, k),
                                          OP_OUTPUT(values, indices), OP_ATTR(sorted, largest, dim));
