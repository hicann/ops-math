...
[📄 查看源码](https://gitcode.com/cann/ops-math/tree/master/math/top_k_v2)
...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用"aclnnTopkV2GetWorkspaceSize"接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用"aclnnTopkV2"接口执行计算。
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>sortPolicy（int64_t）</td>
      <td>输入</td>
      <td>表示TopK候选结果的排序策略。</td>
      <td>0表示使用原有排序行为，1表示在sorted=True且小k值（2≤k≤32）场景下启用Bitonic Small TopK路径，对相等值的索引按原始顺序排列，对齐GPU行为。</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnTopkV2GetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
int main() {
  ...
  bool largest = true;
  bool sorted = true;
  int64_t sortPolicy = 1;

  uint64_t workspaceSize = 0;
  aclOpExecutor* executor;
  // 调用aclnnTopkV2第一段接口
  ret = aclnnTopkV2GetWorkspaceSize(self, k, dim, largest, sorted, sortPolicy, out, indices, &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnTopkV2GetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  // 调用aclnnTopkV2第二段接口
  ret = aclnnTopkV2(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnTopkV2 failed. ERROR: %d\n", ret); return ret);
