...
int main()
{
    ...
    ret = aclnnAtanhGetWorkspaceSize(self.get(), out.get(), &workspaceSize, &executor);
    ...
    // 调用aclnnAtanh第二段接口
    ret = aclnnAtanh(workspaceAddr.get(), workspaceSize, executor, stream.get());
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAtanh failed. ERROR: %d\n", ret); return ret);

    // aclnnInplaceAtanh接口调用示例
    uint64_t inplaceWorkspaceSize = 0;
    aclOpExecutor* inplaceExecutor;
    ret = aclnnInplaceAtanhGetWorkspaceSize(self.get(), &inplaceWorkspaceSize, &inplaceExecutor);
    ...
    ret = aclnnInplaceAtanh(inplaceWorkspaceAddr.get(), inplaceWorkspaceSize, inplaceExecutor, stream.get());
