...
#include "nn_other.h"
#include "../op_graph/atanh_proto.h"
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape, inputValues)                                        \
    do {                                                                                                             \
        ...
        atanh1.set_input_##inputName(placeholder##inputIndex);                                                       \
...
{
    Status ret = SUCCESS;
    auto atanh1 = op::Atanh("atanh1");
    std::vector<int64_t> xShape = {2, 3};
    std::vector<double> xData = {0.5, 0.0, 0.25, 0.5, -0.5, 0.0, 0.0, 0.5, 0.75, -0.5, -0.25, 0.25};
    ADD_INPUT(1, x, inDtype, xShape, xData);

    outputs.push_back(atanh1);
    return SUCCESS;
}
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < input_num; i++) {
        string input_file = "./tc_ge_irrun_test_atanh_npu_input_" + std::to_string(i) + ".bin";
    ...
    for (int i = 0; i < output_num; i++) {
        string output_file = "./tc_ge_irrun_test_atanh_npu_output_" + std::to_string(i) + ".bin";
