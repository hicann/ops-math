...
static const aclTensor* ReduceLogSumExpAiCore(const aclTensor* self, const aclTensor* dim, bool keepDim,
                                              bool noopWithEmptyAxes, const aclTensor* logSumExpOut,
                                              aclOpExecutor* executor)
{
    L0_DFX(ReduceLogSumExpAiCore, self, dim, keepDim, noopWithEmptyAxes, logSumExpOut);
...
const aclTensor* ReduceLogSumExp(const aclTensor* self, const aclIntArray* dim, bool keepDim, aclOpExecutor* executor)
{
    ...
    INFER_SHAPE(ReduceLogSumExp, OP_INPUT(self, dimTensor), OP_OUTPUT(logSumExpOut),
                OP_ATTR(keepDim, noopWithEmptyAxes));

    if (IsAiCoreSupport(self)) {
        return ReduceLogSumExpAiCore(self, dimTensor, keepDim, noopWithEmptyAxes, logSumExpOut, executor);
    }
    return logSumExpOut;
}
} // namespace l0op
