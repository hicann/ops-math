...
const aclTensor* ReduceStdV2Update(const aclTensor* self, const aclTensor* mean, const aclIntArray* dim, bool unbiased,
                                   bool keepdim, aclOpExecutor* executor)
{
    L0_DFX(ReduceStdV2Update, self, mean, dim, unbiased, keepdim);
    auto stdWithV2UpdateOut = executor->AllocTensor(self->GetDataType(), self->GetStorageFormat(),
