...
#ifndef HISTOGRAM_FIXED_WIDTH_SIMT_NOT_FULL_LOAD_SIMT_H
#define HISTOGRAM_FIXED_WIDTH_SIMT_NOT_FULL_LOAD_SIMT_H
...
class HistogramFixedWidthSimtNotFullLoadGmAtomicAdd {
public:
    __aicore__ inline HistogramFixedWidthSimtNotFullLoadGmAtomicAdd(){};
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR range, GM_ADDR y,
                                const HistogramFixedWidthSimtTilingData* tilingData);
...
template <typename X_TYPE, typename COMPUTE_TYPE>
__aicore__ inline void HistogramFixedWidthSimtNotFullLoadGmAtomicAdd<X_TYPE, COMPUTE_TYPE>::Init(
    GM_ADDR x, GM_ADDR range, GM_ADDR y, const HistogramFixedWidthSimtTilingData* tilingData)
{
...
template <typename X_TYPE, typename COMPUTE_TYPE>
__simt_callee__ __aicore__ inline void DoBinAtomic(__gm__ int32_t* yGmAddr, const COMPUTE_TYPE v,
                                                   ...
                                                   const int32_t bins, const int32_t binsM1)
{
    ...
    asc_atomic_add(yGmAddr + binIndex, static_cast<int32_t>(1));
}

template <typename X_TYPE, typename COMPUTE_TYPE>
__simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_NUM) inline void SimtComputeAtomic(
    __gm__ X_TYPE* xGmAddr, __gm__ int32_t* yGmAddr, const int32_t blockIdx, const uint32_t needXCoreNum,
    const int64_t xIndexBase, const int64_t coreDataLength, const COMPUTE_TYPE minValue, const COMPUTE_TYPE maxValue,
    const COMPUTE_TYPE rangeVal, const COMPUTE_TYPE invRange, const int32_t bins)
{
    ...
    int64_t stepN = static_cast<int64_t>(stride) * UNROLL_FACTOR;
    ...
    for (; base < limitN; base += stepN) {
        ...
        DoBinAtomic<X_TYPE, COMPUTE_TYPE>(yGmAddr, v0, minValue, maxValue, rangeVal, invRange, bins, binsM1);
        DoBinAtomic<X_TYPE, COMPUTE_TYPE>(yGmAddr, v1, minValue, maxValue, rangeVal, invRange, bins, binsM1);
        DoBinAtomic<X_TYPE, COMPUTE_TYPE>(yGmAddr, v2, minValue, maxValue, rangeVal, invRange, bins, binsM1);
        DoBinAtomic<X_TYPE, COMPUTE_TYPE>(yGmAddr, v3, minValue, maxValue, rangeVal, invRange, bins, binsM1);
        DoBinAtomic<X_TYPE, COMPUTE_TYPE>(yGmAddr, v4, minValue, maxValue, rangeVal, invRange, bins, binsM1);
        DoBinAtomic<X_TYPE, COMPUTE_TYPE>(yGmAddr, v5, minValue, maxValue, rangeVal, invRange, bins, binsM1);
    }
    for (; base < coreDataLength; base += stride) {
        DoBinAtomic<X_TYPE, COMPUTE_TYPE>(yGmAddr, static_cast<COMPUTE_TYPE>(asc_ldcg(xGmAddr + xIndexBase + base)),
                                          minValue, maxValue, rangeVal, invRange, bins, binsM1);
    }
}

template <typename X_TYPE, typename COMPUTE_TYPE>
__aicore__ inline void HistogramFixedWidthSimtNotFullLoadGmAtomicAdd<X_TYPE, COMPUTE_TYPE>::Process()
{
    if (blockIdx_ < GetBlockNum()) {
        ...
        asc_vf_call<SimtCleanY<X_TYPE, COMPUTE_TYPE, uint32_t>>(
            dim3{THREAD_NUM, 1, 1}, yGmAddr, blockIdx_, this->clearYCoreNum_, clearYIndexBase, clearYDataLength);
#ifndef __CCE_UT_TEST__
        SyncAll();
#endif
        asc_vf_call<SimtComputeAtomic<X_TYPE, COMPUTE_TYPE>>(dim3{THREAD_NUM, 1, 1}, xGmAddr, yGmAddr, blockIdx_,
                                                             needXCoreNum_, xIndexBase, coreDataLength, minValue_,
                                                             maxValue_, rangeVal, invRange, bins_);
    }
}

} // namespace HistogramFixedWidthSIMT

#endif // HISTOGRAM_FIXED_WIDTH_SIMT_NOT_FULL_LOAD_SIMT_H
