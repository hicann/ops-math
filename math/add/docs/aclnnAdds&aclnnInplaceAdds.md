...
- aclnnAdds和aclnnInplaceAdds实现相同的功能，使用区别如下，请根据自身实际场景选择合适的算子。

  - aclnnAdds：需新建一个输出张量对象存储计算结果。
  - aclnnInplaceAdds：无需新建输出张量对象，直接在输入张量的内存中存储计算结果。

- 每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnAddsGetWorkspaceSize”或者“aclnnInplaceAddsGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnAdds”或者“aclnnInplaceAdds”接口执行计算。
...
- **参数说明：**
    ...
    <table style="table-layout: fixed; width: 1510px"><colgroup>
...
- 确定性计算：
  - aclnnAdds&aclnnInplaceAdds默认确定性实现。
