...
uint32_t CheckPermissionType(const CpuKernelContext& ctx)
{
    ...
    if (input0_data_type != output_data_type) {
        KERNEL_LOG_ERROR("[Add] Output must have the same dtype as input, but got "
...
uint32_t AddCpuKernel::Compute(CpuKernelContext& ctx)
{
    ...
    KERNEL_LOG_INFO("[%s] Compute begin, dtype=%d, in0_bytes=%lu, in1_bytes=%lu, out_bytes=%lu.",
                    ctx.GetOpType().c_str(), static_cast<int>(input0_data_type), input0->GetDataSize(),
                    input1->GetDataSize(), ctx.Output(kFirstOutputIndex)->GetDataSize());
    ...
    switch (input0_data_type) {
        ...
        default:
            KERNEL_LOG_ERROR("[%s] Data type of input is not supported, got dtype=[%s]. "
...
uint32_t AddCpuKernel::RunMaybeParallel(const CpuKernelContext& ctx, const char* tag, int64_t total, int64_t elem_bytes,
                                        const Body& body) const
{
    const int64_t total_bytes = total * elem_bytes;
    if (total_bytes < kParallelBytesThresh) {
        KERNEL_LOG_INFO("[%s] %s serial path, total_bytes=%ld.", ctx.GetOpType().c_str(), tag, total_bytes);
        body(0, total);
        return KERNEL_STATUS_OK;
    }
    ...
    if (__builtin_expect(elem_bytes <= 0, 0)) {
        KERNEL_LOG_ERROR("[%s] %s invalid elem_bytes=%ld, fallback to serial.", ctx.GetOpType().c_str(), tag,
                         elem_bytes);
        body(0, total);
        return KERNEL_STATUS_OK;
    }
    const int64_t per_unit = std::max<int64_t>(1, kBytesPerShard / elem_bytes);
    KERNEL_LOG_INFO("[%s] %s parallel path, total=%ld, per_unit=%ld.", ctx.GetOpType().c_str(), tag, total, per_unit);
    const uint32_t rc = CpuKernelUtils::ParallelFor(ctx, total, per_unit, body);
    if (__builtin_expect(rc != KERNEL_STATUS_OK, 0)) {
        KERNEL_LOG_ERROR("[%s] %s ParallelFor failed, rc=%u, total=%ld, per_unit=%ld.", ctx.GetOpType().c_str(), tag,
                         rc, total, per_unit);
        return rc;
    }
    return KERNEL_STATUS_OK;
...
template <typename T>
uint32_t AddCpuKernel::AddSameShape(const CpuKernelContext& ctx, const T* x0, const T* x1, T* y, int64_t total) const
{
    ...
    return RunMaybeParallel(ctx, "same-shape", total, static_cast<int64_t>(sizeof(T)), body);
}

template <typename T>
uint32_t AddCpuKernel::AddScalarBcast(const CpuKernelContext& ctx, const T* vec, T scalar_val, T* y,
                                      int64_t total) const
{
    ...
    return RunMaybeParallel(ctx, "scalar-bcast", total, static_cast<int64_t>(sizeof(T)), body);
}

template <typename T>
uint32_t AddCpuKernel::AddGenericBcast(const CpuKernelContext& ctx, BCalcInfo& calc_info) const
{
    (void)ctx;
    return AddCalculateWithRankCheck<T>(ctx, calc_info);
}

uint32_t AddCpuKernel::ValidateAndBroadcast(const CpuKernelContext& ctx, BCalcInfo& calc_info) const
{
    ...
    if (raw0.size() > static_cast<size_t>(kRank8) || raw1.size() > static_cast<size_t>(kRank8) ||
        raw_out.size() > static_cast<size_t>(kRank8)) {
        KERNEL_LOG_ERROR("[%s] Rank of input/output must be <= 8, got in0_rank=%zu, "
    ...
    if (raw_out.size() != calc_info.shape_out.size()) {
        KERNEL_LOG_ERROR("[%s] Output rank [%zu] does not match broadcast rank [%zu].", ctx.GetOpType().c_str(),
                         raw_out.size(), calc_info.shape_out.size());
        return KERNEL_STATUS_PARAM_INVALID;
    }
    for (size_t i = 0; i < raw_out.size(); ++i) {
        if (raw_out[i] != calc_info.shape_out[i]) {
            KERNEL_LOG_ERROR("[%s] Output dim[%zu]=%ld mismatches broadcast dim=%ld.", ctx.GetOpType().c_str(), i,
                             raw_out[i], calc_info.shape_out[i]);
            return KERNEL_STATUS_PARAM_INVALID;
        }
    }
    return KERNEL_STATUS_OK;
...
uint32_t AddCpuKernel::AddCompute(const CpuKernelContext& ctx) const
{
    ...
    KERNEL_LOG_INFO("[%s] Input[0] size=%lu, Input[1] size=%lu, Output size=%lu.", ctx.GetOpType().c_str(),
    ...
    if (raw0 == raw1 && raw0 == raw_out && n0 == n1 && n0 == ny) {
        KERNEL_LOG_INFO("[%s] same-shape branch selected, elems=%ld.", ctx.GetOpType().c_str(), ny);
        return AddSameShape<T>(ctx, x0_ptr, x1_ptr, y_ptr, ny);
    }
    if (n0 == 1 && n1 == ny && raw1 == raw_out) {
        KERNEL_LOG_INFO("[%s] x0-scalar bcast branch selected, elems=%ld.", ctx.GetOpType().c_str(), ny);
        return AddScalarBcast<T>(ctx, x1_ptr, *x0_ptr, y_ptr, ny);
    }
    if (n1 == 1 && n0 == ny && raw0 == raw_out) {
        KERNEL_LOG_INFO("[%s] x1-scalar bcast branch selected, elems=%ld.", ctx.GetOpType().c_str(), ny);
...
uint32_t AddCpuKernel::AddCalculateWithRankCheck(const CpuKernelContext& ctx, BCalcInfo& calc_info) const
{
    ...
    switch (rank) {
        ...
        default:
            KERNEL_LOG_ERROR("[%s] Rank of output must be in [0,8], got rank=%zu.", ctx.GetOpType().c_str(),
...
{
    Eigen::TensorMap<Eigen::Tensor<T, 1>, OPTION> input0(PtrToPtr<void, T>(calc_info.input_0->GetData()),
                                                         calc_info.input_0->GetTensorShape()->NumElements());
    Eigen::TensorMap<Eigen::Tensor<T, 1>, OPTION> input1(PtrToPtr<void, T>(calc_info.input_1->GetData()),
                                                         calc_info.input_1->GetTensorShape()->NumElements());
    Eigen::TensorMap<Eigen::Tensor<T, 1>, OPTION> output(PtrToPtr<void, T>(calc_info.output->GetData()),
    ...
    output.reshape(shape_out) = input0.reshape(reshape0).broadcast(bcast0) + input1.reshape(reshape1).broadcast(bcast1);
    return KERNEL_STATUS_OK;
}

OPS_MATH_REGISTER_CPU_KERNELV2(kAdd, AddCpuKernel);
} // namespace aicpu
