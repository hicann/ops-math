...
protected:
  ...
  static void TearDownTestCase() { cout << "addcdiv_test TearDown" << endl; }
};

// 普通场景（含精度）
TEST_F(l2_addcdiv_test, case_1) {
  auto input = TensorDesc({3,3}, ACL_FLOAT, ACL_FORMAT_ND)
                   .ValueRange(-1, 1);

  auto tensor1 = TensorDesc({3,3}, ACL_FLOAT, ACL_FORMAT_ND)
                     .ValueRange(-1, 1);

  auto tensor2 = TensorDesc({3,3}, ACL_FLOAT, ACL_FORMAT_ND)
                     .ValueRange(-1, 1);
  ...
  auto ut = OP_API_UT(aclnnAddcdiv,
                      INPUT(input, tensor1, tensor2, scalar),
                      OUTPUT(out));
...
TEST_F(l2_addcdiv_test, case_2) {
  ...
  auto tensor1 = TensorDesc({3}, ACL_FLOAT, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{4, 4, 4});

  auto tensor2 = TensorDesc({3}, ACL_FLOAT, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{1, 1, 1});
...
TEST_F(l2_addcdiv_test, case_3) {
  auto input = TensorDesc({3}, ACL_FLOAT, ACL_FORMAT_ND)
                   .ValueRange(0, 10)
...
TEST_F(l2_addcdiv_test, case_7) {
  ...
  auto tensor1 = TensorDesc({3}, ACL_DT_UNDEFINED, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{4, 4, 4});
...
TEST_F(l2_addcdiv_test, case_8) {
  auto input = TensorDesc({3}, ACL_DT_UNDEFINED, ACL_FORMAT_ND)
                   .ValueRange(0, 10)
                   .Value(vector<float>{1, 2, 3});
...
TEST_F(l2_addcdiv_test, case_15) {
  auto input = TensorDesc({3,3}, ACL_DOUBLE, ACL_FORMAT_ND)
                   .ValueRange(-1, 1);

  auto tensor1 = TensorDesc({3,3}, ACL_DOUBLE, ACL_FORMAT_ND)
                     .ValueRange(-1, 1);

  auto tensor2 = TensorDesc({3,3}, ACL_DOUBLE, ACL_FORMAT_ND)
                     .ValueRange(-1, 1);
...
TEST_F(l2_addcdiv_test, case_16) {
  auto input = TensorDesc({3}, ACL_FLOAT16, ACL_FORMAT_ND)
                   .ValueRange(0, 10)
                   .Value(vector<float>{1, 2, 3});

  auto tensor1 = TensorDesc({3}, ACL_FLOAT16, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{4, 4, 4});

  auto tensor2 = TensorDesc({3}, ACL_FLOAT16, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{1, 1, 1});
...
TEST_F(l2_addcdiv_test, case_17) {
  auto input = TensorDesc({3}, ACL_BF16, ACL_FORMAT_ND)
                   .ValueRange(0, 10)
                   .Value(vector<float>{1, 2, 3});

  auto tensor1 = TensorDesc({3}, ACL_BF16, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{4, 4, 4});

  auto tensor2 = TensorDesc({3}, ACL_BF16, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{1, 1, 1});
...
TEST_F(l2_addcdiv_test, case_18) {
  auto input = TensorDesc({3}, ACL_INT32, ACL_FORMAT_ND)
                   .ValueRange(0, 10)
                   .Value(vector<float>{1, 2, 3});

  auto tensor1 = TensorDesc({3}, ACL_INT32, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{4, 4, 4});

  auto tensor2 = TensorDesc({3}, ACL_INT32, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{1, 1, 1});
...
TEST_F(l2_addcdiv_test, case_19) {
  auto input = TensorDesc({3}, ACL_INT64, ACL_FORMAT_ND)
                   .ValueRange(0, 10)
                   .Value(vector<float>{1, 2, 3});

  auto tensor1 = TensorDesc({3}, ACL_INT64, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{4, 4, 4});

  auto tensor2 = TensorDesc({3}, ACL_INT64, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{1, 1, 1});
...
TEST_F(l2_addcdiv_test, case_20) {
  auto input = TensorDesc({3}, ACL_INT16, ACL_FORMAT_ND)
                   .ValueRange(0, 10)
                   .Value(vector<float>{1, 2, 3});

  auto tensor1 = TensorDesc({3}, ACL_INT16, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{4, 4, 4});

  auto tensor2 = TensorDesc({3}, ACL_INT16, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{1, 1, 1});
...
TEST_F(l2_addcdiv_test, case_21) {
  auto input = TensorDesc({3}, ACL_INT8, ACL_FORMAT_ND)
                   .ValueRange(0, 10)
                   .Value(vector<float>{1, 2, 3});

  auto tensor1 = TensorDesc({3}, ACL_INT8, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{4, 4, 4});

  auto tensor2 = TensorDesc({3}, ACL_INT8, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{1, 1, 1});
...
TEST_F(l2_addcdiv_test, case_22) {
  auto input = TensorDesc({3}, ACL_UINT8, ACL_FORMAT_ND)
                   .ValueRange(0, 10)
                   .Value(vector<float>{1, 2, 3});

  auto tensor1 = TensorDesc({3}, ACL_UINT8, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{4, 4, 4});

  auto tensor2 = TensorDesc({3}, ACL_UINT8, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{1, 1, 1});
...
TEST_F(l2_addcdiv_test, case_23) {
  auto input = TensorDesc({3}, ACL_BOOL, ACL_FORMAT_ND)
                   .ValueRange(0, 10)
                   .Value(vector<float>{1, 2, 3});

  auto tensor1 = TensorDesc({3}, ACL_BOOL, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{4, 4, 4});

  auto tensor2 = TensorDesc({3}, ACL_BOOL, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{1, 1, 1});
...
TEST_F(l2_addcdiv_test, case_24) {
  auto input = TensorDesc({3}, ACL_COMPLEX64, ACL_FORMAT_ND)
                   .ValueRange(0, 10)
                   .Value(vector<float>{1, 2, 3});

  auto tensor1 = TensorDesc({3}, ACL_COMPLEX64, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{4, 4, 4});

  auto tensor2 = TensorDesc({3}, ACL_COMPLEX64, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{1, 1, 1});
...
TEST_F(l2_addcdiv_test, case_25) {
  auto input = TensorDesc({3}, ACL_COMPLEX128, ACL_FORMAT_ND)
                   .ValueRange(0, 10)
                   .Value(vector<float>{1, 2, 3});

  auto tensor1 = TensorDesc({3}, ACL_COMPLEX128, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{4, 4, 4});

  auto tensor2 = TensorDesc({3}, ACL_COMPLEX128, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{1, 1, 1});
...
TEST_F(l2_addcdiv_test, case_26) {
  ...
  auto tensor2 = TensorDesc({3}, ACL_DT_UNDEFINED, ACL_FORMAT_ND)
                     .ValueRange(0, 10)
                     .Value(vector<float>{1, 1, 1});
