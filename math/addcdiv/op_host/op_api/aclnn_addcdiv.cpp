...
static bool CheckPromoteType(const aclTensor* self, const aclTensor* tensor1,
                             const aclTensor* tensor2, const aclTensor* out) {
...
static bool CheckMaxDimension(const aclTensor* self, const aclTensor* tensor1,
                              const aclTensor* tensor2, const aclTensor* out) {
...
static bool CheckInAndOutShape (const aclTensor* self, const aclTensor* tensor1,
                                const aclTensor* tensor2, const aclTensor* out) {
  op::Shape shape1;
  op::Shape shape2;
...
static bool IsMixedDType(const aclTensor* self, const aclScalar* value) {
  ...
  return (selfDtype == op::DataType::DT_FLOAT16 || selfDtype == op::DataType::DT_BF16) && (valueDtype == op::DataType::DT_FLOAT);
}
...
aclnnStatus aclnnAddcdivGetWorkspaceSize(const aclTensor* self, const aclTensor* tensor1, const aclTensor* tensor2,
  ...
  bool isToFloat = GetCurrentPlatformInfo().GetSocVersion() == SocVersion::ASCEND910_95 && IsMixedDType(self, value) &&
...
aclnnStatus aclnnInplaceAddcdivGetWorkspaceSize(const aclTensor* selfRef, const aclTensor* tensor1,
...
aclnnStatus aclnnInplaceAddcdiv(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
