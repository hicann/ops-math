...
#include "../op_graph/xdivy_proto.h"
...
int32_t GenFloatData(
    vector<int64_t> shapes, Tensor& input_tensor, TensorDesc& input_tensor_desc,
    DataType data_type, const vector<float>& values)
{
    ...
    for (size_t i = 0; i < size; ++i) pData[i] = values[i % values.size()];
    ...
    std::vector<Operator>& outputs, Graph& graph)
{
    ...
    // Input x1
    {
        ...
        ret = GenFloatData(shape, tensor, desc, inDtype, xVals);
        if (ret != SUCCESS) {
    ...
    // Input x2 (y in the formula)
    {
        ...
        ret = GenFloatData(shape, tensor, desc, inDtype, yVals);
        if (ret != SUCCESS) {
        ...
        input.push_back(tensor);
        graph.AddOp(data);
        xdivyOp.set_input_x2(data);
        inputs.push_back(data);
    }

    TensorDesc outDesc = TensorDesc(ge::Shape(shape), FORMAT_ND, inDtype);
    xdivyOp.update_output_desc_y(outDesc);
    outputs.push_back(xdivyOp);
    return SUCCESS;
}

int main(int argc, char* argv[])
{
    const char* graph_name = "tc_ge_irrun_xdivy_test";
    ...
    std::string file_path = "./dump_xdivy";
    aclgrphDumpGraph(graph, file_path.c_str(), file_path.length());

    printf("%s - [XIR]: Running xdivy graph\n", GetTime().c_str());
    ...
    for (int i = 0; i < input_num; i++) {
        string input_file = "./xdivy_input_" + std::to_string(i) + ".bin";
    ...
    printf("\n========== GEIR NPU Output (xdivy: x1/x2, x1==0 -> 0) ==========\n");
    for (int i = 0; i < output_num; i++) {
        string output_file = "./xdivy_output_" + std::to_string(i) + ".bin";
    ...
    // Golden: z = x1 / x2, x1==0 -> 0
    printf("\n========== Golden Reference (x1/x2) ==========\n");
    float x1Vals[] = {1.0f, 2.0f, 3.0f, 0.0f, 5.0f, 6.0f};
    float x2Vals[] = {2.0f, 3.0f, 4.0f, 5.0f, 1.0f, 2.0f};
    int allMatch = 1;
    for (int i = 0; i < 6; i++) {
        float golden = (x1Vals[i] == 0.0f) ? 0.0f : x1Vals[i] / x2Vals[i];
        float npuVal = reinterpret_cast<float*>(output[0].GetData())[i];
        int match = (fabs(npuVal - golden) < 1e-5f);
        if (!match) allMatch = 0;
        printf("  [%d] x1=%.1f x2=%.1f -> NPU=%.6f golden=%.6f %s\n",
               i, x1Vals[i], x2Vals[i], npuVal, golden, match ? "OK" : "MISMATCH");
    }
    printf("========================================\n");
