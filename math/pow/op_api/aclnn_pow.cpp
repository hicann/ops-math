...
#include "math/pows/op_api/pows.h"
...
static inline bool IsPowAiCpuOn910B(const op::DataType dtype, const aclScalar* self, const aclTensor* exponent)
{
    ...
    if (selfDtype == op::DataType::DT_BOOL || expDtype == op::DataType::DT_BOOL || selfDtype == op::DataType::DT_BF16 ||
        expDtype == op::DataType::DT_BF16) {
        return false;
    }
    // 910B AICORE支持的dtype
...
aclnnStatus aclnnPowTensorScalarGetWorkspaceSize(const aclTensor* self, const aclScalar* exponent, const aclTensor* out,
...
aclnnStatus aclnnInplacePowTensorScalarGetWorkspaceSize(const aclTensor* self, const aclScalar* exponent,
                                                        uint64_t* workspaceSize, aclOpExecutor** executor)
{
...
static aclnnStatus BuildPowScalarTensorCompute(const aclScalar* self, const aclTensor* exponent, const aclTensor* out,
                                               const op::DataType promoteType, aclOpExecutor* executor)
{
...
aclnnStatus aclnnPowScalarTensorGetWorkspaceSize(const aclScalar* self, const aclTensor* exponent, const aclTensor* out,
                                                 uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    CHECK_RET(BuildPowScalarTensorCompute(self, exponent, out, promoteType, uniqueExecutor.get()) == ACLNN_SUCCESS,
              ACLNN_ERR_INNER);
