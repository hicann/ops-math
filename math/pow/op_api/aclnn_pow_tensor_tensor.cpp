...
static void CheckFormat(const aclTensor* self, const aclTensor* exponent)
{
    ...
    if (selfStorageFormat == ge::Format::FORMAT_FRACTAL_NZ || exponentStorageFormat == ge::Format::FORMAT_FRACTAL_NZ) {
...
aclnnStatus aclnnInplacePowTensorTensorGetWorkspaceSize(const aclTensor* self, const aclTensor* exponent,
                                                        uint64_t* workspaceSize, aclOpExecutor** executor)
{
