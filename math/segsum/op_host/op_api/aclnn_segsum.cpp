...
static bool CheckShape(const aclTensor* self, const aclTensor* out)
{
    ...
    if (selfShape.GetDim(selfDimSize - 1) != outShape.GetDim(outDimSize - 1)) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "It is expected output size at index %zu equals to input size at index %zu, \
            but got input size %s, output size %s.",
                selfDimSize - 1, outDimSize - 1, op::ToString(selfShape).GetString(),
...
static bool CheckFormat(const aclTensor* self, const aclTensor* out)
{
    ...
    if (selfFormat != outFormat) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Format of input and output should be equal, input[%s], out[%s].",
...
aclnnStatus aclnnExpSegsumGetWorkspaceSize(const aclTensor* self, aclTensor* out, uint64_t* workspaceSize,
                                           aclOpExecutor** executor)
{
    ...
    // 入参校验必须先于空张量早退: 顺序颠倒会让空张量绕过全部校验(out 为空指针 / dtype 不一致 /
    // rank 越界 / format 非 ND 一律返回 SUCCESS), 且 self 为空指针时 self->IsEmpty() 先解引用直接崩溃,
    // 与 aclnnSegsum.md 承诺的 161001/161002 对不上。仓内 6 个同类算子(addcmul/sum/argmax/acos/add/add_v3)
    // 均为"先 CheckParams 再 IsEmpty"。
