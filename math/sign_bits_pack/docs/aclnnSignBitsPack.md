...
[📄 查看源码](https://gitcode.com/cann/ops-math-dev/tree/master/math/sign_bits_pack)
...
## 函数原型
每个算子分为[两段式接口](../../../docs/context/两段式接口.md)，必须先调用“aclnnSignBitsPackGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnSignBitsPack”接口执行计算。
...
- **参数说明**：
  ...
  - self（aclTensor\*，计算输入）：表示用于计算的1D张量，Device侧的tensor，支持空tensor场景，数据类型支持FLOAT16和FLOAT。支持[非连续的Tensor](../../../docs/context/非连续的Tensor.md)。支持[数据格式](../../../docs/context/数据格式.md)为ND。

  - size（int64_t，计算输入）：表示处理维度，reshape时输出张量的第一个维度。

  - out（aclTensor\*，计算输出）：输出的tensor，只支持二维，数据类型支持UINT8。输出out总长度在self元素个数不被8整除时为（self元素个数 // 8） + 1，在被8整除时为（self元素个数/8）。支持[数据格式](../../../docs/context/数据格式.md)为ND。
