...
int main()
{
    ...
    // 调用aclnnLog1p第一段接口
    ret = aclnnLog1pGetWorkspaceSize(self.get(), out.get(), &workspaceSize, &executor);
    ...
    // 调用aclnnLog1p第二段接口
    ret = aclnnLog1p(workspaceAddr.get(), workspaceSize, executor, stream.get());
    ...
    ret = aclnnInplaceLog1pGetWorkspaceSize(self.get(), &workspaceSize, &executor);
    ...
    ret = aclnnInplaceLog1p(workspaceAddr.get(), workspaceSize, executor, stream.get());
