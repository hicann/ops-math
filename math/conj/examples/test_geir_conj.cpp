...
// Generate complex input data. Uses COMPLEX128 (std::complex<double>) as the
// default validation dtype (COMPLEX128 > COMPLEX64 > DOUBLE priority).
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto conjOp = op::Conj("conj");

    std::vector<int64_t> xShape = {2, 3};

    // input
    auto placeholder = op::Data("placeholder0").set_attr_index(0);
    TensorDesc placeholder_desc = TensorDesc(ge::Shape(xShape), FORMAT_ND, inDtype);
    placeholder_desc.SetPlacement(ge::kPlacementHost);
    placeholder_desc.SetFormat(FORMAT_ND);
    Tensor tensor_placeholder;
    ret = GenComplexData(xShape, tensor_placeholder, placeholder_desc);
    if (ret != SUCCESS) {
        printf("%s - ERROR - [XIR]: Generate input data failed\n", GetTime().c_str());
        return FAILED;
    }
    placeholder.update_input_desc_x(placeholder_desc);
    input.push_back(tensor_placeholder);
    graph.AddOp(placeholder);
    conjOp.set_input_input(placeholder);
    conjOp.update_input_desc_input(placeholder_desc);
    inputs.push_back(placeholder);

    // output
    TensorDesc output_desc = TensorDesc(ge::Shape(xShape), FORMAT_ND, inDtype);
    conjOp.update_output_desc_output(output_desc);
...
void ProcessIOData(std::vector<ge::Tensor>& input, std::vector<ge::Tensor>& output)
{
    ...
    for (int i = 0; i < input_num; i++) {
        std::cout << "input " << i << " dtype :  " << DataTypeToString(input[i].GetTensorDesc().GetDataType())
                  << std::endl;
        string input_file = "./tc_ge_irrun_conj_input_" + std::to_string(i) + ".bin";
    ...
    for (int i = 0; i < output_num; i++) {
        std::cout << "output " << i << " dtype :  " << DataTypeToString(output[i].GetTensorDesc().GetDataType())
                  << std::endl;
        string output_file = "./tc_ge_irrun_conj_output_" + std::to_string(i) + ".bin";
...
{
    // 1、创建图对象
    const char* graph_name = "tc_ge_irrun_conj_test";
