...
static aclnnStatus CastInput(const aclTensor* input, op::DataType promoteType, aclOpExecutor* executor,
...
static aclnnStatus SelectByNC(bool ncSupported, const aclTensor* original, aclOpExecutor* executor,
                              const aclTensor*& processed)
{
    if (ncSupported) {
...
static aclnnStatus PrepareBinaryOpInputs(const aclTensor* self, const aclTensor* other, const op::Shape& outputShape,
                                         op::DataType promoteType, aclOpExecutor* executor,
                                         const aclTensor*& selfProcessed, const aclTensor*& otherProcessed)
{
    bool selfNeedsCast = self->GetDataType() != promoteType;
    bool otherNeedsCast = other->GetDataType() != promoteType;

    if (selfNeedsCast) {
        auto ret = CastInput(self, promoteType, executor, selfProcessed);
        CHECK_RET(ret == ACLNN_SUCCESS, ret);
    }
    if (otherNeedsCast) {
        auto ret = CastInput(other, promoteType, executor, otherProcessed);
        CHECK_RET(ret == ACLNN_SUCCESS, ret);
    }
    ...
    bool ncSupported = l0op::IsGreaterEqualSupportNonContiguous(selfForCheck, otherForCheck, outputShape);

    if (!selfNeedsCast) {
        auto ret = SelectByNC(ncSupported, self, executor, selfProcessed);
        CHECK_RET(ret == ACLNN_SUCCESS, ret);
    }
    if (!otherNeedsCast) {
        auto ret = SelectByNC(ncSupported, other, executor, otherProcessed);
        CHECK_RET(ret == ACLNN_SUCCESS, ret);
    }
                                       ...
                                       uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    ret = PrepareBinaryOpInputs(self, other, out->GetViewShape(), promoteType, uniqueExecutor.get(), selfProcessed,
                                otherProcessed);
    CHECK_RET(ret == ACLNN_SUCCESS, ret);
...
aclnnStatus aclnnInplaceGeTensorGetWorkspaceSize(aclTensor* selfRef, const aclTensor* other, uint64_t* workspaceSize,
