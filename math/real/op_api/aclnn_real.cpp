...
// Multi-SOC dtype support matrix（在本算子各 SOC 间一致性约束）：
// - ascend950 (regbase / arch35): AICORE 走本仓 op_kernel/arch35/real_apt（FLOAT/FLOAT16/COMPLEX32/COMPLEX64）。
// - ascend910B / ascend910_93: AICORE 同上；COMPLEX128 通过 RealAiCpu 走 canndev AICPU 实现（DOUBLE 输出）。
// - 其它 (legacy 910): 全部走 RealAiCpu（COMPLEX64/COMPLEX128）。
// op_def 仅注册 ascend950 是因本仓 AICORE 实现仅落 950；其它 SOC 的 AICORE 路径由 ASCEND910B_DTYPE_SUPPORT_LIST 校验
// 通过后由 ADD_TO_LAUNCHER_LIST_AICPU 走 canndev Real proto，AICPU 注册不在本仓。
...
// Generated By CANNBot: ascend950 / arch35 (regbase). COMPLEX128 不在本 SOC 支持表，
// 由 CheckDtypeValid 在 L2 入口拒绝，与 op_def AICore 配置一致，避免落到不存在的 AICPU 实现。
...
static const std::initializer_list<DataType> OTHER_DTYPE_SUPPORT_LIST = {DataType::DT_COMPLEX64,
...
static bool CheckFormat(const aclTensor* self, const aclTensor* out)
{
    ...
    OP_CHECK(!::IsPrivateFormat(self->GetStorageFormat()),
             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Real does not support private input format on RegBase."), return false);
    OP_CHECK(!::IsPrivateFormat(out->GetStorageFormat()),
             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Real does not support private output format on RegBase."), return false);
    return true;
}
...
aclnnStatus aclnnRealGetWorkspaceSize(const aclTensor* self, aclTensor* out, uint64_t* workspaceSize,
