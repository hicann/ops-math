 ...
 */
template <typename T, bool largest>
__aicore__ inline void RadixTopKWs<T, largest>::ClearTileHistInWs(
    const uint32_t &dstOffset, const uint32_t &dataNum)
{
    ...
    if (remain > 0) {
        ...
        DataCopyExtParams tileHistOutParams{
            static_cast<uint16_t>(1), static_cast<uint32_t>(sizeof(int32_t) * remain), 0, 0, 0};
        DataCopyPad(tileHistGm_[dstOffset + repeatTimes * MAX_TILE_NUM_IN_UB],
                    tileHist, tileHistOutParams);
    }
}
 ...
 */
template <typename T, bool largest>
__aicore__ inline void RadixTopKWs<T, largest>::CopyTileHistWs2Ub(
    const LocalTensor<int32_t> &dstTensor,
    const uint32_t &srcOffset, const uint32_t &dataNum)
{
    DataCopyExtParams tileHistInParams{
 ...
 */
template <typename T, bool largest>
__aicore__ inline void RadixTopKWs<T, largest>::CopyTileTopKWs2Ub(
    const LocalTensor<int32_t> &dstTensor,
    const uint32_t &srcOffset, const uint32_t &dataNum)
{
    DataCopyExtParams tileTopKInParams{
        static_cast<uint16_t>(1), static_cast<uint32_t>(sizeof(int32_t) * dataNum), 0, 0, 0};
    DataCopyPadExtParams<int32_t> padParams{true, 0, 0, 0};
 ...
 */
template <typename T, bool largest>
__aicore__ inline void RadixTopKWs<T, largest>::CopyTileTopKUb2Ws(
    const LocalTensor<int32_t> &srcTensor,
    const uint32_t &dstOffset, const uint32_t &dataNum)
{
    DataCopyExtParams tileTopKOutParams{
        static_cast<uint16_t>(1), static_cast<uint32_t>(sizeof(int32_t) * dataNum), 0, 0, 0};
 ...
 */
template <typename T, bool largest>
__aicore__ inline void RadixTopKWs<T, largest>::CopyTileHistInWs(
 ...
 */
template <typename T, bool largest>
__aicore__ inline void RadixTopKWs<T, largest>::SubTileHistWs2Ub(
    const LocalTensor<int32_t> &dstTensor,
    const LocalTensor<int32_t> &srcTensor,
    ...
    const uint32_t &dataNum)
{
    ...
    DataCopyPad(srcTensor[MAX_TILE_NUM_IN_UB_BY2], tileHistGm_[srcOffset1],
                tileHistInParams, padParams);
    MTE2ToVSync();
 ...
 */
template <typename T, bool largest>
__aicore__ inline void RadixTopKWs<T, largest>::SubTileHistInWs(
    const uint32_t &dstOffset, const uint32_t &srcOffset0,
 ...
 */
template <typename T, bool largest>
__aicore__ inline void RadixTopKWs<T, largest>::SubTileHistInWsAll(
    const uint32_t &dstOffset, const uint32_t &srcOffset0, const uint32_t &srcOffset1)
{
 ...
 */
template <typename T, bool largest>
__aicore__ inline void RadixTopKWs<T, largest>::AddTileHist2TileTopKInWs(
    const uint64_t &tileHistOffset)
{
    LocalTensor<int32_t> tileHist = tileHistAndTopKBuf_.Get<int32_t>();
...
__aicore__ inline void RadixTopKWs<T, largest>::ClearHistInWs(const int32_t &roundId)
{
    ...
    if (roundId == this->round_ - 1) {
        ...
        if (tileNumRemain_ > 0) {
            ...
            DataCopyPad(tileHistGm_[tileNumRepeatTimes_ * MAX_TILE_NUM_IN_UB],
                        tileHist, tileHistOutParams);
        }
        this->globalHistBoundaryNum_ =
            (this->tileNum_ - 1) * this->tileLen_ + this->tailTileLen_;
    } else {
        if (this->boundaryBin < this->numValue_ - 1) {
            SubTileHistInWsAll(0, this->boundaryBin * this->tileNum_,
                this->boundaryBinPrev * this->tileNum_);
        } else {
