...
// AICORE算子kernel
static const aclTensor *LogicalAndAiCore(const aclTensor *self, const aclTensor *other, aclTensor *logical_andOut,
  ...
  OP_CHECK(ret == ACLNN_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "LogicalAndAiCore ADD_TO_LAUNCHER_LIST_AICORE failed."), return nullptr);
  return logical_andOut;
}
 
// AICPU算子kernel
static const aclTensor *LogicalAndAiCpu(const aclTensor *self, const aclTensor *other, aclTensor *logical_andOut,
                                        aclOpExecutor *executor) {
  ...
  auto ret = ADD_TO_LAUNCHER_LIST_AICPU(LogicalAnd, OP_ATTR_NAMES(), OP_INPUT(self, other), OP_OUTPUT(logical_andOut));
  OP_CHECK(ret == ACLNN_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "LogicalAndAiCpu ADD_TO_LAUNCHER_LIST_AICPU failed."), return nullptr);
  return logical_andOut;
}
