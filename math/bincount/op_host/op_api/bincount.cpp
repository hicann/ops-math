...
// AICPU算子kernel
static const aclTensor *BincountAiCpu(const aclTensor *x, const aclTensor *size, const aclTensor *weights,
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICPU(Bincount,
                                          OP_ATTR_NAMES({"T", "T"}),
                                          OP_INPUT(x, size, weights),
                                          OP_OUTPUT(out),
                                          OP_ATTR(weights->GetDataType(), out->GetDataType()));
    OP_CHECK(ret ==  ACL_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "BincountAiCpu ADD_TO_LAUNCHER_LIST_AICPU failed."), return nullptr);
    return out;
}

// AICORE算子kernel
static const aclTensor *BincountAiCore(const aclTensor *x, const aclTensor *size, const aclTensor *weights,
                                      aclTensor *bins, aclOpExecutor *executor) {
    L0_DFX(BincountAiCore, x, size, weights, bins);

    auto ret = ADD_TO_LAUNCHER_LIST_AICORE(Bincount,
                                          OP_INPUT(x, size, weights),
                                          OP_OUTPUT(bins));
    OP_CHECK(ret == ACL_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "BincountAiCore ADD_TO_LAUNCHER_LIST_AICORE failed."), return nullptr);
    return bins;
}
...
const aclTensor* Bincount(const aclTensor* x, const aclTensor* weights, int64_t size, aclOpExecutor* executor) {
    ...
    auto out = executor->AllocTensor(outShape, outShape, weights->GetDataType(),
