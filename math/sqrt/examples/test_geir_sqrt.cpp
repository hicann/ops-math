...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape)                                                     \
    ...
    auto placeholder##inputIndex =                                                                                   \
        op::Data(std::string("placeholder") + std::to_string(inputIndex)).set_attr_index(0);                        \
    ...
    sqrt1.set_input_##inputName(placeholder##inputIndex);                                                            \
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    ...
    sqrt1.update_output_desc_##outputName(outputName##outputIndex##_desc)
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto sqrt1 = op::Sqrt("sqrt");
    vector<vector<int64_t>> shapes = {{4, 4}, {4, 4}};

    ADD_INPUT(1, x, inDtype, shapes[0]);
    ADD_OUTPUT(1, y, inDtype, shapes[1]);

    outputs.push_back(sqrt1);
    // 添加完毕
    return SUCCESS;
}
...
bool CreateAndConfigGraph(Graph& graph, std::vector<ge::Tensor>& input)
{
    ...
    // Use DT_COMPLEX64 to dispatch to AICPU kernel
    // (TBE Sqrt only supports real types on ascend910b)
...
void ProcessInputData(std::vector<ge::Tensor>& input)
{
    ...
    for (int i = 0; i < input_num; i++) {
        ...
        string input_file = "./tc_ge_irrun_sqrt_input_" + std::to_string(i) + ".bin";
...
void ProcessOutputData(std::vector<ge::Tensor>& output)
{
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        string output_file = "./tc_ge_irrun_sqrt_output_" + std::to_string(i) + ".bin";
        uint8_t* output_data_i = output[i].GetData();
        int64_t output_shape = output[i].GetTensorDesc().GetShape().GetShapeSize();
        std::cout << "this is " << i << "th output, output shape size =" << output_shape << std::endl;
        uint32_t data_size = output_shape * GetDataTypeSize(output[i].GetTensorDesc().GetDataType());
        WriteDataToFile((const char*)output_file.c_str(), data_size, output_data_i);
        // complex64: each element = (float real, float imag)
...
int main(int argc, char* argv[])
{
    ...
    // 创建计算图
    const char* graph_name = "tc_ge_irrun_sqrt_test";
