...
int main()
{
    ...
    // 调用aclnnSqrt第一段接口
    ret = aclnnSqrtGetWorkspaceSize(self.get(), out.get(), &workspaceSize, &executor);
    ...
    // 调用aclnnSqrt第二段接口
    ret = aclnnSqrt(workspaceAddr.get(), workspaceSize, executor, stream.get());
    ...
    ret = aclnnInplaceSqrtGetWorkspaceSize(self.get(), &inplaceWorkspaceSize, &inplaceExecutor);
    ...
    ret = aclnnInplaceSqrt(inplaceWorkspaceAddr.get(), inplaceWorkspaceSize, inplaceExecutor, stream.get());
