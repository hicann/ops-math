...
#define TILE_COMPUTE_DIM2(input_x_data, shape_x, output_data, shape_output)                               \
    do {                                                                                                  \
        ...
        EigenTensorNd output_nd((output_data), (shape_output).at(0), (shape_output).at(1));               \
...
#define TILE_COMPUTE_DIM3(input_x_data, shape_x, output_data, shape_output)                                       \
    do {                                                                                                          \
        ...
        EigenTensorNd input_nd((input_x_data), (shape_x).at(0), (shape_x).at(1), (shape_x).at(2));                \
        EigenTensorNd output_nd((output_data), (shape_output).at(0), (shape_output).at(1), (shape_output).at(2)); \
...
#define TILE_COMPUTE_DIM4(input_x_data, shape_x, output_data, shape_output)                                          \
    do {                                                                                                             \
        ...
        EigenTensorNd input_nd((input_x_data), (shape_x).at(0), (shape_x).at(1), (shape_x).at(kDim2),                \
                               (shape_x).at(kDim3));                                                                 \
        EigenTensorNd output_nd((output_data), (shape_output).at(0), (shape_output).at(1), (shape_output).at(kDim2), \
...
#define TILE_COMPUTE_DIM5(input_x_data, shape_x, output_data, shape_output)                                          \
    do {                                                                                                             \
        ...
        EigenTensorNd output_nd((output_data), (shape_output).at(0), (shape_output).at(1), (shape_output).at(kDim2), \
                                (shape_output).at(kDim3), (shape_output).at(kDim4));                                 \
...
#define TILE_COMPUTE_DIM6(input_x_data, shape_x, output_data, shape_output)                                          \
    do {                                                                                                             \
        ...
        EigenTensorNd output_nd((output_data), (shape_output).at(0), (shape_output).at(1), (shape_output).at(kDim2), \
                                (shape_output).at(kDim3), (shape_output).at(kDim4), (shape_output).at(kDim5));       \
...
#define TILE_COMPUTE_DIM8(input_x_data, shape_x, output_data, shape_output)                                          \
    do {                                                                                                             \
        typedef Eigen::TensorMap<Eigen::Tensor<T, kDim8, Eigen::RowMajor>, Eigen::Aligned> EigenTensorNd;            \
        EigenTensorNd input_nd((input_x_data), (shape_x).at(0), (shape_x).at(1), (shape_x).at(kDim2),                \
                               (shape_x).at(kDim3), (shape_x).at(kDim4), (shape_x).at(kDim5), (shape_x).at(kDim6),   \
                               (shape_x).at(kDim7));                                                                 \
        EigenTensorNd output_nd((output_data), (shape_output).at(0), (shape_output).at(1), (shape_output).at(kDim2), \
...
namespace aicpu {
uint32_t TileCpuKernel::TileComputeUsingMemcpy(void* dst_addr, void* src_addr, size_t copy_len) const
{
...
uint32_t TileCpuKernel::TileComputeUsingSdma(void* dst_addr, void* src_addr, size_t copy_len) const
{
    auto ret = halSdmaCopy(reinterpret_cast<DVdeviceptr>(dst_addr), copy_len, reinterpret_cast<DVdeviceptr>(src_addr),
                           copy_len);
    if (ret != 0) {
...
uint32_t TileCpuKernel::TileComputeWith3DNotUsingEigen(const CpuKernelContext& ctx)
{
    ...
    KERNEL_CHECK_FALSE((output_data_size >= static_cast<uint64_t>(x_third_dim * sizeof(T))), KERNEL_STATUS_INNER_ERROR,
                       "memcpy size=[%ld] should less or equal to output data size=[%lu]", x_third_dim * sizeof(T),
                       output_data_size);
    KERNEL_HANDLE_ERROR(TileCompute3DSharderFirst<T>(ctx, input_x_data, output_data, x_first_dim, x_second_dim,
                                                     x_third_dim, last_axes_dims, second_axes_dims));
...
uint32_t TileCpuKernel::TileKernelCompute(const CpuKernelContext& ctx)
{
    ...
    switch (input_rank) {
        ...
        case kDim3:
            ...
            break;
        case kDim4:
            TILE_COMPUTE_DIM4(input_x_data, input_x_dims, output_data, output_dims);
            break;
        case kDim5:
            TILE_COMPUTE_DIM5(input_x_data, input_x_dims, output_data, output_dims);
            break;
        case kDim6:
            TILE_COMPUTE_DIM6(input_x_data, input_x_dims, output_data, output_dims);
            break;
        case kDim7:
            TILE_COMPUTE_DIM7(input_x_data, input_x_dims, output_data, output_dims);
            break;
        case kDim8:
            TILE_COMPUTE_DIM8(input_x_data, input_x_dims, output_data, output_dims);
            break;
        default:
...
uint32_t TileCpuKernel::TileParamCheck(const CpuKernelContext& ctx)
{
    ...
    KERNEL_CHECK_FALSE((multiples_tensor->NumElements() == (unsigned int)input_x_dims.size()),
                       ...
                       "got [%ld].",
                       input_x_dims.size(), multiples_tensor->NumElements())
    KERNEL_CHECK_FALSE((input_x_dims.size() >= 1), KERNEL_STATUS_PARAM_INVALID,
                       ...
                       "1-D, but got [%zu].",
                       input_x_dims.size())
    KERNEL_CHECK_FALSE((GetMultiplesValue(multiples_tensor, multiples_) == KERNEL_STATUS_OK),
                       KERNEL_STATUS_PARAM_INVALID,
...
{
    KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum), "Tile NormalCheck fail.");
...
template <typename T>
uint32_t TileCpuKernel::TileCompute3DSharderSecond(const CpuKernelContext& ctx, T* output_data, int64_t x_first_dim,
...
template <typename T>
uint32_t TileCpuKernel::TileCompute3DSharderThird(const CpuKernelContext& ctx, T* output_data, int64_t x_first_dim,
