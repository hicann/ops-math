...
ge::graphStatus TileInferShapeCommon(gert::InferShapeContext* context, const T* multiples_data, size_t multiples_len)
{
    ...
    if (in_shape_len < multiples_len) {
        ...
        OP_LOGI(context->GetNodeName(), "The tile multiples len is less than or equal to the input len.");
    ...
    for (uint64_t i = 0; i < in_shape_len; i++) {
        if (in_shape_new[i] >= 0) {
            ...
            OP_LOGE(context->GetNodeName(), "Runtime infershape illegal input dim:%lu, value is %ld", i,
                    in_shape_new[i]);
            return ge::GRAPH_FAILED;
        }
    }
