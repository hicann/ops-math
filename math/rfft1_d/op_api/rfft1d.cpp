...
bool IsRfft1DAiCoreSupported(const aclTensor* self, int64_t n)
{
    ...
    if ((GetCurrentPlatformInfo().GetSocVersion() == SocVersion::ASCEND910B || IsRegBase() ||
                                     ...
                                     int64_t norm, aclOpExecutor* executor)
{
    ...
    ADD_TO_LAUNCHER_LIST_AICORE(Rfft1D, OP_ATTR_NAMES({"n", "norm"}), OP_INPUT(self, dft), OP_OUTPUT(out),
                                OP_ATTR(n, norm));
    return out;
}
                                    ...
                                    int64_t norm, aclOpExecutor* executor)
{
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICPU(Rfft1D, OP_ATTR_NAMES({"n", "norm"}), OP_INPUT(self, dft), OP_OUTPUT(out),
                                          OP_ATTR(n, norm));
