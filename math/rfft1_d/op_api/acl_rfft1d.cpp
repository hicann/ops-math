...
{
    if (IsRegBase() || GetCurrentPlatformInfo().GetSocVersion() == SocVersion::ASCEND910_93 ||
...
static bool CheckParamsValid(const aclTensor* self, int64_t n, int64_t dim, int64_t norm, int64_t dims)
{
    ...
    if (!((dim >= -dims) && (dim < dims))) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "'dim' out of range (expected to be in range of [%ld, %ld], but got %ld",
                -dims, dims - 1, dim);
        return false;
    }
    return true;
...
static aclnnStatus CheckParams(const aclTensor* self, int64_t n, int64_t dim, int64_t norm, int64_t dims)
{
...
static void CalculateBeta(std::complex<double>& b, std::vector<double>& betaTmp, std::vector<double>& betaReverseReal,
                          std::vector<double>& betaReverseImag, std::vector<double>& betaConj,
                          std::vector<double>& betaConjImag, const int64_t len, const uint64_t lenPow2,
...
static std::vector<double> GenerateAlphaBeta(const int64_t& len, const uint64_t& lenPow2)
{
    ...
    }
    {
        ...
        ret.insert(ret.end(), std::make_move_iterator(betaConjImag.begin()),
                   std::make_move_iterator(betaConjImag.end()));
...
static void CalculateIntermediateFactors(std::vector<uint32_t>& interFactors, std::vector<uint32_t> availableFactors,
                                         uint32_t tmpN, int curFactorsIndex)
{
...
static void CalculationDft(std::vector<double>& dftRealCurVal, std::vector<double>& dftImagCurVal,
                           ...
                           size_t curIndex, bool isBluestein, size_t colsNum, size_t curFactor, const size_t& i,
                           size_t& k)
{
...
static void CalculationMatricesValues(std::vector<double>& dftRealVal, std::vector<double>& dftImagVal,
                                      ...
                                      std::vector<double>& twiddleImagCurVal,
                                      std::vector<double>& twiddleImagBackCurVal, int64_t len, int64_t norm,
                                      size_t curIndex, bool isBluestein, size_t rowsNum, size_t colsNum,
                                      size_t curFactor, size_t twiddleCurSize, size_t prevFactors)
{
    size_t k = 0, l = 0;
    double normParam = 1.;
    if (norm != BACKWARD) {
    ...
    for (size_t i = 0; i < rowsNum / (1 + int(curIndex == 0 && isBluestein)); ++i) {
        CalculationDft(dftRealCurVal, dftImagCurVal, dftRealBackCurVal, dftImagBackCurVal, curIndex, isBluestein,
                       colsNum, curFactor, i, k);
    ...
    for (size_t curIndex = 0; curIndex < MAX_FACTORS_LEN; ++curIndex) {
        ...
        if (curFactor != 1) {
            CalculationMatricesValues(dftRealVal, dftImagVal, dftRealBackVal, dftImagBackVal, twiddleRealVal,
                                      twiddleImagVal, twiddleImagBackVal, dftRealCurVal, dftImagCurVal,
                                      dftRealBackCurVal, dftImagBackCurVal, twiddleRealCurVal, twiddleImagCurVal,
                                      twiddleImagBackCurVal, len, norm, curIndex, isBluestein, rowsNum, colsNum,
                                      curFactor, twiddleCurSize, prevFactors);
        }
...
static const aclTensor* GeneratePadInput(const aclTensor* self, int64_t n, int64_t lastDimLength, int64_t batches,
...
static const aclTensor* InputProcessing(const aclTensor* selfContiguous, int64_t n, int64_t dims,
                                        aclOpExecutor* executor)
{
