...
{
    // check params
    KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum), "InvGrad check input and output number failed.");
    ...
    switch (data_type) {
        ...
        case (DT_COMPLEX128):
            ...
            break;
        default:
            KERNEL_LOG_ERROR("InvGrad kernel data type [%s] not support.", DTypeStr(data_type).c_str());
...
uint32_t InvGradCpuKernel::InvGradParamCheck(const CpuKernelContext& ctx) const
{
    ...
    KERNEL_LOG_DEBUG("InvGradCpuKernel[%s], input0: size[%lu];"
...
template <typename T>
void InvGradCpuKernel::SpecialCompute(int64_t start, int64_t end, const T* input1, const T* input2, T* output) const
{
...
uint32_t InvGradCpuKernel::NoBcastCompute(const CpuKernelContext& ctx) const
{
    ...
    if (data_num >= kParallelDataNumSameShape) {
        ...
        KERNEL_HANDLE_ERROR(CpuKernelUtils::ParallelFor(ctx, data_num, data_num / max_core_num, sharder_invgrad),
...
OPS_MATH_REGISTER_CPU_KERNELV2(kInvGrad, InvGradCpuKernel);
} // namespace aicpu
