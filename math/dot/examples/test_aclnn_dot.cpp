...
int main()
{
    ...
    DeviceMemPtr tensorDeviceAddr(nullptr, &aclrtFree);
    DeviceMemPtr outDeviceAddr(nullptr, &aclrtFree);
    TensorPtr self(nullptr, &aclDestroyTensor);
    TensorPtr tensor(nullptr, &aclDestroyTensor);
    ...
    // 创建tensor aclTensor
    ret = CreateAclTensor(tensorHostData, tensorShape, aclDataType::ACL_FLOAT, tensorDeviceAddr, tensor);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建out aclTensor
    ...
    // 调用aclnnDot第一段接口
    ret = aclnnDotGetWorkspaceSize(self.get(), tensor.get(), out.get(), &workspaceSize, &executor);
    ...
    // 调用aclnnDot第二段接口
    ret = aclnnDot(workspaceAddr.get(), workspaceSize, executor, stream.get());
