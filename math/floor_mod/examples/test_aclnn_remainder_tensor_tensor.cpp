...
int main()
{
    ...
    // 创建other aclTensor
    ret = CreateAclTensor(otherHostData, otherShape, aclDataType::ACL_INT64, otherDeviceAddr, other);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建out aclTensor
    ...
    ret = aclnnRemainderTensorTensorGetWorkspaceSize(self.get(), other.get(), out.get(), &workspaceSize, &executor);
    ...
    ret = aclnnRemainderTensorTensor(workspaceAddr.get(), workspaceSize, executor, stream.get());
