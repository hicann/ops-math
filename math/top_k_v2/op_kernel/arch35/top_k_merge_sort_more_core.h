...
/*!
 * \file top_k_merge_sort_more_core.h
 * \brief topk merge sort more-core kernel entry
 */
#ifndef TOP_K_MERGE_SORT_MORE_CORE_H
#define TOP_K_MERGE_SORT_MORE_CORE_H
#include <cmath>
...
constexpr uint32_t MORE_CORE_FP32_DTYPE = 4;
constexpr uint32_t MORE_CORE_DOUBLE_BUFFER = 1;
constexpr uint32_t MORE_CORE_BLOCK_UB = Ops::Base::GetUbBlockSize();
constexpr uint32_t MORE_CORE_MERGE_SORT_LIST_MAX_NUM = 4;
constexpr uint32_t MORE_CORE_DEALING_CONCAT_NUM_ONCE = 16;
constexpr uint32_t MORE_CORE_DEALING_SORT_NUM_ONCE = 32;
constexpr uint32_t MORE_CORE_DEALING_EXTRACT_NUM_ONCE = 32;
constexpr int32_t MORE_CORE_XOR_OP_VALUE_FP = 0x80000000;
constexpr int16_t MORE_CORE_XOR_OP_VALUE_HALF = 0x8000;
...
public:
    ...
    TQue<QuePosition::VECOUT, MORE_CORE_DOUBLE_BUFFER> outIndexQueue_;
    TQue<QuePosition::VECOUT, MORE_CORE_DOUBLE_BUFFER> sortedQueue_;
    TQue<QuePosition::VECIN, MORE_CORE_DOUBLE_BUFFER> copyInQueue_;
    TQue<QuePosition::VECOUT, MORE_CORE_DOUBLE_BUFFER> castValueQueue_;
    TQue<QuePosition::VECOUT, MORE_CORE_DOUBLE_BUFFER> castIndexQueue_;
    // input value
    ...
    uint32_t inputLastDimValue_ = 0;
    uint32_t outputTopKValue_ = 0;
    uint32_t frontCoreNum_ = 0;
    uint32_t rowIdx_ = 0;
    uint32_t rowCoreIdx_ = 0;
    uint32_t vfLenFp32_ = Ops::Base::GetVRegSize() / MORE_CORE_FP32_DTYPE;
    int64_t rowDataOffset_ = 0;
    int64_t rowWorkspaceOffset_ = 0;
    int64_t rowOutputOffset_ = 0;
...
__aicore__ inline int64_t Align(int64_t elementNum, int64_t bytes)
{
    ...
    return (elementNum * bytes + MORE_CORE_BLOCK_UB - 1) / MORE_CORE_BLOCK_UB * MORE_CORE_BLOCK_UB / bytes;
}
    ...
    GM_ADDR value, GM_ADDR indices, GM_ADDR workSpace, const TopKV2TilingDataSimd* tilingData, TPipe* pipe)
{
    ...
    pipe_ = pipe;
    inputLastDimValue_ = static_cast<uint32_t>(tilingData->lastAxisNum);
    outputTopKValue_ = static_cast<uint32_t>(tilingData->topKRealValue);
    ...
    rowDataOffset_ = static_cast<int64_t>(rowIdx_) * static_cast<int64_t>(inputLastDimValue_);
    ...
    pipe_->InitBuffer(inputQueue_, MORE_CORE_DOUBLE_BUFFER, alignTile * sizeof(T));
...
template <typename T, typename CONVERT_TYPE, bool IS_DESCEND, typename INDEX_TYPE>
__aicore__ inline void TopKMergeSortMoreCore<T, CONVERT_TYPE, IS_DESCEND, INDEX_TYPE>::Process()
{
    ...
    if (rowCoreIdx_ < frontCoreNum_ - 1) {
        ...
        uint32_t tailNum = inputLastDimValue_ - numTileData_ * (frontCoreNum_ - 1);
        offsetPerCore = numTileData_ * rowCoreIdx_;
    ...
    while(listNum_ > MORE_CORE_MERGE_SORT_LIST_MAX_NUM) {
        ...
        SortInMultiCore();
        currentCoreNum = (listNum_ + MORE_CORE_MERGE_SORT_LIST_MAX_NUM - 1) / MORE_CORE_MERGE_SORT_LIST_MAX_NUM;
        remainListNum = listNum_ - (currentCoreNum - 1) * MORE_CORE_MERGE_SORT_LIST_MAX_NUM;
        currentTailElements_ = currentElements_ * (remainListNum - 1) + currentTailElements_;
        listNum_ = currentCoreNum;
        currentElements_ = currentElements_ * MORE_CORE_MERGE_SORT_LIST_MAX_NUM;
...
template <typename T, typename CONVERT_TYPE, bool IS_DESCEND, typename INDEX_TYPE>
__aicore__ inline void TopKMergeSortMoreCore<T, CONVERT_TYPE, IS_DESCEND, INDEX_TYPE>::SortInSingleCore(
    uint32_t tileNum, int64_t offsetPerCore)
{
    ...
    int64_t offsetPerCore)
{
    ...
    uint32_t alignTile = (tileNum + MORE_CORE_BLOCK_UB - 1) / MORE_CORE_BLOCK_UB * MORE_CORE_BLOCK_UB;
...
template <typename T, typename CONVERT_TYPE, bool IS_DESCEND, typename INDEX_TYPE>
__aicore__ inline void TopKMergeSortMoreCore<T, CONVERT_TYPE, IS_DESCEND, INDEX_TYPE>::FlipSignBit(
    LocalTensor<CONVERT_TYPE> xLocal, uint32_t aglinTileNum)
{
...
template <typename T, typename CONVERT_TYPE, bool IS_DESCEND, typename INDEX_TYPE>
__aicore__ inline void TopKMergeSortMoreCore<T, CONVERT_TYPE, IS_DESCEND, INDEX_TYPE>::CopyOutWorkSpace(
...
template <typename T, typename CONVERT_TYPE, bool IS_DESCEND, typename INDEX_TYPE>
__aicore__ inline void TopKMergeSortMoreCore<T, CONVERT_TYPE, IS_DESCEND, INDEX_TYPE>::CopyOutMultiCore()
{
...
{
    uint32_t needCoreNum = (listNum_ + MORE_CORE_MERGE_SORT_LIST_MAX_NUM - 1) / MORE_CORE_MERGE_SORT_LIST_MAX_NUM;
...
template <typename T, typename CONVERT_TYPE, bool IS_DESCEND, typename INDEX_TYPE>
__aicore__ inline void TopKMergeSortMoreCore<T, CONVERT_TYPE, IS_DESCEND, INDEX_TYPE>::CopyInMultiCore()
{
    LocalTensor<CONVERT_TYPE> ubMainInput = copyInQueue_.AllocTensor<CONVERT_TYPE>();
    remainListNum_ = 0;
    for (int64_t i = 0, j = 0; i < MORE_CORE_MERGE_SORT_LIST_MAX_NUM; i++) {
...
template <typename T, typename CONVERT_TYPE, bool IS_DESCEND, typename INDEX_TYPE>
__aicore__ inline void TopKMergeSortMoreCore<T, CONVERT_TYPE, IS_DESCEND, INDEX_TYPE>::UpdateSortInfo()
{
    curLoopSortedNum_ = 0;
    for (int64_t i = 0, j = 0; i < MORE_CORE_MERGE_SORT_LIST_MAX_NUM; i++) {
        if (dealLengths_[i] > 0) {
...
template <typename T, typename CONVERT_TYPE, bool IS_DESCEND, typename INDEX_TYPE>
__aicore__ inline void TopKMergeSortMoreCore<T, CONVERT_TYPE, IS_DESCEND, INDEX_TYPE>::MultiCoreInit()
{
    outOffset_ = GetSortLen<CONVERT_TYPE>(rowCoreIdx_ * MORE_CORE_MERGE_SORT_LIST_MAX_NUM * currentElements_);
...
template <typename T, typename CONVERT_TYPE, bool IS_DESCEND, typename INDEX_TYPE>
__aicore__ inline void TopKMergeSortMoreCore<T, CONVERT_TYPE, IS_DESCEND, INDEX_TYPE>::UpdateMrgParam()
{
    if (remainListNum_ == 2) {
...
template <typename T, typename CONVERT_TYPE, bool IS_DESCEND, typename INDEX_TYPE>
__aicore__ inline void TopKMergeSortMoreCore<T, CONVERT_TYPE, IS_DESCEND, INDEX_TYPE>::FinalSortAndCopy()
{
    if (rowCoreIdx_ == 0) {
        FinalSortInit();
        for (; allRemainElements_ > 0 && outOffset_ < outputTopKValue_;) {
            CopyInMultiCore();
            UpdateMrgParam();
            DealingMergeSort();
            UpdateSortInfo();
...
template <typename T, typename CONVERT_TYPE, bool IS_DESCEND, typename INDEX_TYPE>
__aicore__ inline uint32_t TopKMergeSortMoreCore<T, CONVERT_TYPE, IS_DESCEND, INDEX_TYPE>::GetRemainingTopK() const
{
    if (outOffset_ >= static_cast<int64_t>(outputTopKValue_)) {
        return 0;
    }
    return static_cast<uint32_t>(static_cast<int64_t>(outputTopKValue_) - outOffset_);
}

template <typename T, typename CONVERT_TYPE, bool IS_DESCEND, typename INDEX_TYPE>
__aicore__ inline void TopKMergeSortMoreCore<T, CONVERT_TYPE, IS_DESCEND, INDEX_TYPE>::ExtractAndCopyOut()
{
    uint32_t copyCount = curLoopSortedNum_;
    uint32_t remainingTopK = GetRemainingTopK();
    if (remainingTopK == 0) {
        LocalTensor<CONVERT_TYPE> sortTempBufferDrop = sortedQueue_.DeQue<CONVERT_TYPE>();
        sortedQueue_.FreeTensor(sortTempBufferDrop);
        return;
    }
    if (copyCount > remainingTopK) {
        copyCount = remainingTopK;
    }
    ...
    if constexpr (std::is_same<bfloat16_t, T>::value) {
        ...
        DataCopyPad(outValueGm_[rowOutputOffset_ + outOffset_], ubOutput1, copyParamsValue);
        outValueQueue_.FreeTensor(ubOutput1);
    } else {
        castValueQueue_.EnQue(castValue);
        castValue = castValueQueue_.DeQue<T>();
        DataCopyPad(outValueGm_[rowOutputOffset_ + outOffset_], castValue, copyParamsValue);
    }
    ...
    if constexpr (std::is_same<int64_t, INDEX_TYPE>::value) {
        ...
        DataCopyPad(outIndexGm_[rowOutputOffset_ + outOffset_], ubOutput2, copyParamsIndex);
        ...
        DataCopyPad(outIndexGm_[rowOutputOffset_ + outOffset_], castIndexTemp, copyParamsIndex);
    }
    castIndexQueue_.FreeTensor(castIndex);
    sortedQueue_.FreeTensor(sortTempBuffer);
    outOffset_ += copyCount;
}

template <typename T, typename CONVERT_TYPE, bool IS_DESCEND, typename INDEX_TYPE>
__aicore__ inline void TopKMergeSortMoreCore<T, CONVERT_TYPE, IS_DESCEND, INDEX_TYPE>::FinalSortInit()
{
    outOffset_ = 0;
    for (int64_t i = 0; i < MORE_CORE_MERGE_SORT_LIST_MAX_NUM; i++) {
        if (i < listNum_ - 1) {
...
template <typename T, typename CONVERT_TYPE, bool IS_DESCEND, typename INDEX_TYPE>
__aicore__ inline void TopKMergeSortMoreCore<T, CONVERT_TYPE, IS_DESCEND, INDEX_TYPE>::ClearCache()
{
