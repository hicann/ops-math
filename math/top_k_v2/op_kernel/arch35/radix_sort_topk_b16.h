    ...
    UNSIGNED_TYPE involvedDataMask, uint16_t round, uint32_t numTileData)
{
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < repateTime; i++) {
            ...
            Reg::Compares<uint16_t, CMPMODE::EQ>(cmpValueMask, andValueZero, involvedDataMask, histMask);
            // update mask for next step calc
            Reg::MaskReg newHistMask;
            Reg::And(newHistMask, cmpValueMask, histMask, predicateDefaultB16);
            // is first round
            Reg::MaskReg finalMask, firstRoundMask;
            Reg::Compares<uint16_t, CMPMODE::EQ>(firstRoundMask, firstRoundVector, round, histMask);
            // mask sel
