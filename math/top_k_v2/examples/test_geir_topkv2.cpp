...
#include "../op_graph/top_k_v2_proto.h"
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape)                                                    \
    do {                                                                                                            \
        ...
        topKV2Ops.set_input_##inputName(placeholder##inputIndex);                                         \
...
#define ADD_CONST_INPUT(inputIndex, inputName, inputDtype, inputShape, value)                                \
    do {                                                                                                     \
        ...
        graph.AddOp(placeholder##inputIndex);                                                                \
        topKV2Ops.set_input_##inputName(placeholder##inputIndex);                                  \
        topKV2Ops.update_input_desc_##inputName(placeholder##inputIndex##_desc);                   \
        inputs.push_back(placeholder##inputIndex);                                                         \
    } while (0)
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                           \
    do {                                                                                                        \
        ...
        topKV2Ops.update_output_desc_##outputName(outputName##outputIndex##_desc);                    \
    } while (0)

#define ADD_INPUT_ATTR(attrName, attrValue) topKV2Ops.set_attr_##attrName(attrValue)
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto topKV2Ops = op::TopKV2("test_top_k_v2");
    // 输入shape
    ...
    ADD_INPUT(1, x, inDtype, dyShape);
    ADD_CONST_INPUT(2, k, constDtype, constShape, 4);
    // 添加必选属性
    ADD_INPUT_ATTR(sorted, true);
    ADD_INPUT_ATTR(dim, -1);
    ADD_INPUT_ATTR(largest, true);
    // 添加输出
    ADD_OUTPUT(1, values, inDtype, dyShape);
    ADD_OUTPUT(2, indices, indicesDtype, dyShape);

    outputs.push_back(topKV2Ops);
    // 添加完毕
    return SUCCESS;
}
