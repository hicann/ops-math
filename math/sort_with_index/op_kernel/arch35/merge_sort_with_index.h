...
struct MergeSortWithIndex : public MergeSort<T, CONVERT_TYPE, TILING_DATA_TYPE, IS_LARGEST, INDEX_TYPE> {
    ...
    __aicore__ inline void CopyDataOut4Int64Case(GlobalTensor<T> outValueGm, GlobalTensor<INDEX_TYPE> outIndexGm,
    ...
    const TILING_DATA_TYPE* tilingData, TPipe* pipe)
{
    ...
    // vbs init
    vbsSortMe.SetPipePtr(pipe);
    vbsSortMe.InitMergeSortBuffer(this->numTileData_, this->oneCoreRowNum_, this->mergSortAcApiNeedBufferSize_);
}
...
template <typename T, typename CONVERT_TYPE, typename TILING_DATA_TYPE, bool IS_LARGEST, typename INDEX_TYPE>
__aicore__ inline void MergeSortWithIndex<T, CONVERT_TYPE, TILING_DATA_TYPE, IS_LARGEST,
                                          INDEX_TYPE>::ProcessSingleBlockSort(GlobalTensor<T> inputX,
                                                                              GlobalTensor<INDEX_TYPE> presetIndices)
{
    uint32_t tileId = GetBlockIdx();
    uint32_t unsortedDimIndex = (GetBlockIdx() + this->sortLoopRound_ * this->unsortedDimParallel_) *
                                this->oneCoreRowNum_;
    ...
    if (SortIsSame<int32_t, INDEX_TYPE>::value) {
        // int32 index get buffer
        AscendC::LocalTensor<T> sortedValueLocal = this->outValueQueue_.template AllocTensor<T>();
        AscendC::LocalTensor<INDEX_TYPE> sortedValueIndexLocal = this->outIndexQueue_
                                                                     .template AllocTensor<INDEX_TYPE>();
        this->CopyDataIn(inputX, presetIndices, sortedValueIndexLocal, tileOffset, this->numTileData_,
                         nowCoreRealRowNum);
        ...
        if constexpr (SortIsSame<bfloat16_t, T>::value) {
        ...
        // copy result out
        uint64_t gmOffset = this->sortLoopRound_ * this->unsortedDimParallel_ * this->outputLastDimValue_ *
                            this->oneCoreRowNum_;
        ...
        this->RadixSortProcess4Index64Case(this->outValueGm_, this->outIndexGm_, tileOffset, this->numTileData_,
                                           nowCoreRealRowNum);
    }
}
...
__simt_vf__ LAUNCH_BOUND(THREAD_DIM_NUM) __aicore__
    void CopyResultToGm(uint64_t tileOffset, uint32_t currTileSize, uint32_t oneCoreRowNum, uint32_t aglinNum,
                        uint32_t loopOffset,
...
MergeSortWithIndex<T, CONVERT_TYPE, TILING_DATA_TYPE, IS_LARGEST, INDEX_TYPE>::RadixSortProcess4Index64Case(
    GlobalTensor<T> outValueGm, GlobalTensor<INDEX_TYPE> outIndexGm, uint64_t tileOffset, uint32_t currTileSize,
    uint32_t oneCoreRowNum)
{
    // 拷贝数据进来, 逐行处理
    ...
    for (uint32_t i = 0; i < oneCoreRowNum; i++) {
        ...
        if (IS_LARGEST) {
            AscendC::Sort<T, false, sortConfigDesend>(sortedValueLocal, tmpSortedIndexLocal,
                                                      inputValueLocal[i * aglinNum], shareTmpBuffer,
                                                      static_cast<uint32_t>(currTileSize));
        } else {
            AscendC::Sort<T, false, sortConfigAscend>(sortedValueLocal, tmpSortedIndexLocal,
                                                      inputValueLocal[i * aglinNum], shareTmpBuffer,
                                                      static_cast<uint32_t>(currTileSize));
        }
    ...
    Simt::VF_CALL<CopyResultToGm<T, INDEX_TYPE>>(
        Simt::Dim3(THREAD_DIM_NUM), tileOffset, currTileSize, oneCoreRowNum, aglinNum, loopOffset,
        (__ubuf__ T*)(inputValueLocal.GetPhyAddr()), (__ubuf__ INDEX_TYPE*)(inputIndexLocal.GetPhyAddr()),
        (__ubuf__ uint32_t*)(sortedIndexLocal.GetPhyAddr()), (__gm__ T*)(outValueGm.GetPhyAddr()),
        (__gm__ INDEX_TYPE*)(outIndexGm.GetPhyAddr()));
}
    ...
    uint64_t tileOffset, uint32_t currTileSize, uint32_t oneCoreRowNum)
{
    ...
    DataCopyPadExtParams<T> copyDataPadParams;
    copyDataPadParams.isPad = true;
    copyDataPadParams.rightPadding = currTileSizeAlign - currTileSize;
    copyDataPadParams.paddingValue = static_cast<T>(defaultValue);
    ...
    DataCopyPad(xLocal, inputX[tileOffset], dataCopyParam, copyDataPadParams);

    uint32_t currTileSizeAlign4Indice = ROUND_UP_AGLIN(currTileSize * sizeof(INDEX_TYPE)) / sizeof(INDEX_TYPE);
    uint32_t dstStride4Indice = ((aglinOneRowTileSize - currTileSizeAlign4Indice) * sizeof(INDEX_TYPE)) /
                                UB_AGLIN_VALUE;
...
template <typename T, typename CONVERT_TYPE, typename TILING_DATA_TYPE, bool IS_LARGEST, typename INDEX_TYPE>
__aicore__ inline void MergeSortWithIndex<T, CONVERT_TYPE, TILING_DATA_TYPE, IS_LARGEST,
                                          INDEX_TYPE>::CopyDataIn4Int64Case(GlobalTensor<T> inputX,
                                                                            GlobalTensor<INDEX_TYPE> presetIndices,
                                                                            uint64_t tileOffset, uint32_t currTileSize,
                                                                            uint32_t oneCoreRowNum)
{
    ...
    for (uint32_t i = 0; i < oneCoreRowNum; i++) {
        ...
        DataCopyPad(xLocal[i * currTileSizeAglin], inputX[tileOffset + i * currTileSize], dataCopyParam,
                    valuePadParams);
    }
    ...
    for (uint32_t i = 0; i < oneCoreRowNum; i++) {
        ...
        DataCopyPad(indexLocal[i * currTileSizeAglin], presetIndices[tileOffset + i * currTileSize], indexCopyParam,
                    indexPadParams);
    }
