...
struct MergeSort {
    ...
    __aicore__ inline void Init(GM_ADDR inputValue, GM_ADDR value, GM_ADDR indices, GM_ADDR workSpace,
                                const TILING_DATA_TYPE* mergeSortTilingData, TPipe* pipe);
...
public:
    ...
    // merge sort kernel
    MergeSortKernel<T, CONVERT_TYPE, IS_LARGEST> vbsSort;
};

template <typename T, typename CONVERT_TYPE, typename TILING_DATA_TYPE, bool IS_LARGEST, typename INDEX_TYPE>
__aicore__ inline void MergeSort<T, CONVERT_TYPE, TILING_DATA_TYPE, IS_LARGEST, INDEX_TYPE>::Init(
    GM_ADDR inputValue, GM_ADDR value, GM_ADDR indices, GM_ADDR workSpace, const TILING_DATA_TYPE* mergeSortTilingData,
    TPipe* pipe)
{
    InitTilingData(mergeSortTilingData);
...
{
    // vbs init
    vbsSort.SetPipePtr(pipe);
    vbsSort.InitMergeSortBuffer(numTileData_, oneCoreRowNum_, mergSortAcApiNeedBufferSize_);
}
    ...
    GlobalTensor<T> inputX)
{
    ...
    if constexpr (SortIsSame<int64_t, INDEX_TYPE>::value) {
    ...
    if constexpr (SortIsSame<bfloat16_t, T>::value) {
        vbsSort.DoMergeSortBf16(xLocal, sortedValueLocal, sortedValueIndexLocal, numTileData_, nowCoreRealRowNum);
    } else {
        vbsSort.DoMergeSort(xLocal, sortedValueLocal, sortedValueIndexLocal, numTileData_, nowCoreRealRowNum);
    }
    ...
    if constexpr (SortIsSame<int64_t, INDEX_TYPE>::value) {
        ...
        AscendC::Cast(sortedValueIndexInt64Local, sortedValueIndexInt32Local, AscendC::RoundMode::CAST_NONE,
                      nowCoreRealRowNum * ROUND_UP_AGLIN(outputLastDimValue_));
    ...
    GlobalTensor<T> inputX, uint64_t tileOffset, uint32_t currTileSize, uint32_t oneCoreRowNum)
{
    ...
    event_t copyDataInEventId = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::V_MTE2));
    SetFlag<HardEvent::V_MTE2>(copyDataInEventId);
    WaitFlag<HardEvent::V_MTE2>(copyDataInEventId);
    ...
    dataCopyPadExtParams.rightPadding = currTileSizeAlign - currTileSize;
    dataCopyPadExtParams.paddingValue = static_cast<T>(defaultValue);
    ...
    DataCopyPad(xLocal, inputX[tileOffset], dataCopyParam, dataCopyPadExtParams);
