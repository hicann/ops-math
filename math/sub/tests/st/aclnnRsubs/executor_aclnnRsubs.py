...
@register("function_aclnnRsubs")
class aclnnRsubsExecutor(BaseApi):
    ...
    def __call__(self, input_data: InputDataset, with_output: bool = False):
        ...
        if data_type == torch.bool:
            ...
            output = torch.from_numpy(np.logical_xor(x_other_np, np.multiply(x_input_np, x_alpha)))
        else:
            output = torch.sub(x_other, x_input, alpha=x_alpha)
        return output