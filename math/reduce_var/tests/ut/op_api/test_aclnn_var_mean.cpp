...
#include "math/reduce_var/op_api/aclnn_var_mean.h"
...
TEST_F(l2_var_mean_test, var_mean_correction_return_nan)
{
    auto selfDesc = TensorDesc({1, 1, 1, 1}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
    auto dimDesc = IntArrayDesc(vector<int64_t>{0, 1, 2, 3});
    int64_t correction = 1;
    bool keepdim = false;
    auto varOutDesc = TensorDesc({}, ACL_FLOAT, ACL_FORMAT_ND).Precision(0.0001, 0.0001);
...
// correction>1 && shapeProd<=correction 返回INF场景
TEST_F(l2_var_mean_test, var_mean_correction_return_inf)
{
    auto selfDesc = TensorDesc({1, 1, 1, 1}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
    auto dimDesc = IntArrayDesc(vector<int64_t>{0, 1, 2, 3});
    int64_t correction = 2;
...
// BF16数据类型测试
TEST_F(l2_var_mean_test, var_mean_dtype_bfloat16)
{
    auto selfDesc = TensorDesc({1, 2, 6, 4}, ACL_BF16, ACL_FORMAT_ND).ValueRange(-2, 2);
    auto dimDesc = IntArrayDesc(vector<int64_t>{1});
    int64_t correction = 1;
    bool keepdim = false;
    auto varOutDesc = TensorDesc({1, 6, 4}, ACL_BF16, ACL_FORMAT_ND).Precision(0.001, 0.001);
    auto meanOutDesc = TensorDesc({1, 6, 4}, ACL_BF16, ACL_FORMAT_ND).Precision(0.001, 0.001);
...
// dim为负数边界值-1
TEST_F(l2_var_mean_test, var_mean_dim_neg_one)
{
    auto selfDesc = TensorDesc({1, 2, 5, 4}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
    auto dimDesc = IntArrayDesc(vector<int64_t>{-1});
    int64_t correction = 1;
    bool keepdim = false;
    auto varOutDesc = TensorDesc({1, 2, 5}, ACL_FLOAT, ACL_FORMAT_ND).Precision(0.0001, 0.0001);
    auto meanOutDesc = TensorDesc({1, 2, 5}, ACL_FLOAT, ACL_FORMAT_ND).Precision(0.0001, 0.0001);
...
// dim为负数边界值-2
TEST_F(l2_var_mean_test, var_mean_dim_neg_two)
{
    auto selfDesc = TensorDesc({1, 2, 5, 4}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
    auto dimDesc = IntArrayDesc(vector<int64_t>{-2});
    int64_t correction = 1;
    bool keepdim = false;
    auto varOutDesc = TensorDesc({1, 2, 4}, ACL_FLOAT, ACL_FORMAT_ND).Precision(0.0001, 0.0001);
    auto meanOutDesc = TensorDesc({1, 2, 4}, ACL_FLOAT, ACL_FORMAT_ND).Precision(0.0001, 0.0001);
...
// 标量tensor场景（0维）
TEST_F(l2_var_mean_test, var_mean_scalar_tensor)
{
...
// 单维度tensor场景
TEST_F(l2_var_mean_test, var_mean_single_dim_tensor)
{
    auto selfDesc = TensorDesc({10}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
    auto dimDesc = IntArrayDesc(vector<int64_t>{0});
    int64_t correction = 1;
    bool keepdim = true;
    auto varOutDesc = TensorDesc({1}, ACL_FLOAT, ACL_FORMAT_ND).Precision(0.0001, 0.0001);
    auto meanOutDesc = TensorDesc({1}, ACL_FLOAT, ACL_FORMAT_ND).Precision(0.0001, 0.0001);
...
// keepdim为false且需要reshape的场景
TEST_F(l2_var_mean_test, var_mean_keepdim_false_reshape)
{
    auto selfDesc = TensorDesc({1, 2, 6, 4}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
    auto dimDesc = IntArrayDesc(vector<int64_t>{1, 2});
    int64_t correction = 1;
    bool keepdim = false;
    auto varOutDesc = TensorDesc({1, 4}, ACL_FLOAT, ACL_FORMAT_ND).Precision(0.0001, 0.0001);
    auto meanOutDesc = TensorDesc({1, 4}, ACL_FLOAT, ACL_FORMAT_ND).Precision(0.0001, 0.0001);
