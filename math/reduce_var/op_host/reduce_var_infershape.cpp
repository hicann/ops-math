...
static ge::graphStatus InferShape4ReduceVar(gert::InferShapeContext* context) {
    ...
    if (keepDims) {
        inferStat = Ops::Base::ReduceDimsWithKeepDims<int64_t>(inShape, &axes[0], static_cast<int32_t>(axes.size()), varShape);
    } else {
        inferStat = Ops::Base::ReduceDimsWithoutKeepDims<int64_t>(inShape, &axes[0], static_cast<int32_t>(axes.size()), varShape);
    }
