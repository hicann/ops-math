...
#include "nn_other.h"
#include "../op_graph/reduce_var_proto.h"
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape)                                                    \
    do {                                                                                                            \
        ...
        reducevar1.set_input_##inputName(placeholder##inputIndex);                                                       \
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                           \
    do {                                                                                                        \
        ...
        reducevar1.update_output_desc_##outputName(outputName##outputIndex##_desc);                                  \
    } while (0)

#define ADD_INPUT_ATTR(attrName, attrValue) reducevar1.set_attr_##attrName(attrValue)
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto reducevar1 = op::ReduceVar("test_geir_reduce_var");
    // 输入shape
    std::vector<int64_t> xShape = {1, 100};
    // 输出shape
    std::vector<int64_t> y1Shape = {1, 1};
    std::vector<int64_t> y2Shape = {1, 1};

    ADD_INPUT(1, x, inDtype, xShape);

    // 添加必选属性
    ADD_INPUT_ATTR(dim, {1});
    ADD_INPUT_ATTR(correction, 9);
    ADD_INPUT_ATTR(keepdim, true);
    ADD_INPUT_ATTR(is_mean_out, true);

    // 添加输出
    ADD_OUTPUT(1, var, inDtype, y1Shape);
    ADD_OUTPUT(2, mean, inDtype, y2Shape);

    outputs.push_back(reducevar1);
    // 添加完毕
    return SUCCESS;
}
