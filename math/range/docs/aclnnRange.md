...
[📄 查看源码](https://gitcode.com/cann/ops-math-dev/tree/master/math/range)
...
每个算子分为[两段式接口](../../../docs/context/两段式接口.md)，必须先调用“aclnnRangeGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnRange”接口执行计算。
...
- **参数说明：**
  ...
  - <term>昇腾910_95 AI处理器</term>：out不支持DOUBLE数据类型。
