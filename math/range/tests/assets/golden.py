...
__golden__ = {
    "kernel": {"range": "_range_golden"},
    "aclnn": {"aclnnArange": "aclnn_arange_golden", "aclnnRange": "aclnn_range_golden"},
}
...
def _range_golden(start, limit, delta, *, is_closed=False, **kwargs):
    ...
    if (
        step_val == 0
        or (step_val > 0 and start_val > stop_val)
        or (step_val < 0 and start_val < stop_val)
    ):
    ...
    if "bfloat16" in str(src_type):
        if is_closed:
            golden = (
                torch.range(start_val, stop_val, step_val, dtype=torch_dtype)
                .float()
                .numpy()
            )
        else:
            golden = (
                torch.arange(start_val, stop_val, step_val, dtype=torch_dtype)
                .float()
                .numpy()
            )
    else:
        if is_closed:
            golden = torch.range(
                start_val, stop_val, step_val, dtype=torch_dtype
            ).numpy()
        else:
            golden = torch.arange(
                start_val, stop_val, step_val, dtype=torch_dtype
...
def aclnn_range_golden(start, end, step, out, **kwargs):
    """
    ...
    Returns:
        Output tensors.
    """
    return torch.range(start, end, step, dtype=out.dtype)
