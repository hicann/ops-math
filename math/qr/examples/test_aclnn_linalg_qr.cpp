...
aclError CreateInputs(std::vector<int64_t>& selfShape, std::vector<int64_t>& qOutShape, std::vector<int64_t>& rOutShape,
                      void** selfDeviceAddr, void** qOutDeviceAddr, void** rOutDeviceAddr, aclTensor** self,
                      aclTensor** qOut, aclTensor** rOut)
{
...
aclError ExecOpApi(aclTensor* self, aclTensor* qOut, aclTensor* rOut, int64_t mode, void** workspaceAddrOut,
                   uint64_t& workspaceSize, void* qOutDeviceAddr, void* rOutDeviceAddr, std::vector<int64_t>& qOutShape,
                   std::vector<int64_t>& rOutShape, aclrtStream stream)
{
...
int main()
{
    ...
    ret = CreateInputs(selfShape, qOutShape, rOutShape, &selfDeviceAddr, &qOutDeviceAddr, &rOutDeviceAddr, &self, &qOut,
                       &rOut);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ...
    ret = ExecOpApi(self, qOut, rOut, mode, &workspaceAddr, workspaceSize, qOutDeviceAddr, rOutDeviceAddr, qOutShape,
                    rOutShape, stream);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
