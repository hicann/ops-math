...
int main()
{
    ...
    // 调用aclnnAbs第一段接口
    ret = aclnnAbsGetWorkspaceSize(self.get(), out.get(), &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS,
              LOG_PRINT("aclnnAbsGetWorkspaceSize failed. ERROR: %d.\n[ERROR msg]%s", ret, aclGetRecentErrMsg());
              return ret);
    ...
    // 调用aclnnAbs第二段接口
    ret = aclnnAbs(workspaceAddr.get(), workspaceSize, executor, stream.get());
