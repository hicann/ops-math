...
static const std::initializer_list<op::DataType> ARCH_REGBASE_AICORE_DTYPE_SUPPORT_LIST = {
    DataType::DT_FLOAT, DataType::DT_FLOAT16, DataType::DT_BF16,      DataType::DT_INT8,     DataType::DT_INT16,
    DataType::DT_INT32, DataType::DT_INT64,   DataType::DT_COMPLEX32, DataType::DT_COMPLEX64};
...
static const aclTensor* AbsAiCpu(const aclTensor* self, const aclTensor* out, aclOpExecutor* executor)
{
    ...
    OP_CHECK(ret == ACLNN_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "AbsAiCpu ADD_TO_LAUNCHER_LIST_AICPU failed."),
             return nullptr);
    return out;
}
...
const aclTensor* Abs(const aclTensor* self, aclOpExecutor* executor)
{
    ...
    if (!AbsInferShape(self->GetViewShape(), outShape)) {
        OP_LOGE(ACL_ERROR_INVALID_PARAM, "Abs infer shape failed, input dim num: %zu.",
