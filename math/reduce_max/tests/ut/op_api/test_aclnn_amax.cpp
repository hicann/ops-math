...
protected:
    static void SetUpTestCase() { cout << "l2_amax_test SetUp" << endl; }

    static void TearDownTestCase() { cout << "l2_amax_test TearDown" << endl; }
};

// self为空指针
TEST_F(l2_amax_test, l2_amax_test_nullptr_self)
{
    auto outDesc = TensorDesc({1, 4}, ACL_FLOAT, ACL_FORMAT_ND);
    auto dim = IntArrayDesc(vector<int64_t>{0});
    bool keepDim = true;

    auto ut = OP_API_UT(aclnnAmax, INPUT(nullptr, dim, keepDim), OUTPUT(outDesc));
...
// dim为空指针
TEST_F(l2_amax_test, l2_amax_test_nullptr_dim)
{
    auto selfDesc = TensorDesc({2, 4}, ACL_FLOAT, ACL_FORMAT_ND);
    auto outDesc = TensorDesc({1, 4}, ACL_FLOAT, ACL_FORMAT_ND);
    bool keepDim = true;

    auto ut = OP_API_UT(aclnnAmax, INPUT(selfDesc, nullptr, keepDim), OUTPUT(outDesc));
...
// out为空指针
TEST_F(l2_amax_test, l2_amax_test_nullptr_out)
{
    auto selfDesc = TensorDesc({2, 4}, ACL_FLOAT, ACL_FORMAT_ND);
    auto dim = IntArrayDesc(vector<int64_t>{0});
    bool keepDim = true;

    auto ut = OP_API_UT(aclnnAmax, INPUT(selfDesc, dim, keepDim), OUTPUT(nullptr));
...
// 正常路径，float32
TEST_F(l2_amax_test, l2_amax_test_dtype_float32)
{
...
// 正常路径，float16
TEST_F(l2_amax_test, l2_amax_test_dtype_float16)
{
...
// 正常路径，int64
TEST_F(l2_amax_test, l2_amax_test_dtype_int64)
{
...
// 正常路径，dim为1
TEST_F(l2_amax_test, l2_amax_test_dim_1)
{
    auto selfDesc = TensorDesc({2, 4}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
...
// 正常路径，dim为-1
TEST_F(l2_amax_test, l2_amax_test_dim_neg1)
{
    auto selfDesc = TensorDesc({2, 4}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
    auto outDesc = TensorDesc({2, 1}, ACL_FLOAT, ACL_FORMAT_ND).Precision(0.0001, 0.0001);
    auto dim = IntArrayDesc(vector<int64_t>{-1});
...
// 正常路径，keepDim为false
TEST_F(l2_amax_test, l2_amax_test_keepdim_false)
{
    auto selfDesc = TensorDesc({2, 4}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
...
// self为空tensor
TEST_F(l2_amax_test, l2_amax_test_empty_self)
{
...
// 用例不支持，最大维度超过8
TEST_F(l2_amax_test, l2_amax_test_dimension)
{
...
// 异常路径，dim值不在self范围内
TEST_F(l2_amax_test, l2_amax_dim_invalid)
{
    auto selfDesc = TensorDesc({2, 4}, ACL_FLOAT, ACL_FORMAT_ND);
    auto outDesc = TensorDesc({1, 4}, ACL_FLOAT, ACL_FORMAT_ND);
