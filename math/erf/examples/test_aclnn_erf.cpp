...
int main()
{
    ...
    // 调用aclnnErf第一段接口
    ret = aclnnErfGetWorkspaceSize(self.get(), out.get(), &workspaceSize, &executor);
    ...
    // 调用aclnnErf第二段接口
    ret = aclnnErf(workspaceAddr.get(), workspaceSize, executor, stream.get());
    ...
    ret = aclnnInplaceErfGetWorkspaceSize(self.get(), &inplaceWorkspaceSize, &inplaceExecutor);
    ...
    ret = aclnnInplaceErf(inplaceWorkspaceAddr.get(), inplaceWorkspaceSize, inplaceExecutor, stream.get());
