...
int main()
{
    ...
    DeviceMemPtr gradYDeviceAddr(nullptr, &aclrtFree);
    DeviceMemPtr groupIdxDeviceAddr(nullptr, &aclrtFree);
    DeviceMemPtr outDeviceAddr(nullptr, &aclrtFree);
    TensorPtr gradY(nullptr, &aclDestroyTensor);
    TensorPtr groupIdx(nullptr, &aclDestroyTensor);
    ...
    // 创建gradY aclTensor
    ret = CreateAclTensor(gradYHostData, gradYShape, aclDataType::ACL_FLOAT, gradYDeviceAddr, gradY);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建groupIdxOptional aclTensor
    ret = CreateAclTensor(groupIdxHostData, groupIdxShape, aclDataType::ACL_INT32, groupIdxDeviceAddr, groupIdx);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建out aclTensor
    ...
    ret = aclnnGroupedBiasAddGradGetWorkspaceSize(gradY.get(), groupIdx.get(), out.get(), &workspaceSize, &executor);
    ...
    ret = aclnnGroupedBiasAddGrad(workspaceAddr.get(), workspaceSize, executor, stream.get());
