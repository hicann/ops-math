...
int main()
{
    ...
    DeviceMemPtr windowDeviceAddr(nullptr, &aclrtFree);
    DeviceMemPtr outDeviceAddr(nullptr, &aclrtFree);
    TensorPtr self(nullptr, &aclDestroyTensor);
    TensorPtr window(nullptr, &aclDestroyTensor);
    ...
    // 调用aclStft第一段接口
    ret = aclStftGetWorkspaceSize(self.get(), window.get(), out.get(), n_fft, hop_length, win_length, normalized,
                                  onesided, returnComplex, &workspaceSize, &executor);
    ...
    // 调用aclStft第二段接口
    ret = aclStft(workspaceAddr.get(), workspaceSize, executor, stream.get());
