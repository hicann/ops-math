...
/*!
 * \file exp_segsum_grad_arch22.h
 * \brief
 */

#ifndef EXP_SEGSUM_GRAD_ARCH22_H
#define EXP_SEGSUM_GRAD_ARCH22_H
...
private:
    ...
    __aicore__ inline void ComputeSum(int64_t colLoopTimes, LocalTensor<float> currentRow, LocalTensor<float> castRow,
                                      LocalTensor<T> yTensor);
    __aicore__ inline void ComputeMulBatches(int64_t row, int64_t calNumAlign, int64_t rowIdx,
                                             LocalTensor<float> currentRow, LocalTensor<float> castRow);
                                                      ...
                                                      GM_ADDR workspace, const ExpSegsumGradTilingData* tilingData)
{
    ...
    if (tailDimLength == 0 || slideSize == 0 || batchStart >= batchEnd) {
        return;
    }
