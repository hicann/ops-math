...
/**
 * \file cosh_arch35.cpp
 * \brief Cosh operator kernel entry (arch35 / Ascend910B)
 *
 * Template parameters (matching ASCENDC_TPL_ARGS_DECL in cosh_tiling_key.h):
...
__global__ __aicore__ void cosh(GM_ADDR self, GM_ADDR out, GM_ADDR workspace, GM_ADDR tiling)
{
    REGISTER_TILING_DEFAULT(CoshTilingData);
    GET_TILING_DATA_WITH_STRUCT(CoshTilingData, tilingData, tiling);
    NsCosh::CoshOp<D_T, BUFFER_MODE> op;
