...
/**
 * \file cosh.h
 * \brief Cosh operator kernel class definition (arch35 / Ascend910B)
 *
...
class CoshOp {
    ...
    // All paths compute in float: fp16/bf16 cast to float for precision,
    // fp32 computes natively
    using ComputeT = float;

public:
    __aicore__ inline CoshOp() {}

    __aicore__ inline void Init(GM_ADDR self, GM_ADDR out, const CoshTilingData* tilingData);
...
template <typename T, int BUFFER_MODE>
__aicore__ inline void CoshOp<T, BUFFER_MODE>::Init(GM_ADDR self, GM_ADDR out,
...
template <typename T, int BUFFER_MODE>
__aicore__ inline void CoshOp<T, BUFFER_MODE>::CopyIn(int64_t progress, int64_t currentNum)
{
...
template <typename T, int BUFFER_MODE>
__aicore__ inline void CoshOp<T, BUFFER_MODE>::Compute()
{
    ...
    // Use ubLength_ (guaranteed aligned) for all vector operations.
    // CopyIn pads the tail with zeros via DataCopyPad; CopyOut copies only
    // the precise element count.  Computing cosh(0)=1 on padded zeros is
    // harmless since those elements are discarded by CopyOut.
    ...
    if constexpr (std::is_same_v<T, bfloat16_t> || std::is_same_v<T, half>) {
        ...
        AscendC::Cast(castLocal, selfLocal, AscendC::RoundMode::CAST_NONE, ubLength_);

        // Step 2: |x|
        AscendC::Abs(fExpPos, castLocal, ubLength_);

        // Step 3: -|x|
        AscendC::Muls(fExpNeg, fExpPos, -1.0f, ubLength_);

        // Step 4: |x| - ln2
        AscendC::Adds(castLocal, fExpPos, -LN2_F, ubLength_);

        // Step 5: exp(|x| - ln2) = exp(|x|) / 2
        AscendC::Exp(castLocal, castLocal, ubLength_);

        // Step 6: exp(-|x|)
        AscendC::Exp(fExpNeg, fExpNeg, ubLength_);

        // Step 7: exp(-|x|) / 2
        AscendC::Muls(fExpNeg, fExpNeg, 0.5f, ubLength_);

        // Step 8: exp(|x| - ln2) + exp(-|x|) / 2 = cosh(x)
        AscendC::Add(castLocal, castLocal, fExpNeg, ubLength_);

        // Step 9: Cast fp32 -> input dtype
        AscendC::Cast(outLocal, castLocal, AscendC::RoundMode::CAST_ROUND, ubLength_);
    } else {
        // fp32 path: compute directly in native precision
        // Step 1: |x|
        AscendC::Abs(expPos, selfLocal, ubLength_);

        // Step 2: -|x|
        AscendC::Muls(expNeg, expPos, -1.0f, ubLength_);

        // Step 3: |x| - ln2
        AscendC::Adds(outLocal, expPos, -LN2_F, ubLength_);

        // Step 4: exp(|x| - ln2) = exp(|x|) / 2
        AscendC::Exp(outLocal, outLocal, ubLength_);

        // Step 5: exp(-|x|)
        AscendC::Exp(expNeg, expNeg, ubLength_);

        // Step 6: exp(-|x|) / 2
        AscendC::Muls(expNeg, expNeg, 0.5f, ubLength_);

        // Step 7: exp(|x| - ln2) + exp(-|x|) / 2 = cosh(x)
        AscendC::Add(outLocal, outLocal, expNeg, ubLength_);
    }
...
template <typename T, int BUFFER_MODE>
__aicore__ inline void CoshOp<T, BUFFER_MODE>::CopyOut(int64_t progress, int64_t currentNum)
{
...
template <typename T, int BUFFER_MODE>
__aicore__ inline void CoshOp<T, BUFFER_MODE>::Process()
{
