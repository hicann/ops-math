...
/*!
 * \file test_geir_cosh.cpp
 * \brief GE IR test for Cosh operator
 */
    ...
    std::vector<Operator>& outputs, Graph& graph)
{
    ...
    TensorDesc data_desc = TensorDesc(ge::Shape(xShape), ge::FORMAT_ND, inDtype);
    data_desc.SetPlacement(ge::kPlacementHost);
    data_desc.SetFormat(ge::FORMAT_ND);

    Tensor input_tensor;
    const double coshInputValue = 2.0;
    ret = GenOnesData(xShape, input_tensor, data_desc, inDtype, coshInputValue);
    if (ret != SUCCESS) {
