...
static aclnnStatus CheckParams(const aclTensor* self, const aclTensor* out)
{
    ...
    if (IsRegBase(curArch)) {
        OP_CHECK(!::IsPrivateFormat(self->GetStorageFormat()),
                 OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Cosh does not support private input format on RegBase."),
                 return ACLNN_ERR_PARAM_INVALID);
        OP_CHECK(!::IsPrivateFormat(out->GetStorageFormat()),
                 OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Cosh does not support private output format on RegBase."),
                 return ACLNN_ERR_PARAM_INVALID);
    }
...
aclnnStatus aclnnCoshGetWorkspaceSize(const aclTensor* self, aclTensor* out, uint64_t* workspaceSize,
