 ...
 *   Step 2: pSafe = clamp(p, FLT_MIN, 1 - 2^-24)
    ...
    const NdtriTilingData* tilingData)
{
    ...
    // uint8 mask 按 bit 存储；+32B 余量保证 mask 写操作（按 256bit/cycle 批量写）
    // 末尾即使越过 logical 边界也不踩到下一段 buffer。32B = 一条 V 写入的最小粒度。
    ...
    int32_t len)
{
    ...
    //
    // NaN 检测策略：
    //   通过 scratch = Mins(Maxs(p, -INF), +INF) 把所有有限/无穷值映射到自身，
    //   并把 NaN 映射为 -INF（arch35 上 Maxs/Mins 对 NaN 采用 IEEE 754 "minNumber/maxNumber" 语义：
    //   当任一操作数为 NaN 时返回非 NaN 操作数）。然后 Compare(p, scratch, NE) 即可捕获 NaN。
    //   该行为在 arch35 IEEE 754 兼容模式下硬件保证。已通过 docs/precision-report.md 的
    //   NaN 用例（boundary_nan_*）验证。
    //
...
__aicore__ inline void Ndtri<T, K_ALIGN>::Compute(int64_t currentNum)
{
    ...
    Mins(pSafe, pSafe, NDTRI_SAFE_HI, lenAligned);
