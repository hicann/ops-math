...
#include "nn_other.h"
#include "../op_graph/trace_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                         \
    ...
    trace1.set_input_##intputName(placeholder##intputIndex);                                                \
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    ...
    trace1.update_output_desc_##outputName(outputName##outputIndex##_desc);
...
{
    Status ret = SUCCESS;
    auto trace1 = op::Trace("trace1");
    std::vector<int64_t> xShape = {4, 4};
    ADD_INPUT(1, x, inDtype, xShape);

    // Output is scalar (0-dim), dtype depends on input dtype
    DataType outDtype = inDtype;
    if (inDtype == ge::DT_INT8 || inDtype == ge::DT_INT16 || inDtype == ge::DT_INT32 ||
        inDtype == ge::DT_UINT8 || inDtype == ge::DT_UINT16 || inDtype == ge::DT_UINT32 ||
        inDtype == ge::DT_BOOL) {
        outDtype = ge::DT_INT64;
    }
    std::vector<int64_t> yShape = {};
    ADD_OUTPUT(1, y, outDtype, yShape);

    outputs.push_back(trace1);
    return SUCCESS;
}
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        if (data_size == 0) data_size = GetDataTypeSize(output[i].GetTensorDesc().GetDataType());
