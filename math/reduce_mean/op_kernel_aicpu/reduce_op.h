...
struct ComplexFloatMeanReducer : Eigen::internal::MeanReducer<std::complex<float>> {
    ...
    inline std::complex<float> FinalizeBoth(std::complex<float> saccum, const Packet& vaccum) const
    {
        ...
        return sum_op(saccum, predux(vaccum)) /
               static_cast<std::complex<float>>(scalarCount_ +
                                                packetCount_ * Eigen::internal::unpacket_traits<Packet>::size);
    }
};
...
struct ComplexDoubleMeanReducer : Eigen::internal::MeanReducer<std::complex<double>> {
    ...
    inline std::complex<double> FinalizeBoth(std::complex<double> saccum, const Packet& vaccum) const
    {
        ...
        return sum_op(saccum, predux(vaccum)) /
               static_cast<std::complex<double>>(scalarCount_ +
                                                 packetCount_ * Eigen::internal::unpacket_traits<Packet>::size);
    }
};
...
uint32_t CheckAxes(const CpuKernelContext* context, const QuickVector& axes, bool (&bitmap)[kMaxDimNum])
{
    ...
    for (size_t i = 0U; i < axes.GetDimNum(); i++) {
        ...
        if (index < -input_dim_num || index >= input_dim_num) {
            KERNEL_LOG_ERROR("%s kernel reduction dimension axes[%zu]=%ld is invalid, input dimNum is %ld",
...
void Reduce(const QuickVector& input_reshape, const QuickVector& out_reshape, const ReductionAxes& axis_eigen,
...
template <typename T, typename Reducer>
void ReduceBigger(const QuickVector& input_reshape, const QuickVector& out_reshape, const bool reduce_first_axis,
                  T* input_data, T* out_data)
{
    if (input_reshape.GetDimNum() == DIM5 && reduce_first_axis) { // input_reshape dims = 5
        Reduce<T, Reducer, Eigen::array<int32_t, DIM3>, DIM5,
               DIM2>( // input_reshape dims = 5, reduce dims = 2, unreduce dims = 3
        ...
        Reduce<T, Reducer, Eigen::array<int32_t, DIM2>, DIM5,
               DIM3>( // input_reshape dims = 5, reduce dims = 2, unreduce dims = 3
        ...
        Reduce<T, Reducer, Eigen::array<int32_t, DIM3>, DIM6,
               DIM3>( // input_reshape dims = 6, reduce dims = 3, unreduce dims = 3
            input_reshape, out_reshape, kReductionDimsZeroTwoForth, input_data, out_data);
    } else if (input_reshape.GetDimNum() == DIM6 && !reduce_first_axis) { // input_reshape dims = 6
        Reduce<T, Reducer, Eigen::array<int32_t, DIM3>, DIM6,
               DIM3>( // input_reshape dims = 6, reduce dims = 3, unreduce dims = 3
        ...
        Reduce<T, Reducer, Eigen::array<int32_t, DIM4>, DIM7,
               DIM3>( // input_reshape dims = 7, reduce dims = 4, unreduce dims = 3
        ...
        Reduce<T, Reducer, Eigen::array<int32_t, DIM3>, DIM7,
               DIM4>( // input_reshape dims = 7, reduce dims = 3, unreduce dims = 4
            input_reshape, out_reshape, kReductionDimsOneThreeFive, input_data, out_data);
    } else if (input_reshape.GetDimNum() == DIM8 && reduce_first_axis) { // input_reshape dims = 8
        Reduce<T, Reducer, Eigen::array<int32_t, DIM4>, DIM8,
               DIM4>( // input_reshape dims = 8, reduce dims = 4, unreduce dims = 4
            input_reshape, out_reshape, kReductionDimsZeroTwoForthSix, input_data, out_data);
    } else if (input_reshape.GetDimNum() == DIM8 && !reduce_first_axis) { // input_reshape dims = 8
        Reduce<T, Reducer, Eigen::array<int32_t, DIM4>, DIM8,
               DIM4>( // input_reshape dims = 8, reduce dims = 4, unreduce dims = 4
        ...
        KERNEL_LOG_ERROR("reductionOp kernel input_reshape dims should be less or equal than 8, but now is [%zu]",
                         input_reshape.GetDimNum());
    }
}

template <typename T, typename Reducer>
void ReduceDim3AndDim4(const QuickVector& input_reshape, const QuickVector& out_reshape, const bool reduce_first_axis,
                       T* input_data, T* out_data)
{
...
uint32_t ProcessingEmptyTensor(CpuKernelContext* context)
{
    ...
    KERNEL_CHECK_NULLPTR(out_tensor, KERNEL_STATUS_PARAM_INVALID, "%s get out_tensor failed.",
                         context->GetOpType().c_str());
    auto out_data = static_cast<T*>(out_tensor);
    KERNEL_CHECK_NULLPTR(out_data, KERNEL_STATUS_PARAM_INVALID, "%s get out_data failed.",
                         context->GetOpType().c_str());
    ...
    if (find(OutputZeroValue.begin(), OutputZeroValue.end(), input_type) != OutputZeroValue.end()) {
        ...
        KERNEL_LOG_WARN("Empty tensor type not supported for input tensor, output num is [%ld], "
                        "data type is [%s]",
                        ...
                        T* input_data, T* out_data)
{
    ...
    } else if (input_reshape.GetDimNum() > DIM4 &&
               input_reshape.GetDimNum() <= DIM8) { // input_reshape dims > 4 and <= 8
