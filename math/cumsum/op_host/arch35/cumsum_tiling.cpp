...
static ge::graphStatus TilingCumsumForAscendc(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF((input == nullptr), OP_LOGE_WITH_INVALID_INPUT(context->GetNodeName(), "input"),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF((input->GetDataType() != ge::DT_FLOAT && input->GetDataType() != ge::DT_FLOAT16 &&
                ...
                OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(
                    context->GetNodeName(), "input", Ops::Base::ToString(input->GetDataType()).c_str(),
                    "The dtype of input must be within the range [DT_FLOAT, DT_FLOAT16, DT_BF16, DT_INT32, DT_INT64, "
                    "DT_INT8, DT_UINT8, DT_UINT64]."),
                return ge::GRAPH_FAILED);
...
static ge::graphStatus TilingPrepare4CumsumAscendc(gert::TilingParseContext* context)
{
    ...
    OP_CHECK_IF((compileInfo->clSize <= 0), OP_LOGE(context->GetNodeName(), "Failed to get cache line size."),
                return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF((compileInfo->vRegSize <= 0), OP_LOGE(context->GetNodeName(), "Failed to get vReg size."),
                return ge::GRAPH_FAILED);
