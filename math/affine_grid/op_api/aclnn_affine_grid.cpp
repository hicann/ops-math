...
static bool CheckShape(const aclTensor* theta, const aclIntArray* size, const aclTensor* out)
{
    ...
    if (size->Size() == 4) { // size的大小是4时，theta的维度为(N, 2, 3)
        if (theta->GetViewShape().GetDimNum() != DIM_LEN || theta->GetViewShape().GetDim(1) != DIM_H_2D ||
            theta->GetViewShape().GetDim(SECOND_DIM) != DIM_W_2D ||
            theta->GetViewShape().GetDim(DIM_N) != (*size)[DIM_N]) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                    "Expected a batch of 2D affine matrices of shape Nx2x3 for size "
                    "[%ld, %ld, %ld, %ld]. Got %s.",
                    (*size)[DIM_N], (*size)[DIM_C], (*size)[DIM_H_2D], (*size)[DIM_W_2D],
                    op::ToString(theta->GetViewShape()).GetString());
            return false;
        }
        op::Shape outShape;
        ...
        if (theta->GetViewShape().GetDimNum() != DIM_LEN || theta->GetViewShape().GetDim(1) != DIM_H ||
            theta->GetViewShape().GetDim(SECOND_DIM) != DIM_W ||
            theta->GetViewShape().GetDim(DIM_N) != (*size)[DIM_N]) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                    "Expected a batch of 3D affine matrices of shape Nx3x4 for size "
                    "[%ld, %ld, %ld, %ld, %ld]. Got %s.",
                    (*size)[DIM_N], (*size)[DIM_C], (*size)[DIM_D], (*size)[DIM_H], (*size)[DIM_W],
                    op::ToString(theta->GetViewShape()).GetString());
            return false;
        }
        op::Shape outShape;
                                            ...
                                            aclTensor* out, uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    auto outReshape = l0op::Reshape(
        affineGridOut, uniqueExecutor.get()->AllocIntArray((size->Size() == 4) ? dim4Shape : dim5Shape, size->Size()),
        uniqueExecutor.get());
