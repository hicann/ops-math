...
int main()
{
    ...
    DeviceMemPtr thetaDeviceAddr(nullptr, &aclrtFree);
    DeviceMemPtr outDeviceAddr(nullptr, &aclrtFree);
    TensorPtr theta(nullptr, &aclDestroyTensor);
    ...
    // 创建theta aclTensor
    ret = CreateAclTensor(thetaHostData, thetaShape, aclDataType::ACL_FLOAT, thetaDeviceAddr, theta);
    ...
    ret = aclnnAffineGridGetWorkspaceSize(theta.get(), size, alignCorners, out.get(), &workspaceSize, &executor);
    ...
    ret = aclnnAffineGrid(workspaceAddr.get(), workspaceSize, executor, stream.get());
