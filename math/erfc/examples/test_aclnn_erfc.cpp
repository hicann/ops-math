...
#define CHECK_RET(cond, return_erfcr) \
    do {                              \
        if (!(cond)) {                \
            return_erfcr;             \
        }                             \
...
int main()
{
    ...
    // 调用aclnnErfc第一段接口
    ret = aclnnErfcGetWorkspaceSize(self.get(), out.get(), &workspaceSize, &executor);
    ...
    // 调用aclnnErfc第二段接口
    ret = aclnnErfc(workspaceAddr.get(), workspaceSize, executor, stream.get());
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnErfc failed. ERROR: %d\n", ret); return ret);

    uint64_t inplaceWorkspaceSize = 0;
    aclOpExecutor* inplaceExecutor;
    // 调用aclnnInplaceErfc第一段接口
    ret = aclnnInplaceErfcGetWorkspaceSize(self.get(), &inplaceWorkspaceSize, &inplaceExecutor);
    ...
    ret = aclnnInplaceErfc(inplaceWorkspaceAddr.get(), inplaceWorkspaceSize, inplaceExecutor, stream.get());
