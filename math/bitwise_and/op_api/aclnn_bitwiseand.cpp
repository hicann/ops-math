...
static bool CheckFormat(const aclTensor* self, const aclTensor* other, const aclTensor* y)
{
    ...
    if (self->GetStorageFormat() != other->GetStorageFormat() || self->GetStorageFormat() != y->GetStorageFormat()) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Format of input and output should be equal, self [%s], other [%s], out [%s].",
...
aclnnStatus aclnnBitwiseAndTensorOutGetWorkspaceSize(const aclTensor* self, const aclTensor* other, aclTensor* out,
...
aclnnStatus aclnnBitwiseAndTensorOut(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                     aclrtStream stream)
{
...
aclnnStatus aclnnInplaceBitwiseAndTensorOut(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                            aclrtStream stream)
{
