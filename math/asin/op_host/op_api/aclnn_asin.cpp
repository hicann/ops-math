...
// 1个输入1个输出的参数校验
static aclnnStatus CheckParams1In1Out(const aclTensor* self, const aclTensor* out, const std::initializer_list<op::DataType>& dtypeSupportList, const std::initializer_list<op::DataType>& dtypeOutList) {
  // 1. 检查参数是否为空指针
...
static aclnnStatus ExecAsinGetWorkspaceSize(const aclTensor* self, aclTensor* out, uint64_t* workspaceSize,
  ...
  std::initializer_list<op::DataType> dtypeSupportList = (isSupportBf16Version()) ? DTYPE_SUPPORT_LIST : DTYPE_SUPPORT_LIST_V100;
...
aclnnStatus aclnnInplaceAsin(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
