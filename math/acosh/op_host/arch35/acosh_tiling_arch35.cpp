 ...
 *   - dtype 由 op def 的 DataType profile 展开，不进入 TilingKey
 *
...
static ge::graphStatus ParseInputAndCheckDtype(gert::TilingContext* context, int64_t* totalNum, ge::DataType* dtype)
{
    ...
    OP_CHECK_IF(inputStorageShape.GetDimNum() > MAX_DIM_NUM,
                OP_LOGE(context, "Acosh: input rank must be <= 8, got %zu", inputStorageShape.GetDimNum()),
                return ge::GRAPH_FAILED);
    *totalNum = inputStorageShape.GetShapeSize();
    OP_CHECK_IF(*totalNum < 0, OP_LOGE(context, "Acosh: invalid negative shape size %ld", *totalNum),
                return ge::GRAPH_FAILED);
    ...
    *dtype = inputDesc->GetDataType();
    ...
    OP_CHECK_IF(supported.count(*dtype) == 0, OP_LOGE(context, "Acosh: unsupported dtype %d", static_cast<int>(*dtype)),
                return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(outputStorageShape.GetDimNum() > MAX_DIM_NUM,
                OP_LOGE(context, "Acosh: output rank must be <= 8, got %zu", outputStorageShape.GetDimNum()),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(inputStorageShape != outputStorageShape,
                OP_LOGE(context, "Acosh: input and output shapes must be identical"), return ge::GRAPH_FAILED);
    OP_CHECK_IF(outputDesc->GetDataType() != *dtype,
                OP_LOGE(context, "Acosh: output dtype must match input dtype: input=%d, output=%d",
...
static ge::graphStatus ComputeUbFactor(gert::TilingContext* context, uint64_t ubSize, int64_t ubBlockSize,
                                       int64_t* ubFactor)
{
    constexpr int64_t logTmpReserveBytes = 0;
    int64_t ubAvail = static_cast<int64_t>(ubSize) - logTmpReserveBytes;
    OP_CHECK_IF(ubAvail <= 0, OP_LOGE(context, "Acosh: ubAvail <= 0, ubSize=%lu", static_cast<unsigned long>(ubSize)),
                return ge::GRAPH_FAILED);
    ...
    *ubFactor = FloorAlign(elemsPerFp32, alignElems);
    OP_CHECK_IF(*ubFactor <= 0, OP_LOGE(context, "Acosh: ubFactor too small (=%ld)", *ubFactor),
                return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
static ge::graphStatus AcoshTilingFunc(gert::TilingContext* context)
{
    ...
    OP_LOGI(context, "Acosh: totalNum=%ld, blockFactor=%ld, ubFactor=%ld, usedCoreNum=%ld, dtype=%d", tiling->totalNum,
            tiling->blockFactor, tiling->ubFactor, usedCoreNum, static_cast<int>(dtype));
    return ge::GRAPH_SUCCESS;
}
