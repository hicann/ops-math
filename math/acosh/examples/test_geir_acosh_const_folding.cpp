...
/*!
 * \file test_geir_acosh_const_folding.cpp
 * \brief Verifies that Acosh is constant folded at graph compile time.
 *
 ...
 * successful fold GE removes the Acosh node and replaces it with a Const carrying the attribute
...
constexpr const char* kOpType = "Acosh";
...
// COMPLEX128 is used so the AI Core implementation cannot claim the node and the AICPU host kernel is exercised.
ge::Tensor CreateComplexTensor(const std::vector<int64_t>& shape, const std::vector<std::complex<double>>& values)
{
    ge::TensorDesc desc(ge::Shape(shape), ge::FORMAT_ND, ge::DT_COMPLEX128);
    desc.SetPlacement(ge::kPlacementHost);
    desc.SetRealDimCnt(shape.size());
    return ge::Tensor(desc, reinterpret_cast<const uint8_t*>(values.data()),
...
int32_t CreateGraph(ge::Graph& graph)
{
    ...
    const std::vector<std::complex<double>> input_values = {{1.0, 0.0}, {2.0, 0.0}, {5.0, 0.0}, {10.0, 0.0}};
    ge::Tensor input_tensor = CreateComplexTensor(shape, input_values);

    auto input = ge::op::Const("acosh_const_input").set_attr_value(input_tensor);
    input.update_output_desc_y(input_tensor.GetTensorDesc());

    auto acosh = ge::op::Acosh("acosh_const_folding").set_input_x(input);
    acosh.update_input_desc_x(input_tensor.GetTensorDesc());
    acosh.update_output_desc_y(input_tensor.GetTensorDesc());

    graph.SetInputs({input}).SetOutputs({acosh});
    return kSuccess;
}
...
int32_t VerifyOutput(const std::vector<ge::Tensor>& outputs)
{
    const std::vector<std::complex<double>> expected = {
        {0.0, 0.0}, {std::acosh(2.0), 0.0}, {std::acosh(5.0), 0.0}, {std::acosh(10.0), 0.0}};
    if (outputs.size() != 1U ||
        outputs[0].GetTensorDesc().GetShape().GetShapeSize() != static_cast<int64_t>(expected.size())) {
        std::printf("Unexpected Acosh output shape or count.\n");
        return kFailed;
    }

    const auto* actual = reinterpret_cast<const std::complex<double>*>(outputs[0].GetData());
    if (actual == nullptr) {
        std::printf("Acosh output data is null.\n");
        return kFailed;
    }
    for (size_t i = 0; i < expected.size(); ++i) {
        std::printf("output[%zu] = (%.6f, %.6f)\n", i, actual[i].real(), actual[i].imag());
        if (std::fabs(actual[i].real() - expected[i].real()) > kTolerance ||
            std::fabs(actual[i].imag() - expected[i].imag()) > kTolerance) {
            std::printf("Acosh output mismatch at index %zu, expected (%.6f, %.6f).\n", i, expected[i].real(),
                        expected[i].imag());
            return kFailed;
        }
    }
...
{
    ge::Graph graph(folding_enabled ? "acosh_constant_folding_test" : "acosh_constant_folding_disabled_test");
    ...
    if (session.RunGraph(graph_id, {}, outputs) != ge::SUCCESS || VerifyOutput(outputs) != kSuccess) {
        std::printf("Acosh graph run failed (folding_enabled=%d).\n", static_cast<int>(folding_enabled));
        return kFailed;
    }
    ...
    if (folding_enabled) {
        if (remaining != 0 || !folded_const) {
            std::printf("Acosh was NOT constant folded: it is still dispatched to the device. Check that "
                        "math/acosh/CMakeLists.txt keeps HOSTCPU TRUE and the kernel uses "
                        "OPS_MATH_REGISTER_CPU_KERNELV2.\n");
            return kFailed;
        }
    } else if (remaining == 0) {
        std::printf("The folding assertion is vacuous: the Acosh node disappeared even with constant folding "
...
int main()
{
    ...
    std::printf("Acosh constant folding graph passed.\n");
    return kSuccess;
}
