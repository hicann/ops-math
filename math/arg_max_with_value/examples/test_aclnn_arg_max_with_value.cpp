...
int main()
{
    ...
    // 调用aclnnMaxDim第一段接口
    ret = aclnnMaxDimGetWorkspaceSize(self.get(), dim, keepDim, out.get(), indices.get(), &workspaceSize, &executor);
    ...
    // 调用aclnnMaxDim第二段接口
    ret = aclnnMaxDim(workspaceAddr.get(), workspaceSize, executor, stream.get());
