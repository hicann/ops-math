...
protected:
    static void SetUpTestCase() { std::cout << "FusedMulAddInfershape SetUp" << std::endl; }

    static void TearDownTestCase() { std::cout << "FusedMulAddInfershape TearDown" << std::endl; }
};

// Case 1: three inputs share the same shape, fp16.
TEST_F(FusedMulAddInfershape, same_shape_fp16)
{
    gert::InfershapeContextPara para("FusedMulAdd",
                                     {
...
TEST_F(FusedMulAddInfershape, infer_datatype_fp16)
{
    auto spaceRegistry = gert::DefaultOpImplSpaceRegistryV2::GetInstance().GetSpaceRegistry();
    ASSERT_NE(spaceRegistry, nullptr);
    auto opImpl = spaceRegistry->GetOpImpl("FusedMulAdd");
    ASSERT_NE(opImpl, nullptr);
    ASSERT_NE(opImpl->infer_datatype, nullptr);

    gert::OpInferDataTypeContextBuilder builder;
    builder.OpType("FusedMulAdd").OpName("FusedMulAdd");
    builder.IONum(3, 1);
...
TEST_F(FusedMulAddInfershape, infer_datatype_fp32)
{
    auto spaceRegistry = gert::DefaultOpImplSpaceRegistryV2::GetInstance().GetSpaceRegistry();
    ASSERT_NE(spaceRegistry, nullptr);
    auto opImpl = spaceRegistry->GetOpImpl("FusedMulAdd");
...
TEST_F(FusedMulAddInfershape, infer_datatype_int32)
{
    auto spaceRegistry = gert::DefaultOpImplSpaceRegistryV2::GetInstance().GetSpaceRegistry();
    ASSERT_NE(spaceRegistry, nullptr);
    auto opImpl = spaceRegistry->GetOpImpl("FusedMulAdd");
