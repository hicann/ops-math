...
int main()
{
    ...
    DeviceMemPtr realDeviceAddr(nullptr, &aclrtFree);
    DeviceMemPtr imagDeviceAddr(nullptr, &aclrtFree);
    DeviceMemPtr outDeviceAddr(nullptr, &aclrtFree);
    TensorPtr real(nullptr, &aclDestroyTensor);
    TensorPtr imag(nullptr, &aclDestroyTensor);
    ...
    // 创建real aclTensor
    ret = CreateAclTensor(realHostData, realShape, aclDataType::ACL_FLOAT, realDeviceAddr, real);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建imag aclTensor
    ret = CreateAclTensor(imagHostData, imagShape, aclDataType::ACL_FLOAT, imagDeviceAddr, imag);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建out aclTensor
    ret = CreateAclTensor(outHostData, outShape, aclDataType::ACL_COMPLEX64, outDeviceAddr, out);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ...
    // 调用aclnnComplex第一段接口
    ret = aclnnComplexGetWorkspaceSize(real.get(), imag.get(), out.get(), &workspaceSize, &executor);
    ...
    // 调用aclnnComplex第二段接口
    ret = aclnnComplex(workspaceAddr.get(), workspaceSize, executor, stream.get());
