 ...
 *   - z:  输出梯度
 *
 * Computes: z = dy / sqrt(1 - y^2)
 *
 * Template parameters:
 *   - StorageT: Data type for GM storage and data copy (half/float/bfloat16_t)
 *   - ComputeT: Data type for actual computation (always float in production dispatch)
 *     For fp32: ComputeT == StorageT (direct computation)
 *     For fp16/bf16: ComputeT == float (Cast low-precision -> float for computation)
 *   - BUFFER_MODE: Buffer mode (0=single buffer, 1=double buffer)
 *
 * Iteration 3 (full coverage):
 *   - fp32 path: shared template, 4 Queue buffers (y, dy, tmp, z) all in float
...
private:
    ...
    TQue<QuePosition::VECIN, BUFFER_NUM> yInQueue;
    TQue<QuePosition::VECIN, BUFFER_NUM> dyInQueue;
    TQue<QuePosition::VECOUT, BUFFER_NUM> zOutQueue;
    ...
    TBuf<TPosition::VECCALC> tmpBufYF32;
    TBuf<TPosition::VECCALC> tmpBufDyF32;
    TBuf<TPosition::VECCALC> tmpBufTmpF32;
    TBuf<TPosition::VECCALC> tmpBufZF32;

    GlobalTensor<StorageT> yGM;
    GlobalTensor<StorageT> dyGM;
    GlobalTensor<StorageT> zGM;
    ...
    GM_ADDR y, GM_ADDR dy, GM_ADDR z, const AsinGradTilingData* tilingData)
{
    ...
    if constexpr (NEED_CAST) {
        // fp16/bf16 path: TQue for low-precision I/O, TBuf<VECCALC> for float intermediate compute
        // Input queues: low-precision for DataCopyPad (GM -> UB)
        pipe.InitBuffer(yInQueue, BUFFER_NUM, ubLength_ * sizeof(StorageT));
        pipe.InitBuffer(dyInQueue, BUFFER_NUM, ubLength_ * sizeof(StorageT));
        // Output queue: low-precision for DataCopyPad (UB -> GM)
        pipe.InitBuffer(zOutQueue, BUFFER_NUM, ubLength_ * sizeof(StorageT));
        // Float computation buffers: TBuf (no double-buffer overhead for purely local compute)
        pipe.InitBuffer(tmpBufYF32, ubLength_ * sizeof(ComputeT));
        pipe.InitBuffer(tmpBufDyF32, ubLength_ * sizeof(ComputeT));
        pipe.InitBuffer(tmpBufTmpF32, ubLength_ * sizeof(ComputeT));
        pipe.InitBuffer(tmpBufZF32, ubLength_ * sizeof(ComputeT));
    } else {
        // fp32 path: 4 Queue buffers in ComputeT (== StorageT == float)
        // yInQueue:  input y (forward)
        // dyInQueue: input dy (gradient)
        // tmpQueue:  intermediate (y*y, 1-y*y, sqrt)
        // zOutQueue: output z (grad input)
        pipe.InitBuffer(yInQueue, BUFFER_NUM, ubLength_ * sizeof(ComputeT));
        ...
        pipe.InitBuffer(zOutQueue, BUFFER_NUM, ubLength_ * sizeof(ComputeT));
    }
}
    ...
    int64_t progress, int64_t currentNum)
{
    ...
    AscendC::DataCopyPad(yLocal, yGM[progress * ubLength_], copyParams, {false, 0, 0, 0});
    AscendC::DataCopyPad(dyLocal, dyGM[progress * ubLength_], copyParams, {false, 0, 0, 0});

    yInQueue.EnQue(yLocal);
    ...
    int64_t progress, int64_t currentNum)
{
    ...
    AscendC::DataCopyPad(zGM[progress * ubLength_], zLocal, copyParams);
    zOutQueue.FreeTensor(zLocal);
}

template <typename StorageT, typename ComputeT, int BUFFER_MODE>
__aicore__ inline void AsinGrad<StorageT, ComputeT, BUFFER_MODE>::Compute(int64_t currentNum)
{
    AscendC::LocalTensor<StorageT> yLocal = yInQueue.template DeQue<StorageT>();
    AscendC::LocalTensor<StorageT> dyLocal = dyInQueue.template DeQue<StorageT>();

    if constexpr (NEED_CAST) {
        // fp16/bf16 path: Cast low-precision -> float, compute in float, Cast float -> low-precision
        // Use TBuf<VECCALC> for float intermediate buffers (probe-validated approach)
        AscendC::LocalTensor<ComputeT> yF32 = tmpBufYF32.template Get<ComputeT>();
        ...
        AscendC::LocalTensor<ComputeT> zF32 = tmpBufZF32.template Get<ComputeT>();

        // Cast low-precision -> float
        AscendC::Cast(yF32, yLocal, AscendC::RoundMode::CAST_NONE, currentNum);
        AscendC::Cast(dyF32, dyLocal, AscendC::RoundMode::CAST_NONE, currentNum);

        // Free low-precision input buffers (no longer needed after Cast)
        yInQueue.FreeTensor(yLocal);
        dyInQueue.FreeTensor(dyLocal);

        // Compute: tmp = y * y
        AscendC::Mul(tmpF32, yF32, yF32, currentNum);
        // tmp = -(y*y)
        ...
        // z = dy / sqrt(1 - y*y)
        AscendC::Div(zF32, dyF32, tmpF32, currentNum);

        // Alloc low-precision output buffer and Cast float -> low-precision
        AscendC::LocalTensor<StorageT> zLocal = zOutQueue.template AllocTensor<StorageT>();
        AscendC::Cast(zLocal, zF32, AscendC::RoundMode::CAST_RINT, currentNum);
        zOutQueue.template EnQue<StorageT>(zLocal);
    } else {
        // fp32 path: direct computation (ComputeT == StorageT == float)
        AscendC::LocalTensor<ComputeT> tmpLocal = tmpQueue.template AllocTensor<ComputeT>();
        AscendC::LocalTensor<StorageT> zLocal = zOutQueue.template AllocTensor<StorageT>();

        AscendC::LocalTensor<ComputeT> yComp = yLocal.template ReinterpretCast<ComputeT>();
        AscendC::LocalTensor<ComputeT> dyComp = dyLocal.template ReinterpretCast<ComputeT>();
        AscendC::LocalTensor<ComputeT> zComp = zLocal.template ReinterpretCast<ComputeT>();

        // tmp = y * y
        AscendC::Mul(tmpLocal, yComp, yComp, currentNum);
        // tmp = -(y*y)
        ...
        // z = dy / sqrt(1 - y*y)
        AscendC::Div(zComp, dyComp, tmpLocal, currentNum);
