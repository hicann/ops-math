...
// arch35 (RegBase / ascend950) tiling UT. This file is compiled only for the ascend950 build
// (wired via TILING_DIR arch35 in the op_host tests CMakeLists), where IsRegbaseSocVersion() is
// true and the HistogramV2SimtTiling template is selected. No runtime SoC probing is needed:
// the SIMT template is the only capable template here, so failure cases assert directly.
...
protected:
    static void SetUpTestCase() { std::cout << "HistogramV2Tiling (arch35/SIMT) SetUp" << std::endl; }
    static void TearDownTestCase() { std::cout << "HistogramV2Tiling (arch35/SIMT) TearDown" << std::endl; }
};
...
TEST_F(HistogramV2Tiling, tiling_fp16_out_fp32)
{
    auto ci = MakeCompileInfo();
...
TEST_F(HistogramV2Tiling, tiling_fp32_out_fp32)
{
    auto ci = MakeCompileInfo();
...
TEST_F(HistogramV2Tiling, tiling_ub_not_full)
{
    auto ci = MakeCompileInfo();
...
TEST_F(HistogramV2Tiling, tiling_ub_not_full_simt)
{
    auto ci = MakeCompileInfo();
...
TEST_F(HistogramV2Tiling, tiling_bins_non_positive)
{
    auto ci = MakeCompileInfo();
...
TEST_F(HistogramV2Tiling, tiling_minmax_shape_invalid)
{
    auto ci = MakeCompileInfo();
...
TEST_F(HistogramV2Tiling, tiling_minmax_dtype_mismatch)
{
    auto ci = MakeCompileInfo();
...
TEST_F(HistogramV2Tiling, tiling_unsupported_dtype)
{
    auto ci = MakeCompileInfo();
...
TEST_F(HistogramV2Tiling, tiling_out_size_ne_bins)
{
    auto ci = MakeCompileInfo();
...
TEST_F(HistogramV2Tiling, tiling_fp32_out_int_input)
{
    auto ci = MakeCompileInfo();
...
TEST_F(HistogramV2Tiling, tiling_out_dtype_invalid)
{
    auto ci = MakeCompileInfo();
...
TEST_F(HistogramV2Tiling, tiling_core_num_zero)
{
