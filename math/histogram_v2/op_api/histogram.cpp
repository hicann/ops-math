                                        ...
                                        const aclTensor* out, int64_t bins, aclOpExecutor* executor)
{
    ...
    OP_CHECK(ret == ACLNN_SUCCESS,
             OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "HistogramAiCore ADD_TO_LAUNCHER_LIST_AICORE failed."), return nullptr);
    return out;
}

// AiCPU的执行逻辑
inline const aclTensor* HistogramAiCPU(const aclTensor* self, const aclTensor* out, int64_t bins, float min, float max,
                                       aclOpExecutor* executor)
{
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICPU(Histogram, OP_ATTR_NAMES({"bins", "min", "max"}), OP_INPUT(self),
                                          OP_OUTPUT(histogramOut), OP_ATTR(bins, min, max));
    OP_CHECK(ret == ACLNN_SUCCESS,
             OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "HistogramAiCPU ADD_TO_LAUNCHER_LIST_AICPU failed."), return nullptr);
    return histogramOut;
}
                           ...
                           int64_t bins, float minValue, float maxValue, aclOpExecutor* executor)
{
    if (IsAiCoreSupport(self, out)) {
        ...
        auto desDtype = ((out->GetDataType() == op::DataType::DT_FLOAT16 ||
