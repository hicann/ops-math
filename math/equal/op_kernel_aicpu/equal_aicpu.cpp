...
uint32_t BcastCompute(const CpuKernelContext& ctx, const Bcast& bcast)
{
    ...
    if (data_num >= kParallelDataNum) {
        ...
        KERNEL_HANDLE_ERROR(CpuKernelUtils::ParallelFor(ctx, data_num, 1, sharder_equal), "Equal Compute failed.")
    } else {
...
uint32_t NoBcastCompute(const CpuKernelContext& ctx)
{
    ...
    if (data_num >= kParallelDataNumSameShape) {
        auto sharder_equal = [&type, &ctx](int64_t start, int64_t end) { SpecialCompute<T>(type, start, end, ctx); };
        KERNEL_HANDLE_ERROR(CpuKernelUtils::ParallelFor(ctx, data_num, 1, sharder_equal), "Equal Compute failed.")
    } else {
...
OPS_MATH_REGISTER_CPU_KERNELV2(kEqual, EqualCpuKernel);
} // namespace aicpu
