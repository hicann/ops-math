...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用"aclnnExpm1GetWorkspaceSize"或者"aclnnInplaceExpm1GetWorkspaceSize"接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用"aclnnExpm1"或者"aclnnInplaceExpm1"接口执行计算。
...
- 确定性计算：aclnnExpm1&aclnnInplaceExpm1默认确定性实现。
