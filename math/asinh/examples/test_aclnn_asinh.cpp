...
static int Init(int32_t deviceId, StreamPtr& stream, bool& initialized, bool& deviceSet)
{
...
template <typename T>
static int CreateAclTensor(const std::vector<T>& hostData, const std::vector<int64_t>& shape, aclDataType dataType,
...
int main()
{
    ...
    std::vector<float> selfHostData = {-4.0f, -2.0f,   -1.5f, -1.0f, -0.5f, -0.1f, -0.0001f, -0.0f,
    ...
    ret = aclnnAsinhGetWorkspaceSize(self.get(), out.get(), &workspaceSize, &executor);
    ...
    // 5. 两段式接口 - 第二段：执行算子
    ret = aclnnAsinh(workspaceAddr.get(), workspaceSize, executor, stream.get());
    ...
    ret = aclrtMemcpy(resultData.data(), outNumel * sizeof(float), outDeviceAddr.get(), outNumel * sizeof(float),
