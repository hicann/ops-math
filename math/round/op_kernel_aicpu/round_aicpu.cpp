...
inline auto DoCompute(const T& x) -> const T
{
    ...
    bool is_point_five = std::is_same<T, double>::value ? IsValueEqualForDouble(fraction, point_five) :
                                                          aicpu::IsValueEqual<T>(fraction, point_five);
    if (fraction > point_five) {
        roundval += T(1.0);
    } else if (is_point_five) {
        const T nearest_even_int = roundval - T(2) * Eigen::numext::floor(point_five * x);
        bool is_odd = std::is_same<T, double>::value ? IsValueEqualForDouble(nearest_even_int, T(1)) :
                                                       aicpu::IsValueEqual<T>(nearest_even_int, T(1));
        if (is_odd) {
            roundval += T(1);
        }
    }
...
uint32_t RoundCpuKernel::Compute(CpuKernelContext& ctx)
{
    ...
    switch (inputtype) {
        case DT_FLOAT16:
            ret = RangeRound(ctx, static_cast<Eigen::half*>(inputdata), static_cast<Eigen::half*>(outputdata),
                             decimals);
            break;
        case DT_FLOAT:
            ret = RangeRound(ctx, static_cast<float*>(inputdata), static_cast<float*>(outputdata), decimals);
            break;
        case DT_DOUBLE:
            ret = RangeRound(ctx, static_cast<double*>(inputdata), static_cast<double*>(outputdata), decimals);
            break;
        case DT_INT32:
            ret = RangeRound(ctx, static_cast<int32_t*>(inputdata), static_cast<int32_t*>(outputdata), decimals);
            break;
        case DT_INT64:
            ret = RangeRound(ctx, static_cast<int64_t*>(inputdata), static_cast<int64_t*>(outputdata), decimals);
            break;
        default:
...
OPS_MATH_REGISTER_CPU_KERNELV2(kRound, RoundCpuKernel);
} // namespace aicpu
