...
/*!
 * \file xlogy_kernel.h
 * \brief Xlogy Kernel — 2入(x1,x2)1出(y) broadcast, VF 全链融合
 */
#pragma once
#include "kernel_operator.h"
#include "xlogy_tiling_struct.h"
#include "xlogy_struct.h"
...
// VF 函数声明
template <typename T>
__simd_vf__ inline void XlogyVF(
...
class XlogyKernel {
    ...
    AscendC::TPipe pipe_;
    const XlogyTilingData<RANK>* td_;
...
private:
    // FP32 路径: CopyIn x1→B0, x2→B1 → XlogyVF B2←B0,B1 → CopyOut B2
        ...
        int32_t evMTE2toV, int32_t evVtoMTE2, int32_t evVtoMTE3)
    {
        ...
        // XlogyVF B2←B0(x1),B1(x2)
        // UB: [B0, B1, B2] = 3 ← 峰值
        CallXlogyVF(B_Y, B_X1, B_X2, count);
        // B0, B1 已消费，可释放
    ...
    // FP16 路径: CopyIn→Cast→XlogyVF→Cast→CopyOut
        ...
        int32_t evMTE2toV, int32_t evVtoMTE2, int32_t evVtoMTE3)
    {
        ...
        // Step 3: XlogyVF B1←B1,B2 (B1 作为 dst, 就地覆写)
        // B0, B3 已消费
        CallXlogyVF(B_X1, B_X1, B_X2, count);
        // B2 已消费

        // Step 4: Cast FP32→低精度: B1→B0 (bf16 不支持 CAST_NONE, 用 CAST_RINT)
        AscendC::Cast(buf_[B_TEMP_X1].template Get<T>(), buf_[B_X1].template Get<float>(),
    ...
    __aicore__ inline void CallXlogyVF(int dstSlot, int srcXSlot, int srcYSlot, int64_t count)
    {
        ...
        asc_vf_call<XlogyVF<float>>(dstAddr, srcXAddr, srcYAddr, (uint32_t)count, VL, repeatTimes);
    }
...
// ============================================================
// XlogyVF — 寄存器全链融合: Ln→Mul→Compare→Select→Compare→Select
// 链长=6 ≤ 7, 中间值不进 UB, dst 复用 B1 (FP32) 或 B0 (FP16 cast后)
// ============================================================
...
__simd_vf__ inline void XlogyVF(
    ...
    uint32_t count, uint32_t oneRepeatSize, uint16_t repeatTimes)
{
    AscendC::Reg::RegTensor<T> regX, regY, regLogY, regMul, regRes0, regFinal, regZero;
    ...
    for (uint16_t i = 0; i < repeatTimes; ++i) {
        ...
        // S1: Ln(y) → log_y
        AscendC::Reg::Ln(regLogY, regY, mask);

        // S2: Mul(x, log_y) → mul_res
        AscendC::Reg::Mul(regMul, regX, regLogY, mask);

        // S3: Compare(x, 0, EQ) → mask_eq
        ...
        // S4: Select(mask_eq, 0, mul_res) → res0  (maskEq? zero : mul_res)
        AscendC::Reg::Select<float>(regRes0, regZero, regMul, maskEq);

        // S5: Compare(y, y, EQ) → mask_nan (NaN≠NaN → mask=0)
        AscendC::Reg::Compare<float, AscendC::CMPMODE::EQ>(maskNaN, regY, regY, mask);

        // S6: Select(mask_nan, res0, y) → final  (maskNaN? res0 : y)
