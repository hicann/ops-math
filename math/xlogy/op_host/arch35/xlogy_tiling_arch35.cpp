...
ge::graphStatus XlogyTiling::GetShapeInfo()
{
    ...
    OP_LOGI(ctx_->GetNodeName(), "Xlogy GetShapeInfo done rank %ld phys_nodes %ld ub %lu core %lu", rank_, phys_nodes_,
...
ge::graphStatus XlogyTiling::DoTilingAndSet()
{
    ...
    OP_LOGI(ctx_->GetNodeName(),
            "Xlogy TilingData: per_buf_bytes=%ld rank=%ld->R=%d "
            "max_bro_shape=%s "
    ...
    for (int64_t i = 0; i < num_in; i++)
        OP_LOGI(ctx_->GetNodeName(), "Xlogy TilingData input[%ld]: shape=%s stride=%s", i,
                Arr2String(tiling->input_shapes[i], R).c_str(), Arr2String(tiling->input_strides[i], R).c_str());
    for (int64_t i = 0; i < num_out; i++)
        OP_LOGI(ctx_->GetNodeName(), "Xlogy TilingData output[%ld]: shape=%s stride=%s", i,
