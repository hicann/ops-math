...
/*!
 * \file xlogy_tiling_arch35.cpp
 * \brief Xlogy Tiling — arch35 实现
 */
#include "xlogy_tiling_arch35.h"
#include "../../op_kernel/arch35/xlogy_struct.h"
#include <algorithm>
#include <sstream>
...
// ============================================================
// XlogyTiling — CANN 主线：单次归一化 → 映射 rank → 模板填充
// ============================================================

namespace optiling {

using namespace xlogy;
...
XlogyTiling::XlogyTiling(gert::TilingContext* ctx) : ctx_(ctx) {}

ge::graphStatus XlogyTiling::GetShapeInfo()
{
    auto compileInfo = reinterpret_cast<const XlogyCompileInfo*>(ctx_->GetCompileInfo());
    ...
    if (dtype == ge::DT_FLOAT) {
        dtype_size_ = 4;
        phys_nodes_ = kPhysNodesFp32;
    } else if (dtype == ge::DT_FLOAT16 || dtype == ge::DT_BF16) {
        dtype_size_ = 2;
        phys_nodes_ = kPhysNodesFp16;
    } else {
        OP_LOGE(ctx_->GetNodeName(), "Xlogy unsupported dtype");
        return GRAPH_FAILED;
    }
    ...
    OP_LOGI(ctx_->GetNodeName(), "Xlogy GetShapeInfo done rank %ld phys_nodes %ld ub %lu core %lu",
...
template<int64_t R>
ge::graphStatus XlogyTiling::DoTilingAndSet()
{
    auto* tiling = ctx_->GetTilingData<XlogyTilingData<R>>();
    OP_CHECK_NULL_WITH_CONTEXT(ctx_, tiling);

    auto* compileInfo = reinterpret_cast<const XlogyCompileInfo*>(ctx_->GetCompileInfo());
    int64_t ub_per_core = (int64_t)compileInfo->ubSize;

    // per_buf_bytes = (UB / P) & ~31, per_buf_elems = per_buf_bytes / kBufDtypeSize
    ...
    // 维测: TilingData 全部字段
    OP_LOGI(ctx_->GetNodeName(), "Xlogy TilingData: per_buf_bytes=%ld rank=%ld->R=%d "
            "max_bro_shape=%s "
    ...
    for (int64_t i = 0; i < num_in; i++)
        OP_LOGI(ctx_->GetNodeName(), "Xlogy TilingData input[%ld]: shape=%s stride=%s",
                i, Arr2String(tiling->input_shapes[i], R).c_str(),
                   Arr2String(tiling->input_strides[i], R).c_str());
    for (int64_t i = 0; i < num_out; i++)
        OP_LOGI(ctx_->GetNodeName(), "Xlogy TilingData output[%ld]: shape=%s stride=%s",
...
ge::graphStatus XlogyTiling::RunTiling()
{
    ...
    if (mapped == 4) {
        ...
        ctx_->SetTilingKey(GET_TPL_TILING_KEY(XLOGY_RANK_4));
    } else {
        ret = DoTilingAndSet<8>();
        ctx_->SetTilingKey(GET_TPL_TILING_KEY(XLOGY_RANK_8));
    }
    return ret;
}

static ge::graphStatus TilingFuncXlogy(gert::TilingContext* context)
{
    XlogyTiling xlogyTiling(context);
    auto ret = xlogyTiling.RunTiling();
    if (ret != GRAPH_SUCCESS) return ret;
...
ge::graphStatus TilingPrepareForXlogy(gert::TilingParseContext* context)
{
    fe::PlatFormInfos* platformInfo = context->GetPlatformInfo();
    auto compileInfo = context->GetCompiledInfo<XlogyCompileInfo>();
...
IMPL_OP_OPTILING(Xlogy).Tiling(TilingFuncXlogy).TilingParse<XlogyCompileInfo>(TilingPrepareForXlogy);
