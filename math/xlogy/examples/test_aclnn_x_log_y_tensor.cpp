...
int main()
{
    ...
    ret = aclnnXLogYTensorGetWorkspaceSize(self.get(), other.get(), out.get(), &workspaceSize, &executor);
    ...
    ret = aclnnXLogYTensor(workspaceAddr.get(), workspaceSize, executor, stream.get());
    ...
    ret = aclnnInplaceXLogYTensorGetWorkspaceSize(self.get(), other.get(), &inplaceWorkspaceSize, &inplaceExecutor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnInplaceXLogYTensorGetWorkspaceSize failed. ERROR: %d\n", ret);
              return ret);
    // 根据第一段接口计算出的workspaceSize申请device内存
    DeviceMemPtr inplaceWorkspaceAddr(nullptr, &aclrtFree);
    if (inplaceWorkspaceSize > static_cast<uint64_t>(0)) {
        void* rawInplaceWorkspaceAddr = nullptr;
        ret = aclrtMalloc(&rawInplaceWorkspaceAddr, inplaceWorkspaceSize, ACL_MEM_MALLOC_HUGE_FIRST);
        CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("allocate workspace failed. ERROR: %d\n", ret); return ret);
        inplaceWorkspaceAddr.reset(rawInplaceWorkspaceAddr);
    }
    // 调用aclnnInplaceXLogYTensor第二段接口
    ret = aclnnInplaceXLogYTensor(inplaceWorkspaceAddr.get(), inplaceWorkspaceSize, inplaceExecutor, stream.get());
    ...
    ret = aclrtMemcpy(inplaceResultData.data(), inplaceResultData.size() * sizeof(inplaceResultData[0]),
                      selfDeviceAddr.get(), inplaceSize * sizeof(inplaceResultData[0]), ACL_MEMCPY_DEVICE_TO_HOST);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("copy result from device to host failed. ERROR: %d\n", ret); return ret);
