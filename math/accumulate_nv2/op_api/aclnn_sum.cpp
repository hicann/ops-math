...
static aclnnStatus SplitToSumN(const aclTensorList* tensors, const aclIntArray* broadcastShapeArray,
                               const aclTensor** sumOut, aclOpExecutor* executor, bool promoteFp16)
{
    ...
    while (tensorList.size() >= MAX_TENSOR_SIZE) {
        ...
        auto onesComputeOut = l0op::AccumulateNV2(
            executor->AllocTensorList(tensorListOnes.data(), tensorListOnes.size()), executor);
                                     ...
                                     aclOpExecutor** executor)
{
    ...
    // The broadcast result can be empty even when the first input is non-empty.
    // Skip all Contiguous/BroadcastTo/AccumulateNV2 nodes based on the validated
    // output shape; BroadcastToAiCore does not accept a non-empty input expanded
    // to a zero-element output.
    if (out->IsEmpty()) {
        *workspaceSize = 0;
