...
#include "math/accumulate_nv2/op_api/aclnn_sum.h"
...
// 广播结果为空，但第一个输入非空：应直接返回，不能调用 BroadcastToAiCore。
TEST_F(l2_sum_test, l2_sum_test_empty_broadcast_first_input_nonempty)
{
    auto tensor_1_desc = TensorDesc({1, 2, 1, 2}, ACL_FLOAT, ACL_FORMAT_ND);
    auto tensor_2_desc = TensorDesc({0, 1, 3, 2}, ACL_FLOAT, ACL_FORMAT_ND);
    auto tensor_list_desc = TensorListDesc({tensor_1_desc, tensor_2_desc});
    auto out_tensor_desc = TensorDesc({0, 2, 3, 2}, ACL_FLOAT, ACL_FORMAT_ND);
    auto ut = OP_API_UT(aclnnSum, INPUT(tensor_list_desc), OUTPUT(out_tensor_desc));
    uint64_t workspace_size = 1;
    aclnnStatus aclRet = ut.TestGetWorkspaceSize(&workspace_size);
    EXPECT_EQ(aclRet, ACL_SUCCESS);
...
TEST_F(l2_sum_test, l2_sum_test_int8_out_shape_fail)
{
    ...
    auto tensor_2_desc = TensorDesc({2, 3}, ACL_INT8, ACL_FORMAT_ND).Value(vector<float>{7, 8, 9, 10, 11, 12});
...
TEST_F(l2_sum_test, l2_sum_test_dtype_int32)
{
    ...
    auto tensor_2_desc = TensorDesc({2, 3}, ACL_INT32, ACL_FORMAT_ND).Value(vector<float>{7, 8, 9, 10, 11, 12});
...
TEST_F(l2_sum_test, l2_sum_test_dtype_uint8)
{
    ...
    auto tensor_2_desc = TensorDesc({2, 3}, ACL_UINT8, ACL_FORMAT_ND).Value(vector<float>{7, 8, 9, 10, 11, 12});
...
TEST_F(l2_sum_test, l2_sum_test_dtype_double)
{
    ...
    auto tensor_2_desc = TensorDesc({2, 3}, ACL_DOUBLE, ACL_FORMAT_ND).Value(vector<float>{7, 8, 9, 10, 11, 12});
