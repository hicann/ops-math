...
/*!
 * \file test_geir_truncate_div.cpp
 * \brief TruncateDiv算子GEIR测试示例
 */
...
#include "nn_other.h"
#include "../op_graph/truncate_div_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape, value)                                   \
    do {                                                                                                     \
        ...
        truncateDiv1.set_input_##intputName(placeholder##intputIndex);                                       \
...
{
    Status ret = SUCCESS;
    auto truncateDiv1 = op::TruncateDiv("truncateDiv1");
    std::vector<int64_t> xShape = {4, 2};
    ADD_INPUT(1, x1, inDtype, xShape, 10.0f);
    ADD_INPUT(2, x2, inDtype, xShape, 3.0f);

    outputs.push_back(truncateDiv1);
    return SUCCESS;
}
...
void SaveInputOutput(std::vector<ge::Tensor>& input, std::vector<ge::Tensor>& output)
{
    ...
    for (int i = 0; i < input_num; i++) {
        ...
        string input_file = "./tc_ge_irrun_test_truncate_div_npu_input_" + std::to_string(i) + ".bin";
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        string output_file = "./tc_ge_irrun_test_truncate_div_npu_output_" + std::to_string(i) + ".bin";
...
{
    const char* graph_name = "tc_ge_irrun_test_truncate_div";
    ...
    std::string file_path = "./dump_truncate_div";
