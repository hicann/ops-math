...
inline std::uint32_t SinhExtraCheck(const CpuKernelContext& ctx)
{
    ...
    if (input_dims.size() != output_dims.size()) {
        KERNEL_LOG_ERROR("The data dim size of the input [%llu] need be the same as the output "
                         "[%llu].",
...
{
    return detail::SinhCheck(ctx, kSinhInputNum, kSinhOutputNum) ? KERNEL_STATUS_PARAM_INVALID :
