...
class l2_addcmul_test : public testing::Test {
 protected:
  static void SetUpTestCase() { cout << "addcmul_test SetUp" << endl; }

  static void TearDownTestCase() { cout << "addcmul_test TearDown" << endl; }
};
...
TEST_F(l2_addcmul_test, test_empty_tensor) {
  ...
  auto ut = OP_API_UT(aclnnAddcmul, INPUT(self_tensor_desc, other_tensor_desc, other_tensor_desc, scalar_desc),
...
TEST_F(l2_addcmul_test, test_null_scalar) {
  ...
  auto ut = OP_API_UT(aclnnAddcmul, INPUT(tensor_desc, other_tensor_desc, other_tensor_desc, scalar_desc),
                      OUTPUT(out_tensor_desc));
...
TEST_F(l2_addcmul_test, test_invalid_data_type) {
  auto tensor_desc = TensorDesc({10, 20}, ACL_UINT32, ACL_FORMAT_ND).ValueRange(0, 255).Value(vector<uint8_t>(200, 1));
  auto other_tensor_desc =
...
TEST_F(l2_addcmul_test, test_invalid_data_type_2) {
  ...
  auto ut = OP_API_UT(aclnnAddcmul, INPUT(self_tensor_desc, other_tensor_desc, other_tensor_desc2, scalar_desc),
                      OUTPUT(out_tensor_desc));
...
// Test for precision
TEST_F(l2_addcmul_test, test_int8) {
  auto tensor_desc = TensorDesc({4, 4}, ACL_INT8, ACL_FORMAT_NCHW).ValueRange(-2, 2);
  auto out_tensor_desc = TensorDesc(tensor_desc).Precision(0.0001, 0.0001);
  auto scalar_desc = ScalarDesc(1.0f);

  auto ut = OP_API_UT(aclnnAddcmul, INPUT(tensor_desc, tensor_desc, tensor_desc, scalar_desc), OUTPUT(out_tensor_desc));
...
// Test for precision
TEST_F(l2_addcmul_test, test_uint8) {
...
// Test for precision
TEST_F(l2_addcmul_test, test_int64) {
...
// Test for precision
TEST_F(l2_addcmul_test, test_double) {
...
//  auto scalar_desc = ScalarDesc(1.0f);
//
//  auto ut = OP_API_UT(aclnnAddcmul, INPUT(tensor_desc1, tensor_desc2, tensor_desc3, scalar_desc), OUTPUT(out_tensor_desc));
//
//  // SAMPLE: only test GetWorkspaceSize
//  uint64_t workspace_size = 0;
//  aclnnStatus aclRet = ut.TestGetWorkspaceSize(&workspace_size);
