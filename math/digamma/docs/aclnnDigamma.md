...
每个算子分为[两段式接口](./../../../docs/context/两段式接口.md)，必须先调用“aclnnDigammaGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnDigamma”接口执行计算。
...
- **参数说明：**
  ...
  - out(aclTensor\*, 计算输出): Device侧的aclTensor，数据类型支持FLOAT、FLOAT16、DOUBLE，支持[非连续的Tensor](./../../../docs/context/非连续的Tensor.md)，[数据格式](./../../../docs/context/数据格式.md)支持ND。
