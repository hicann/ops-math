...
inline KernelStatus CheckShape(const std::vector<int64_t>& sequence_shape, const std::vector<int64_t>& values_shape)
{
    ...
    if (!(dim_num == 1 || matched_before_last_dim(sequence_shape, values_shape))) {
        KERNEL_LOG_ERROR("sequence should be 1D tensor or the first N-1 dims of sequence and "
...
KernelStatus SearchSortedKernel::GetInputAndCheck(const CpuKernelContext& ctx)
{
    ...
    KERNEL_LOG_DEBUG("sequence shape is [%s], value shape is [%s].", VectorToString(sequence_dims).c_str(),
    ...
    if (sorter_t_ != nullptr) {
        ...
        if (sorter_t_->NumElements() != sequence_t_->NumElements()) {
            KERNEL_LOG_ERROR("sorted_sequence and sorter must have the same size, but got "
        ...
        if (!(sorter_min >= 0 && sorter_max < last_dim)) {
            KERNEL_LOG_ERROR("sorter index out of range, sorter_max is [%ld], sorter_min is "
    ...
    if (ctx.GetOutputsSize() != kOutputSize) {
        KERNEL_LOG_ERROR("Output number is: [%d], but SearchSorted needs [%zu] outputs.", ctx.GetOutputsSize(),
                         kOutputSize);
        return KERNEL_STATUS_PARAM_INVALID;
    }

    if (output_t_->NumElements() != values_t_->NumElements()) {
        KERNEL_LOG_ERROR("The output size should be same with values size, but got output "
...
KernelStatus CalSearchSorted(bool right, const Tensor* sequence_t, const Tensor* values_t, const Tensor* sort_t,
                             const Tensor* output_t, const CpuKernelContext& ctx)
{
    // Empty tensor
    ...
    auto task = [&seq_dim, &search_repeat, &search_len, &values, &sequence, &sort, &output, &right](int64_t start,
                                                                                                    int64_t end) {
...
uint32_t SearchSortedKernel::Compute(CpuKernelContext& ctx)
{
    ...
    KERNEL_CHECK_FALSE((res == KERNEL_STATUS_OK), static_cast<uint32_t>(res), "GetInputAndCheck failed, result = [%u].",
                       res);
    ...
    if (iter == calls.end()) {
        ...
        if (iter->second.find(output_dtype_) == iter->second.end()) {
            KERNEL_LOG_ERROR("SearchSorted op doesn't support output[0] tensor types: [%s]",
