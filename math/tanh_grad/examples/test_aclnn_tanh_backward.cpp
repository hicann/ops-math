...
int main()
{
    ...
    // 创建output aclTensor
    ret = CreateAclTensor(outputHostData, outputShape, aclDataType::ACL_FLOAT, outputDeviceAddr, output);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ...
    ret = aclnnTanhBackwardGetWorkspaceSize(gradOutput.get(), output.get(), gradInput.get(), &workspaceSize, &executor);
    ...
    ret = aclnnTanhBackward(workspaceAddr.get(), workspaceSize, executor, stream.get());
