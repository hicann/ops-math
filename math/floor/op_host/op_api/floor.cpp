...
static const aclTensor *FloorAiCore(const aclTensor *self, aclTensor *out, aclOpExecutor *executor) {
  ...
  OP_CHECK(ret == ACLNN_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "FloorAiCore ADD_TO_LAUNCHER_LIST_AICORE failed."), return nullptr);
  return out;
}
...
static const aclTensor *FloorAiCpu(const aclTensor *self, aclTensor *out, aclOpExecutor *executor) {
  ...
  OP_CHECK(ret == ACLNN_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "FloorAiCpu ADD_TO_LAUNCHER_LIST_AICPU failed."), return nullptr);
  return out;
}
