...
static const std::initializer_list<op::DataType> ASCEND910_DTYPE_OUT_LIST = {op::DataType::DT_FLOAT,
...
static bool CheckFormat(const aclTensor* self, const aclTensor* out)
{
    ...
    if (op::IsPrivateFormat(self->GetViewFormat())) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Format only supports ND, NCHW, NHWC, HWCN, NDHWC, NCDHW, not support [%s].",
...
aclnnStatus ExecTanhGetWorkspaceSize(const aclTensor* self, aclTensor* out, uint64_t* workspaceSize,
...
aclnnStatus aclnnTanhGetWorkspaceSize(const aclTensor* self, aclTensor* out, uint64_t* workspaceSize,
                                      aclOpExecutor** executor)
{
