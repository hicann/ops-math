...
{
    const Eigen::half val{static_cast<Eigen::half>(std::atan(static_cast<std::float_t>(x)))};
    return Eigen::half_impl::isnan(val) ? Eigen::half{0.0f} : val;
}

inline std::uint32_t ParallelFor(const CpuKernelContext& ctx, std::int64_t total, std::int64_t per_unit_size,
...
inline std::uint32_t ComputeAtanKernel(const CpuKernelContext& ctx)
{
    ...
    return ParallelFor(ctx, total, per_unit_size, [&](std::int64_t begin, std::int64_t end) {
        std::transform(input0 + begin, input0 + end, output + begin, ScalarAtan<T>);
    });
}
template <typename T>
...
inline std::uint32_t ExtraCheckAtan(const CpuKernelContext& ctx)
{
    ...
    if (ctx.Input(0)->GetDataSize() != ctx.Output(0)->GetDataSize()) {
        KERNEL_LOG_ERROR("The data size of the input [%lu] need to be the same as the output [%lu].",
...
inline std::uint32_t CheckAtan(CpuKernelContext& ctx, std::uint32_t inputs_num, std::uint32_t outputs_num)
{
    return NormalCheck(ctx, inputs_num, outputs_num) ? KERNEL_STATUS_PARAM_INVALID : ExtraCheckAtan(ctx);
}
...
{
    return detail::CheckAtan(ctx, kAtanInputNum, kAtanOutputNum) ? KERNEL_STATUS_PARAM_INVALID :
