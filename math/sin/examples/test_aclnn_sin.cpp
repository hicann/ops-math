...
int main()
{
    ...
    // 调用aclnnSin第一段接口
    ret = aclnnSinGetWorkspaceSize(self.get(), out.get(), &workspaceSize, &executor);
    ...
    // 调用aclnnSin第二段接口
    ret = aclnnSin(workspaceAddr.get(), workspaceSize, executor, stream.get());
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnSin failed. ERROR: %d\n", ret); return ret);

    uint64_t inplaceWorkspaceSize = 0;
    aclOpExecutor* inplaceExecutor;
    // 调用aclnnInplaceSin第一段接口
    ret = aclnnInplaceSinGetWorkspaceSize(self.get(), &inplaceWorkspaceSize, &inplaceExecutor);
    ...
    ret = aclnnInplaceSin(inplaceWorkspaceAddr.get(), inplaceWorkspaceSize, inplaceExecutor, stream.get());
