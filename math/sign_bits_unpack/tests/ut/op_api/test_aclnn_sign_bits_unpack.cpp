...
protected:
    ...
    static void TearDownTestCase() { cout << "sign_bits_unpack_test TearDown" << endl; }
};
...
// CheckValue size is negative
TEST_F(sign_bits_unpack_test, ascend910B2_case_value_size_negative)
{
    int64_t size = -1;
    aclDataType dType = ACL_FLOAT;
    auto self_tensor_desc = TensorDesc({2}, ACL_UINT8, ACL_FORMAT_ND);
...
// CheckFormat self
TEST_F(sign_bits_unpack_test, ascend910B2_case_format_self)
{
    int64_t size = 2;
    aclDataType dType = ACL_FLOAT;
    auto self_tensor_desc = TensorDesc({1, 1, 1, 2}, ACL_UINT8, ACL_FORMAT_NCHW);
    auto out_tensor_desc = TensorDesc({2, 8}, ACL_FLOAT, ACL_FORMAT_ND);
    auto ut = OP_API_UT(aclnnSignBitsUnpack, INPUT(self_tensor_desc, size, dType), OUTPUT(out_tensor_desc));
...
// CheckFormat out
TEST_F(sign_bits_unpack_test, ascend910B2_case_format_out)
{
    int64_t size = 2;
    aclDataType dType = ACL_FLOAT;
    auto self_tensor_desc = TensorDesc({2}, ACL_UINT8, ACL_FORMAT_ND);
    auto out_tensor_desc = TensorDesc({1, 1, 2, 8}, ACL_FLOAT, ACL_FORMAT_NCHW);
    auto ut = OP_API_UT(aclnnSignBitsUnpack, INPUT(self_tensor_desc, size, dType), OUTPUT(out_tensor_desc));
...
// Boundary: size equals the total unpacked element count.
TEST_F(sign_bits_unpack_test, ascend910B2_case_size_max)
{
    int64_t size = 16;
    aclDataType dType = ACL_FLOAT16;
    auto self_tensor_desc = TensorDesc({2}, ACL_UINT8, ACL_FORMAT_ND).Value(vector<uint8_t>{159, 15});
    auto out_tensor_desc = TensorDesc({16, 1}, ACL_FLOAT16, ACL_FORMAT_ND).Value(vector<int16_t>(16, 0));
    auto ut = OP_API_UT(aclnnSignBitsUnpack, INPUT(self_tensor_desc, size, dType), OUTPUT(out_tensor_desc));
...
TEST_F(sign_bits_unpack_test, ascend950_case_float_float)
{
    const auto originalSocVersion = op::GetCurrentPlatformInfo().GetSocVersion();
    op::SetPlatformSocVersion(op::SocVersion::ASCEND950);

    int64_t size = 2;
    aclDataType dType = ACL_FLOAT;
    auto self_tensor_desc = TensorDesc({2}, ACL_UINT8, ACL_FORMAT_ND).Value(vector<uint8_t>{159, 15});
    auto out_tensor_desc = TensorDesc({2, 8}, ACL_FLOAT, ACL_FORMAT_ND).Value(vector<float>(16, 0));
    auto ut = OP_API_UT(aclnnSignBitsUnpack, INPUT(self_tensor_desc, size, dType), OUTPUT(out_tensor_desc));
    ...
    op::SetPlatformSocVersion(originalSocVersion);
}
