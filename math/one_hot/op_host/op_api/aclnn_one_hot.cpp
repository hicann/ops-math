...
static bool CheckMaxDimension(const aclTensor *self, const aclTensor *onValue, const aclTensor *offValue, const aclTensor *out) {
  static bool isSimtVersion = GetCurrentPlatformInfo().GetSocVersion() == SocVersion::ASCEND910_95;
  if (isSimtVersion) {
...
static bool CheckMaxNumClasses(const aclTensor* self, int64_t numClasses)
{
  ...
  if (self->IsEmpty()) {
    ...
    OP_LOGE(ACLNN_ERR_PARAM_INVALID, "NumClasses should be greater and equal than %ld, but got %ld.", MIN_NUM_CLASSES,
            numClasses);
    return false;
  }
  return true;
