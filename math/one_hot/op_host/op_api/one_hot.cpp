...
const aclTensor* OneHot(const aclTensor* self, const aclTensor* depth, const aclTensor* onValue,
                        const aclTensor* offValue, int64_t axis, aclOpExecutor* executor)
{
  ...
  auto retAicore = ADD_TO_LAUNCHER_LIST_AICORE(OneHot, OP_INPUT(self, depth, onValue, offValue), OP_ATTR(axis), OP_OUTPUT(out));
  OP_CHECK(retAicore ==  ACLNN_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "OneHotAiCore ADD_TO_LAUNCHER_LIST_AICORE failed."),
    return nullptr);
  return out;
}
}  // namespace l0op
