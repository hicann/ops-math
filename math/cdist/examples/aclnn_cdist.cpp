...
int main()
{
    ...
    // 调用aclnnCdist第一段接口
    ret = aclnnCdistGetWorkspaceSize(x1.get(), x2.get(), p, compute_mode, out.get(), &workspaceSize, &executor);
    ...
    // 调用aclnnCdist第二段接口
    ret = aclnnCdist(workspaceAddr.get(), workspaceSize, executor, stream.get());
