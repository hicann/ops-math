...
static inline int64_t BrcCeilDiv(int64_t a, int64_t b)
{
    int64_t d = (b > 0) ? b : 1;
    return (a + d - 1) / d;
}
static inline int64_t BrcAlignDown(int64_t a, int64_t al)
{
    int64_t d = (al > 0) ? al : 1;
    return (a / d) * d;
}
...
template <int IN_NUM>
inline void ComputeTiling(BroadcastTilingData<IN_NUM>& td, int dtSize, int64_t coreNum, int64_t ubSize,
                          int64_t aliveBuf)
{
    ...
    for (int ax = rank - 1; ax >= 0; ax--) {
        if (innerBelow * td.outputDims[ax] > td.elemNum) {
            splitAxis = ax;
            break;
        }
    ...
    int64_t outerProd = 1;
    for (int j = 0; j < td.ubSplitAxis; j++)
        outerProd *= td.outputDims[j];
...
__aicore__ inline void GetAxesIndices(int64_t (&axes)[BROADCAST_MAX_DIMS], int64_t tileLinear,
                                      const int64_t (&outputDims)[BROADCAST_MAX_DIMS], int64_t ubSplitAxis,
                                      int64_t tileTotal)
{
                                         ...
                                         int64_t ubOuter)
{
    ...
    if (axes[ubSplitAxis] == ubOuter) {
        axes[ubSplitAxis] = 0;
        if (ubSplitAxis > 0)
            axes[ubSplitAxis - 1]++; // 防 ubSplitAxis=0 时 axes[-1] 越界
    }
    for (int64_t idx = ubSplitAxis - 1; idx >= 1; idx--) {
        if (axes[idx] == outputDims[idx]) {
            axes[idx] = 0;
            axes[idx - 1]++;
        }
    }
...
__aicore__ inline int64_t GetGmOffset(const int64_t (&axes)[BROADCAST_MAX_DIMS],
                                      const int64_t (&strides)[BROADCAST_MAX_DIMS], int64_t ubSplitAxis,
                                      int64_t ubFormer)
{
    int64_t off = 0;
    for (int64_t idx = 0; idx < ubSplitAxis; idx++)
        off += axes[idx] * strides[idx];
    off += axes[ubSplitAxis] * strides[ubSplitAxis] * ubFormer; // 切分轴以 ubFormer 行为步长
    return off;
}

// ④ 填 UB 内 Broadcast 的 dst/src shape（首轴随主块/尾块变化），供 ③ 使用
__aicore__ inline void FillUbShape(uint32_t (&dstShape)[BROADCAST_MAX_DIMS], uint32_t (&srcShape)[BROADCAST_MAX_DIMS],
                                   const int64_t (&outputDims)[BROADCAST_MAX_DIMS],
                                   ...
                                   int64_t rows)
{
    ...
    int64_t inner = 1;
    for (int64_t j = ubSplitAxis + 1; j < shapeLen; j++)
        inner *= inDims[j];
...
__aicore__ inline AscendC::MultiCopyParams<T, NDDMA_DIM> MakeNddmaParams(
    ...
    const int64_t (&inputStrides)[BROADCAST_MAX_DIMS], int64_t shapeLen, int64_t ubSplitAxis, int64_t ubSplitSize)
{
    ...
    if (axisInsideUb < 0)
        axisInsideUb = 0; // 仅防越界；rank>5 应由 Host 拦截
...
__aicore__ inline void BroadcastNddma(const GlobalTensor<T>& gm, const LocalTensor<T>& ub,
                                      const int64_t (&outputDims)[BROADCAST_MAX_DIMS],
                                      const int64_t (&outputStrides)[BROADCAST_MAX_DIMS],
                                      const int64_t (&inputStrides)[BROADCAST_MAX_DIMS], int64_t shapeLen,
                                      int64_t ubSplitAxis, int64_t ubSplitSize)
{
...
{
    dimLen = 1;
    for (int j = 0; j < td.shapeLen; j++)
        dimLen *= td.outputDims[j];
    scalarFlag = 0;
    for (int i = 0; i < IN_NUM; i++) {
        int64_t cnt = 1;
        for (int j = 0; j < td.shapeLen; j++)
            cnt *= td.inputDims[i][j];
        if (cnt == 1)
            scalarFlag |= (1 << i); // 纯标量
        else if (cnt != dimLen)
            return false; // 既非标量又非满 shape → 不能塌一维
    }
    return true;
}
...
inline void ComputeOneDimTiling(OneDimTilingData& t, int64_t dimLen, int32_t scalarFlag, int dtSize, int64_t coreNum,
                                int64_t ubSize, int64_t aliveBuf)
{
    ...
    int64_t cn = (coreNum < 1) ? 1 : coreNum;
    if (ubOuter < cn)
        cn = (ubOuter < 1) ? 1 : ubOuter; // 块数不足核数 → 减核（小 shape 自适应）
...
template <typename T>
__aicore__ inline void OneDimLoadInput(const LocalTensor<T>& dst, const GlobalTensor<T>& gm, int64_t offset,
                                       int64_t len, bool isScalar, bool firstTile)
{
    if (isScalar) {
        if (firstTile)
            AscendC::Duplicate(dst, gm.GetValue(0), len); // 标量值铺满；之后复用
    } else {
