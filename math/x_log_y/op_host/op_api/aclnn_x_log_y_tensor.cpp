...
static bool CheckPromoteType(const aclTensor* self, const aclTensor* other, op::DataType& promoteType)
{
    promoteType = op::PromoteType(self->GetDataType(), other->GetDataType());
    promoteType = (IsIntegralType(promoteType, true)) ? op::DataType::DT_FLOAT : promoteType;
...
static aclnnStatus CheckParams(
    const aclTensor* self, const aclTensor* other, const aclTensor* out, op::DataType& promoteType)
{
    // 1. 检查参数是否为空指针
...
aclnnStatus XLogYTensorGetWorkspaceSize(
