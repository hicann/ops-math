...
int main()
{
    ...
    // 调用aclnnDiv第一段接口
    ret = aclnnDivGetWorkspaceSize(self.get(), other.get(), out.get(), &workspaceSize, &executor);
    ...
    // 调用aclnnDiv第二段接口
    ret = aclnnDiv(workspaceAddr.get(), workspaceSize, executor, stream.get());
    ...
    ret = aclnnInplaceDivGetWorkspaceSize(self.get(), other.get(), &workspaceSize, &executor);
    ...
    ret = aclnnInplaceDiv(workspaceAddr.get(), workspaceSize, executor, stream.get());
