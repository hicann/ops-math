...
#include "nn_other.h"
#include "../op_graph/sparse_bincount_proto.h"
...
{
    Status ret = SUCCESS;
    // SparseBincount: 5 inputs (indices, values, dense_shape, size, weights), 1 output
    auto op1 = op::SparseBincount("sparse_bincount1");

    // indices: shape [4, 1], dtype int64
    std::vector<int64_t> indicesShape = {4, 1};
    ADD_INPUT(1, indices, ge::DT_INT64, indicesShape);

    // values: shape [4], dtype int32
    std::vector<int64_t> valuesShape = {4};
    ADD_INPUT(2, values, ge::DT_INT32, valuesShape);

    // dense_shape: shape [1], dtype int64 (1D mode)
    std::vector<int64_t> denseShapeShape = {1};
    ADD_CONST_INPUT(3, dense_shape, ge::DT_INT64, denseShapeShape);

    // size: shape [1], dtype int32
    std::vector<int64_t> sizeShape = {1};
    ADD_CONST_INPUT(4, size, ge::DT_INT32, sizeShape);

    // weights: shape [4], dtype float32
    std::vector<int64_t> weightsShape = {4};
    ADD_INPUT(5, weights, ge::DT_FLOAT, weightsShape);

    // output: shape [8], dtype float32
