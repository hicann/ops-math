                                   ...
                                   int64_t correction, bool keepdim, bool invert, float eps, aclOpExecutor* executor)
{
    ...
    auto stdWithMeanOut = executor->AllocTensor(self->GetDataType(), self->GetStorageFormat(),
    ...
    INFER_SHAPE(ReduceStdWithMean, OP_INPUT(self, mean), OP_OUTPUT(stdWithMeanOut),
                OP_ATTR(dim, unbiased, keepdim, invert, eps, correction));
    auto ret = ADD_TO_LAUNCHER_LIST_AICORE(ReduceStdWithMean, OP_INPUT(self, mean), OP_OUTPUT(stdWithMeanOut),
                                           OP_ATTR(dim, unbiased, keepdim, invert, eps, correction));
