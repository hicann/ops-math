...
#include "nn_other.h"
#include "../op_graph/fresnel_sin_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    fresnel_sin1.set_input_##intputName(placeholder##intputIndex);                                                  \
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    ...
    fresnel_sin1.update_output_desc_##outputName(outputName##outputIndex##_desc);
                    ...
                    int value)
{
    ...
    FILE* fp = fopen("./test_input.bin", "rb");
    if (fp != nullptr) {
        size_t read = fread(pData, 1, data_len, fp);
        fclose(fp);
        if (read != data_len) {
            printf("[WARN] test_input.bin: read %zu bytes, expected %u\n", read, data_len);
        }
    } else {
        memset(pData, 0, data_len);
        printf("[WARN] test_input.bin not found, using zeros\n");
    }
...
int main(int argc, char* argv[])
{
    ...
    if (argc < 2) {
        printf("%s - ERROR - [XIR]: Missing required argument. Usage: %s <dtype_index>\n", GetTime().c_str(), argv[0]);
        ge::GEFinalize();
        return FAILED;
    }
