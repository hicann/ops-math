...
/*!
 * \file atan_grad_tiling_arch35.cpp
 * \brief AtanGrad Tiling 实现（arch35 / Ascend950）
 *
 * Tiling 核心逻辑：
...
#include "../../op_kernel/arch35/atan_grad_tiling_key.h"
...
static ge::graphStatus QueryPlatform(gert::TilingContext* context, uint64_t& ubSize, int64_t& coreNum)
{
    ...
    OP_CHECK_IF(ubSize == 0 || coreNum == 0,
                OP_LOGE(context, "platform info invalid: ubSize=%lu coreNum=%ld", ubSize, coreNum),
    ...
    gert::TilingContext* context, int64_t& totalNum, ge::DataType& dataType)
{
    ...
    const auto& storageShape = inputY->GetStorageShape();
    // Scalar (0-D) tensors are treated as 1-element vectors for tiling purposes.
    totalNum = (storageShape.GetDimNum() == 0) ? 1 : storageShape.GetShapeSize();
