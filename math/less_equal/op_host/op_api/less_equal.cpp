...
static const std::initializer_list<op::DataType> ASCEND910_95_DTYPE_SUPPORT_LIST = {
  ...
  op::DataType::DT_INT8, op::DataType::DT_UINT8, op::DataType::DT_BF16, op::DataType::DT_UINT64,
  op::DataType::DT_BOOL};
...
// AICORE算子kernel
static const aclTensor *LessEqualAiCore(const aclTensor *self, const aclTensor *other,
                                        aclTensor *leOut, aclOpExecutor *executor) {
  L0_DFX(LessEqualAiCore, self, other, leOut);
  // 使用框架宏 ADD_TO_LAUNCHER_LIST_AICORE，将LessEqual算子加入任务队列
  auto ret = ADD_TO_LAUNCHER_LIST_AICORE(LessEqual,
                                        OP_INPUT(self, other),
                                        OP_OUTPUT(leOut));
  OP_CHECK(ret == ACLNN_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "LessEqualAiCore ADD_TO_LAUNCHER_LIST_AICORE failed."), return nullptr);
  return leOut;
}

// AICPU算子 TF
static const aclTensor *LessEqualAiCpuTf(const aclTensor *self, const aclTensor *other,
                                         aclTensor *leOut, aclOpExecutor *executor) {
  ...
  auto ret = ADD_TO_LAUNCHER_LIST_AICPU(LessEqual,
                                        OP_ATTR_NAMES({"T", "incompatible_shape_error"}),
                                        OP_INPUT(self, other),
                                        OP_OUTPUT(leOut),
                                        OP_ATTR(self->GetDataType(), true));
  OP_CHECK(ret == ACLNN_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "LessEqualAiCpuTf ADD_TO_LAUNCHER_LIST_AICPU failed."), return nullptr);
  return leOut;
}
