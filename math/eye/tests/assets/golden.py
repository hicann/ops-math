...
def eye_golden(num_rows: int,
    ...
    dtype_str = _DATA_TYPE_INT_TO_STR.get(dtype, str(dtype))
    if dtype_str in _SPECIAL_DTYPES:
        np_dtype = _resolve_custom_numpy_dtype(dtype_str)
    elif dtype_str == "bool":
        np_dtype = np.bool_
    else:
        np_dtype = getattr(np, dtype_str)

    eye = np.eye(n, m, dtype=np_dtype)
    if batch_shape:
