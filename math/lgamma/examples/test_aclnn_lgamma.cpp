...
void PrintOutResult(std::vector<int64_t>& shape, DeviceMemPtr& deviceAddr)
{
    ...
    for (int64_t i = 0; i < size; i++) {
        LOG_PRINT("lgamma result[%ld] is: %f\n", i, resultData[i]);
    }
}
...
int main()
{
    ...
    // 调用aclnnLgamma第一段接口
    ret = aclnnLgammaGetWorkspaceSize(self.get(), out.get(), &workspaceSize, &executor);
    ...
    // 调用aclnnLgamma第二段接口
    ret = aclnnLgamma(workspaceAddr.get(), workspaceSize, executor, stream.get());
