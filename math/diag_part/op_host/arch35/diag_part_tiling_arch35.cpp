...
ge::graphStatus DiagPartTiling::Init()
{
    ...
    OP_CHECK_IF((coreNum_ <= 0),
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(tilingContext_->GetNodeName(), "core num",
                                                      std::to_string(coreNum_).c_str(), "must be greater than 0"),
                return ge::GRAPH_FAILED);
    ubSize_ = compileInfo->ub_size;
    OP_CHECK_IF((ubSize_ <= 0),
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(tilingContext_->GetNodeName(), "ub size",
    ...
    OP_CHECK_IF((res != ge::GRAPH_SUCCESS), OP_LOGE(tilingContext_->GetNodeName(), "SetLocalMemorySize ubSize failed."),
                return ge::GRAPH_FAILED);
    OP_LOGD(tilingContext_->GetNodeName(), "Init DiagPartTiling success.");
...
ge::graphStatus DiagPartTiling::RunDiagPartTiling()
{
    ...
    OP_CHECK_IF(DiagPartVerifying() != ge::GRAPH_SUCCESS,
                OP_LOGE(tilingContext_->GetNodeName(), "DiagPartTiling failed to verify params!"),
                return ge::GRAPH_FAILED);
...
ge::graphStatus DiagPartTiling::DiagPartVerifying()
{
    ...
    OP_CHECK_IF((xDimNum <= 0 || (xDimNum % TWO) != 0),
                OP_LOGE_FOR_INVALID_SHAPEDIM(tilingContext_->GetNodeName(), "x", std::to_string(xDimNum).c_str(),
    ...
    for (uint64_t i = 0; i < xDimNum / TWO; i++) {
        OP_CHECK_IF((xShape.GetDim(i) != xShape.GetDim(i + xDimNum / TWO)),
    ...
    OP_CHECK_IF((yDimNum != xDimNum / TWO),
                OP_LOGE_FOR_INVALID_SHAPEDIM(tilingContext_->GetNodeName(), "y", std::to_string(yDimNum).c_str(),
                                             "equal to half of the dim num of x"),
                return ge::GRAPH_FAILED);
    for (uint64_t i = 0; i < yDimNum; i++) {
        OP_CHECK_IF((xShape.GetDim(i) != yShape.GetDim(i)),
    ...
    OP_LOGD(tilingContext_->GetNodeName(), "DiagPartVerifying success.");
    return ge::GRAPH_SUCCESS;
}
...
{
    OP_LOGI(tilingContext_->GetNodeName(), "tilingData is sideLength:%ld, \
