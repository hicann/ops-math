...
static bool CheckPromoteType(const op::DataType selfDtype, const op::DataType otherDtype,
                             const op::DataType outDtype, const bool isOtherScalar) {
  ...
  if (GetCurrentPlatformInfo().GetSocVersion() == SocVersion::ASCEND910_95) {
    ...
    if (!CheckType(promoteType, supportList)) {
      OP_LOGE(ACLNN_ERR_PARAM_INVALID, "aclnnFloorDivide not implemented for input dtype %s," \
...
aclnnStatus aclnnInplaceFloorDivideGetWorkspaceSize(aclTensor* selfRef, const aclTensor* other,
                                                    uint64_t* workspaceSize, aclOpExecutor** executor) {
