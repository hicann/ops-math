...
- **参数说明：**
  ...
  - self(aclTensor*, 计算输入)：Device侧的aclTensor，shape需要与other满足[broadcast关系](common/broadcast关系.md)。支持[非连续的Tensor](common/非连续的Tensor.md)，[数据格式](common/数据格式.md)支持ND。
     ...
     * <term>昇腾910_95 AI处理器</term>：FLOAT、FLOAT16、DOUBLE、INT32、INT64、INT8、UINT8、INT16、BOOL、BFLOAT16，数据类型与other的数据类型需满足[TensorScalar互推导关系](common/TensorScalar互推导关系.md)。
  - other(aclScalar*, 计算输入)：Host侧的aclScalar。
     ...
     * <term>昇腾910_95 AI处理器</term>：FLOAT、FLOAT16、DOUBLE、INT32、INT64、INT8、UINT8、INT16、BOOL、BFLOAT16，数据类型与self的数据类型需满足[TensorScalar互推导关系](common/TensorScalar互推导关系.md)。
...
- **参数说明：**
  ...
  - other(aclScalar*, 计算输入)：Host侧的aclScalar。
     ...
     * <term>昇腾910_95 AI处理器</term>：FLOAT、FLOAT16、DOUBLE、INT32、INT64、INT8、UINT8、INT16、BOOL、BFLOAT16，数据类型与selfRef的数据类型需满足[TensorScalar互推导关系](common/TensorScalar互推导关系.md)。
  - workspaceSize(uint64_t*, 出参)：返回需要在Device侧申请的workspace大小。
  - executor(aclOpExecutor**, 出参)：返回op执行器，包含了算子计算流程。
