...
static const aclTensor *CeilAiCore(const aclTensor *self, aclTensor *out, aclOpExecutor *executor) {
  ...
  OP_CHECK(ret == ACLNN_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "Ceil ADD_TO_LAUNCHER_LIST_AICORE failed."), return nullptr);
  return out;
}
...
static const aclTensor *CeilAiCpu(const aclTensor *self, aclTensor *out, aclOpExecutor *executor) {
  ...
  OP_CHECK(ret ==  ACL_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "CeilAiCpu ADD_TO_LAUNCHER_LIST_AICPU failed."), return nullptr);
  return out;
}
