...
#include "nn_other.h"
#include "../op_graph/kl_div_v2_proto.h"
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape)                          \
    ...
    klDiv1.set_input_##inputName(placeholder##inputIndex);                                   \
...
#define ADD_CONST_INPUT(inputIndex, inputName, inputDtype, inputShape)                    \
    ...
    graph.AddOp(placeholder##inputIndex);                                                   \
    klDiv1.set_input_##inputName(placeholder##inputIndex);                                   \
    klDiv1.update_input_desc_##inputName(placeholder##inputIndex##_desc);                    \
    inputs.push_back(placeholder##inputIndex)
...
#define ADD_INPUT_ATTR(attrName, attrValue)                                                  \
    klDiv1.set_attr_##attrName(attrValue)
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto klDiv1 = op::KLDivV2("kl_div_v2");
    std::vector<std::vector<int64_t>> shapes = {{216}, {216}, {216}};

    ADD_INPUT(1, x, inDtype1, shapes[0]);
    ADD_CONST_INPUT(2, target, inDtype2, shapes[1]);
    ADD_OUTPUT(3, y, inDtype1, shapes[2]);
    ADD_INPUT_ATTR(reduction, "mean");
    ADD_INPUT_ATTR(log_target, true);
