...
int main()
{
    ...
    DeviceMemPtr targetDeviceAddr(nullptr, &aclrtFree);
    DeviceMemPtr outDeviceAddr(nullptr, &aclrtFree);
    TensorPtr self(nullptr, &aclDestroyTensor);
    TensorPtr target(nullptr, &aclDestroyTensor);
    ...
    // 创建target aclTensor
    ret = CreateAclTensor(targetHostData, targetShape, aclDataType::ACL_FLOAT, targetDeviceAddr, target);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建out aclTensor
    ...
    // 调用aclnnKlDiv第一段接口
    ret = aclnnKlDivGetWorkspaceSize(self.get(), target.get(), reduction, log_target, out.get(), &workspaceSize,
                                     &executor);
    ...
    // 调用aclnnKlDiv第二段接口
    ret = aclnnKlDiv(workspaceAddr.get(), workspaceSize, executor, stream.get());
