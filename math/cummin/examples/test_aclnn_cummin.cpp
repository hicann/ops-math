...
int main()
{
    ...
    DeviceMemPtr valuesOutDeviceAddr(nullptr, &aclrtFree);
    DeviceMemPtr indicesOutDeviceAddr(nullptr, &aclrtFree);
    TensorPtr self(nullptr, &aclDestroyTensor);
    TensorPtr valuesOut(nullptr, &aclDestroyTensor);
    TensorPtr indicesOut(nullptr, &aclDestroyTensor);
    ...
    ret = CreateAclTensor(valuesOutHostData, valuesOutShape, aclDataType::ACL_FLOAT, valuesOutDeviceAddr, valuesOut);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建indicesOut aclTensor
    ret = CreateAclTensor(indicesOutHostData, indicesOutShape, aclDataType::ACL_INT64, indicesOutDeviceAddr,
                          indicesOut);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ...
    // 调用aclnnCummin第一段接口
    ret = aclnnCumminGetWorkspaceSize(self.get(), dim, valuesOut.get(), indicesOut.get(), &workspaceSize, &executor);
    ...
    // 调用aclnnCummin第二段接口
    ret = aclnnCummin(workspaceAddr.get(), workspaceSize, executor, stream.get());
    ...
    ret = aclrtMemcpy(resultData.data(), resultData.size() * sizeof(resultData[0]), valuesOutDeviceAddr.get(),
    ...
    ret = aclrtMemcpy(indicesResultData.data(), indicesResultData.size() * sizeof(indicesResultData[0]),
                      indicesOutDeviceAddr.get(), indicesSize * sizeof(indicesResultData[0]),
