...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < (int)output.size(); i++) {
        ...
        float* data = reinterpret_cast<float*>(t.GetData());
    ...
    bool pass = true;
    if (output.size() >= 3) {
        float* d0 = reinterpret_cast<float*>(output[0].GetData());
        float* d1 = reinterpret_cast<float*>(output[1].GetData());
        float* d2 = reinterpret_cast<float*>(output[2].GetData());
