...
int main()
{
    ...
    DeviceMemPtr inputAddr(nullptr, &aclrtFree);
    DeviceMemPtr outMantissaAddr(nullptr, &aclrtFree);
    DeviceMemPtr outFixedAddr(nullptr, &aclrtFree);
    DeviceMemPtr outVarAddr(nullptr, &aclrtFree);
    DeviceMemPtr pdfAddr(nullptr, &aclrtFree);
    DeviceMemPtr recoverAddr(nullptr, &aclrtFree);
    TensorPtr input(nullptr, &aclDestroyTensor);
    TensorPtr outMantissa(nullptr, &aclDestroyTensor);
    TensorPtr outFixed(nullptr, &aclDestroyTensor);
    TensorPtr pdf(nullptr, &aclDestroyTensor);
    TensorPtr outVar(nullptr, &aclDestroyTensor);
    TensorPtr recover(nullptr, &aclDestroyTensor);
    // 创建out aclTensor
    ret = CreateAclTensor(inputHost, {1, 65536}, aclDataType::ACL_FLOAT, inputAddr, input);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ret = CreateAclTensor(mantissaHost, {1, 49152}, aclDataType::ACL_FLOAT, outMantissaAddr, outMantissa);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ret = CreateAclTensor(fixedHost, {1, 16384}, aclDataType::ACL_FLOAT, outFixedAddr, outFixed);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ret = CreateAclTensor(varHost, {1, 16384}, aclDataType::ACL_FLOAT, outVarAddr, outVar);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ret = CreateAclTensor(pdfHost, {1, 256}, aclDataType::ACL_INT32, pdfAddr, pdf);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ret = CreateAclTensor(recoverHost, {1, 65536}, aclDataType::ACL_FLOAT, recoverAddr, recover);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 3. 调用CANN算子库API，需要修改为具体的Api名称
    uint64_t workspaceSize = 0;
    aclOpExecutor* executor;
    // 调用aclnnHansEncode第一段接口
    ret = aclnnHansEncodeGetWorkspaceSize(input.get(), pdf.get(), statistic, reshuff, outMantissa.get(), outFixed.get(),
                                          outVar.get(), &workspaceSize, &executor);
    ...
    ret = aclnnHansEncode(workspaceAddr.get(), workspaceSize, executor, stream.get());
    ...
    ret = aclrtMemcpy(resultData.data(), size, outFixedAddr.get(), size, ACL_MEMCPY_DEVICE_TO_HOST);
    ...
    ret = aclnnHansDecodeGetWorkspaceSize(outMantissa.get(), outFixed.get(), outVar.get(), pdf.get(), reshuff,
                                          recover.get(), &workspaceSizeDecode, &executorDecode);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnHansDecodeGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    // 根据第一段接口计算出的workspaceSize申请device内存
    DeviceMemPtr workspaceAddrDecode(nullptr, &aclrtFree);
    if (workspaceSizeDecode > static_cast<uint64_t>(0)) {
        void* rawWorkspaceAddr = nullptr;
        ret = aclrtMalloc(&rawWorkspaceAddr, workspaceSizeDecode, ACL_MEM_MALLOC_HUGE_FIRST);
        CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("allocate workspace failed. ERROR: %d\n", ret); return ret);
    ...
    ret = aclrtMemcpy(recoverData.data(), 65536 * sizeof(recoverData[0]), recoverAddr.get(),
                      65536 * sizeof(recoverData[0]), ACL_MEMCPY_DEVICE_TO_HOST);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("copy result from device to host failed. ERROR: %d\n", ret); return ret);
