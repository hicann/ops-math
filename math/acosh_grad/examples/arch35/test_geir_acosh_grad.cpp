...
#include "array_ops.h"
#include "nn_other.h"
#include "../../op_graph/acosh_grad_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                          \
    ...
    acoshGrad.set_input_##intputName(placeholder##intputIndex);                              \
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                    \
    ...
    graph.AddOp(placeholder##intputIndex);                                                   \
    acoshGrad.set_input_##intputName(placeholder##intputIndex);                              \
    acoshGrad.update_input_desc_##intputName(placeholder##intputIndex##_desc);               \
    inputs.push_back(placeholder##intputIndex)

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                        \
    TensorDesc outputName##outputIndex##_desc =                                              \
        TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype);                          \
    acoshGrad.update_output_desc_##outputName(outputName##outputIndex##_desc)
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto acoshGrad = op::AcoshGrad("acosh_grad");

    std::vector<int64_t> yShape = {4, 2};
    ADD_INPUT(1, y, inDtype, yShape);
    ADD_INPUT(2, dy, inDtype, yShape);

    ADD_OUTPUT(1, z, inDtype, yShape);

    outputs.push_back(acoshGrad);
    // 添加完毕
    return SUCCESS;
}
...
void ProcessIOData(std::vector<ge::Tensor>& input, std::vector<ge::Tensor>& output) {
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        float expected = 2.0f / 1.7320508f;  // dy/sqrt(y*y-1) = 2/sqrt(3)
        bool allok = true;
        for (int64_t j = 0; j < output_shape; j++) {
            LOG_PRINT("result[%ld] is: %f (expected %f)\n", j, resultData[j], expected);
            if (resultData[j] < expected - 0.01f || resultData[j] > expected + 0.01f) {
                allok = false;
            }
        }
        LOG_PRINT("%s - INFO - [XIR]: AcoshGrad precision %s\n", GetTime().c_str(), allok ? "PASS" : "FAIL");
    }
}
...
{
    // 1、创建图对象
    const char *graph_name = "tc_ge_irrun_acosh_grad_test";
