...
int main()
{
    ...
    // 调用aclnnArgMin第一段接口
    ret = aclnnArgMinGetWorkspaceSize(self.get(), realDim, realKeepDim, out.get(), &workspaceSize, &executor);
    ...
    // 调用aclnnArgMin第二段接口
    ret = aclnnArgMin(workspaceAddr.get(), workspaceSize, executor, stream.get());
