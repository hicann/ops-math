...
public:
    ...
    __aicore__ inline void MergeAcrossCores(bool rowActive)
    {
        ...
        static_cast<Derived*>(this)->FinalSortAndCopy(rowActive);
    }
    ...
    {
        // A full window needs no refill. Consume every run in one merge rather
        // than stopping when the first source is exhausted.
        bool allLoaded = true;
        for (uint32_t i = 0; i < MERGE_LIST_MAX_NUM; ++i) {
            allLoaded = allLoaded && this->dealLengths_[i] == this->listRemainElements_[i];
        }
        if (allLoaded) {
            MergeLoadedLists<false>();
            return;
        }
        MergeLoadedLists<true>();
    }

    template <bool SuspendOnExhaustion>
    __aicore__ inline void MergeLoadedLists()
    {
        ...
        if (this->remainListNum_ == TWO_WAY_MERGE_LIST_NUM) {
            ...
            MrgSortSrcList sortListTail = MrgSortSrcList(tmpUbInputs[0], tmpUbInputs[1], tmpUbInputs[0],
                                                         tmpUbInputs[0]);
            MrgSort<CONVERT_TYPE, SuspendOnExhaustion>(sortTempBuffer, sortListTail, this->elementCountList_,
                                                       this->listSortedNums_, this->validBitTail_, 1);
        } else if (this->remainListNum_ == THREE_WAY_MERGE_LIST_NUM) {
        ...
        if constexpr (!SuspendOnExhaustion) {
            // Non-suspending MrgSort does not report per-list consumption. All
            // remaining records were loaded, so their input lengths are exact.
            for (uint32_t i = 0; i < this->remainListNum_; ++i) {
                this->listSortedNums_[i] = this->elementCountList_[i];
            }
        }
