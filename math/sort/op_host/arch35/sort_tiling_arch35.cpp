...
ge::graphStatus CheckSortOutputDtypes(
    gert::TilingContext *context, SortKthTileInfo &sortTileInfo, ge::DataType dataType)
{
...
ge::graphStatus SortCheckParams(gert::TilingContext *context, SortKthTileInfo &sortTileInfo)
{
    ...
    OP_CHECK_IF(!ge::TypeUtils::GetDataTypeLength(dataType, sortTileInfo.dtypeSize),
    ...
    OP_CHECK_IF(sortAxis < 0 || sortAxis >= static_cast<int32_t>(sortTileInfo.rank),
        OP_LOGE_WITH_INVALID_ATTR(context->GetNodeName(), "axis",
            std::to_string(sortAxis).c_str(), "range [-dimNum, dimNum - 1)"),
        return ge::GRAPH_FAILED);
...
// =============================================================================
static bool TryGetSortNonLastTileCount(const SortKthTileInfo &info, uint32_t innerChunk,
    uint32_t &innerLoopNum, uint64_t &tileCount)
{
    if (innerChunk == 0U || info.innerSize <= 0 || info.outerSize <= 0) {
        return false;
    }
    uint64_t innerLoopNum64 = Ops::Base::CeilDiv(
        static_cast<uint64_t>(info.innerSize), static_cast<uint64_t>(innerChunk));
...
bool ComputeSortNonLastBatchPlan(const SortKthTileInfo &sortTileInfo, uint32_t &batchSize,
    uint32_t &innerLoopNum, uint32_t &batchNum)
{
    batchSize = static_cast<uint32_t>(std::min<uint64_t>(
        static_cast<uint64_t>(batchSize), static_cast<uint64_t>(sortTileInfo.innerSize)));
...
bool IsRadixSortOneCore(SortKthTileInfo &sortTileInfo)
{
    ...
    if (!ComputeRadixOneCoreUbSizes(sortTileInfo.lastAxis, sortTileInfo.dtypeSize,
        static_cast<uint32_t>(sizeof(int32_t)), sortTileInfo.blockUbSize, xUbSize, y2UbSize)) {
        return false;
    }
    ...
    SetSortTmpSize(sortTileInfo.dataType, static_cast<uint32_t>(sortTileInfo.lastAxis),
...
// =============================================================================
bool IsAxisOneCopy(const SortKthTileInfo &sortTileInfo)
{
    return sortTileInfo.lastAxis == static_cast<int64_t>(1);
}
...
void FillSmallAxisBatched(gert::TilingContext *context, SortKthTileInfo &sortTileInfo,
...
ge::graphStatus SetRadixOneCoreTiling(gert::TilingContext *context, SortKthTileInfo &sortTileInfo)
{
    ...
    uint64_t sortLoopTimes64 =
        Ops::Base::CeilDiv(sortTileInfo.unsortedDim, static_cast<int64_t>(sortTileInfo.maxCoreNum));
...
ge::graphStatus SetRadixMoreCoreTiling(gert::TilingContext *context, SortKthTileInfo &info)
{
    OP_CHECK_IF(!FillRadixMoreCoreInfo(info),
        OP_LOGE(context->GetNodeName(), "radix more-core plan failed"), return ge::GRAPH_FAILED);
...
void PrintTilingDataSort(gert::TilingContext *context, SortKthTileInfo &sortTileInfo)
{
    OP_LOGI(context->GetNodeName(),
        ...
        sortTileInfo.keyParams5, sortTileInfo.tmpUbSize, sortTileInfo.lastAxis, sortTileInfo.unsortedDim,
        sortTileInfo.outerSize, sortTileInfo.innerSize, sortTileInfo.innerChunk);
    return;
}
...
bool TrySmallAxis(gert::TilingContext *context, SortKthTileInfo &sortTileInfo, uint64_t &schId)
{
    ...
    bool selected = sortTileInfo.isNonLastAxis ?
        SelectNonLastSmallAxisRoute(sortTileInfo, smallAxisRoutePlan) :
...
static bool ComputeSortNonLastUbLayout(const SortKthTileInfo &sortTileInfo, uint32_t innerChunk,
    uint32_t sortCount, bool useMergeSort, SortNonLastUbLayout &layout)
{
...
static bool EstimateSortNonLastSmallAxisUb(SortKthTileInfo &sortTileInfo, uint32_t innerChunk,
    uint64_t &peakUb, bool useMergeSort, NonLastSmallAxisCandidate &candidate)
{
    ...
    // 8/16-bit gather uses uint16_t offsets; verify row strides stay addressable.
    ...
    if (sortTileInfo.dtypeSize <= sizeof(uint16_t) &&
        ...
        // 8/16-bit vector gather uses 16-bit offsets after packing, so both source
        // and destination row strides must stay within uint16_t addressable elements.
        return false;
    }
    ...
    if (useMergeSort && sortTileInfo.dataType == ge::DT_BF16) {
        ...
        if (!TryAlignBytesToUint32(static_cast<uint64_t>(sortCount) * sortTileInfo.dtypeSize,
            sortTileInfo.blockUbSize, inputValueAxisBytes)) {
            return false;
        }
...
bool TryNonLastSmallAxis(gert::TilingContext *context, SortKthTileInfo &sortTileInfo, uint64_t &schId)
{
    ...
    uint32_t sortCount = GetNonLastSortCount(sortTileInfo.dataType,
        static_cast<uint32_t>(sortTileInfo.lastAxis));
    bool useMergeSort = UseNonLastMergeSort(sortTileInfo.dataType,
        static_cast<uint32_t>(sortTileInfo.lastAxis));
    ...
    auto estimateUb = [useMergeSort](SortKthTileInfo &candidateInfo, uint32_t innerChunk,
        uint64_t &peakUb, NonLastSmallAxisCandidate &candidate) -> bool {
...
bool TryMerge(gert::TilingContext *context, SortKthTileInfo &sortTileInfo, uint64_t &schId)
{
    if (IsMergeSortSupported(sortTileInfo.dataType, sortTileInfo.lastAxis)) {
        ...
        if (SetMergeSortTiling(context, candidate) == ge::GRAPH_SUCCESS) {
            ...
            schId = (sortTileInfo.lastAxis <= SORT32_SMALL_AXIS_THRESHOLD) ?
                SORT_SCHID_8 : static_cast<uint64_t>(0);
            return true;
        }
    }

    if (IsMergeMoreCoreSupported(sortTileInfo.dataType, sortTileInfo.lastAxis,
        sortTileInfo.unsortedDim, sortTileInfo.maxCoreNum)) {
        SortKthTileInfo candidate = sortTileInfo;
...
bool TryRadixOneCore(gert::TilingContext *context, SortKthTileInfo &sortTileInfo, uint64_t &schId)
{
    ...
    if (!IsRadixSortOneCore(candidate) ||
        SetRadixOneCoreTiling(context, candidate) != ge::GRAPH_SUCCESS) {
        return false;
    }
    sortTileInfo = candidate;
...
{
    if (!IsMergeIntraCoreSupported(sortTileInfo.dataType, sortTileInfo.lastAxis,
        sortTileInfo.unsortedDim, sortTileInfo.maxCoreNum, sortTileInfo.ubSize)) {
        return false;
    }
    SortKthTileInfo candidate = sortTileInfo;
...
ge::graphStatus SelectSortSchedule(gert::TilingContext *context, SortKthTileInfo &sortTileInfo, uint64_t &schId)
{
    ...
    if (TryMerge(context, sortTileInfo, schId) ||
