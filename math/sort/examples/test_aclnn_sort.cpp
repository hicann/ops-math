...
int main()
{
    ...
    DeviceMemPtr outValuesDeviceAddr(nullptr, &aclrtFree);
    DeviceMemPtr outIndicesDeviceAddr(nullptr, &aclrtFree);
    TensorPtr self(nullptr, &aclDestroyTensor);
    TensorPtr outValues(nullptr, &aclDestroyTensor);
    TensorPtr outIndices(nullptr, &aclDestroyTensor);
    ...
    ret = CreateAclTensor(outValuesHostData, outValuesShape, aclDataType::ACL_INT64, outValuesDeviceAddr, outValues);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ...
    // 调用aclnnSort第一段接口
    ret = aclnnSortGetWorkspaceSize(self.get(), stable, dim, descending, outValues.get(), outIndices.get(),
    ...
    // 调用aclnnSort第二段接口
    ret = aclnnSort(workspaceAddr.get(), workspaceSize, executor, stream.get());
    ...
    ret = aclrtMemcpy(resultData.data(), resultData.size() * sizeof(resultData[0]), outValuesDeviceAddr.get(),
    ...
    ret = aclrtMemcpy(resultData2.data(), resultData2.size() * sizeof(resultData2[0]), outIndicesDeviceAddr.get(),
