...
static bool CheckDtypeValid(const aclTensor* self, const aclTensor* other, const aclTensor* out)
{
    ...
    auto outSupportList = IsRegBase(npuArch) ? GetOutputDtypeSupportList() : inputSupportList;
...
aclnnStatus aclnnNeTensorCommon(const aclTensor* self, const aclTensor* other, aclTensor* out, uint64_t* workspaceSize,
...
aclnnStatus aclnnInplaceNeTensorGetWorkspaceSize(aclTensor* selfRef, const aclTensor* other, uint64_t* workspaceSize,
                                                 aclOpExecutor** executor)
{
