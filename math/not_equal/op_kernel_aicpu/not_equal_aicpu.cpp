...
uint32_t NotEqualBcastCompute(const CpuKernelContext& ctx, const Bcast& bcast)
{
    ...
    if (data_num >= kParallelDataNum) {
        ...
        KERNEL_HANDLE_ERROR(CpuKernelUtils::ParallelFor(ctx, data_num, 1, sharder_not_equal), "Equal Compute failed.")
    } else {
...
uint32_t NotEqualNoBcastCompute(const CpuKernelContext& ctx)
{
    ...
    BcastShapeType type = (element_num_x1 == element_num_x2 ?
...
uint32_t NotEqualCompute(const CpuKernelContext& ctx)
{
    ...
    KERNEL_LOG_INFO("CpuKernel[%s], input x1 : size[%lu], input x2: size[%lu], output: size[%lu]",
                    ctx.GetOpType().c_str(), tensorx1->GetDataSize(), tensorx2->GetDataSize(), output->GetDataSize());

    bool no_need_bcast = (shapex1 == shapex2) || (tensorx1->NumElements() == 1) || (tensorx2->NumElements() == 1);
    if (no_need_bcast) {
...
{
    // check params
    KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum), "Check NotEqual params failed.");
    ...
    KERNEL_CHECK_FALSE((x1_type == x2_type), KERNEL_STATUS_PARAM_INVALID,
                       "DataType of x1 [%d] should be same as x2 [%d].", static_cast<int32_t>(x1_type),
                       static_cast<int32_t>(x2_type))
...
OPS_MATH_REGISTER_CPU_KERNELV2(kNotEqual, NotEqualCpuKernel);
} // namespace aicpu
