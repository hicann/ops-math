...
<tbody>
  <tr>
    <td>x</td>
    <td>输入</td>
    <td>输入张量，对应公式中的x。支持1-8维Tensor。</td>
  ...
  <tr>
    <td>y</td>
    <td>输出</td>
    <td>输出张量，对应公式中的y。数据类型与输入张量x保持一致，shape与x一致。</td>
    <td>FLOAT16、FLOAT32、BFLOAT16、INT16、INT32、INT64</td>
    <td>ND</td>
  </tr>
</tbody></table>
...
- **数据类型约束**：输入张量x和输出张量y的数据类型必须一致。
- **shape约束**：输出张量y的shape必须与输入张量x的shape完全一致。
- **维度约束**：输入输出张量支持1-8维。
