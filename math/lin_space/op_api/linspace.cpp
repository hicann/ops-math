                                      ...
                                      aclTensor* out, aclOpExecutor* executor)
{
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICPU(LinSpace, OP_ATTR_NAMES({"Tidx"}), OP_INPUT(start, end, steps),
                                          OP_OUTPUT(out), OP_ATTR(start->GetDataType()));
    OP_CHECK(ret == ACLNN_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "LinspaceAiCpu ADD_TO_LAUNCHER_LIST_AICPU failed."),
             return nullptr);
    return out;
}
                                       ...
                                       aclTensor* out, aclOpExecutor* executor)
{
    ...
    OP_CHECK(ret == ACLNN_SUCCESS,
             OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "LinspaceAiCore ADD_TO_LAUNCHER_LIST_AICORE failed."), return nullptr);
    return out;
}
