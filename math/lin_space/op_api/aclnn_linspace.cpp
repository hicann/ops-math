...
aclnnStatus aclnnLinspaceGetWorkspaceSize(const aclScalar* start, const aclScalar* end, int64_t steps, aclTensor* out,
                                          uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    auto startWithBool = (start->GetDataType() == op::DataType::DT_BOOL) ?
                             (uniqueExecutor.get()->AllocScalar(startFloatValue)) :
                             start;
    auto endWithBool = (end->GetDataType() == op::DataType::DT_BOOL) ?
                           (uniqueExecutor.get()->AllocScalar(endFloatValue)) :
                           end;
