...
ge::graphStatus SquareTiling::CheckShape() const
{
...
ge::graphStatus SquareTiling::RunTiling()
{
    ...
    OP_CHECK_IF(status == ge::GRAPH_FAILED, OP_LOGE(tilingContext->GetNodeName(), "get input dtype failed"),
                return ge::GRAPH_FAILED);
    status = CalcOutputDtype();
    OP_CHECK_IF(status == ge::GRAPH_FAILED, OP_LOGE(tilingContext->GetNodeName(), "get output dtype failed"),
                return ge::GRAPH_FAILED);
    status = CheckShape();
