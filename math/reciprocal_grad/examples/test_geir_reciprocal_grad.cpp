...
#include "../op_graph/reciprocal_grad_proto.h"
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape, inputValue)                                           \
    ...
    reciprocalGradOp.set_input_##inputName(placeholder##inputIndex);                                                   \
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    ...
    reciprocalGradOp.update_output_desc_##outputName(outputName##outputIndex##_desc);
                    ...
                    float value)
{
    ...
    if (data_type == DT_FLOAT) {
        ...
        for (uint32_t i = 0; i < size; ++i) {
            iData[i] = static_cast<int32_t>(value);
        }
    }
...
{
    Status ret = SUCCESS;
    auto reciprocalGradOp = op::ReciprocalGrad("reciprocal_grad1");
    std::vector<int64_t> xShape = {4, 4};
    // y = 2.0, dy = 1.0, 期望输出 z = -y * y * dy = -4.0
    ADD_INPUT(1, y, inDtype, xShape, 2.0f);
    ADD_INPUT(2, dy, inDtype, xShape, 1.0f);

    ADD_OUTPUT(1, z, inDtype, xShape);

    outputs.push_back(reciprocalGradOp);
    return SUCCESS;
}

int main(int argc, char* argv[])
{
    const char* graph_name = "tc_ge_irrun_test_reciprocal_grad";
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        string output_file = "./tc_ge_irrun_test_reciprocal_grad_npu_output_" + std::to_string(i) + ".bin";
        ...
        if (output[i].GetTensorDesc().GetDataType() == DT_FLOAT) {
            ...
            std::cout << "expected z = -y * y * dy = -4.0" << std::endl;
