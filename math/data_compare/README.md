...
- 算子功能：逐元素比较两个相同shape和dtype的输入张量`x1`和`x2`，统计差异超出容差范围的元素总个数（All Reduce），输出FLOAT标量表示不匹配元素总数。
- 计算公式：
  ...
  $$
  output = \sum_{i} mismatch_i \quad (\text{dtype: FLOAT})
  $$
...
<tbody>
  ...
  <tr>
    <td>num</td>
    <td>输出</td>
    <td>不匹配元素总数，对应公式中的output。固定为FLOAT标量（0维张量）。</td>
    <td>FLOAT</td>
    <td>ND</td>
  </tr>
</tbody></table>
...
- 输出固定为FLOAT标量。
- 确定性实现：相同输入保证产生相同输出。
- 当输入元素总数超过$2^{24}$（约16M）时，由于FLOAT精确表示范围限制，计数值可能不精确。
