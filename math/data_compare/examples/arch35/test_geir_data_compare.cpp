...
#include "../../op_graph/data_compare_proto.h"
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape)                                                    \
    do {                                                                                                            \
        ...
        ret = GenOnesDataFloat32(inputShape, tensor_placeholder##inputIndex, placeholder##inputIndex##_desc, 1.0f); \
        if (ret != SUCCESS) {                                                                                       \
        ...
        dataCompare1.set_input_##inputName(placeholder##inputIndex);                                                \
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                           \
    do {                                                                                                        \
        ...
        dataCompare1.update_output_desc_##outputName(outputName##outputIndex##_desc);                           \
    } while (0)

#define ADD_INPUT_ATTR(attrName, attrValue) dataCompare1.set_attr_##attrName(attrValue)
...
{
    Status ret = SUCCESS;
    auto dataCompare1 = op::DataCompare("test_geir_data_compare");
    ...
    ADD_INPUT_ATTR(atol, 1e-5f);
    ADD_INPUT_ATTR(rtol, 1e-3f);

    ADD_OUTPUT(1, num, DT_FLOAT, yShape);

    outputs.push_back(dataCompare1);
    return SUCCESS;
}
