...
#include "nn_other.h"
#include "../op_graph/dense_bincount_proto.h"
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape, inputValue)                                      \
    ...
    ret = GenScalarData(placeholder##inputIndex##_shape, tensor_placeholder##inputIndex,                          \
                        placeholder##inputIndex##_desc, inputDtype, inputValue);                                  \
    if (ret != SUCCESS) {                                                                                         \
...
#define ADD_CONST_INPUT(inputIndex, inputName, inputDtype, inputShape, inputValue)                                \
    vector<int64_t> placeholder##inputIndex##_shape = inputShape;                                                 \
    ...
    graph.AddOp(placeholder##inputIndex);                                                                         \
    op1.set_input_##inputName(placeholder##inputIndex);                                                           \
    op1.update_input_desc_##inputName(placeholder##inputIndex##_desc);                                            \
    inputs.push_back(placeholder##inputIndex);
...
int32_t GenScalarData(vector<int64_t> shapes, Tensor& input_tensor, TensorDesc& input_tensor_desc, DataType data_type,
                      int value)
{
    ...
    if (data_type == ge::DT_INT32) {
        ...
        std::fill_n(reinterpret_cast<int64_t*>(pData), size, static_cast<int64_t>(value));
...
{
    Status ret = SUCCESS;
    // DenseBincount: input, size and weights, 1 output
    auto op1 = op::DenseBincount("dense_bincount1");

    std::vector<int64_t> inputShape = {4};
    ADD_INPUT(1, input, ge::DT_INT32, inputShape, 0);

    // size: shape [1], dtype int32
    std::vector<int64_t> sizeShape = {1};
    ADD_CONST_INPUT(2, size, ge::DT_INT32, sizeShape, 1);

    // weights: shape [4], dtype float32
    std::vector<int64_t> weightsShape = {4};
    ADD_INPUT(3, weights, ge::DT_FLOAT, weightsShape, 1);

    // Four zero-valued bins with unit weights produce output [4].
    std::vector<int64_t> outputShape = {1};
    ADD_OUTPUT(1, output, ge::DT_FLOAT, outputShape);
...
int main(int argc, char* argv[])
{
    ...
    if (output_num != 1 || output[0].GetTensorDesc().GetShape().GetShapeSize() != 1 ||
        *(reinterpret_cast<const float*>(output[0].GetData())) != 4.0f) {
        printf("%s - ERROR - [XIR]: Precision check failed\n", GetTime().c_str());
        delete session;
        GEFinalize();
        return FAILED;
    }
