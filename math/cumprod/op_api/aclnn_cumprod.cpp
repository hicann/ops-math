...
static inline bool CheckShape(const aclTensor* input, const aclTensor* out, const aclScalar* axis)
{
    ...
    if (inputDim == 0 && inputAxis != 0 && inputAxis != -1) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Dimension out of range (expected to be in range of [-1, 0], but got %d)",
                inputAxis);
        return false;
...
static aclnnStatus CheckParams(const aclTensor* input, const aclTensor* out, const aclScalar* inputDim,
...
static aclnnStatus doGetWorkspaceSize(aclTensor* input, const aclScalar* inputDim, const aclDataType dtype,
