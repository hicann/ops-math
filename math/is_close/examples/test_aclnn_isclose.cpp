...
int main()
{
    ...
    // 调用aclnnIsClose第一段接口
    ret = aclnnIsCloseGetWorkspaceSize(self.get(), other.get(), rtol, atol, equal_nan, out.get(), &workspaceSize,
                                       &executor);
    ...
    // 调用aclnnIsClose第二段接口
    ret = aclnnIsClose(workspaceAddr.get(), workspaceSize, executor, stream.get());
