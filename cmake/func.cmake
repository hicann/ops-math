...
macro(add_all_modules_sources)
  ...
  # AICPU built-in package manifest: check if this op is allowed in built-in package
  # Only affects device-side AICPU (.so + json); host constant folding is not affected.
  # Key is op name (e.g. "log", "cumsum")
  set(_AICPU_BUILTIN_PKG_ALLOWED OFF)
  if(AICPU_BUILTIN_PACKAGE_FORCE_ALL)
    set(_AICPU_BUILTIN_PKG_ALLOWED ON)
  else()
    foreach(_entry ${AICPU_BUILTIN_PACKAGE_MANIFEST})
      if(_entry STREQUAL "${OP_NAME}=ON")
        set(_AICPU_BUILTIN_PKG_ALLOWED ON)
        break()
      endif()
    endforeach()
  ...
  if(AICPU_OP_DEF_SRCS AND (ENABLE_CUSTOM OR _AICPU_BUILTIN_PKG_ALLOWED))
    set_property(GLOBAL APPEND PROPERTY AICPU_OPDEF_FILES ${AICPU_OP_DEF_SRCS})
  endif()
  if(AICPU_JSON_FILES AND (ENABLE_CUSTOM OR _AICPU_BUILTIN_PKG_ALLOWED))
    set_property(GLOBAL APPEND PROPERTY AICPU_JSON_FILES ${AICPU_JSON_FILES})
  endif()

  if(AICPU_SRCS AND NOT DISABLE_AICPU)
    if(ENABLE_CUSTOM)
    ...
    elseif(_AICPU_BUILTIN_PKG_ALLOWED AND BUILD_WITH_INSTALLED_DEPENDENCY_CANN_PKG)
      add_aicpu_cust_kernel_modules(${OP_NAME} "${AICPU_SRCS}")
    endif()
  endif()
  ...
  # Not subject to manifest control — preserves existing behavior
  ...
  file(GLOB_RECURSE OP_GRAPH_SRCS ${SOURCE_DIR}/op_graph/*.cpp)
  if(OP_GRAPH_SRCS)
...
macro(add_graph_plugin_sources)
  ...
  file(GLOB_RECURSE OP_GRAPH_SRCS ${SOURCE_DIR}/*.cpp)
