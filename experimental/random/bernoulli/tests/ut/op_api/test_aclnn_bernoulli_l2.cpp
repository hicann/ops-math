...
#include "../../../op_api/aclnn_bernoulli.h"
...
protected:
    static void SetUpTestCase() { cout << "bernoulli_test SetUp" << endl; }

    static void TearDownTestCase() { cout << "bernoulli_test TearDown" << endl; }
};
...
TEST_F(l2_bernoulli_test, ascend910B_test_bool_nchw_prob05)
{
    auto tensor_desc = TensorDesc({4, 4}, ACL_BOOL, ACL_FORMAT_NCHW).ValueRange(0, 1);
    auto out_tensor_desc = TensorDesc(tensor_desc).Precision(0.0, 0.0);
...
TEST_F(l2_bernoulli_test, ascend910B_test_int64_nchw_prob05)
{
...
TEST_F(l2_bernoulli_test, ascend910B_test_double_nchw_prob05)
{
...
TEST_F(l2_bernoulli_test, test_single_element_random_alias)
{
    auto tensor_desc = TensorDesc({1}, ACL_UINT8, ACL_FORMAT_ND).Value(vector<float>{0});
    auto out_tensor_desc = TensorDesc(tensor_desc).Precision(0.0, 0.0);
    auto scalar_desc = ScalarDesc(0.5f);
    auto ut = OP_API_UT(aclnnBernoulli, INPUT(tensor_desc, scalar_desc, 0, 0), OUTPUT(out_tensor_desc));
...
TEST_F(l2_bernoulli_test, test_inplace_scalar_probability)
{
    auto tensor_desc = TensorDesc({4, 4}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
    auto scalar_desc = ScalarDesc(0.5f);
    auto ut = OP_API_UT(aclnnInplaceBernoulli, INPUT(tensor_desc, scalar_desc, 0, 0), OUTPUT());
...
TEST_F(l2_bernoulli_test, test_err_offset_not_multiple_of_four)
{
    auto tensor_desc = TensorDesc({4, 4}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
    auto out_tensor_desc = TensorDesc(tensor_desc).Precision(0.0, 0.0);
    auto scalar_desc = ScalarDesc(0.5f);
    auto ut = OP_API_UT(aclnnBernoulli, INPUT(tensor_desc, scalar_desc, 0, 2), OUTPUT(out_tensor_desc));
...
TEST_F(l2_bernoulli_test, test_err_nan_probability)
{
    auto tensor_desc = TensorDesc({4, 4}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
    auto out_tensor_desc = TensorDesc(tensor_desc).Precision(0.0, 0.0);
    auto scalar_desc = ScalarDesc(std::numeric_limits<float>::quiet_NaN());
    auto ut = OP_API_UT(aclnnBernoulli, INPUT(tensor_desc, scalar_desc, 0, 0), OUTPUT(out_tensor_desc));
