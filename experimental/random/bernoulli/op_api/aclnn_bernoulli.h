 ...
 * @return aclnnStatus: 返回状态码。
 */
ACLNN_API aclnnStatus aclnnInplaceBernoulliGetWorkspaceSize(const aclTensor* selfRef, const aclScalar* prob,
 ...
 *
 * 算子功能：从伯努利分布中提取二进制随机数
 * 计算公式：$$ selfRef_i ∼ Bernoulli(prob_i) $$
 *
 * @param [in] selfRef: npu
 * device侧的aclTensor，数据类型支持整型，浮点类型，支持非连续的Tensor，数据格式支持ND
 * @param [in] prob: npu
