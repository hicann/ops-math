...
int main()
{
    ...
    std::vector<int64_t> selfXShape = {1, 2, 3, 4};
    ...
    std::vector<int64_t> selfYShape = {1, 2, 3, 4};
    std::vector<DataType> selfYHostData(24, 9);
    ret = CreateAclTensor(selfYHostData, selfYShape, &selfYDeviceAddr, aclDataType::ACL_FLOAT, &selfY);
    CHECK_RET(ret == ACL_SUCCESS, return ret);

    aclTensor* out = nullptr;
    void* outDeviceAddr = nullptr;
    std::vector<int64_t> outShape = {1, 2, 3, 8};
    std::vector<DataType> outHostData(24, 3);
    ...
    // 4. 调用aclnnPackV2Example第一段接口
    ret = aclnnPackV2GetWorkspaceSize(selfX, selfY, axis,out, &workspaceSize, &executor);
    LOG_PRINT("aclnnPackV2GetWorkspaceSize returned %d, workspaceSize=%llu, executor=%p\n",
          ret, (unsigned long long)workspaceSize, (void*)executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnPackV2ExampleGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnPackExample failed. ERROR: %d\n", ret); return ret);
    ...
    std::vector<int64_t> outShape1 = {48};
