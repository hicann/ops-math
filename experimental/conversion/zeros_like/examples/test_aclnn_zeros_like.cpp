...
bool PrintOutResult(std::vector<int64_t>& shape, void** deviceAddr)
{
    ...
    LOG_PRINT("result check PASS: all %ld elements are 0\n", size);
    return true;
}
...
int main()
{
    ...
    // 7. 获取输出的值，将device侧内存上的结果拷贝至host侧，并校验全 0
    bool ok = PrintOutResult(selfShape, &selfDeviceAddr);
