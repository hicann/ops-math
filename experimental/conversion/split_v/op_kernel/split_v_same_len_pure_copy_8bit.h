...
#ifndef OP_KERNEL_SPLIT_V_SAME_LEN_PURE_COPY_8BIT_H_
#define OP_KERNEL_SPLIT_V_SAME_LEN_PURE_COPY_8BIT_H_
...
template <typename T>
class SplitVSameLenPureCopy8Bit : private SplitVPureCopyState<T, 1> {
public:
    __aicore__ inline SplitVSameLenPureCopy8Bit() {}

    __aicore__ inline void Init(GM_ADDR x, GM_ADDR yList, const SplitVTilingDataSameLenPureCopy8Bit* tilingData,
                                TPipe* pipe)
    {
    ...
    __aicore__ inline void Process()
    {
        if constexpr (!Is8BitType()) {
            ...
            const bool ready = PreparePureCopyCore(IsValidTiling());
            if (!ready) {
                return;
            }
...
private:
    ...
    __aicore__ inline uint32_t AlignUp32Bytes(uint32_t bytes) const
    {
        return (bytes + BLOCK_SIZE - 1U) & ~(BLOCK_SIZE - 1U);
    }

    __aicore__ inline bool IsValidTiling() const
    {
        if (splitSize == 0 || splitNum == 0) {
            return false;
        }
    ...
    __aicore__ inline LocalTensor<T> CopyInSplitMajor(uint64_t outerBase, uint32_t outerReal, uint32_t splitIndex)
    {
        ...
        const uint32_t splitBase = splitIndex * splitSize;
        return CopyInPureSplitMajor(outerBase, outerReal, splitBase, curSplitSize, splitPitch);
    }
    ...
    {
        const uint32_t curSplitSize = GetSplitSize(splitIndex);
        CopyOutPureSplitMajor(local, outerBase, outerReal, splitIndex, curSplitSize);
    }
    ...
    __aicore__ inline void ProcessSplitMajor()
    {
        for (uint32_t outerOff = 0; outerOff < coreRows; outerOff += outerTile) {
            ...
            for (uint32_t splitIndex = 0; splitIndex < splitNum; ++splitIndex) {
                LocalTensor<T> local = CopyInSplitMajor(outerBase, outerReal, splitIndex);
                CopyOutSplitMajor(local, outerBase, outerReal, splitIndex);
    ...
    {
        const uint32_t localPitch = (AlignUp32Bytes(copyElems * sizeof(T))) / sizeof(T);
        return CopyInPureRowChunk(srcOffset, copyElems, localPitch);
    }
                                           ...
                                           uint32_t splitOffset, uint32_t copyElems)
    {
        ...
        const uint64_t dstOffset = outerIndex * static_cast<uint64_t>(curSplitSize) + splitOffset;
    ...
    __aicore__ inline void ProcessRowLengthChunk()
    {
        for (uint32_t rowOff = 0; rowOff < coreRows; ++rowOff) {
            ...
            for (uint32_t splitIndex = 0; splitIndex < splitNum; ++splitIndex) {
                const uint32_t curSplitSize = GetSplitSize(splitIndex);
                const uint32_t splitBase = splitIndex * splitSize;
                for (uint32_t splitOff = 0; splitOff < curSplitSize; splitOff += colTileLength) {
                    const uint32_t remain = curSplitSize - splitOff;
...
#endif // OP_KERNEL_SPLIT_V_SAME_LEN_PURE_COPY_8BIT_H_
