...
template <
    typename TYPE_X, typename TYPE_CLIP_VALUE_MIN, typename TYPE_CLIP_VALUE_MAX, typename TYPE_Y, bool IsExistBigCore>
class KernelClipByValueV2 {
public:
    __aicore__ inline KernelClipByValueV2() {}
    __aicore__ inline void Init(
        GM_ADDR x, GM_ADDR clip_value_min, GM_ADDR clip_value_max, GM_ADDR y, uint64_t smallCoreDataNum,
        uint64_t bigCoreDataNum, uint64_t bigCoreLoopNum, uint64_t smallCoreLoopNum, uint64_t ubPartDataNum,
        uint64_t smallCoreTailDataNum, uint64_t bigCoreTailDataNum, uint64_t tailBlockNum, TPipe* pipeIn)
    {
