...
#include "../op_kernel/cos_tiling_data.h"
#include "../op_kernel/cos_tiling_key.h"
...
struct CosCompileInfo {};

// tiling 分发入口
static ge::graphStatus CosTilingFunc(gert::TilingContext* context)
{
    CosTilingData* tiling = context->GetTilingData<CosTilingData>();
    uint64_t ubLength = 0;
...
static ge::graphStatus TilingParseForCos([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

// tiling注册入口.
IMPL_OP_OPTILING(Cos).Tiling(CosTilingFunc).TilingParse<CosCompileInfo>(TilingParseForCos);
} // namespace optiling
