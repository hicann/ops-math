 ...
 *
 * FresnelSin Kernel (arch35 / Ascend950)
 *
 * Algorithm: Cephes-style three-branch (small / large / clamp) for S(x):
 *   - Small branch (x^2 < 2.5625):  S(|x|) = |x|^3 * P_S(x^4) / Q_S(x^4)
 *   - Large branch (else):          S(|x|) = 0.5 - (f*cos(phase) + g*sin(phase)) / (pi*|x|)
 *     phase = (pi/2)*(x^2 mod 4) reduced via Cody-Waite, sin/cos via default polynomial.
 *   - Clamp |x| >= 36974:           S(x)   = sign(x) * 0.5
 *   - Pre-saturate |x| > CLAMP_THRESH to SAFE_SATURATE to bound x^2 / handle Inf/3.4e38.
 *   - S is odd: compute on |x| then multiply by sign(x) (single post-multiply).
 *   - Cast fp16/bf16 -> fp32 for math, cast back at output.
 */
...
// ============================================================================
// FresnelSin small-branch polynomials SN/SD (Cephes fresnl.c).
// HornerEval evaluates P(u) = coeffs[0]*u^(n-1) + coeffs[1]*u^(n-2) + ... + coeffs[n-1]
...
class KernelFresnelSin {
    static constexpr int32_t BUFFER_NUM = 2;

public:
    __aicore__ inline KernelFresnelSin() {}
...
// ============================================================================
template <typename T, uint32_t TILING_KEY>
__aicore__ inline void KernelFresnelSin<T, TILING_KEY>::Init(
    GM_ADDR x, GM_ADDR y, GM_ADDR /*workspace*/, GM_ADDR tiling)
{
    REGISTER_TILING_DEFAULT(FresnelSinTilingData);
    GET_TILING_DATA_WITH_STRUCT(FresnelSinTilingData, td, tiling);
    ...
    // tileLength_/8 为 mask 所需字节数（每个元素 1bit），向上对齐到 256B。
...
// ============================================================================
template <typename T, uint32_t TILING_KEY>
__aicore__ inline void KernelFresnelSin<T, TILING_KEY>::Process()
{
...
// ============================================================================
template <typename T, uint32_t TILING_KEY>
__aicore__ inline void KernelFresnelSin<T, TILING_KEY>::CopyIn(int64_t progress, int64_t curLen)
{
    ...
    // so any garbage produced in the tail region is discarded. See the matching
    // divide-by-zero guard on the same rationale.
    DataCopyPad(xLocal, gmX_[progress * tileLength_], copyParams, {false, 0, 0, 0});
    inQueX_.EnQue(xLocal);
}

template <typename T, uint32_t TILING_KEY>
__aicore__ inline void KernelFresnelSin<T, TILING_KEY>::CopyOut(int64_t progress, int64_t curLen)
{
...
// ============================================================================
// Compute (FresnelSin three-branch: small / large / clamp with RADIAN_REDUCTION)
//
// Algorithm differences vs FresnelCos:
//   - Small branch numerator: S(|x|) = |x|^3 * P_S(x^4) / Q_S(x^4)   (not x*P_C/Q_C)
//   - Large branch:           S(|x|) = 0.5 - (f*cos(phase) + g*sin(phase)) / (pi*|x|)
//                             (cos+sin and minus, vs FresnelCos sin-cos and plus)
//   - Result on |x|: S is odd, multiply final result by sign(x) once at the end.
//   - Clamp |x|>=CLAMP_THRESH -> sign(x)*0.5 (same as FresnelCos pattern).
//
// Pipeline:
//   1) Cast input -> fp32; saturate |x|>CLAMP_THRESH to SAFE_SATURATE for math path.
//   2) Compute small branch on |x|.
//   3) Compute large branch on |x|.
//   4) Select small vs large by (x^2 < SMALL_THRESH).
//   5) Multiply by sign(x) (odd function recovery).
//   6) Apply clamp (sign(x)*0.5).
// ============================================================================
template <typename T, uint32_t TILING_KEY>
__aicore__ inline void KernelFresnelSin<T, TILING_KEY>::Compute(int64_t curLen)
{
    ...
    // 1) Cast to fp32
    if constexpr (TILING_KEY == FRESNEL_SIN_KEY_FP16 || TILING_KEY == FRESNEL_SIN_KEY_BF16) {
    ...
    // === Apply sign(x): S is odd, multiply unified |x|-domain result by sign ===
    // Branch via mask on saturated signed xF32: negative -> negate result.
    Muls(polyDen, result, -1.0f, calcLen);
    {
        LocalTensor<uint8_t> maskNeg = maskBuf1_.template Get<uint8_t>();
        Duplicate(phase, 0.0f, cmpCount);
        Compare(maskNeg, xF32, phase, CMPMODE::LT, cmpCount);
        Select(result, maskNeg, polyDen, result, SELMODE::VSEL_TENSOR_TENSOR_MODE, calcLen);
    }
