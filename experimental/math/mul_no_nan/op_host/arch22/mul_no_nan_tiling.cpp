...
/*!
 * \file mul_no_nan_tiling_arch22.cpp
 * \brief MulNoNan A2/A3 tiling implementation.
 *
 * Generates the inline MulNoNanTilingData (totalNum/rank/outputDims/strides) for
 * the scalar 910b kernel. Multi-core block dim is selected from the output word
 * count, matching the Gcd pattern.
 */

#include "log/log.h"
#include "register/op_impl_registry.h"
#include "op_common/op_host/util/platform_util.h"
#include "experimental/math/mul_no_nan/op_kernel/mul_no_nan_tiling_data.h"
#include "experimental/math/mul_no_nan/op_kernel/mul_no_nan_tiling_key.h"
#include "mul_no_nan_tiling.h"

namespace optiling {
namespace {

constexpr int64_t MUL_NO_NAN_MIN_OUTPUT_WORDS_PER_BLOCK = 256L;
...
static ge::graphStatus CheckDtype(gert::TilingContext* context)
{
    ...
    bool supported = x1Type == x2Type && x1Type == yType &&
                     (x1Type == ge::DT_FLOAT || x1Type == ge::DT_FLOAT16 || x1Type == ge::DT_BF16);
    OP_CHECK_IF(
        !supported,
        OP_LOGE_WITH_INVALID_INPUT_DTYPE(context->GetNodeName(), "x", "unsupported", "DT_FLOAT16, DT_FLOAT, DT_BF16"),
...
static ge::graphStatus GetOutputWordCount(gert::TilingContext* context, int64_t totalNum, int64_t& wordCount)
{
    ...
    OP_CHECK_IF(wordCount <= 0, OP_LOGE(context, "MulNoNan output word count must be positive"),
                return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
{
    int64_t blockDim = CeilDiv(outputWordCount, MUL_NO_NAN_MIN_OUTPUT_WORDS_PER_BLOCK);
    if (blockDim < 1) {
        blockDim = 1;
    }
    if (blockDim > coreNum) {
...
template <typename ShapeT>
static ge::graphStatus FillOutputDims(gert::TilingContext* context, const ShapeT& outputShape,
                                      MulNoNanTilingData* tiling)
{
    tiling->rank = GetRank(outputShape);
    OP_CHECK_IF(tiling->rank <= 0 || tiling->rank > MUL_NO_NAN_MAX_DIMS,
                OP_LOGE(context, "MulNoNan output rank must be in [1, 8]"), return ge::GRAPH_FAILED);

    tiling->totalNum = 1;
    for (int64_t i = 0; i < tiling->rank; ++i) {
        int64_t dim = GetDimOrOne(outputShape, i);
        OP_CHECK_IF(dim <= 0, OP_LOGE(context, "MulNoNan output dim must be positive"), return ge::GRAPH_FAILED);
...
static ge::graphStatus FillInputStrides(gert::TilingContext* context, const ShapeT& inputShape,
                                        const MulNoNanTilingData* tiling, int64_t* strides)
{
    int64_t inputRankRaw = static_cast<int64_t>(inputShape.GetDimNum());
    OP_CHECK_IF(inputRankRaw < 0 || inputRankRaw > MUL_NO_NAN_MAX_DIMS,
                OP_LOGE(context, "MulNoNan input rank must be in [0, 8]"), return ge::GRAPH_FAILED);
    OP_CHECK_IF(inputRankRaw > tiling->rank, OP_LOGE(context, "MulNoNan input rank cannot exceed output rank"),
                return ge::GRAPH_FAILED);

    int64_t contiguousStrides[MUL_NO_NAN_MAX_DIMS] = {0, 0, 0, 0, 0, 0, 0, 0};
    int64_t runningStride = 1;
    for (int64_t i = inputRankRaw - 1; i >= 0; --i) {
        int64_t dim = inputShape.GetDim(i);
        OP_CHECK_IF(dim <= 0, OP_LOGE(context, "MulNoNan input dim must be positive"), return ge::GRAPH_FAILED);
    ...
    for (int64_t outDim = 0; outDim < tiling->rank; ++outDim) {
        ...
        if (dim == outputDim) {
            ...
            OP_LOGE(context, "MulNoNan input shape is not broadcast-compatible with output shape");
            return ge::GRAPH_FAILED;
        }
    }
    return ge::GRAPH_SUCCESS;
...
static ge::graphStatus FillShapeInfo(gert::TilingContext* context, MulNoNanTilingData* tiling)
{
...
static ge::graphStatus TilingForMulNoNanArch22(gert::TilingContext* context)
{
    OP_LOGD("MulNoNanTilingArch22", "Enter TilingForMulNoNanArch22");
    ...
    MulNoNanTilingData* tiling = context->GetTilingData<MulNoNanTilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    *tiling = MulNoNanTilingData{};
    ...
    context->SetTilingKey(MUL_NO_NAN_TILING_KEY);
    return ge::GRAPH_SUCCESS;
}

} // namespace

static ge::graphStatus TilingParseForMulNoNanArch22([[maybe_unused]] gert::TilingParseContext* context)
{
...
IMPL_OP_OPTILING(MulNoNan)
    .Tiling(TilingForMulNoNanArch22)
    .TilingParse<MulNoNanArch22CompileInfo>(TilingParseForMulNoNanArch22);
} // namespace optiling
