...
static bool IsAiCoreSupport(const aclTensor* x, const aclTensor* y)
{
    ...
    OP_CHECK(npuArch == NpuArch::DAV_3510 || npuArch == NpuArch::DAV_2201,
             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "MulNoNan not supported on this platform: npuArch=%d.",
...
{
    OP_CHECK(BroadcastInferShape(xShape, yShape, outShape), OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Shape broadcast failed."),
             return false);
    return true;
}

static const aclTensor* MulNoNanAiCore(const aclTensor* x, const aclTensor* y, const aclTensor* out,
