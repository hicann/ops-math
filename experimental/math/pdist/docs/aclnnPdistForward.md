...
计算输入二维 tensor 各行之间的 p-范数成对距离（Forward 接口）。功能与 aclnnPdist 一致，区别在于 p 参数以 aclScalar 指针传入。等价于 PyTorch 的 `torch.nn.functional.pdist`。
...
其中 x 为输入 tensor，形状 (N, M)；p 为距离参数（aclScalar，p ≥ 0）；输出为一维 tensor，形状 (N*(N-1)/2,)，按上三角行优先顺序排列。

## 函数原型

每个算子分为两段式接口，必须先调用"aclnnPdistForwardGetWorkspaceSize"接口获取计算所需 workspace 大小以及包含了算子计算流程的执行器，再调用"aclnnPdistForward"接口执行计算。
...
| p（const aclScalar*） | 输入 | 距离参数，对应公式中p。p≥0，含inf。通过aclCreateScalar创建。 | FLOAT | - | - |
...
| 返回值 | 错误码 | 描述 |
|--------|--------|------|
| ACLNN_ERR_PARAM_NULLPTR | 161001 | self、p、out存在空指针。 |
...
| workspaceSize | 输入 | 在Device侧申请的workspace大小，由第一段接口aclnnPdistForwardGetWorkspaceSize获取。 |
...
| 调用方式 | 样例代码 | 说明 |
| :------- | :------- | :--- |
| aclnn接口 | [test_aclnn_pdist_forward](../examples/test_aclnn_pdist_forward.cpp) | 通过 aclnnPdistForward 接口调用 Pdist 算子，p 以 aclScalar 传入。 |
...
// 省略 Init、CreateAclTensor 等通用函数，完整代码见 examples/test_aclnn_pdist_forward.cpp
...
int main()
{
    ...
    // 构造 p 值为 aclScalar
    float pValue = 2.0f;
    aclScalar* pScalar = aclCreateScalar(&pValue, ACL_FLOAT);

    // 调用第一段接口
    uint64_t workspaceSize = 0;
    aclOpExecutor* executor;
    aclnnPdistForwardGetWorkspaceSize(inputTensor, pScalar, outputTensor, &workspaceSize, &executor);
    ...
    // 调用第二段接口
    aclnnPdistForward(workspaceAddr, workspaceSize, executor, stream);
