...
int main()
{
    ...
    std::vector<int64_t> selfXShape = {2, 2};
    std::vector<DataType> selfXHostData(4, 7);
    ...
    std::vector<int64_t> selfYShape = {2, 2};
    std::vector<DataType> selfYHostData(4, 1);
    ...
    std::vector<DataType> outHostData(4, 0);
    ...
    ret = aclnnComplexV2GetWorkspaceSize(selfX, selfY, out, &workspaceSize, &executor);
    LOG_PRINT("aclnnComplexV2GetWorkspaceSize returned %d, workspaceSize=%llu, executor=%p\n",
          ret, (unsigned long long)workspaceSize, (void*)executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnComplexV2ExampleGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    ret = aclnnComplexV2(workspaceAddr, workspaceSize, executor, stream);
