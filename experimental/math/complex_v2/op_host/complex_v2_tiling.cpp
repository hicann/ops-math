...
static ge::graphStatus CalculateCoreNum(
    gert::TilingContext* context, ComplexV2TilingData* tiling, const PlatformInfo& platformInfo, int64_t totalIdx)
{
    auto coreNum = platformInfo.coreNum;
    auto blockSize = platformInfo.blockSize;
    auto ubSize = platformInfo.ubSize;
...
static ge::graphStatus ComplexV2TilingFunc(gert::TilingContext* context)
{
    ...
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(
        memset_s(tiling, sizeof(ComplexV2TilingData), 0, sizeof(ComplexV2TilingData)) != EOK,
