...
#include "../op_kernel/complex_v2_tiling_data.h"
#include "../op_kernel/complex_v2_tiling_key.h"
...
static ge::graphStatus CalculateCoreNum(gert::TilingContext* context, ComplexV2TilingData* tiling,
...
// --- safer numeric types ---
    ...
    // 总 block 数（向上取整）
    uint64_t blocksTotal = Ops::Base::CeilDiv(inputLengthBytes,blockSize);
...
// tiling 分发入口
// 可直接替换你的 ComplexV2TilingFunc 内部实现（保留函数签名）
static ge::graphStatus ComplexV2TilingFunc(gert::TilingContext* context)
{
// 1. platform
    ...
    // handle empty input
    if (totalIdx <= 0) {
        ComplexV2TilingData* tiling = context->GetTilingData<ComplexV2TilingData>();
        context->GetRawTilingData()->SetDataSize(sizeof(ComplexV2TilingData));
        OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
        memset_s(tiling, sizeof(ComplexV2TilingData), 0, sizeof(ComplexV2TilingData));
    ...
    ComplexV2TilingData* tiling = context->GetTilingData<ComplexV2TilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(ComplexV2TilingData), 0, sizeof(ComplexV2TilingData)) != EOK,
...
// tiling注册入口.
IMPL_OP_OPTILING(ComplexV2).Tiling(ComplexV2TilingFunc);
} // namespace optiling
