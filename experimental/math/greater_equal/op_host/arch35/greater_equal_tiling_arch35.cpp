...
#include "../../op_kernel/arch35/greater_equal_dag.h"
#include "../../op_kernel/arch35/greater_equal_struct.h"
...
ge::graphStatus GreaterEqualTiling::DoOpTiling()
{
    ...
    if (input0DType != input1DType) {
        ...
        OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(
            context_->GetNodeName(), "x1 and x2", dtypesStr.c_str(), "The dtypes of x1 and x2 must be the same");
    ...
    if (input0DType == ge::DT_INT64) {
        ...
        BroadcastBaseTiling<GreaterEqualOp::GreaterEqualCompute<uint32_t>::OpDag> brcBaseTiling(context_);
        ret = brcBaseTiling.DoTiling();
        tilingKey = GET_TPL_TILING_KEY(brcBaseTiling.GetSchMode());
        ...
        BroadcastBaseTiling<GreaterEqualOp::GreaterEqualCompute<uint16_t>::OpDag> brcBaseTiling(context_);
        ret = brcBaseTiling.DoTiling();
        tilingKey = GET_TPL_TILING_KEY(brcBaseTiling.GetSchMode());
    } else if (input0DType == ge::DT_INT16) {
        BroadcastBaseTiling<GreaterEqualOp::GreaterEqualCompute<int16_t>::OpDag> brcBaseTiling(context_);
        ret = brcBaseTiling.DoTiling();
        tilingKey = GET_TPL_TILING_KEY(brcBaseTiling.GetSchMode());
        ...
        OP_LOGE_FOR_INVALID_DTYPE(
            context_->GetNodeName(), "x1", dtypeStr.c_str(),
            "fp16, bf16, fp32, uint64, uint32, int64, int32, uint16, int16, uint8, int8 or bool");
        return ge::GRAPH_FAILED;
    }
