 ...
 */
ACLNN_API aclnnStatus aclnnGeTensorGetWorkspaceSize(
...
aclnnGeTensor(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor, const aclrtStream stream);

/**
 * @brief aclnnInplaceGeTensor的第一段接口，根据具体的计算流程，计算workspace大小。
 * @domain aclnn_math
 * @param [in] selfRef: npu device侧的aclTensor，与 out 复用同一缓冲；语义同 aclnnGeTensor 的 self。
 * @param [in] other: npu device侧的aclTensor，语义同 aclnnGeTensor 的 other。
 * @param [out] workspaceSize: 返回用户需要在npu device侧申请的workspace大小。
 * @param [out] executor: 返回op执行器，包含算子计算流程。
 * @return aclnnStatus: 返回状态码。
 */
ACLNN_API aclnnStatus aclnnInplaceGeTensorGetWorkspaceSize(
 ...
 */
ACLNN_API aclnnStatus
aclnnInplaceGeTensor(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor, const aclrtStream stream);
