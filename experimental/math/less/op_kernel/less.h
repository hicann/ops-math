...
public:
    ...
    __aicore__ inline void Init(
        GM_ADDR x1, GM_ADDR x2, GM_ADDR y, uint32_t smallCoreDataNum, uint32_t bigCoreDataNum, uint32_t bigCoreLoopNum,
        uint32_t smallCoreLoopNum, uint32_t ubPartDataNum, uint32_t smallCoreTailDataNum, uint32_t bigCoreTailDataNum,
        uint32_t tailBlockNum, uint32_t bigprocessDataNumComputes, uint32_t smallprocessDataNumComputes,
...
private:
    ...
    __aicore__ inline void ComputeFp16(int32_t progress)
    {
        ...
        AscendC::Select(
            x1Local, yLocal, x1Local, (TYPE_X2)0, AscendC::SELMODE::VSEL_TENSOR_SCALAR_MODE,
            this->processDataNumComputes);
    ...
    __aicore__ inline void ComputeInt(int32_t progress)
    {
        ...
        AscendC::Select(
            p1, yLocal, p1, (half)1, AscendC::SELMODE::VSEL_TENSOR_SCALAR_MODE, this->processDataNumComputes);
        AscendC::Cast(yLocal, p1, AscendC::RoundMode::CAST_NONE, this->processDataNumComputes);
        inQueueX1.FreeTensor(x1Local);
        inQueueX2.FreeTensor(x2Local);
        outQueueY.EnQue<int8_t>(yLocal);
    ...
    __aicore__ inline void ComputeBf16(int32_t progress)
    {
        ...
        AscendC::Select(
            y2, yLocal, y1, (float)0, AscendC::SELMODE::VSEL_TENSOR_SCALAR_MODE, this->processDataNumComputes);
