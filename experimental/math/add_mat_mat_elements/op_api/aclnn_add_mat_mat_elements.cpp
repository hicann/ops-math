...
static bool CheckNotNull(const aclTensor* a, const aclTensor* b, const aclTensor* c, const aclScalar* alpha,
                         const aclScalar* beta, const aclTensor* cOut)
{
...
static aclnnStatus CheckParams(const aclTensor* a, const aclTensor* b, const aclTensor* c, const aclScalar* alpha,
                               const aclScalar* beta, const aclTensor* cOut)
{
 ...
 */
extern "C" aclnnStatus aclnnAddMatMatElementsGetWorkspaceSize(const aclTensor* a, const aclTensor* b,
                                                              const aclTensor* c, const aclScalar* alpha,
                                                              const aclScalar* beta, aclTensor* cOut,
                                                              uint64_t* workspaceSize, aclOpExecutor** executor)
{
    L2_DFX_PHASE_1(aclnnAddMatMatElements, DFX_IN(a, b, c, alpha, beta), DFX_OUT(cOut));
    ...
    // 调用 L0 API
    const aclTensor* opResult = l0op::AddMatMatElements(aContiguous, bContiguous, cContiguous, alpha->ToFloat(),
                                                        beta->ToFloat(), uniqueExecutor.get());
 ...
 * @brief 第二段接口：执行计算
 */
extern "C" aclnnStatus aclnnAddMatMatElements(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                              aclrtStream stream)
{
