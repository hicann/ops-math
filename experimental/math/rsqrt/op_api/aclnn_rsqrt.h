 ...
 * 数据类型支持浮点、整数类型，shape为非空，支持非连续的Tensor，数据格式支持ND。
...
aclnnRsqrtGetWorkspaceSize(const aclTensor* self, aclTensor* out, uint64_t* workspaceSize, aclOpExecutor** executor);
/**
 * @brief: aclnnRsqrt的第二段接口，用于执行计算
 *
 * 算子功能： 对输入Tensor完成求算术平方根倒数的操作
 * @param [in] workspace: 在npu device侧申请的workspace内存起址。
 * @param [in] workspaceSize: 在npu device侧申请的workspace大小，由第一段接口aclnnRsqrtGetWorkspaceSize获取。
 ...
 */
ACLNN_API aclnnStatus
aclnnInplaceRsqrtGetWorkspaceSize(aclTensor* selfRef, uint64_t* workspaceSize, aclOpExecutor** executor);

ACLNN_API aclnnStatus
aclnnInplaceRsqrt(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor, aclrtStream stream);
