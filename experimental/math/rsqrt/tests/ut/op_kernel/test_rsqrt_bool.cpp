...
#define DTYPE_X bool
#define RSQRT_KERNEL_SUFFIX bool_
...
class RsqrtBoolTest : public testing::Test {
    RSQRT_SETUP_TEARDOWN(RsqrtBoolTest)
};

TEST_F(RsqrtBoolTest, bool_0)
{
    constexpr size_t elemCnt = 128;
    size_t byteSize = elemCnt * sizeof(bool);
    size_t tilingSize = sizeof(RsqrtTilingData);
    uint32_t blockDim = 1;

    int ret = system("cd ./rsqrt_data/ && python3 gen_data.py '(128,)' 'bool'");
    EXPECT_EQ(ret, 0);

    uint8_t* x = (uint8_t*)AscendC::GmAlloc(byteSize);
    ReadFile("./rsqrt_data/bool_input_rsqrt.bin", byteSize, x, byteSize);
    ...
    td->tileDataNum = 97792;
    td->smallTailDataNum = 128;
    td->bigTailDataNum = 160;
    td->tailBlockNum = 0;

    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    ICPU_RUN_KF(rsqrt_bool_<1>, blockDim, x, y, wk, (uint8_t*)td);

    WriteFile("./rsqrt_data/bool_output_rsqrt.bin", y, byteSize);
    ...
    ret = system("cd ./rsqrt_data/ && python3 compare_data.py 'bool'");
    EXPECT_EQ(ret, 0);
}
