...
#define DTYPE_X int16_t
#define RSQRT_KERNEL_SUFFIX int16
...
class RsqrtInt16Test : public testing::Test {
    RSQRT_SETUP_TEARDOWN(RsqrtInt16Test)
};

TEST_F(RsqrtInt16Test, int16_0)
{
    constexpr size_t elemCnt = 128;
    size_t byteSize = elemCnt * sizeof(int16_t);
    size_t tilingSize = sizeof(RsqrtTilingData);
    uint32_t blockDim = 1;

    int ret = system("cd ./rsqrt_data/ && python3 gen_data.py '(128,)' 'int16'");
    EXPECT_EQ(ret, 0);

    uint8_t* x = (uint8_t*)AscendC::GmAlloc(byteSize);
    ReadFile("./rsqrt_data/int16_input_rsqrt.bin", byteSize, x, byteSize);
    ...
    ICPU_RUN_KF(rsqrt_int16<1>, blockDim, x, y, wk, (uint8_t*)td);

    WriteFile("./rsqrt_data/int16_output_rsqrt.bin", y, byteSize);
    ...
    ret = system("cd ./rsqrt_data/ && python3 compare_data.py 'int16'");
    EXPECT_EQ(ret, 0);
}
