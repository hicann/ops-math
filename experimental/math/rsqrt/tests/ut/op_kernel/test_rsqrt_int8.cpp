...
class RsqrtInt8Test : public testing::Test {
    RSQRT_SETUP_TEARDOWN(RsqrtInt8Test)
};

TEST_F(RsqrtInt8Test, int8_0)
{
    constexpr size_t elemCnt = 128;
    size_t byteSize = elemCnt * sizeof(int8_t);
    size_t tilingSize = sizeof(RsqrtTilingData);
    uint32_t blockDim = 1;

    int ret = system("cd ./rsqrt_data/ && python3 gen_data.py '(128,)' 'int8'");
    EXPECT_EQ(ret, 0);

    uint8_t* x = (uint8_t*)AscendC::GmAlloc(byteSize);
    ReadFile("./rsqrt_data/int8_input_rsqrt.bin", byteSize, x, byteSize);
    ...
    td->tileDataNum = 32576;
    td->smallTailDataNum = 128;
    td->bigTailDataNum = 160;
    td->tailBlockNum = 0;

    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    ICPU_RUN_KF(rsqrt_int8<1>, blockDim, x, y, wk, (uint8_t*)td);

    WriteFile("./rsqrt_data/int8_output_rsqrt.bin", y, byteSize);
    ...
    ret = system("cd ./rsqrt_data/ && python3 compare_data.py 'int8'");
    EXPECT_EQ(ret, 0);
}
