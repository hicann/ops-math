...
#define DTYPE_X half
#define RSQRT_KERNEL_SUFFIX fp16
...
class RsqrtFp16Test : public testing::Test {
    RSQRT_SETUP_TEARDOWN(RsqrtFp16Test)
};

TEST_F(RsqrtFp16Test, fp16_0)
{
    constexpr size_t elemCnt = 128;
    size_t byteSize = elemCnt * sizeof(half);
    size_t tilingSize = sizeof(RsqrtTilingData);
    uint32_t blockDim = 1;

    int ret = system("cd ./rsqrt_data/ && python3 gen_data.py '(128,)' 'float16'");
    EXPECT_EQ(ret, 0);

    uint8_t* x = (uint8_t*)AscendC::GmAlloc(byteSize);
    ReadFile("./rsqrt_data/float16_input_rsqrt.bin", byteSize, x, byteSize);
    ...
    td->tileDataNum = 32592;
    td->smallTailDataNum = 128;
    td->bigTailDataNum = 144;
    td->tailBlockNum = 0;

    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    ICPU_RUN_KF(rsqrt_fp16<1>, blockDim, x, y, wk, (uint8_t*)td);

    WriteFile("./rsqrt_data/float16_output_rsqrt.bin", y, byteSize);
    ...
    ret = system("cd ./rsqrt_data/ && python3 compare_data.py 'float16'");
    EXPECT_EQ(ret, 0);
}
