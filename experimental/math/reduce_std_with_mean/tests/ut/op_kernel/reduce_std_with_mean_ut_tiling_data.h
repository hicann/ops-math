...
/*
 * Static preset of the tiling head file normally generated by
 * tests/ut/op_kernel/scripts/gen_tiling_head_file.sh at build time.
 * Preset here so the op_kernel UT build does not invoke that script
 * (its `import tbe` chain depends on CI numpy/scipy version match),
 * see tests/ut/op_kernel/CMakeLists.txt. Regenerate this file with
 * gen_tiling_head_file.sh when ReduceStdWithMeanTilingData changes.
 */
#ifndef __REDUCE_STD_WITH_MEAN_HEADER__
#define __REDUCE_STD_WITH_MEAN_HEADER__
...
#endif
#else
#if defined(__DAV_C310__) && defined(__ASCENDC_ENABLE_VEC_TAIL_TILING_COPY__)
...
#endif // __REDUCE_STD_WITH_MEAN_HEADER__
