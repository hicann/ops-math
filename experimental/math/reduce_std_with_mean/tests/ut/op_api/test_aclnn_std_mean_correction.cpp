...
#include "experimental/math/reduce_std_with_mean/op_api/aclnn_std_mean_correction_experimental.h"
...
TEST_F(l2_std_mean_correction_test, std_mean_correction_dim_nullptr)
{
    ...
    auto ut = OP_API_UT(aclnnStdMeanCorrection, INPUT(selfDesc, nullptr, correction, keepdim),
                        OUTPUT(stdOutDesc, meanOutDesc));
    uint64_t workspaceSize = 0;
    aclnnStatus aclRet = ut.TestGetWorkspaceSize(&workspaceSize);
    EXPECT_EQ(aclRet, ACLNN_SUCCESS);
