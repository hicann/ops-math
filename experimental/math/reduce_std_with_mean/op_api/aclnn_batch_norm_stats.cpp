 ...
 */
#include "aclnn_batch_norm_stats_experimental.h"
...
static aclnnStatus aclnnBatchNormStatsImplUnify(const aclTensor* input, const aclIntArray* dim, double eps,
                                                aclTensor* meanOut, aclTensor* invstdOut, uint64_t* workspaceSize,
                                                ...
                                                aclTensor* invstdOut, uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    // The Welford kernel requires reduce-dimension elements to be contiguous in memory.
    const aclTensor* selfForKernel = reformat;
    const aclTensor* meanForKernel = broadcast;
    const aclIntArray* dimForKernel = axes;
    {
        auto selfViewShape = reformat->GetViewShape();
        ...
        if (rank > 1) {
            ...
            if (numReduceDims > 0 && numReduceDims < rank) {
                ...
                if (needTranspose) {
                    // Build perm: non-reduce dims first, then reduce dims
                    ...
                    for (int64_t d = 0; d < rank; d++)
                        if (isReduce[d])
                            perm.push_back(d);
                    auto permArray = uniqueExecutor.get()->AllocIntArray(perm.data(), rank);
                    ...
                    CHECK_RET(meanForKernel != nullptr, ACLNN_ERR_INNER_NULLPTR);
                    // New dims: reduce dims are now the last numReduceDims dims
                    FVector<int64_t> newDimVec;
                    for (int64_t s = 0; s < numReduceDims; s++)
                        newDimVec.push_back(rank - numReduceDims + s);
                    dimForKernel = uniqueExecutor.get()->AllocIntArray(newDimVec.data(), numReduceDims);
    ...
    int64_t correction = 0;
    auto reduceStdWithMeanResult = l0op::ReduceStdWithMean(selfForKernel, meanForKernel, dimForKernel, correction,
                                                           false, true, static_cast<float>(eps), uniqueExecutor.get());
...
aclnnStatus aclnnBatchNormStats(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor, aclrtStream stream)
{
