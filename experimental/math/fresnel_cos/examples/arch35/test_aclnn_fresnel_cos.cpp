...
int main()
{
    ...
    // 3. 调用 aclnnFresnelCos 第一段接口（GetWorkspaceSize）
    uint64_t workspaceSize = 0;
    aclOpExecutor* executor = nullptr;
    ret = aclnnFresnelCosGetWorkspaceSize(x, out, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnFresnelCosGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    // 4. 调用 aclnnFresnelCos 第二段接口
    ret = aclnnFresnelCos(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnFresnelCos failed. ERROR: %d\n", ret); return ret);
    ...
    LOG_PRINT("aclnnFresnelCos example finished successfully.\n");
    return 0;
}
