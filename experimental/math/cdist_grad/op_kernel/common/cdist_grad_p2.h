 ...
 * \brief CdistGrad p=2 (Euclidean distance) Kernel implementation
 *
 * For each (b, i, j):
 *   diff = x1[b,i,:] - x2[b,j,:]
 *   dist_safe = (dist == 0) ? 1.0 : dist
 *   local_grad = diff / dist_safe
...
template <typename T>
__aicore__ inline void CdistGradP2<T>::CopyInX1Row(int64_t b, int64_t i)
{
...
template <typename T>
__aicore__ inline void CdistGradP2<T>::CopyInRChunk(int64_t b, int64_t i, int64_t rStart, int64_t currentRTile)
{
    // x2[b, rStart:rEnd, :]
...
template <typename T>
__aicore__ inline void CdistGradP2<T>::CopyOutAccum(int64_t b, int64_t i)
{
    PipeBarrier<PIPE_ALL>();
...
__aicore__ inline void CdistGradP2<T>::ComputeForJ(int64_t j)
{
    ...
    // diff = x1Row - x2Chunk[j * mAligned_ ... (j+1) * mAligned_ - 1]
    int64_t x2Offset = j * mAligned_;
    Sub(diff, x1Row, x2Chunk[x2Offset], count);

    // Read scalar values from UB local tensor (small R-tile chunk already in UB, not GM access)
    float distVal = distChunk.GetValue(j);
    float gradVal = gradChunk.GetValue(j);

    // dist_safe: avoid division by zero
    float distSafe = (distVal == 0.0f) ? 1.0f : distVal;

    // Broadcast distSafe to M-dim vector
    Duplicate(distBroad, distSafe, count);

    // local_grad = diff / distBroad
    Div(localGrad, diff, distBroad, count);

    // Zero-distance mask: if dist == 0, set local_grad to 0
    if (distVal == 0.0f) {
...
template <typename T>
__aicore__ inline void CdistGradP2<T>::ProcessTask(int64_t taskIdx)
{
...
template <typename T>
__aicore__ inline void CdistGradP2<T>::Process()
{
