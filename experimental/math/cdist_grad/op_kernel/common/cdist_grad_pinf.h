 ...
 * \brief CdistGrad p=inf (Chebyshev distance) Kernel implementation
 *
 * For each (b, i, j):
 *   diff = x1[b,i,:] - x2[b,j,:]
 *   abs_diff = |diff|
 *   max_val = ReduceMax(abs_diff)     // max over M dimension
 *   mask = (abs_diff == max_val)      // which elements achieve the max
 *   local_grad = sign(diff) * mask
 *   accum += grad_output[b,i,j] * local_grad
 *
 * p=inf does not need cdist_result. Needs ReduceMax + Compare for max mask.
...
class CdistGradPInf {
public:
    __aicore__ inline CdistGradPInf() {}
...
template <typename T>
__aicore__ inline void CdistGradPInf<T>::Init(GM_ADDR gradOutput, GM_ADDR x1, GM_ADDR x2,
                                                GM_ADDR cdistResult, GM_ADDR gradX1,
                                                const CdistGradTilingData* tilingData)
{
...
template <typename T>
__aicore__ inline void CdistGradPInf<T>::CopyInX1Row(int64_t b, int64_t i)
{
...
template <typename T>
__aicore__ inline void CdistGradPInf<T>::CopyInRChunk(int64_t b, int64_t i, int64_t rStart, int64_t currentRTile)
{
    // x2[b, rStart:rEnd, :]
    ...
    // p=inf does not need cdist_result
}

template <typename T>
__aicore__ inline void CdistGradPInf<T>::CopyOutAccum(int64_t b, int64_t i)
{
    PipeBarrier<PIPE_ALL>();
...
__aicore__ inline void CdistGradPInf<T>::ComputeForJ(int64_t j)
{
    ...
    LocalTensor<float> tmpReduce = tmpReduceCalcBuf.Get<float>();
    LocalTensor<float> accum = accumBuf.Get<float>();
    ...
    // max_val = ReduceMax(abs_diff)
    // Use distBroad as workspace for ReduceMax to avoid dst/workspace aliasing
    ReduceMax(tmpReduce, tmp, distBroad, static_cast<int32_t>(count));
    PipeBarrier<PIPE_ALL>();
    // GetValue from UB local tensor: standard way to retrieve ReduceMax scalar result from UB
    float maxVal = tmpReduce.GetValue(0);
    ...
    // Apply max mask via arithmetic:
    // tmp = ||diff| - maxVal| * 1e6, clamped to [0,1], inverted
    Abs(tmp, diff, count);
    float negMaxVal = -maxVal;
    Adds(tmp, tmp, negMaxVal, count);
    Abs(tmp, tmp, count);
    Muls(tmp, tmp, 1e6f, count);
    PipeBarrier<PIPE_ALL>();
    Mins(tmp, tmp, 1.0f, count);
    Muls(tmp, tmp, -1.0f, count);
    Adds(tmp, tmp, 1.0f, count);
    PipeBarrier<PIPE_ALL>();

    // Apply mask: localGrad = sign(diff) * mask_float
    Mul(localGrad, localGrad, tmp, count);
...
template <typename T>
__aicore__ inline void CdistGradPInf<T>::ProcessTask(int64_t taskIdx)
{
...
template <typename T>
__aicore__ inline void CdistGradPInf<T>::Process()
{
