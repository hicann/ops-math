 ...
 * \brief CdistGrad p=1 (Manhattan distance) Kernel implementation
 *
 * For each (b, i, j):
 *   diff = x1[b,i,:] - x2[b,j,:]
 *   local_grad = sign(diff)
 *   accum += grad_output[b,i,j] * local_grad
 *
 * p=1 does not need cdist_result. Computation is the simplest among all branches.
                                              ...
                                              const CdistGradTilingData* tilingData)
{
    ...
    // Init buffers - all in fp32 computation precision
...
template <typename T>
__aicore__ inline void CdistGradP1<T>::CopyInX1Row(int64_t b, int64_t i)
{
    ...
    if constexpr (IS_FP16) {
        // fp16: copy to castBuf, then Cast to fp32
...
template <typename T>
__aicore__ inline void CdistGradP1<T>::CopyInRChunk(int64_t b, int64_t i, int64_t rStart, int64_t currentRTile)
{
    // x2[b, rStart:rEnd, :]
    ...
    if constexpr (IS_FP16) {
        // Use castBuf to load fp16 grad, then Cast to fp32
        // For grad/dist chunks, rTileAligned may be larger than mAligned.
        // But castBuf is mAligned * sizeof(half). We need rTileAligned elements.
        // Since rTileAligned <= rTile and castBuf size = mAligned * sizeof(half),
        // we need to ensure mAligned >= rTileAligned. If not, we load in pieces.
        // For safety, since grad/dist are scalar-per-j, currentRTile is typically small.
        // We use castBuf which has mAligned half elements. rTileAligned could be > mAligned
        // in cases where rTile is large and M is small. In that case we load in batches.
        LocalTensor<half> castLocal = castBuf.Get<half>();
        int64_t castCapacity = mAligned_; // number of half elements in castBuf
    ...
    // p=1 does not need cdist_result, but we keep distChunkBuf allocated
    // for buffer uniformity across branches (no-op load)
}

template <typename T>
__aicore__ inline void CdistGradP1<T>::CopyOutAccum(int64_t b, int64_t i)
{
    PipeBarrier<PIPE_ALL>();
    ...
    if constexpr (IS_FP16) {
        // Cast fp32 accum to fp16, then write to GM
        LocalTensor<half> castLocal = castBuf.Get<half>();
...
template <typename T>
__aicore__ inline void CdistGradP1<T>::ProcessTask(int64_t taskIdx)
{
...
template <typename T>
__aicore__ inline void CdistGradP1<T>::Process()
{
