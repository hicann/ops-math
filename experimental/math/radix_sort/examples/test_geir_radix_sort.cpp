...
#include "nn_other.h"
#include "../op_graph/radix_sort_proto.h"
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::RadixSort("add1");
    std::vector<int64_t> xShape = {2048};
    ADD_INPUT(1, x, inDtype, xShape);

    ADD_OUTPUT(1, sorted_values, inDtype, xShape);
    ADD_OUTPUT(2, sorted_indices, DT_INT64, xShape);
