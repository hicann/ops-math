...
static const aclTensor* CastToOutDtypeIfNeeded(const aclTensor* input, const aclTensor* out, aclOpExecutor* executor)
{
    if (input->GetDataType() == out->GetDataType()) {
        return input;
    }
    return l0op::Cast(input, out->GetDataType(), executor);
}
    ...
    aclTensor* out, uint64_t* workspaceSize, UniqueExecutor& uniqueExecutor, aclOpExecutor** executor)
{
    ...
    auto castOut = CastToOutDtypeIfNeeded(stdOut, out, uniqueExecutor.get());
    ...
    bool keepdim, aclTensor* out, uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    // ReduceMean uses keepdim=true, so its output can be consumed by ReduceStdWithMean directly.
    // This avoids registering a generic custom Expand L0 for aclnnStd's internal broadcast.
    ...
    auto castOut = CastToOutDtypeIfNeeded(stdWithMeanOpOut, out, uniqueExecutor.get());
    CHECK_RET(castOut != nullptr, ACLNN_ERR_INNER_NULLPTR);
