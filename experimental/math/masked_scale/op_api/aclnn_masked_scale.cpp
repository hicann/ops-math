...
#include "aclnn_exp_masked_scale.h"
#include "masked_scale_opapi.h"
...
static const std::initializer_list<DataType> REGBASE_X_DTYPE_SUPPORT_LIST = {DataType::DT_FLOAT, DataType::DT_FLOAT16,
                                                                             DataType::DT_BF16};
...
static const std::initializer_list<DataType>& GetXDtypeSupportList()
{
    if (GetCurrentPlatformInfo().GetSocVersion() == SocVersion::ASCEND910B ||
        GetCurrentPlatformInfo().GetCurNpuArch() == NpuArch::DAV_3510 || IsRegBase()) {
...
static aclnnStatus CheckParams(const aclTensor* self, const aclTensor* mask, const aclTensor* out)
{
    ...
    // 4. 检查self和out的format一致
    CHECK_RET(CheckFormat(self, out), ACLNN_ERR_PARAM_INVALID);

    return ACLNN_SUCCESS;
}

aclnnStatus aclnnMaskedScaleGetWorkspaceSize(const aclTensor* self, const aclTensor* mask, float scale, aclTensor* out,
