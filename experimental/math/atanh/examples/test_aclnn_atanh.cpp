...
/**
 * @file test_aclnn_atanh.cpp
 * @brief aclnnAtanh 算子调用示例
 *
 * 功能说明：
 *   计算输入 Tensor 的逆双曲正切 atanh(x) = 0.5 * ln((1+x)/(1-x))，输入值域 (-1, 1)
 *
 * 接口原型：
 *   aclnnAtanhGetWorkspaceSize(inputTensor, outputTensor, &workspaceSize, &executor)
 *   aclnnAtanh(workspace, workspaceSize, executor, stream)
 *
...
int main()
{
    ...
    std::vector<float> inputHostData = {0.0f, 0.5f, -0.5f, 0.9f, -0.9f, 0.3f};
    ...
    // 调用aclnnAtanh第一段接口
    ret = aclnnAtanhGetWorkspaceSize(inputTensor, outputTensor, &workspaceSize, &executor);
    ...
    ret = aclrtMemcpy(
        resultData.data(), resultData.size() * sizeof(resultData[0]), outputDeviceAddr, size * sizeof(float),
        ACL_MEMCPY_DEVICE_TO_HOST);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("copy result from device to host failed. ERROR: %d\n", ret); return ret);
    // 预期输出: atanh(0.0)=0.0, atanh(0.5)≈0.5493, atanh(-0.5)≈-0.5493,
    //          atanh(0.9)≈1.4722, atanh(-0.9)≈-1.4722, atanh(0.3)≈0.3095
