...
protected:
    static void SetUpTestCase() { cout << "l2_asinh_test SetUp" << endl; }

    static void TearDownTestCase() { cout << "l2_asinh_test TearDown" << endl; }
};
...
TEST_F(l2_asinh_test, ascend910B2_asinh_dtype_all)
{
    ...
    for (auto dtype1 : INPUT_VALID_DTYPE_LIST) {
        ...
        for (auto dtype2 : OUTPUT__VALID_DTYPE_LIST) {
            ...
            // EXPECT_EQ(aclRet, ACL_SUCCESS);  // 当前UT不支持CastAiCPU，暂时注释掉返回值校验
        }
    }
}
