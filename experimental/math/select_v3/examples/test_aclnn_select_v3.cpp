...
int main()
{
    ...
    std::vector<int64_t> selfXShape = {15};
    std::vector<DataType> selfXHostData(15, 2.6f);
    // std::vector<DataType> selfXHostData={1,2,3,4,5};
    ...
    std::vector<int64_t> selfYShape = {15};
    std::vector<DataType> selfYHostData(15, 7.8f);
    // std::vector<DataType> selfYHostData={1,2,3,4,5,6,7};
    ret = CreateAclTensor(selfYHostData, selfYShape, &selfYDeviceAddr, aclDataType::ACL_FLOAT, &selfY);
    CHECK_RET(ret == ACL_SUCCESS, return ret);

    aclTensor* selfB = nullptr;
    ...
    std::vector<DataType> outHostData(15, 0);
    ...
    ret = aclnnSelectV3(workspaceAddr, workspaceSize, executor, stream);
