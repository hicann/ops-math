                    ...
                    aclDataType dataType, aclTensor** tensor)
{
    ...
    if (dataType == aclDataType::ACL_FLOAT16) {
        ...
        auto copySize = std::min(static_cast<int64_t>(elemCount * sizeof(T)), size);
...
int main()
{
    ...
    std::vector<float> yHostData(3, 1);
    ret = CreateAclTensor(yHostData, yShape, &yDeviceAddr, aclDataType::ACL_FLOAT16, &y);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    std::vector<int64_t> dyShape = {3};
    std::vector<float> dyHostData(3, 1);
    ...
    std::vector<int64_t> dxShape = {3};
    std::vector<float> dxHostData(3, 0);
    ret = CreateAclTensor(dxHostData, dxShape, &dxDeviceAddr, aclDataType::ACL_FLOAT16, &dx);
    CHECK_RET(ret == ACL_SUCCESS, return ret);

    // 调用 aclnnAcoshGradGetWorkspaceSize 第一段接口
    uint64_t workspaceSize = 0;
    aclOpExecutor* executor = nullptr;
    ret = aclnnAcoshGradGetWorkspaceSize(y, dy, dx, &workspaceSize, &executor);
    CHECK_RET(ret == ACLNN_SUCCESS, LOG_PRINT("aclnnAcoshGradGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    // 调用 aclnnAcoshGrad 第二段接口
    ret = aclnnAcoshGrad(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACLNN_SUCCESS, LOG_PRINT("aclnnAcoshGrad failed. ERROR: %d\n", ret); return ret);
