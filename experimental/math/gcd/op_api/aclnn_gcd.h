 ...
 *
 * @param [in] self:
 * 输入`self`，shape需要与other满足broadcast规则，支持非连续的Tensor，数据格式支持ND。
 * 支持FLOAT、FLOAT16、BFLOAT16、INT8、UINT8、INT16、INT32、INT64，支持promote/cast。
 * @param [in] other:
 * 输入`other`，shape需要与self满足broadcast规则，支持非连续的Tensor，数据格式支持ND。
 * 数据类型与self一起参与promote。
 * @param [in] out: 输出`out`，shape需要与self和other进行broadcast后的shape一致，支持非连续的Tensor，数据格式支持ND。
 * 可由promote结果cast到out。
