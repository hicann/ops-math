...
int32_t GetDeviceId()
{
    const char* env = std::getenv("GCD_DEVICE_ID");
...
int main()
{
    ...
    std::vector<int16_t> otherHostData = {0, 1, 2, 3, 4, 5, 6, 7};
    std::vector<int16_t> outHostData = {0, 0, 0, 0, 0, 0, 0, 0};

    ret = CreateAclTensor(selfHostData, selfShape, &selfDeviceAddr, aclDataType::ACL_INT16, &self);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ret = CreateAclTensor(otherHostData, otherShape, &otherDeviceAddr, aclDataType::ACL_INT16, &other);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
