 ...
 * 由第一段接口aclnnEqTensorGetWorkspaceSize获取。
 ...
 * @domain aclnn_math
 *
 * 算子功能：计算selfRef中元素的值与other的值是否相等，
 * 将selfRef每个元素与other的值的比较结果原地返回到selfRef中；
 *
 * 计算公式：$$ selfRef = (selfRef == other)  ?  [True] : [False] $$
 *
 * @param [in] selfRef: npu device侧的aclTensor，
 * 数据类型支持FLOAT16,FLOAT,INT64,INT32,INT8,UINT8,BOOL,BFLOAT16(仅1971支持),DOUBLE,INT16,
 ...
 * @param [in] workspace: 在npu device侧申请的workspace内存起址。
 * @param [in] workspaceSize: 在npu device侧申请的workspace大小，
 * 由第一段接口aclnnInplaceEqTensorGetWorkspaceSize获取。
 * @param [in] executor: op执行器，包含了算子计算流程。
 * @param [in] stream: acl stream流。
 * @return aclnnStatus: 返回状态码。
 */
