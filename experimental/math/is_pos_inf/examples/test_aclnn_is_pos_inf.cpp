...
int main()
{
    ...
    const std::vector<float> inputHost = {posInf, -2.0f, 0.0f, 3.0f, posInf, 5.0f};
    ...
    ret = aclnnIsPosInfGetWorkspaceSize(inputTensor, outputTensor, &workspaceSize, &executor);
    if (ret != ACL_SUCCESS) {
        exitCode = ret;
        goto CLEANUP;
    }
    ...
    ret = aclnnIsPosInf(workspace, workspaceSize, executor, stream);
    if (ret != ACL_SUCCESS) {
        exitCode = ret;
        goto CLEANUP;
    }
    ...
    if (std::memcmp(outputHost.data(), expected.data(), outputBytes) != 0) {
        std::printf("is_pos_inf output mismatch\n");
        exitCode = 1;
        goto CLEANUP;
    }
    std::printf("is_pos_inf example passed\n");
