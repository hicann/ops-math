...
#include "../op_kernel/is_neg_inf_tiling_data.h"
...
static IsNegInfDtypeId GetDtypeId(DataType dtype)
{
    if (dtype == DT_FLOAT) {
        return IsNegInfDtypeId::FP32;
    }
    if (dtype == DT_FLOAT16) {
        return IsNegInfDtypeId::FP16;
    }
    if (dtype == DT_BF16) {
        return IsNegInfDtypeId::BF16;
    }
    return IsNegInfDtypeId::INVALID;
}
                                 ...
                                 IsNegInfDtypeId& dtypeId)
{
    ...
    if (dtypeId == IsNegInfDtypeId::INVALID) {
...
static graphStatus ComputeTilePolicy(int64_t totalLength, int64_t dtypeSize, IsNegInfDtypeId dtypeId, int64_t coreNum,
                                     ...
                                     int64_t& tailLength, int64_t& tileLength)
{
    ...
    if (dtypeId == IsNegInfDtypeId::FP32) {
        tileLength = std::min<int64_t>(tileLength, DEBUG_FP32_TILE_ELEMENTS);
    } else if (dtypeId == IsNegInfDtypeId::BF16) {
...
static graphStatus IsNegInfTilingFunc(gert::TilingContext* context)
{
    int64_t totalLength = 0;
    int64_t dtypeSize = 0;
    IsNegInfDtypeId dtypeId = IsNegInfDtypeId::INVALID;
...
static graphStatus TilingParseForIsNegInf([[maybe_unused]] gert::TilingParseContext* context) { return GRAPH_SUCCESS; }

IMPL_OP_OPTILING(IsNegInf).Tiling(IsNegInfTilingFunc).TilingParse<IsNegInfCompileInfo>(TilingParseForIsNegInf);
} // namespace optiling
