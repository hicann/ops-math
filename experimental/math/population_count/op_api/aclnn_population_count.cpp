...
static const std::initializer_list<op::DataType> X_DTYPE_SUPPORT_LIST = {DataType::DT_INT16, DataType::DT_UINT16};

static bool IsXDtypeSupported(DataType dtype) { return CheckType(dtype, X_DTYPE_SUPPORT_LIST); }
...
{
    OP_CHECK(
        IsXDtypeSupported(x->GetDataType()),
        OP_LOGE(
            ACLNN_ERR_PARAM_INVALID, "PopulationCount: x dtype not supported: %d (must be INT16 or UINT16).",
            static_cast<int>(x->GetDataType())),
        return false);
    OP_CHECK(
        y->GetDataType() == DataType::DT_UINT8,
        OP_LOGE(
            ACLNN_ERR_PARAM_INVALID, "PopulationCount: y dtype must be UINT8, got %d.",
...
static bool CheckFormat(const aclTensor* x, const aclTensor* y)
{
    ...
    OP_CHECK(
        !(IsPrivateFormat(xFmt) || IsPrivateFormat(yFmt)),
        OP_LOGE(
            ACLNN_ERR_PARAM_INVALID, "PopulationCount: private format not supported. x=%d, y=%d",
...
static bool CheckShape(const aclTensor* x, const aclTensor* y)
{
    ...
    OP_CHECK(
        ...
        OP_LOGE(
            ACLNN_ERR_PARAM_INVALID, "PopulationCount: x/y dim num mismatch: %zu vs %zu", xShape.GetDimNum(),
            yShape.GetDimNum()),
        return false);
    for (size_t i = 0; i < xShape.GetDimNum(); i++) {
        OP_CHECK(
            ...
            OP_LOGE(
                ACLNN_ERR_PARAM_INVALID, "PopulationCount: x/y shape mismatch at dim %zu: %ld vs %ld", i,
                xShape.GetDim(i), yShape.GetDim(i)),
            return false);
    }
    return true;
...
static aclnnStatus CheckParams(const aclTensor* x, const aclTensor* y)
{
    ...
    CHECK_COND(
        CheckDtypeValid(x, y), ACLNN_ERR_PARAM_INVALID, "CheckDtypeValid failed: x_dtype=%d, y_dtype=%d",
