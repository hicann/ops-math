...
static bool IsAiCoreSupport(const aclTensor* x)
{
    ...
    OP_CHECK(
        ...
        OP_LOGE(
            ACLNN_ERR_PARAM_INVALID, "PopulationCount: only Ascend950 (arch35/DAV_3510) is supported, got npuArch=%d.",
            static_cast<int>(npuArch)),
        return false);
    OP_CHECK(
        CheckType(x->GetDataType(), X_DTYPE_SUPPORT_LIST),
        OP_LOGE(
            ACLNN_ERR_PARAM_INVALID, "PopulationCount: x dtype %d not supported (must be INT16/UINT16).",
            static_cast<int>(x->GetDataType())),
        return false);
    return true;
}

static const aclTensor* PopulationCountAiCore(const aclTensor* x, const aclTensor* out, aclOpExecutor* executor)
{
    L0_DFX(PopulationCountAiCore, x, out);

    auto ret = ADD_TO_LAUNCHER_LIST_AICORE(PopulationCount, OP_INPUT(x), OP_OUTPUT(out));
    OP_CHECK(ret == ACLNN_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "PopulationCountAiCore failed."), return nullptr);
    return out;
}
