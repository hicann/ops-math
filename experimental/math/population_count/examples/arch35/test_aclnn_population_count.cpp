...
static int RunOneCase(
    const char* label, aclDataType dtype, const uint16_t* rawU16, int64_t elemCount, aclrtStream stream)
{
    ...
    xTensor = aclCreateTensor(shape, ndim, dtype, strides, 0, ACL_FORMAT_ND, shape, ndim, devX);
    yTensor = aclCreateTensor(shape, ndim, ACL_UINT8, strides, 0, ACL_FORMAT_ND, shape, ndim, devY);
...

    {
        ...
        CHECK_ACL(aclnnPopulationCountGetWorkspaceSize(xTensor, yTensor, &workspaceSize, &executor));
        ...
        CHECK_ACL(aclrtMemcpy(hostY.data(), outputBytes, devY, outputBytes, ACL_MEMCPY_DEVICE_TO_HOST));

        std::cout << "PopulationCount Example [" << label << "] (shape: [" << elemCount << "], y: uint8)" << std::endl;
        ...
        for (int64_t i = 0; i < elemCount; ++i) {
            ...
            passCount += pass ? 1 : 0;
            std::printf(
                "  %4lld | %11u | 0x%04X | %6u | %6u | %s\n", static_cast<long long>(i),
                static_cast<unsigned int>(hostX[i]), static_cast<unsigned int>(hostX[i]),
                static_cast<unsigned int>(hostY[i]), static_cast<unsigned int>(expected[i]), pass ? "PASS" : "FAIL");
        }
...
int main()
{
...

    {
        ...
        std::cout << "=================================================================" << std::endl;
        std::cout << "Overall: " << ((retInt16 == 0 && retUint16 == 0) ? "ALL PASS" : "FAILED") << std::endl;
        std::cout << "=================================================================" << std::endl;
