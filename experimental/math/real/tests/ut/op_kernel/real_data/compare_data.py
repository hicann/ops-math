...
def compare_data(golden_file_lists, output_file_lists, d_type):
    if d_type == "float16":
        ...
        np_dtype = np.float64
    elif d_type == "complex32":
        np_dtype = np.float16  # complex32 的实部是 float16
    elif d_type == "complex64":
        np_dtype = np.float32  # complex64 的实部是 float32
    elif d_type == "complex128":
        np_dtype = np.float64  # complex128 的实部是 float64
    elif d_type == "int32":
        np_dtype = np.float32
    elif d_type == "bool":
        np_dtype = np.bool_
    else:
        raise ValueError("d_type must in [float16, float32, float64, complex32, complex64, complex128, int32, bool]")
