...
int main()
{
    ...
    ret = aclInit(nullptr);
    if (ret != ACL_SUCCESS) { LOG_PRINT("FATAL: aclInit=%d", ret); return 1; }
    ret = aclrtSetDevice(0);
    if (ret != ACL_SUCCESS) { LOG_PRINT("FATAL: setDevice=%d", ret); goto fail_dev; }
    ret = aclrtCreateStream(&stream);
    if (ret != ACL_SUCCESS) { LOG_PRINT("FATAL: createStream=%d", ret); goto fail_dev; }
