...
#include "nn_other.h"
#include "../op_graph/tabulate_fusion_grad_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    placeholder##intputIndex##_desc.SetRealDimCnt(placeholder##intputIndex##_shape.size());                         \
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    ...
    outputName##outputIndex##_desc.SetRealDimCnt(outputShape.size());                                       \
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::TabulateFusionGrad("add1");
    // nloc=2, nnei=3, last_layer_size=4 -> size_align64 = ceil(4/64)*64 = 64
    // table:        (10, 64*6=384)
    // table_info:   (6,)
    // em_x:         (2*3=6, 1)
    // em:           (2, 3, 4)
    ...
    // dy_dem_x:     (6, 1)
    // dy_dem:       (2, 3, 4)
    std::vector<int64_t> tableShape = {10, 384};
    std::vector<int64_t> tableInfoShape = {6};
    std::vector<int64_t> emXShape = {6, 1};
    std::vector<int64_t> emShape = {2, 3, 4};
    std::vector<int64_t> dyShape = {2, 4, 4};
    std::vector<int64_t> descShape = {2, 4, 4};
    std::vector<int64_t> dyDemXShape = {6, 1};
    std::vector<int64_t> dyDemShape = {2, 3, 4};
    ADD_INPUT(1, table, inDtype, tableShape);
    ADD_INPUT(2, table_info, inDtype, tableInfoShape);
    ADD_INPUT(3, em_x, inDtype, emXShape);
    ADD_INPUT(4, em, inDtype, emShape);
    ADD_INPUT(5, dy, inDtype, dyShape);
    ADD_INPUT(6, descriptor, inDtype, descShape);

    ADD_OUTPUT(1, dy_dem_x, inDtype, dyDemXShape);
    ADD_OUTPUT(2, dy_dem, inDtype, dyDemShape);
