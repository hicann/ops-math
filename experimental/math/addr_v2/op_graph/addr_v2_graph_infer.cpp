...
static ge::graphStatus InferDataType4AddrV2(gert::InferDataTypeContext* context)
{
    OP_LOGD(context->GetNodeName(), "InferDataType4AddrV2 enter");
    ...
    OP_CHECK_IF(x1_data_type != x2_data_type || x1_data_type != x3_data_type || x1_data_type != beta_data_type ||
                ...
                OP_LOGE(context->GetNodeName(), "[InferDataType4AddrV2] Failed, input datatype must be same."),
                return ge::GRAPH_FAILED);

    context->SetOutputDataType(0, x1_data_type);
    OP_LOGD(context->GetNodeName(), "InferDataType4AddrV2 end");
    return ge::GRAPH_SUCCESS;
}

IMPL_OP(AddrV2).InferDataType(InferDataType4AddrV2).InputsDataDependency({INPUT_NUM_THREE, INPUT_NUM_FOUR});
