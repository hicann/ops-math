# aclnnAddrV2&aclnnInplaceAddrV2

[📄 查看源码](https://gitcode.com/cann/ops-math/tree/master/experimental/math/addr_v2)
...
- aclnnAddrV2和aclnnInplaceAddrV2实现相同的功能，使用区别如下，请根据自身实际场景选择合适的算子。
  - aclnnAddrV2：需新建一个输出张量对象存储计算结果。
  - aclnnInplaceAddrV2：无需新建输出张量对象，直接在输入张量的内存中存储计算结果。
- 每个算子分为[两段式接口](../../../../docs/zh/context/two_phase_api.md)，必须先调用"aclnnAddrV2GetWorkspaceSize"或者"aclnnInplaceAddrV2GetWorkspaceSize"接口获取入参并根据流程计算所需workspace大小，再调用"aclnnAddrV2"或者"aclnnInplaceAddrV2"接口执行计算。

```cpp
aclnnStatus aclnnAddrV2GetWorkspaceSize(
...
```cpp
aclnnStatus aclnnInplaceAddrV2GetWorkspaceSize(
  aclTensor*          selfRef,
  const aclTensor*    vec1,
  const aclTensor*    vec2,
  const aclScalar*    betaOptional,
...
```cpp
aclnnStatus aclnnInplaceAddrV2(
  void*             workspace,
  uint64_t          workspaceSize,
  aclOpExecutor*    executor,
  const aclrtStream stream)
  ...
  <table style="undefined;table-layout: fixed; width: 1570px"><colgroup>
    ...
    <tbody>
      <tr>
        ...
        <td>输入</td>
        <td>外积扩展矩阵。</td>
        <td>shape维度不能超过2，并且需要与vec1、vec2满足<a href="../../../../docs/zh/context/broadcast_relationship.md" target="_blank">broadcast关系</a>。</td>
  ...
  <col style="width: 833px">
    ...
    <tbody>
      ...
      <tr>
        <td>workspaceSize</td>
        <td>输入</td>
        <td>在Device侧申请的workspace大小，由第一段接口aclnnAddrV2GetWorkspaceSize获取。</td>
      </tr>
      <tr>
        <td>executor</td>
        <td>输入</td>
...
## aclnnInplaceAddrV2GetWorkspaceSize
  ...
  <table style="undefined;table-layout: fixed; width: 1570px"><colgroup>
    ...
    <tbody>
      <tr>
        ...
        <td>输入/输出</td>
        <td>外积扩展矩阵及输出矩阵。</td>
        <td>shape维度为2，不支持空Tensor，并且需要与vec1、vec2满足<a href="../../../../docs/zh/context/broadcast_relationship.md" target="_blank">broadcast关系</a>。</td>
        <td>FLOAT、FLOAT16、DOUBLE、INT8、INT16、INT32、INT64、UINT8、BOOL、BFLOAT16</td>
        <td>ND</td>
        <td>-</td>
        <td>√</td>
  ...
  <col style="width: 833px">
    ...
    <tbody>
      ...
      <tr>
        <td>workspaceSize</td>
        <td>输入</td>
        <td>在Device侧申请的workspace大小，由第一段接口aclnnInplaceAddrV2GetWorkspaceSize获取。</td>
      </tr>
      <tr>
        <td>executor</td>
        <td>输入</td>
...
- 确定性计算：
  - aclnnAddrV2&aclnnInplaceAddrV2默认确定性实现。
...
int main() {
  ...
  // 调用aclnnAddrV2第二段接口
  ret = aclnnAddrV2(workspaceAddr, workspaceSize, executor, stream);
  ...
  ret = aclnnInplaceAddrV2(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAddrV2 failed. ERROR: %d\n", ret); return ret);
