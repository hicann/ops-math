...
/*!
 * \file reduce_mean_v2.h
 * \brief
 */
#ifndef _REDUCE_MEAN_V2_H
#define _REDUCE_MEAN_V2_H

#include "kernel_operator.h"
#include "kernel_tiling/kernel_tiling.h"
#include "reduce_mean_v2_tiling_data.h"
#include "reduce_mean_v2_tiling_key.h"

namespace NsReduceMeanV2 {
...
class ReduceMeanV2 {
public:
    __aicore__ inline ReduceMeanV2(){};

    __aicore__ inline void Init(GM_ADDR x, GM_ADDR z,GM_ADDR workspace,const ReduceMeanV2TilingData* tilingData);
...
template <typename T>
__aicore__ inline void ReduceMeanV2<T>::Init(GM_ADDR x, GM_ADDR z,GM_ADDR workspace, const ReduceMeanV2TilingData* tilingData)
{
        ...
        if(axes == 0) {
            ...
            if(this->keyType == 1){
                pipe.InitBuffer(tmpBase, this->cols * SLOT_STRIDE * sizeof(T));
            }
        } else if(axes == 1) {
            pipe.InitBuffer(rowSum, this->rows * sizeof(float));
            if(this->keyType == 1){
                pipe.InitBuffer(tmpBase, this->rows * SLOT_STRIDE * sizeof(T));
            }
        }else{
            pipe.InitBuffer(tmpBase, DATA_CACHE_CLEAN_NEED * sizeof(float));
        }
...
template <typename T>
__aicore__ inline void ReduceMeanV2<T>::Process()
{
   if(this->axes == 0){
        ReduceMeanV2Axes0();
    }else if (this->axes == 1){
...
template <typename T>
__aicore__ inline void ReduceMeanV2<T>::ReduceMeanV2Axes0(){
    AscendC::SyncAll();
    ...
    uint32_t totalWorkSize = colNum * SLOT_STRIDE; //需要的float个数
    ...
    if(this->blockIdx == 0){
        ...
        float val = 1.0f / rowNum;
        AscendC::Muls(tmpBuf, tmpBuf, val, totalWorkSize);
        if constexpr (AscendC::Std::is_same<T, float16_t>::value){
            int32_t eventIDVToV = static_cast<int32_t>(GetTPipePtr()->FetchEventID(AscendC::HardEvent::V_V));
            AscendC::SetFlag<AscendC::HardEvent::V_V>(eventIDVToV);
            AscendC::WaitFlag<AscendC::HardEvent::V_V>(eventIDVToV);
            AscendC::LocalTensor<T> tmpBase0 = tmpBase.Get<T>();
            AscendC::Cast(tmpBase0, tmpBuf, AscendC::RoundMode::CAST_ROUND, totalWorkSize);
            ...
            AscendC::DataCopyExtParams copyParams{static_cast<uint16_t>(colNum), sizeof(T), static_cast<uint32_t>(1), 0, 0}; // 结构体DataCopyExtParams最后一个参数是rsv保留位
            AscendC::DataCopyPad(zGm, tmpBase0, copyParams);
        }else{
            int32_t eventIDSToMTE3 = static_cast<int32_t>(GetTPipePtr()->FetchEventID(AscendC::HardEvent::V_MTE3));
            AscendC::SetFlag<AscendC::HardEvent::V_MTE3>(eventIDSToMTE3);
            AscendC::WaitFlag<AscendC::HardEvent::V_MTE3>(eventIDSToMTE3);
            AscendC::DataCopyExtParams copyParams{static_cast<uint16_t>(colNum), sizeof(float), static_cast<uint32_t>(1), 0, 0}; // 结构体DataCopyExtParams最后一个参数是rsv保留位
...
template <typename T>
__aicore__ inline void ReduceMeanV2<T>::ReduceMeanV2Axes1(){
    AscendC::SyncAll();
    const uint32_t colNum = this->cols;
    const uint32_t rowNum = this->rows;
    const uint32_t totalElements = rowNum * colNum;
    ...
    if(this->blockIdx == 0){
        ...
        if constexpr (AscendC::Std::is_same<T, float16_t>::value){
            ...
            AscendC::SetFlag<AscendC::HardEvent::V_MTE3>(eventIDSToMTE3);
            AscendC::WaitFlag<AscendC::HardEvent::V_MTE3>(eventIDSToMTE3);
            AscendC::DataCopyExtParams copyParams{static_cast<uint16_t>(rowNum), sizeof(float), static_cast<uint32_t>(1), 0, 0}; // 结构体DataCopyExtParams最后一个参数是rsv保留位
            AscendC::DataCopyPad(zGm, tmpBuf, copyParams);
        }
    }
}
...
template <typename T>
__aicore__ inline void ReduceMeanV2<T>::ReduceMeanV2AxesAll(){
    ...
    float globalSum = workGm.GetValue(0) / totalElements;
    if(this->blockIdx == 0){
        zGm.SetValue(0, static_cast<T>(globalSum));
    }
}
} // namespace NsReduceMeanV2
#endif // ReduceMeanV2_H