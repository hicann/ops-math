...
/*!
 * \file reduce_mean_v2_tiling.cpp
 * \brief
 */
#include "log/log.h"
...
#include "../op_kernel/reduce_mean_v2_tiling_data.h"
#include "../op_kernel/reduce_mean_v2_tiling_key.h"

namespace optiling {
    const uint32_t BUFFER_NUM = 2;
    const uint32_t WS_SYS_SIZE =  16U * 1024U* 1024U; // 16MB
    const uint64_t UB_DATA_NUMBER_32 = 6;
    struct ReduceMeanV2CompileInfo {};
    
    struct ReduceMeanV2ShapeInfo {
        uint64_t inputNum{0};
    ...
    static ge::graphStatus GetShapeAttrsInfo(gert::TilingContext* context, uint64_t ubSize, ReduceMeanV2ShapeInfo& info)
    {
        OP_CHECK_IF(context == nullptr, OP_LOGE(context, "context is nullptr"), return ge::GRAPH_FAILED);
    ...
    static ge::graphStatus CalculateCoreBlockNums(int64_t coreNum, ReduceMeanV2ShapeInfo& info)
    {
    ...
    // tiling 分发入口
    static ge::graphStatus ReduceMeanV2TilingFunc(gert::TilingContext* context)
    {
        // ReduceMeanV2TilingData tiling;
        ReduceMeanV2TilingData* tiling = context->GetTilingData<ReduceMeanV2TilingData>();
        ReduceMeanV2ShapeInfo shapeInfo;
        OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
        OP_CHECK_IF(
            memset_s(tiling, sizeof(ReduceMeanV2TilingData), 0, sizeof(ReduceMeanV2TilingData)) != EOK,
    ...
    static ge::graphStatus TilingParseForReduceMeanV2([[maybe_unused]] gert::TilingParseContext* context)
    {   
        return ge::GRAPH_SUCCESS;
    }

    // tiling注册入口.
    IMPL_OP_OPTILING(ReduceMeanV2).Tiling(ReduceMeanV2TilingFunc).TilingParse<ReduceMeanV2CompileInfo>(TilingParseForReduceMeanV2);
} // namespace optiling