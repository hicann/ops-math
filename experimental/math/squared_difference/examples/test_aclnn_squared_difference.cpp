...
#include <algorithm>
#include "acl/acl.h"
#include "aclnn_squared_difference.h"
...
int main()
{
    ...
    std::vector<int64_t> x1Shape = {1, 19};
    std::vector<float> x1HostData(19, 1);
    ret = CreateAclTensor(x1HostData, x1Shape, &x1DeviceAddr, aclDataType::ACL_BF16, &x1);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    std::vector<int64_t> x2Shape = {1, 19};
    std::vector<float> x2HostData(19, 1);
    ...
    std::vector<int64_t> yShape = {1, 19};
    std::vector<float> yHostData(19, 0);
    ret = CreateAclTensor(yHostData, yShape, &yDeviceAddr, aclDataType::ACL_BF16, &y);
    CHECK_RET(ret == ACL_SUCCESS, return ret);

    // 调用 aclnnSquaredDifferenceGetWorkspaceSize 第一段接口
    uint64_t workspaceSize = 0;
    aclOpExecutor* executor = nullptr;
    ret = aclnnSquaredDifferenceGetWorkspaceSize(x1, x2, y, &workspaceSize, &executor);
    CHECK_RET(ret == ACLNN_SUCCESS, LOG_PRINT("aclnnSquaredDifferenceGetWorkspaceSize failed. ERROR: %d\n", ret);
              return ret);
    ...
    // 调用 aclnnSquaredDifference 第二段接口
    ret = aclnnSquaredDifference(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACLNN_SUCCESS, LOG_PRINT("aclnnSquaredDifference failed. ERROR: %d\n", ret); return ret);
