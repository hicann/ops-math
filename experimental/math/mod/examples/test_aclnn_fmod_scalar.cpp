...
int Compute(aclrtStream stream, aclTensor* self, aclScalar* other, aclTensor* out)
{
    uint64_t workspaceSize = 0;
    aclOpExecutor* executor;
    auto ret = aclnnFmodScalarGetWorkspaceSize(self, other, out, &workspaceSize, &executor);
    if (ret != ACL_SUCCESS) {
    ...
    if (ret != ACL_SUCCESS) {
        LOG_PRINT("aclnnFmodScalar failed. ERROR: %d\n", ret);
        return ret;
    }
...
void DestroyResources(void* selfAddr, void* outAddr, aclTensor* self, aclScalar* other, aclTensor* out,
...
int main()
{
    ...
    // 打印结果
    if (PrintResult(outAddr, outShape) != 0) {
        return -1;
    }

    // 释放资源
    DestroyResources(selfAddr, outAddr, self, other, out, stream, deviceId);
    return 0;
}
