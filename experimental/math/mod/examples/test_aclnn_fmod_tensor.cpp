...
int Compute(aclrtStream stream, aclTensor* self, aclTensor* other, aclTensor* out)
{
    ...
    auto ret = aclnnFmodTensorGetWorkspaceSize(self, other, out, &workspaceSize, &executor);
    if (ret != ACL_SUCCESS) {
        LOG_PRINT("aclnnFmodTensorGetWorkspaceSize failed. ERROR: %d\n", ret);
        return ret;
    }
    ...
    if (ret != ACL_SUCCESS) {
        LOG_PRINT("aclnnFmodTensor failed. ERROR: %d\n", ret);
        return ret;
    }
