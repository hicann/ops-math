...
class L2InplaceFmodScalarTest : public testing::Test {};

TEST_F(L2InplaceFmodScalarTest, float_scalar)
{
    auto self = TensorDesc({10, 10}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(0, 100);
    auto scalar = ScalarDesc(3.0, ACL_FLOAT);
    auto ut = OP_API_UT(aclnnInplaceFmodScalar, INPUT(self, scalar), OUTPUT());
    uint64_t workspaceSize = 0;
    EXPECT_EQ(ut.TestGetWorkspaceSize(&workspaceSize), ACL_SUCCESS);
}

TEST_F(L2InplaceFmodScalarTest, int32_scalar)
{
