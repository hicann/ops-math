...
/*!
 * \file add_mat_mat_elements_plus_tiling_arch35.cpp
 * \brief AddMatMatElementsPlus Tiling 实现（arch35，Ascend950）
 *
...
#include "../../op_kernel/arch35/add_mat_mat_elements_plus_tiling_key_arch35.h"
#include "add_mat_mat_elements_plus_tiling_arch35.h"
...
ge::graphStatus AddMatMatElementsPlusTilingFunc(gert::TilingContext* context)
{
    ...
    // 4. 设置 workspace（本算子逐元素计算无需额外 workspace）
    size_t* currentWorkspace = context->GetWorkspaceSizes(1);
    if (currentWorkspace != nullptr) {
        currentWorkspace[0] = 0U;
    }

    // 5. 设置 TilingKey（按 dtype 选择模板）
    ASCENDC_TPL_SEL_PARAM(context, static_cast<uint32_t>(dtype));

    // 6. 填写 TilingData（alpha/beta 已改为 tensor input，由 kernel 从 GM 读取）
...
struct AddMatMatElementsPlusCompileInfo {}; // 必须定义，入图场景依赖
