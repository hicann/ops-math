...
/*!
 * \file asinh_with_agent_tiling_arch22.cpp
 * \brief AsinhWithAgent Tiling 实现（arch22 架构）
 *
 * Tiling 策略：
...
static ge::graphStatus AsinhWithAgentTilingFunc(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(dataType != ge::DT_FLOAT16 && dataType != ge::DT_FLOAT,
                OP_LOGE(context, "AsinhWithAgent: unsupported dtype %d", static_cast<int>(dataType)),
    ...
    OP_CHECK_IF(tmpBufSize <= 0, OP_LOGE(context, "AsinhWithAgent: tmpBufSize is invalid: %ld", tmpBufSize),
                return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(memset_s(tiling, sizeof(AsinhWithAgentTilingData), 0, sizeof(AsinhWithAgentTilingData)) != EOK,
