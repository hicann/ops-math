 ...
 * \brief Host tiling for ArgMaxWithValue (A2). Flattens the reduce axis to firstDim x axisSize x lastDim,
...
#include "../op_kernel/arg_max_with_value_tiling_data.h"
#include "../op_kernel/arg_max_with_value_tiling_key.h"
...
{
    auto td = context->GetTilingData<ArgMaxWithValueTilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, td);
    OP_CHECK_IF(memset_s(td, sizeof(ArgMaxWithValueTilingData), 0, sizeof(ArgMaxWithValueTilingData)) != EOK,
...
static ge::graphStatus TilingParseForArgMaxWithValue([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}
...
IMPL_OP_OPTILING(ArgMaxWithValue)
    ...
    .TilingParse<ArgMaxWithValueCompileInfo>(TilingParseForArgMaxWithValue);
} // namespace optiling
