 ...
 * \brief ArgMaxWithValue tiling unit tests.
 */
...
// ArgMaxWithValueCompileInfo is empty (host reads platform info directly per-call), so any non-null
// pointer satisfies the framework's CompileInfo plumbing.
struct ArgMaxWithValueCompileInfo {};

TEST_F(ArgMaxWithValueTiling, arg_max_with_value_test_tiling_last_mode)
{
    ArgMaxWithValueCompileInfo compileInfo = {};
