...
// AICORE算子kernel
std::tuple<aclTensor*, aclTensor*> Max2AiCore(const aclTensor* self, const int64_t dim, const bool keepdim,
                                              aclTensor* indices, aclTensor* out, aclOpExecutor* executor)
{
    L0_DFX(Max2AiCore, self, dim, keepdim, indices, out);
    auto ret = ADD_TO_LAUNCHER_LIST_AICORE(ArgMaxWithValue, OP_INPUT(self), OP_OUTPUT(indices, out),
                                           OP_ATTR(dim, keepdim));
    if (ret != ACL_SUCCESS) {
...
std::tuple<aclTensor*, aclTensor*> ArgMaxWithValue(const aclTensor* self, const int64_t dim, const bool keepdim,
