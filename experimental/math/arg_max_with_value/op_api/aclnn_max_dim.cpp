...
static const std::initializer_list<op::DataType> DTYPE_SUPPORT_A2_A3_LIST = {
    op::DataType::DT_FLOAT, op::DataType::DT_FLOAT16, op::DataType::DT_BF16, op::DataType::DT_INT16};

static inline bool IsA2OrA3(void)
{
...
{
    if (!IsA2OrA3()) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "aclnnMaxDim is only supported on Atlas A2/A3 products.");
        return false;
    }

    OP_CHECK_DTYPE_NOT_SUPPORT(self, DTYPE_SUPPORT_A2_A3_LIST, return false);
...
aclnnStatus aclnnMaxDimGetWorkspaceSize(const aclTensor* self, int64_t dim, bool keepdim, aclTensor* out,
                                        aclTensor* indices, uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    result = l0op::ArgMaxWithValue(self_contiguous, dim, keepdim, op::DataType::DT_INT32, uniqueExecutor.get());
