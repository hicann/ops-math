                    ...
                    const std::vector<T>& expectedValues, const std::vector<int32_t>& expectedIndices)
{
    ...
    ret = aclnnMaxDimGetWorkspaceSize(x, dim, keepDims, values, indices, &workspaceSize, &executor);
    if (ret != ACL_SUCCESS)
        return fail("aclnnMaxDimGetWorkspaceSize", ret);
    if (workspaceSize > 0) {
        ret = aclrtMalloc(&workspace, workspaceSize, ACL_MEM_MALLOC_HUGE_FIRST);
        if (ret != ACL_SUCCESS)
            return fail("aclrtMalloc(workspace)", ret);
    }
    ret = aclnnMaxDim(workspace, workspaceSize, executor, stream);
    if (ret != ACL_SUCCESS)
        return fail("aclnnMaxDim", ret);
...
int main()
{
    ...
    ok &= RunCase<float>(stream, "fp32 ties", ACL_FLOAT, {3, 4}, {3, 4}, {4, 1}, {5, 2, 8, 2, 1, 9, 3, 7, 4, 4, 0, 6},
                         1, false, {3}, {8, 9, 6}, {2, 1, 3});
    ok &= RunCase<uint16_t>(stream, "fp16 negative dim keepdim", ACL_FLOAT16, {2, 3}, {2, 3}, {3, 1},
                            {0x4200, 0x3c00, 0x3c00, 0x4000, 0x4400, 0x0000}, -1, true, {2, 1}, {0x4200, 0x4400},
                            {0, 1});
    ok &= RunCase<uint16_t>(stream, "bf16 first dimension", ACL_BF16, {2, 3}, {2, 3}, {3, 1},
                            {0x40a0, 0x4000, 0x40e0, 0x3f80, 0x4080, 0x4040}, 0, false, {3}, {0x40a0, 0x4080, 0x40e0},
                            {0, 1, 0});
    ...
    ok &= RunCase<float>(stream, "non-contiguous fp32 input", ACL_FLOAT, {2, 3}, {2, 4}, {4, 1},
                         {6, 1, 4, 99, 2, 8, 2, 99}, 1, false, {2}, {6, 8}, {0, 1});

    aclrtDestroyStream(stream);
    aclrtResetDevice(0);
    aclFinalize();
    std::printf("ArgMaxWithValue examples: %s\n", ok ? "ALL PASS" : "FAILED");
    return ok ? 0 : 1;
}
