...
// AICORE算子kernel
std::tuple<aclTensor*, aclTensor*> Min2AiCore(const aclTensor* self, const int64_t dim, const bool keepdim,
                                              aclTensor* indices, aclTensor* out, aclOpExecutor* executor)
{
    L0_DFX(Min2AiCore, self, dim, keepdim, indices, out);
    auto ret = ADD_TO_LAUNCHER_LIST_AICORE(ArgMinWithValue, OP_INPUT(self), OP_OUTPUT(indices, out),
                                           OP_ATTR(dim, keepdim));
    if (ret != ACL_SUCCESS) {
...
std::tuple<aclTensor*, aclTensor*> ArgMinWithValue(const aclTensor* self, const int64_t dim, const bool keepdim,
                                                   const op::DataType indicesDtype, aclOpExecutor* executor)
{
    ...
    int64_t normalizedDim = 0;
    if (keepdim) {
        op::Shape inputShape = self->GetViewShape();
        if (dim < 0) {
            normalizedDim = self->GetViewShape().GetDimNum() + dim;
        } else {
            normalizedDim = dim;
        }
        inputShape.SetDim(normalizedDim, 1);
