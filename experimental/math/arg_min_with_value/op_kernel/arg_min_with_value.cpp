 ...
 */
/*!
 * \file arg_min_with_value.cpp
 * \brief ArgMinWithValue (A2) kernel entry. The host SetTilingKey picks schMode; this dispatches at
 ...
 */
#include "arg_min_with_value.h"

template <uint32_t schMode, bool gather>
__global__ __aicore__ void arg_min_with_value(GM_ADDR x, GM_ADDR indice, GM_ADDR values, GM_ADDR workspace,
                                              GM_ADDR tiling)
{
    REGISTER_TILING_DEFAULT(ArgMinWithValueTilingData);
    GET_TILING_DATA_PTR_WITH_STRUCT(ArgMinWithValueTilingData, tilingData, tiling);
    ArgWithValueNs::RunArgWithValue<DTYPE_X, true, schMode, gather>(x, indice, values, workspace, tilingData);
}
