...
/*!
 * \file arg_min_with_value_infershape.cpp
 * \brief self-contained infershape for ArgMinWithValue (dual output: indice + values)
 */
...
static ge::graphStatus InferShapeArgMinWithValue(gert::InferShapeContext* context)
{
    OP_LOGD(context->GetNodeName(), "InferShapeArgMinWithValue begin");
    ...
    *indiceShape = *valueShape;
    OP_LOGD(context->GetNodeName(), "InferShapeArgMinWithValue end");
    return GRAPH_SUCCESS;
}

IMPL_OP_INFERSHAPE(ArgMinWithValue).InferShape(InferShapeArgMinWithValue);
} // namespace ops
