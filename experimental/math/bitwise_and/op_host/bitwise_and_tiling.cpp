...
#define UB_NUM 6U
#define INT32_DOUBLE_DATA 2U
constexpr uint32_t BUFFER_NUM = 2;
constexpr uint32_t WS_SYS_SIZE = 0;

struct BitwiseAndCompileInfo {};

static ge::graphStatus TilingParseForBitwiseAnd([[maybe_unused]] gert::TilingParseContext* context)
{
...
static ge::graphStatus GetShapeAttrsInfo(
    gert::TilingContext* context, uint64_t BLOCK_SIZE, uint64_t ubSize, uint64_t& inputNum, uint64_t& inputBytes, uint64_t& tileBlockNum,
...
// tiling 分发入口
static ge::graphStatus BitwiseAndTilingFunc(gert::TilingContext* context)
{
    // 1、获取平台运行信息
    ...
    OP_CHECK_IF(
        ...
        return ge::GRAPH_FAILED);
    // 4、设置tiling信息
    BitwiseAndTilingData* tiling = context->GetTilingData<BitwiseAndTilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(
        memset_s(tiling, sizeof(BitwiseAndTilingData), 0, sizeof(BitwiseAndTilingData)) != EOK,
    ...
    //int32类型 tmpLocal相关数据
    uint64_t tmpTileDataNum = INT32_DOUBLE_DATA * tileDataNum;
    uint64_t tmpSmallTailDataNum = INT32_DOUBLE_DATA * smallTailDataNum;
    uint64_t tmpBigTailDataNum = INT32_DOUBLE_DATA * bigTailDataNum;
    tiling->tmpTileDataNum = static_cast<uint64_t>(tmpTileDataNum);
    tiling->tmpSmallTailDataNum = static_cast<uint64_t>(tmpSmallTailDataNum);
    tiling->tmpBigTailDataNum = static_cast<uint64_t>(tmpBigTailDataNum);
...
// tiling注册入口.
IMPL_OP_OPTILING(BitwiseAnd).Tiling(BitwiseAndTilingFunc).TilingParse<BitwiseAndCompileInfo>(TilingParseForBitwiseAnd);
} // namespace optiling
