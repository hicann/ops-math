...
def _compare_float(
    golden: torch.Tensor, actual: torch.Tensor, dtype: torch.dtype
) -> bool:
    ...
    if golden_f.numel() == 0:
        ...
        print("    empty tensor, skip numeric comparison (matching shapes pass)")
        return True

    # 特殊位置（NaN/Inf）掩码
    special = (
        torch.isnan(golden_f)
        | torch.isinf(golden_f)
        | torch.isnan(actual_f)
    ...
    if special.any():
        ...
        inf_match = (
            torch.isinf(gs) & torch.isinf(as_) & (torch.sign(gs) == torch.sign(as_))
        )
        ...
        if not special_pass:
            ...
            first = bad[0].item()
            print(
                f"    special-position (NaN/Inf) mismatch count: {bad.numel()} "
                f"(first: golden={gs[first].item()}, actual={as_[first].item()})"
            )
    ...
    if normal.any():
        ...
        if not num_pass:
            print(
                f"    MERE: {mere:.6e} (threshold: {threshold:.6e}) {'PASS' if mere_pass else 'FAIL'}"
            )
            print(
                f"    MARE: {mare:.6e} (threshold: {mare_threshold:.6e}) {'PASS' if mare_pass else 'FAIL'}"
            )
            diff = (gn - an).abs()
            midx = diff.argmax().item()
            print(
                f"    maximum difference: golden={gn[midx].item():.6f}, actual={an[midx].item():.6f}"
            )
    else:
        num_pass = True
...
def _compare_integer(golden: torch.Tensor, actual: torch.Tensor) -> bool:
    ...
    if not passed:
        ...
        print(f"    Mismatch element count: {mismatch} (first position: {first_idx})")
...
def compare_results(golden: torch.Tensor, actual: torch.Tensor) -> bool:
    ...
    if tuple(golden.shape) != tuple(actual.shape):
        print(
            f"    [FAIL] shape mismatch: golden={tuple(golden.shape)} vs actual={tuple(actual.shape)}"
        )
        return False
