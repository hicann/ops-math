...
/**
 * @file test_geir_fused_mul_add_n.cpp
 * @brief FusedMulAddN 图模式（GE IR）构图调用示例
 *
 * 算子功能：逐元素融合标量乘加  y_i = x1_i * x3[0] + x2_i
 *   - x1、x2、y 同 shape、同 dtype（逐元素，非矩阵乘）
 *   - x3 为单元素标量张量（ShapeSize = 1），仅取 x3[0] 作为标量乘数广播到全部元素
 *
 * 本示例构造一张仅含 op::FusedMulAddN 节点的计算图并执行：
 *   x1 = x2 = x3 = 2  =>  y = 2 * 2 + 2 = 6
 */
...
#include "../op_graph/fused_mul_add_n_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                             \
    ...
    fusedMulAddn.set_input_##intputName(placeholder##intputIndex);                                              \
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    ...
    fusedMulAddn.update_output_desc_##outputName(outputName##outputIndex##_desc);
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto fusedMulAddn = op::FusedMulAddN("fusedMulAddn");
    // x1、x2 同 shape；x3 为单元素标量张量（ShapeSize = 1）；y 与 x1 同 shape
    ...
    ADD_INPUT(3, x3, inDtype, x3Shape);
    ADD_OUTPUT(1, y, inDtype, outShape);
    outputs.push_back(fusedMulAddn);
    // 添加完毕
    return SUCCESS;
}
...
int main(int argc, char* argv[])
{
    ...
    // 默认数据类型 FLOAT；支持 FLOAT、FLOAT16、BFLOAT16、INT32、INT16
