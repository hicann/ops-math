...
static bool CheckDtypeValid(const aclTensor* self, const aclTensor* target, const aclTensor* out)
{
    ...
    if (npuArch != NpuArch::DAV_2201) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "KLDivV2 only supports NpuArch DAV_2201.");
        return false;
    }
    // 检查输入输出数据类型是否在支持列表内
    OP_CHECK_DTYPE_NOT_SUPPORT(self, ASCEND910B_DTYPE_DTYPE_SUPPORT_LIST, return false);
    OP_CHECK_DTYPE_NOT_SUPPORT(target, ASCEND910B_DTYPE_DTYPE_SUPPORT_LIST, return false);
...
static bool CheckReduction(int64_t reduction)
{
    ...
    if (reduction < 0 || reduction > 3) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                "Invalid reduction value: %ld. Allowed values are 0 (None), 1 (Mean), 2 (Sum), 3 (Batchmean).",
                reduction);
        return false;
    }
    return true;
                                       ...
                                       aclOpExecutor** executor)
{
    ...
    // 进行计算
    auto klRes = l0op::KlDiv(selfBroadcast, targetBroadcast, GetReductionStr(reduction), logTarget,
                             uniqueExecutor.get());
