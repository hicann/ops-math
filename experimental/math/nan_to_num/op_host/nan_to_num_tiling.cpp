...
static ge::graphStatus NanToNumTilingFunc(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(
        memset_s(tiling, sizeof(NanToNumTilingData), 0, sizeof(NanToNumTilingData)) != EOK,
    ...
    if (attrs) {
        ...
        if (attrS1 != nullptr) {
            posinf = *attrS2;
        } else if (attrS1 != nullptr && context->GetInputDesc(0)->GetDataType() == ge::DT_FLOAT) {
            posinf = 3.4028235e+38;
        } else if (attrS1 != nullptr && context->GetInputDesc(0)->GetDataType() == ge::DT_FLOAT16) {
            posinf = 65504.0;
        } else {
            posinf = 3.3895314e+38;
        }
        if (attrS1 != nullptr) {
            neginf = *attrS3;
        } else if (attrS1 != nullptr && context->GetInputDesc(0)->GetDataType() == ge::DT_FLOAT) {
