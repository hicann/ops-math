...
static ge::DataType ResolveOutputDtype(const ge::Operator& op)
{
    ...
    OP_LOGW(GetOpName(op).c_str(), "RandomNormalLike has no dtype attr.");
    return ge::DT_FLOAT;
}
...
static Status ParseOpToGraphRandomNormalLike(const ge::Operator& op, ge::Graph& graph)
{
    ...
    if (supported_dtypes.find(out_dtype) == supported_dtypes.end()) {
        OP_LOGE(GetOpName(op).c_str(),
                "RandomNormalLike output dtype[%d] not supported, only support float/float16/double.",
...
REGISTER_CUSTOM_OP("PartitionedCall")
    .FrameworkType(ONNX)
    .OriginOpType({ge::AscendString("ai.onnx::8::RandomNormalLike"), ge::AscendString("ai.onnx::9::RandomNormalLike"),
