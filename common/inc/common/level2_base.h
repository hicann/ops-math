...
// [[maybe_unused]] static bool CheckNotNull2In1Out(const aclTensor *gradOutput, const aclTensor *input, const aclTensor *grid,
...
// [[maybe_unused]] static bool CheckNotNull2In2Out(const aclTensor *gradOut, const aclIntArray *outputSize, const aclIntArray *inputSize,
...
// [[maybe_unused]] static bool CheckSameShapeNotlimit1In1Out(const aclTensor *self, const aclTensor *out) {
//   // self和out的shape必须一致
//   OP_CHECK_SHAPE_NOT_EQUAL(self, out, return false);
//   return true;
// }

// [[maybe_unused]] static bool CheckShapeNotlimitDim1In1Out(const aclTensor* self) {
//   OP_CHECK_MAX_DIM(self, MAX_SUPPORT_DIMS_NUMS, return false);
//   return true;
// }
...
// // 检查1个输入和1个输出的数据类型是否在算子的支持列表内
// [[maybe_unused]] static bool CheckDtypeValid1In1OutScalar(const aclTensor *self, const aclScalar* attrScalar, const aclTensor *out, const std::initializer_list<op::DataType>& dtypeSupportList, const std::initializer_list<op::DataType>& attrScalarSupportList, const std::initializer_list<op::DataType>& dtypeOutList) {
//   // 检查self的数据类型是否在支持列表内
//   OP_CHECK_DTYPE_NOT_SUPPORT(self, dtypeSupportList, return false);
//   // 检查attr的数据类型是否在支持列表内
...
// // 检查1个输入和1个输出的数据类型是否在算子的支持列表内
// [[maybe_unused]] static bool CheckDtypeValid1In1OutTensor(const aclTensor *self, const aclTensor* attrTensor, const aclTensor *out, const std::initializer_list<op::DataType>& dtypeSupportList, const std::initializer_list<op::DataType>& attrTensorSupportList, const std::initializer_list<op::DataType>& dtypeOutList) {
//   // 检查self的数据类型是否在支持列表内
//   OP_CHECK_DTYPE_NOT_SUPPORT(self, dtypeSupportList, return false);
//   // 检查attr的数据类型是否在支持列表内
...
// [[maybe_unused]] static bool CheckDtypeValid1In1OutMatch(const aclTensor* self, aclTensor* valueOut, aclTensor* inverseOut, aclTensor* countsOut, const std::initializer_list<op::DataType>& dtypeSupportList) {
...
// [[maybe_unused]] static aclnnStatus CheckArrayDataAvgPoolBackWard(const aclIntArray* kernelSize, const aclIntArray* stride,
...
//         auto size = (*stride)[i];
//         if (size <= 0) {
//             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "stride [%lu] data [%li] is less than or equal to 0",
//                     i, size);
//             return ACLNN_ERR_PARAM_INVALID;
//         }
//     }
...
//             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "padding [%lu] data [%li] should less than kernelSize / 2 [%ld]",
//                     i, padSize, halfKsize);
//             return ACLNN_ERR_PARAM_INVALID;
//         }
//         if (padSize < 0) {
//             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "padding [%lu] data [%li] is less than 0",
//                     i, padSize);
//             return ACLNN_ERR_PARAM_INVALID;
//         }
//     }
...
// [[maybe_unused]] static bool CheckPaddingValidAvgPool2D(const aclIntArray *kernelSize, const aclIntArray *padding) {
...
// [[maybe_unused]] static bool CheckDtypeValid1Out1In(const aclTensor *gradOut, const aclTensor *gradInput) {
...
// [[maybe_unused]] static bool CheckUpsampleShape(const aclTensor* gradOut, const aclIntArray *outputSize, const aclIntArray *inputSize) {
...
// // 循环判断size是否相等
// [[maybe_unused]] static bool CheckSizeLoop(size_t dimNum, Shape gradOutShape, FVector<int64_t> &fullOutputSize) {
//   for (size_t i = 0; i < dimNum; ++i) {
//       if (gradOutShape.GetDim(i) != fullOutputSize[i]) {
//         OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Expected grad_output to have the same shape as output;"
...
// [[maybe_unused]] static inline bool CheckInpuNullTensorMaxUnPool3D(const aclTensor* self)
// {
...
// [[maybe_unused]] static inline bool CheckIntArrayShapeMaxUnPool3D(const aclTensor* self, const aclIntArray* outputSize,
...
// [[maybe_unused]] static void CalculateValuesSliceUpper(const aclTensor* self, int64_t& dim, int64_t& start, int64_t& end)
// {
//     if (dim < 0) {
...
// [[maybe_unused]] static void CalculateValuesSliceLower(const aclTensor* self, int64_t& dim, int64_t& start, int64_t& end)
// {
//     if (start < 0) {
//         start = 0;
...
// [[maybe_unused]] static bool CheckDimValueWithUnique(const aclTensor* self, int64_t dim)
// {
