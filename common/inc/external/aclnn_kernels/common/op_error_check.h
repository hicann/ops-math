...
static inline bool IsNullptr(const aclScalar* scalar, const char* name)
{
    if (scalar == nullptr) {
        OP_LOGE(
            ACLNN_ERR_PARAM_NULLPTR, "Expected a value of type number for argument %s but instead found type null.",
            name);
        return true;
    }
    return false;
...
static inline bool IsNullptr(const aclIntArray* intArr, const char* name)
{
    if (intArr == nullptr) {
        OP_LOGE(
            ACLNN_ERR_PARAM_NULLPTR, "Expected a value of type List[int] for argument %s but instead found type null.",
            name);
        return true;
    }
    return false;
...
static inline bool IsNullptr(const aclBoolArray* boolArr, const char* name)
{
    if (boolArr == nullptr) {
        OP_LOGE(
            ACLNN_ERR_PARAM_NULLPTR, "Expected a value of type List[bool] for argument %s but instead found type null.",
            name);
        return true;
    }
    return false;
...
static inline bool IsNullptr(const aclFloatArray* floatArr, const char* name)
{
    if (floatArr == nullptr) {
        OP_LOGE(
            ...
            "Expected a value of type List[float] for argument %s but instead found type \
            null.",
            name);
        return true;
    }
...
#define OP_CHECK_DTYPE_NOT_SUPPORT(tensor, supportList, retExpr)                                                       \
    if (!CheckType(tensor->GetDataType(), supportList)) {                                                              \
        OP_LOGE(                                                                                                       \
            ACLNN_ERR_PARAM_INVALID, "Tensor %s not implemented for %s, should be in dtype support list %s.", #tensor, \
...
#define OP_CHECK_DTYPE_NOT_SAME(tensor1, tensor2, retExpr)                                                             \
    if (tensor1->GetDataType() != tensor2->GetDataType()) {                                                            \
        OP_LOGE(                                                                                                       \
            ACLNN_ERR_PARAM_INVALID, "Expected both tensors to have same dtype, but found %s %s and %s %s.", #tensor1, \
            op::ToString(tensor1->GetDataType()).GetString(), #tensor2,                                                \
