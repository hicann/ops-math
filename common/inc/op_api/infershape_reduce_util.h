    ...
    const gert::Shape* xShape, const T* axesDims, int32_t axesSize, gert::Shape* outputShape)
{
    ...
    *outputShape = *xShape;
    for (int32_t i = 0; i < axesSize; i++) {
        OP_CHECK_IF(
            (!CheckAxisBounds<T, T>(dimNum, axesDims[i])), OP_LOGE("reduce", "axesDims is invalid"),
...
template <typename T>
ge::graphStatus ReduceDims(
    const gert::Shape* xShape, const gert::Tensor* axesTensor, int32_t axesSize, const bool keepDims,
