...
// Build a ge::Tensor from a vector of scalars. Moved here from onnx_common.h so that
// protobuf-decoupled plugins (which must not include onnx_common.h) can reuse it.
// The helper itself only depends on ge::Shape/TensorDesc/Tensor (no protobuf).
template <typename T>
inline ge::Tensor Vec2Tensor(std::vector<T>& vals, const std::vector<int64_t>& dims, ge::DataType dtype,
...
// protobuf-decoupled plugins (which must not include onnx_common.h) can reuse it. The
// helper itself only depends on ge::Shape/TensorDesc/Tensor (no protobuf).
template <typename T>
