...
template <typename T>
std::unique_ptr<Ops::Base::TilingBaseClass> TILING_CLASS(gert::TilingContext* context)
{
    return std::unique_ptr<T>(new (std::nothrow) T(context));
}

using TilingClassCase = std::unique_ptr<Ops::Base::TilingBaseClass> (*)(gert::TilingContext*);
...
public:
    ...
    ge::graphStatus DoTilingImpl(gert::TilingContext* context)
    {
        ...
        if (platformInfoPtr == nullptr) {
            auto compileInfoPtr = reinterpret_cast<const Ops::Base::CompileInfoCommon*>(context->GetCompileInfo());
            OP_CHECK_IF(
