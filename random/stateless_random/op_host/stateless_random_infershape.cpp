...
/*!
 * \file stateless_random_infershape.cpp
 * \brief Infer shape for stateless_random operator
 */
...
static graphStatus InferShapeStatelessRandom(gert::InferShapeContext* context)
{
    const std::unordered_map<std::string, size_t>& inputMap = {
        {"shape", INPUT_SHAPE_IDX},
        {"seed", INPUT_SEED_IDX},
        {"offset", INPUT_OFFSET_IDX},
        {"from", INPUT_FROM_IDX},
        {"to", INPUT_TO_IDX}};
...
IMPL_OP_INFERSHAPE(StatelessRandom).InputsDataDependency({0}).InferShape(InferShapeStatelessRandom);
