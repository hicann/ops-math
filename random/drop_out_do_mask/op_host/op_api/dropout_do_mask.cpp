...
// AICORE算子kernel
static const aclTensor* DropoutDoMaskAiCore(const aclTensor* input, const aclTensor* mask, const aclTensor* prob,
  ...
  OP_CHECK(ret == ACL_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "DropoutDoMaskAiCore ADD_TO_LAUNCHER_LIST_AICORE failed."), return nullptr);
  return out;
}

// AICPU算子kernel
static const aclTensor* DropoutDoMaskAiCpu(const aclTensor* input, const aclTensor* mask, const aclTensor* prob,
                                           const aclTensor* out, aclOpExecutor* executor) {
  ...
  OP_CHECK(ret ==  ACL_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "DropoutDoMaskAiCpu ADD_TO_LAUNCHER_LIST_AICPU failed."), return nullptr);
  return out;
}
const aclTensor* DropoutDoMask(const aclTensor* input, const aclTensor* mask, const aclTensor* prob,
