...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnNormalTensorFloatGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnNormalTensorFloat”接口执行计算。
...
<table style="undefined;table-layout: fixed; width: 1547px"><colgroup>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>数据类型需要同mean数据类型一致。shape需要和mean做broadcast之后的shape相等。当mean为空tensor或元素个数为1，且out为空tensor且维度与mean相同时，直接返回成功，不执行计算。</td>
      <td>BF16、FLOAT16、FLOAT、DOUBLE</td>
      <td>ND</td>
      <td>0-8</td>
      <td>支持</td>
