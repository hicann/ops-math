...
- **参数说明：**
  ...
  - mean(aclTensor*, 计算输入)：生成随机数分布均值的张量，Device侧的aclTensor，数据类型支持FLOAT16、FLOAT和DOUBLE，shape不超过8维。[数据格式](common/数据格式.md)支持ND。
  ...
  - out(aclTensor*, 计算输出 ): 输出张量，Device侧的aclTensor，数据类型支持FLOAT16、FLOAT和DOUBLE, shape需要和mean的shape相等，shape不超过8维。[数据格式](common/数据格式.md)支持ND。
