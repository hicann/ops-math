...
/* !
 * \file drop_out_v3_grad_tiling_arch35.cpp
 * \brief
 */
...
constexpr int64_t GRAD_Y_IDX = 0;
constexpr int64_t MASK_IDX = 1;
constexpr int64_t SCALE_IDX = 2;
...
bool DropOutV3GradTiling::IsCapable() { return true; }

ge::graphStatus DropOutV3GradTiling::GetPlatformInfo()
{
    auto compileInfo = context_->GetCompileInfo<DropOutV3GradCompileInfo>();
...
ge::graphStatus DropOutV3GradTiling::CheckInputShape()
{
    auto gradYShapePtr = context_->GetInputShape(GRAD_Y_IDX);
    OP_CHECK_NULL_WITH_CONTEXT(context_, gradYShapePtr);
    auto gradYShape = gradYShapePtr->GetStorageShape();
    allAxis_ = gradYShape.GetShapeSize();

    auto scaleShapePtr = context_->GetInputShape(SCALE_IDX);
    OP_CHECK_NULL_WITH_CONTEXT(context_, scaleShapePtr);
    auto& scaleShape = Ops::Base::EnsureNotScalar(scaleShapePtr->GetStorageShape());
    int64_t scaleAxis = scaleShape.GetShapeSize();
    if (scaleAxis != 1) {
        std::string valueStr = std::to_string(scaleAxis);
        std::string reasonMsg = "size of scale has to be 1";
        OP_LOGE_FOR_INVALID_SHAPESIZE_WITH_REASON(context_->GetNodeName(), "input scale", valueStr.c_str(),
                                                  reasonMsg.c_str());
...
ge::graphStatus DropOutV3GradTiling::GetShapeAttrsInfo()
{
    auto gradYPtr = context_->GetInputDesc(GRAD_Y_IDX);
    OP_CHECK_NULL_WITH_CONTEXT(context_, gradYPtr);
    ...
    auto scalePtr = context_->GetInputDesc(SCALE_IDX);
    OP_CHECK_NULL_WITH_CONTEXT(context_, scalePtr);
    auto scaleDtype = scalePtr->GetDataType();
    if (scaleDtype != ge::DT_FLOAT) {
        std::string valueStr = ToString(scaleDtype);
        std::string reasonMsg = "scale dtype must be float32";
        OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(context_->GetNodeName(), "input scale", valueStr.c_str(),
                                              reasonMsg.c_str());
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus DropOutV3GradTiling::DoLibApiTiling() { return ge::GRAPH_SUCCESS; }

uint64_t DropOutV3GradTiling::GetTilingKey() const
{
    uint64_t tilingKey = 100;
    return tilingKey;
}

ge::graphStatus DropOutV3GradTiling::GetWorkspaceSize() { return ge::GRAPH_SUCCESS; }

ge::graphStatus DropOutV3GradTiling::PostTiling()
{
...
void DropOutV3GradTiling::DumpTilingInfo()
{
...
ge::graphStatus DropOutV3GradTilingForAscendC(gert::TilingContext* context)
{
    DropOutV3GradTiling tiling(context);
    return tiling.DoTiling();
}

REGISTER_OPS_TILING_TEMPLATE(DropOutV3Grad, DropOutV3GradTiling, 1000);
} // namespace optiling
