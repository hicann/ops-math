...
__golden__ = {"kernel": {"stateless_randperm": "stateless_randperm_golden"}}
...
def stateless_randperm_golden(n, seed, offset, layout=0, dtype=9, **kwargs):
    ...
    result = client.compute_on_gpu(
        seed=seed_val, offset=offset_val, n=n_val, dtype=dtype
    )
    if dtype == 27:
        result = result.astype(bfloat16)
    logging.info(
        "remote gpu computation is done, the result type is {}.".format(type(result))
    )

    if result is None:
        logging.warning(
            "remote gpu computation failed, switch to local cpu computation."
        )
