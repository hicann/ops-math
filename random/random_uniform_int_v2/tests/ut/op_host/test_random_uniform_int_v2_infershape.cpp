...
class random_uniform_int_v2 : public testing::Test {
protected:
    static void SetUpTestCase()
    {
        std::cout << "random_uniform_int_v2 SetUp" << std::endl;
    }

    static void TearDownTestCase()
    {
        std::cout << "random_uniform_int_v2 TearDown" << std::endl;
    }
};
...
static void ExeTestCase(
    ...
    const std::vector<int32_t> input1Values, const std::vector<int64_t> input2Values, const std::vector<int64_t> input3Values, const std::vector<int64_t> input4Values, std::vector<gert::StorageShape *>& outStorageShape,
    ge::graphStatus testCaseResult = ge::GRAPH_SUCCESS)
{
    // 从vector中取出对应参数（保持原顺序）
    const auto& shapeStorageShape = inputShapes[0];
    const auto& minStorageShape = inputShapes[1];
    const auto& maxStorageShape = inputShapes[2];
    ...
    auto min_input_tensor = std::unique_ptr<uint8_t[]>(new uint8_t[sizeof(gert::Tensor) + sizeof(int64_t) * input2Values.size()]);
    auto min_tensor = ConstructInputConstTensor<int64_t>(min_input_tensor, input2Values, ge::DT_INT64);
    auto max_input_tensor = std::unique_ptr<uint8_t[]>(new uint8_t[sizeof(gert::Tensor) + sizeof(int64_t) * input3Values.size()]);
    auto max_tensor = ConstructInputConstTensor<int64_t>(max_input_tensor, input3Values, ge::DT_INT64);
    auto offset_input_tensor = std::unique_ptr<uint8_t[]>(new uint8_t[sizeof(gert::Tensor) + sizeof(int64_t)]);
    auto offset_tensor = ConstructInputConstTensor<int64_t>(offset_input_tensor, input4Values, ge::DT_INT64);

    /* make infershape context */
    std::vector<gert::Tensor*> inputTensors = {shape_tensor, min_tensor, max_tensor, offset_tensor};

    std::vector<gert::StorageShape*> outputShapes = outStorageShape;
    auto contextHolder = gert::InferShapeContextFaker()
                             .SetOpType("RandomUniformIntV2")
                             .NodeIoNum(4, 2)
                             ...
                             .NodeOutputTd(0, input2Dtype, ge::FORMAT_ND, ge::FORMAT_ND)
                             .NodeOutputTd(1, input3Dtype, ge::FORMAT_ND, ge::FORMAT_ND)
    ...
    auto inferShapeFunc = spaceRegistry->GetOpImpl("RandomUniformIntV2")->infer_shape;
...
TEST_F(random_uniform_int_v2, random_uniform_int_v2_infershape_case_0)
{
    ...
    std::vector<ge::DataType> dtypes = {ge::DT_INT32, ge::DT_INT64, ge::DT_INT64};

    std::vector<int32_t> input1Values = {32, 512};
    std::vector<int64_t> input2Values = {1};
    std::vector<int64_t> input3Values = {10};
    std::vector<int64_t> input4Values = {0};
    std::vector<std::vector<int64_t>> expectResult = {{32, 512}, {1}};
    std::vector<gert::StorageShape*> outStorageShape = {};

    // 简化后的函数调用
    ExeTestCase(
        expectResult, inputShapes, dtypes, input1Values, input2Values, input3Values, input4Values, outStorageShape, ge::GRAPH_SUCCESS);
}