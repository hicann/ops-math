    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::RandomUniformIntV2("add1");
    ...
    ADD_INT_INPUT(2, min, inDtype, minShape, 0);
    ADD_INT_INPUT(3, max, inDtype, maxShape, 10);
    ADD_INT_INPUT(4, offset, inDtype, offsetShape, 0);

    ADD_INPUT_ATTR(seed, 10);
    ADD_INPUT_ATTR(seed2, 5);
    
    ADD_OUTPUT(1, y, ge::DT_INT64, outShape);
