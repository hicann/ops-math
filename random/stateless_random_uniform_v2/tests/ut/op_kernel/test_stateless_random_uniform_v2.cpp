...
struct StatelessRandomUniformV2TilingLayout {
    uint32_t blockNum;
...
class StatelessRandomUniformV2KernelTest : public testing::Test {
};

TEST_F(StatelessRandomUniformV2KernelTest, smoke_float)
{
    ...
    auto* tiling = static_cast<uint8_t*>(AscendC::GmAlloc(Align32(sizeof(StatelessRandomUniformV2TilingLayout))));

    std::memset(y, 0, kElementCount * sizeof(float));
    std::memset(tiling, 0, sizeof(StatelessRandomUniformV2TilingLayout));
    ...
    *reinterpret_cast<int32_t*>(alg) = 1;
    ...
    auto* tilingData = reinterpret_cast<StatelessRandomUniformV2TilingLayout*>(tiling);
    ...
    ICPU_RUN_KF(stateless_random_uniform_v2, kNumBlocks, shape, key, counter, alg, y, workspace, tiling);
