...
public:
    ...
    __aicore__ inline void Init(
        GM_ADDR p, GM_ADDR mask, GM_ADDR workspace, const RandomUnifiedSimtTilingDataStruct* tilingData, TPipe* pipe);
    ...
    uint64_t magic, uint64_t shift, int64_t elementNum, int64_t seed, int64_t offset, float p)
{
    ...
    for (int64_t linearIndex = idx * VEC; linearIndex < elementNum; linearIndex += blockDim.x * gridDim.x * VEC) {
        ...
        for (uint8_t randIdx = 0; randIdx < randSize; randIdx++) {
            ...
            ThreadMappingAndSkip<VEC, CONTINUOUS_USE>(
                linearIndex + randIdx * NUM_4, counterTmp, magic, shift, totalThreads);
...
template <typename T>
__simt_vf__ __aicore__ LAUNCH_BOUND(CORE_THREAD_NUM) inline void ProcessZero(
    __gm__ volatile T* outputGM, __gm__ volatile uint8_t* maskGM, int64_t elementNum)
{
    ...
    for (int64_t loopIdx = 0; loopIdx < repeatTime; loopIdx++) {
        for (int64_t linearIndex = idx; linearIndex < totalThreads; linearIndex += blockDim.x * gridDim.x) {
            ...
            ThreadMappingAndSkip<STEP, DIS_CONTINUOUS_USE>(
                linearIndex + totalThreads * UNROLL * loopIdx, counterTmp, magic, shift, totalThreads);
            float results[ALG_COUNTER_SIZE];
            PhiloxRandomSimt(key, counterTmp, results);
...
__aicore__ inline void DropOutV3Impl<T, U>::CopyInMask(const int64_t offset, const uint32_t count)
{
    ...
    DataCopyExtParams copyParams{
        static_cast<uint16_t>(1), static_cast<uint32_t>(count * sizeof(uint8_t)), static_cast<uint32_t>(0),
        static_cast<uint32_t>(0), static_cast<uint32_t>(0)};
    DataCopyPadExtParams<uint8_t> padParams{
        true, static_cast<uint8_t>(0), static_cast<uint8_t>(Ops::Base::CeilAlign(count, NUM_8) - count),
...
__aicore__ inline void DropOutV3Impl<T, U>::CopyOutMask(const int64_t offset, const uint32_t count)
{
    ...
    DataCopyExtParams copyParamsMaskOut{
        static_cast<uint16_t>(1), static_cast<uint32_t>(Ops::Base::CeilDiv(count, NUM_8) * sizeof(uint8_t)),
    ...
    GM_ADDR x, GM_ADDR y, GM_ADDR mask, const RandomUnifiedSimtTilingDataStruct* tilingData)
{
    ...
    if (IsProbEqual(prob_, 0.0f)) {
        asc_vf_call<ProcessZero<T>>(
            dim3(CORE_THREAD_NUM), (__gm__ volatile T*)y, (__gm__ volatile uint8_t*)(maskWorkspace_.GetPhyAddr()),
    ...
    switch (vecSize) {
        case VEC_16:
            asc_vf_call<SimtDropOutVec<T, VEC_16>>(
                dim3(CORE_THREAD_NUM), (__gm__ volatile T*)x, (__gm__ volatile T*)y,
                (__gm__ volatile uint8_t*)(maskWorkspace_.GetPhyAddr()), totalThreads, magic, shift,
                tilingData->outputSize, tilingData->seed, tilingData->offset, prob_);
            break;
        case VEC_8:
            asc_vf_call<SimtDropOutVec<T, VEC_8>>(
                dim3(CORE_THREAD_NUM), (__gm__ volatile T*)x, (__gm__ volatile T*)y,
                (__gm__ volatile uint8_t*)(maskWorkspace_.GetPhyAddr()), totalThreads, magic, shift,
                tilingData->outputSize, tilingData->seed, tilingData->offset, prob_);
            break;
        case VEC_4:
            asc_vf_call<SimtDropOutVec<T, VEC_4>>(
                dim3(CORE_THREAD_NUM), (__gm__ volatile T*)x, (__gm__ volatile T*)y,
                (__gm__ volatile uint8_t*)(maskWorkspace_.GetPhyAddr()), totalThreads, magic, shift,
                tilingData->outputSize, tilingData->seed, tilingData->offset, prob_);
            break;
        case VEC_2:
            asc_vf_call<SimtDropOutVec<T, VEC_2>>(
                dim3(CORE_THREAD_NUM), (__gm__ volatile T*)x, (__gm__ volatile T*)y,
                (__gm__ volatile uint8_t*)(maskWorkspace_.GetPhyAddr()), totalThreads, magic, shift,
                tilingData->outputSize, tilingData->seed, tilingData->offset, prob_);
            break;
        default:
            asc_vf_call<SimtDropOut<T>>(
                dim3(CORE_THREAD_NUM), (__gm__ volatile T*)x, (__gm__ volatile T*)y,
                (__gm__ volatile uint8_t*)(maskWorkspace_.GetPhyAddr()), totalThreads, magic, shift,
                tilingData->outputSize, tilingData->seed, tilingData->offset, prob_);
            break;
