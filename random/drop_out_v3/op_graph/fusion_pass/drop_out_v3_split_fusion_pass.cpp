...
bool DropOutV3SplitFusionPass::CheckDtypes(const GNode& node) const
{
    ...
    if (xDtype != yDtype) {
        OP_LOGE(
            kPassName.c_str(), "x dtype should same with y dtype, x: %d, y: %d", static_cast<int>(xDtype),
...
void DropOutV3SplitFusionPass::UpdateTensorDescs(
    const InputInfo& info, const es::EsTensorHolder& rX, const es::EsTensorHolder& rProb,
    const es::EsTensorHolder& rSeed, const es::EsTensorHolder& rOffset, const es::EsTensorHolder& genMask,
    const es::EsTensorHolder& doMask, const es::EsTensorHolder& rShapeConst, const es::EsTensorHolder& rSeed1) const
{
...
Status DropOutV3SplitFusionPass::Run(GraphPtr& graph, [[maybe_unused]] CustomPassContext& passContext)
{
    ...
    for (auto& node : dropOutV3Nodes) {
        ...
        if (replaceStatus != SUCCESS) {
            ...
            node.GetName(nodeName);
            OP_LOGE(
                kPassName.c_str(), "SubgraphRewriter::Replace failed for node %s, status=%d", nodeName.GetString(),
