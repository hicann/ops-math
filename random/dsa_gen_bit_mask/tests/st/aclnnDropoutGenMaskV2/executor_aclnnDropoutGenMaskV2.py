...
def uniform_golden(torch_tensor, params):
    ...
    if params["dtype_input"][0] == torch.bfloat16:
        ...
        uniform_data = tf.raw_ops.StatelessRandomUniformV2(
            shape=matrix_shape, key=seed, counter=offset, alg=1, dtype=tf.dtypes.float16
        )
    else:
        uniform_data = tf.raw_ops.StatelessRandomUniformV2(
...
class MethodAclnnDropoutGenMaskV2Api(BaseApi):
    ...
    def init_by_input_data(self, input_data: InputDataset):
        input_data.kwargs["prob"] = (
            torch.tensor(
                [input_data.kwargs["prob"]], dtype=input_data.kwargs["probDataType"]
            )
            .to(torch.float32)
            .numpy()[0]
        )
