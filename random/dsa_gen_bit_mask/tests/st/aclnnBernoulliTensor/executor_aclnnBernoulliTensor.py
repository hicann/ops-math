...
@register("ascend_aclnn_bernoulli_tensor")
class MethodAclnnBernoulliTensorApi(BaseApi):

    def __call__(self, task_result: TaskResult):
        super(MethodAclnnBernoulliTensorApi, self).__init__(task_result)
        OpsDataset.seed_everything()
        self.change_flag = None
        
    def __call__(self, input_data: InputDataset, with_output: bool = False):
        # 获取yaml中所需参数
        self.tensor_ = input_data.kwargs['self']
        self.dtype_ = self.tensor_.dtype
        self.prob_ = input_data.kwargs['prob']
        self.input = self.prob_
        
        self.input = self.input.to(self.dtype_)
        
        if self.device == "cpu":
            if self.dtype_ in [torch.float16, torch.bfloat16]:
                self.input = self.input.to(torch.float32)
        output = torch.bernoulli(self.input)
        if self.device == 'npu' and self.dtype_ in [torch.float16, torch.bfloat16]:
            output = output.to(torch.float32)
            
        return output

@register("aclnn_bernoulli_tensor")
class BernoulliTensorAclnnApi(AclnnBaseApi):
    def __call__(self):
        super().__call__()
