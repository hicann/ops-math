...
#ifndef _I_SIM_THREAD_EXPONENTIAL_TILING_H_
#define _I_SIM_THREAD_EXPONENTIAL_TILING_H_
...
inline[aicore] void InitTilingData(const __gm__ uint8_t* tiling, RandomUnifiedSimtTilingDataStruct* constData)
{
    ...
    for (size_t i = 0; i < sizeof(RandomUnifiedSimtTilingDataStruct) / 4; i++) {
        *(dst + i) = *(src + i);
    }
}
#else
...
{
    memcpy(constData, tiling, sizeof(RandomUnifiedSimtTilingDataStruct));
}
...
#endif // _I_SIM_THREAD_EXPONENTIAL_TILING_H_
