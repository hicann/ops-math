 ...
 * numsamples为正整数，当replacement为false时，numsamples不大于C。输入为INT64_T数据类型。
 ...
 * @param [in] out: npu device侧的aclTensor，数据类型支持INT64。shape为(N,
 * numsamples)或(numsamples)，self与out的维度一致。 支持非连续的Tensor，数据格式支持ND。
 * @param [out] workspaceSize: 返回用户需要在npu device侧申请的workspace大小。
 * @param [out] executor: 返回op执行器，包含算子计算流程。
 * @return aclnnStatus: 返回状态码。
 */
ACLNN_API aclnnStatus aclnnMultinomialTensorGetWorkspaceSize(const aclTensor* self, int64_t numsamples,
                                                             bool replacement, const aclTensor* seedTensor,
 ...
 * device侧申请的workspace大小，由第一段接口aclnnMultinomialTensorGetWorkspaceSize获取。
