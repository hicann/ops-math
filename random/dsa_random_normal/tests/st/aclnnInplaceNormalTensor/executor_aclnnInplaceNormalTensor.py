...
@register("ascend_aclnn_inplace_normal_tensor")
class MethodAclnnInplaceNormalTensorApi(BaseApi):
    def __init__(self, task_result: TaskResult):
        super(MethodAclnnInplaceNormalTensorApi, self).__init__(task_result)
    ...
    def __call__(self, input_data: InputDataset, with_output: bool = False):
        ...
        self.seed_ = input_data.kwargs['seedTensor'].cpu().numpy()
        self.offset_ = input_data.kwargs['offsetTensor'].cpu().numpy()
        self.offset2_ = input_data.kwargs['offset']

        params = {"mean": self.mean_, "std": self.std_, "seed": self.seed_[0], "offset": self.offset_[0] + self.offset2_, "is_contiguous": True,
...
@register("aclnn_inplace_normal_tensor")
class InplaceNormalTensorAclnnApi(AclnnBaseApi):
    def __call__(self):
        super().__call__()
