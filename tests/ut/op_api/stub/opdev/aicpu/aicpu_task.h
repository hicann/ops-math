...
public:
    ...
    virtual aclnnStatus Init(
        const FVector<const aclTensor*>& inputs, const FVector<aclTensor*>& outputs, const AicpuAttrs& attrs) = 0;
...
public:
    ...
    AicpuTaskSpace(
        const std::string& opType, const ge::UnknowShapeOpType unknownType = ge::DEPEND_IN_SHAPE,
...
#define ADD_TO_LAUNCHER_LIST_AICPU(KERNEL_NAME, attrNames, opArgs...) \
    ({                                                                \
        OP_LOGI("mock ADD_TO_LAUNCHER_LIST_AICPU for AICPU UT");      \
        ACLNN_SUCCESS;                                                \
    })
} // namespace internal
} // namespace op
