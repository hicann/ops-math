...
本指南旨在帮助您快速上手CANN和`ops-math`算子仓的使用，**基于WebIDE或Docker环境**提供极简化软件安装和编译运行指导。注意，WebIDE或Docker环境默认提供**最新商发版本CANN软件包**，目前是CANN 8.5.0。
...
为方便您能快速了解算子开发的全流程，我们将以**AddExample**算子作为实践对象，其源文件位于`ops-math/examples/add_example`，具体操作步骤如下：
...
对于无环境的用户，可直接使用WebIDE开发平台，即“**算子一站式开发平台**”，该平台为您提供在线可直接运行的昇腾环境，环境中已安装必备的软件包，无需手动安装。更多关于开发平台的介绍请参考[website](https://gitcode.com/org/cann/discussions/54)。

1. 进入ops-math开源项目，单击“`云开发`”按钮，使用已认证过的华为云账号登录。若未注册或认证，请根据页面提示进行注册和认证。
...
1. 获取项目源码。
    ...
    若出现”`fatal: destination path 'ops-math' already exists and is not an empty directory.`“说明项目源码已存在，如需刷新项目代码可使用`git pull`命令。
