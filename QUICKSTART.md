...
本指南旨在帮助您快速上手CANN和`ops-math`算子仓的使用，**基于WebIDE或Docker环境**提供极简化软件安装、编译部署及算子运行指导，两种环境默认提供最新商发版本CANN软件包，目前是CANN 8.5.0。
...
这里将以**AddExample**算子作为实践对象，源文件位于`ops-math/examples/add_example`目录，请参考下述步骤快速体验起来吧！
...
1. 进入ops-math开源项目，单击“`云开发 > WebIDE for Ascend`”进入算子一站式开发平台，根据页面提示启动云开发环境。
...
1. 获取项目源码。
    ...
    ```
    若出现”`fatal: destionation path 'ops-math' already exists and is not an empty directory.`“说明项目源码已存在，如需刷新项目代码可使用`git pull`命令。
