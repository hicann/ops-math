...
创建需求类 Issue 后，需申报 SIG 议题，并参加 Ops-basic SIG 评审。会议安排、议题申请方式及 Maintainer 联系方式请查阅 [Ops-basic SIG 说明](https://gitcode.com/cann/community/blob/master/CANN/sigs/ops-basic/README.md)。

**紧急需求处理：**

若需求紧急，可根据 Ops-basic SIG 说明联系 Maintainer，申请临时 SIG 评审。

**需求接纳：**

若需求被接纳，[SIG 成员](https://gitcode.com/cann/community/blob/master/CANN/sigs/ops-basic/sig-info.yaml)将为您分配合适的算子分类路径（如：`experimental/math`），请将贡献算子提交至 `experimental` 对应算子分类目录。
...
- **PR 提交**：向 ops-math 仓目标分支提交 PR，检查 PR 标题是否清晰、PR 描述是否规范（指明更改内容和原因、是否关联对应 Issue）。
...
- **贡献目录**：按 SIG 成员意见提交至指定目录 `experimental/${op_class}`，可参考已有算子文件放置规则。
- **PR 提交**：向 ops-math 仓目标分支提交 PR，检查 PR 标题是否清晰、PR 描述是否规范（指明更改内容和原因、是否关联对应 Issue）、是否签署 CLA。
