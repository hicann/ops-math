...
/* !
 * \file mirror_pad_reflect.h
 * \brief mirror_pad_reflect
 */

#ifndef ASCENDC_MIRROR_PAD_REFLECT_H
#define ASCENDC_MIRROR_PAD_REFLECT_H
...
template <typename T>
class KernelMirrorPadReflect {
public:
    __aicore__ inline KernelMirrorPadReflect()
    {}
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR paddings, GM_ADDR y, MirrorPadACTilingData tilingData)
    {
        ...
        event_t eventId_ = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::MTE2_V));
    ...
    __aicore__ inline void InitCopyParams()
    {
        ...
        firstDataPieceParam_ = InitFirstDataPieceParams(
            inputShapes_[inputDims_ - 1], afterPadStrides_[inputDims_ - TWO_DIM], firstMainDataNum_, constValue_);
        mainDataPieceParam_ = InitMainDataPieceParams(
            inputShapes_[inputDims_ - 1], afterPadStrides_[inputDims_ - TWO_DIM], tileAxisInner_, constValue_);
        lastDataPieceParam_ = InitLastDataPieceParams(
            inputShapes_[inputDims_ - 1], afterPadStrides_[inputDims_ - TWO_DIM], lastMainDataNum_, constValue_);
        if (tileDim_ == inputDims_ - TWO_DIM) {
            ...
            copyParams_ = {
                static_cast<uint16_t>(1), static_cast<uint32_t>(ubOutputLen_ * sizeof(T)), static_cast<uint32_t>(0), 0,
                0};

            copyParamsTail_ = {
                static_cast<uint16_t>(1), static_cast<uint32_t>(tailOutputLen_ * sizeof(T)), static_cast<uint32_t>(0),
                0, 0};
        }
    }

    __aicore__ inline void MoveGMBackPad2UB(
        uint64_t& srcStart, uint32_t dstStart, const LocalTensor<T>& outBuffer, uint32_t loopSize)
    {
    ...
    __aicore__ inline void MoveGMFrontPad2UB(
        uint64_t& srcStart, uint32_t& dstStart, const LocalTensor<T>& outBuffer, uint32_t loopSize)
    {
    ...
    struct GetDimRefPadIdx {
        __aicore__ inline GetDimRefPadIdx()
        {}
        __aicore__ inline void operator()(
            uint32_t dimIdx[], uint16_t& validNum, uint32_t currentCount, uint32_t currentFrontPad,
            uint32_t currentbackPad, uint32_t currentDimShape)
        {
            dimIdx[0] = currentCount + currentFrontPad;
            if (currentCount != 0 && currentCount <= currentFrontPad) {
                dimIdx[validNum] = currentFrontPad - currentCount;
                ++validNum;
            }
            if (currentCount != currentDimShape - 1 && currentDimShape - currentCount - 1 <= currentbackPad) {
                dimIdx[validNum] = currentFrontPad + currentDimShape * TWO_DIM - currentCount - TWO_DIM;
                ++validNum;
            }
        }
    };

    __aicore__ inline void CustomCopyOut(
        const uint32_t counts[], uint16_t highDims, const LocalTensor<T>& outBuffer, DataCopyExtParams& copyParams,
        const uint64_t offset)
    {
        switch (highDims) {
            case 0:
                ...
                break;
            case 1:
                CopyOut2MultiPos1Dim<T, GetDimRefPadIdx>(
                    counts, outBuffer, copyParams, offset, yGm_, frontPads_, backPads_, inputShapes_, afterPadStrides_);
                break;
            case TWO_DIM:
                CopyOut2MultiPos2Dim<T, GetDimRefPadIdx>(
                    counts, outBuffer, copyParams, offset, yGm_, frontPads_, backPads_, inputShapes_, afterPadStrides_);
                break;
            case THREE_DIM:
                CopyOut2MultiPos3Dim<T, GetDimRefPadIdx>(
                    counts, outBuffer, copyParams, offset, yGm_, frontPads_, backPads_, inputShapes_, afterPadStrides_);
                break;
            case FOUR_DIM:
                CopyOut2MultiPos4Dim<T, GetDimRefPadIdx>(
                    counts, outBuffer, copyParams, offset, yGm_, frontPads_, backPads_, inputShapes_, afterPadStrides_);
                break;
            case FIVE_DIM:
                CopyOut2MultiPos5Dim<T, GetDimRefPadIdx>(
                    counts, outBuffer, copyParams, offset, yGm_, frontPads_, backPads_, inputShapes_, afterPadStrides_);
                break;
            case SIX_DIM:
                CopyOut2MultiPos6Dim<T, GetDimRefPadIdx>(
                    counts, outBuffer, copyParams, offset, yGm_, frontPads_, backPads_, inputShapes_, afterPadStrides_);
                break;
            default:
                break;
    ...
    __aicore__ inline void FrontPadCaseRef(const uint32_t counts[], uint64_t& dstStart, uint64_t& frontPadSrcStart)
    {
        uint32_t lastAxisFrontpad = frontPads_[inputDims_ - 1];
        ...
        AscendC::Simt::VF_CALL<SimtRefModeCompute<T>>(
            AscendC::Simt::Dim3{THREAD_DIM}, tileAxisInner_, lastAxisFrontpad, lastAxisBackpad,
            (__ubuf__ T*)srcLocal.GetPhyAddr(), inputShapes_[inputDims_ - 1], afterPadShapes_[inputDims_ - 1]);
    ...
    __aicore__ inline void FrontPadAndDataAndBackPadCaseRef(
        const uint32_t counts[], uint64_t& srcStart, uint64_t& dstStart, uint64_t& frontPadSrcStart,
        uint64_t& backPadSrcStart, uint32_t loopIdx)
    {
        ...
        MoveGMBackPad2UB(
            backPadSrcStart, (firstMainDataNum_ + mainFrontPad_) * afterPadShapes_[inputDims_ - 1], srcLocal,
            mainBackPad_);
        SetFlag<HardEvent::MTE2_V>(eventId_);
        WaitFlag<HardEvent::MTE2_V>(eventId_);
        AscendC::Simt::VF_CALL<SimtRefModeCompute<T>>(
    ...
    __aicore__ inline void FrontPadAndDataCaseRef(
        const uint32_t counts[], uint64_t& srcStart, uint64_t& dstStart, uint64_t& frontPadSrcStart)
    {
        uint32_t lastAxisFrontpad = frontPads_[inputDims_ - 1];
        uint32_t lastAxisBackpad = backPads_[inputDims_ - 1];
    ...
    __aicore__ inline void OriginalDataCaseRef(const uint32_t counts[], uint64_t& srcStart, uint64_t& dstStart)
    {
        uint32_t lastAxisFrontpad = frontPads_[inputDims_ - 1];
        uint32_t lastAxisBackpad = backPads_[inputDims_ - 1];
        uint32_t srcLastAxisLen = inputShapes_[inputDims_ - 1];
    ...
    __aicore__ inline void DataAndBackPadCaseRef(
        const uint32_t counts[], uint64_t& srcStart, uint64_t& dstStart, uint64_t& backPadSrcStart, uint32_t loopIdx)
    {
        uint32_t lastAxisFrontpad = frontPads_[inputDims_ - 1];
        uint32_t lastAxisBackpad = backPads_[inputDims_ - 1];
        uint32_t srcLastAxisLen = inputShapes_[inputDims_ - 1];
    ...
    __aicore__ inline void BackPadCaseRef(const uint32_t counts[], uint64_t& dstStart, uint64_t& backPadSrcStart)
    {
        uint32_t lastAxisFrontpad = frontPads_[inputDims_ - 1];
        uint32_t lastAxisBackpad = backPads_[inputDims_ - 1];
        LocalTensor<T> srcLocal = inQueue_.AllocTensor<T>();
    ...
    __aicore__ inline void BackPadTailCaseRef(const uint32_t counts[], uint64_t& dstStart, uint64_t& backPadSrcStart)
    {
        uint32_t lastAxisFrontpad = frontPads_[inputDims_ - 1];
        uint32_t lastAxisBackpad = backPads_[inputDims_ - 1];
        LocalTensor<T> srcLocal = inQueue_.AllocTensor<T>();
        ...
        WaitFlag<HardEvent::MTE2_V>(eventId_);
        AscendC::Simt::VF_CALL<SimtRefModeCompute<T>>(
            AscendC::Simt::Dim3{THREAD_DIM}, tailsize_, lastAxisFrontpad, lastAxisBackpad,
            (__ubuf__ T*)srcLocal.GetPhyAddr(), inputShapes_[inputDims_ - 1], afterPadShapes_[inputDims_ - 1]);
        inQueue_.EnQue(srcLocal);
        LocalTensor<T> dstLocal = inQueue_.DeQue<T>();
        CustomCopyOut(counts, inputDims_ - TWO_DIM, dstLocal, copyParamsTail_, dstStart);
    ...
    __aicore__ inline void SubProcessCutHOpt()
    {
        ...
        uint64_t frontPadOffset = frontPads_[inputDims_ - TWO_DIM] * inputShapes_[inputDims_ - 1];
        uint64_t backPadOffset = (tileAxisRealLen_ - TWO_DIM) * inputShapes_[inputDims_ - 1];
        ...
        for (uint32_t i = 0; i < maxLoopInCore_; ++i) {
            ...
            if (innerIdx < frontPaddingLoop_) {
                ...
                FrontPadCaseRef(counts, dstStart, frontPadSrcStart);
                ...
                FrontPadAndDataAndBackPadCaseRef(
                    counts, srcStart, dstStart, frontPadSrcStart, backPadSrcStart, innerIdx);
            } else if (innerIdx == frontPaddingLoop_) {
                dstStart += (ubOutputLen_ * frontPaddingLoop_);
                frontPadSrcStart -= (innerOffset * frontPaddingLoop_);
                FrontPadAndDataCaseRef(counts, srcStart, dstStart, frontPadSrcStart);
                ...
                OriginalDataCaseRef(counts, srcStart, dstStart);
            } else if (innerIdx == lastLoopBeforePad_) {
                dstStart += (ubOutputLen_ * innerIdx);
                srcStart += (firstMainData + (innerIdx - frontPaddingLoop_ - 1) * innerOffset);
                DataAndBackPadCaseRef(counts, srcStart, dstStart, backPadSrcStart, innerIdx);
                ...
                BackPadCaseRef(counts, dstStart, backPadSrcStart);
            } else if (innerIdx == tileAxisOuter_ - 1) {
                dstStart += (ubOutputLen_ * innerIdx);
                backPadSrcStart -= (lastMainBackPad + (innerIdx - lastLoopBeforePad_ - 1) * innerOffset);
                BackPadTailCaseRef(counts, dstStart, backPadSrcStart);
            }
        }
    }
