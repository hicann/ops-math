...
template <typename T>
class KernelMirrorPadSymmetric {
public:
        ...
        const uint64_t offset)
    {
        switch (highDims) {
            case 0:
                ...
                break;
            case 1:
                CopyOut2MultiPos1Dim<T, GetDimSymPadIdx>(
                    counts, outBuffer, copyParams, offset, yGm_, frontPads_, backPads_, inputShapes_, afterPadStrides_);
                break;
            case TWO_DIM:
                CopyOut2MultiPos2Dim<T, GetDimSymPadIdx>(
                    counts, outBuffer, copyParams, offset, yGm_, frontPads_, backPads_, inputShapes_, afterPadStrides_);
                break;
            case THREE_DIM:
                CopyOut2MultiPos3Dim<T, GetDimSymPadIdx>(
                    counts, outBuffer, copyParams, offset, yGm_, frontPads_, backPads_, inputShapes_, afterPadStrides_);
                break;
            case FOUR_DIM:
                CopyOut2MultiPos4Dim<T, GetDimSymPadIdx>(
                    counts, outBuffer, copyParams, offset, yGm_, frontPads_, backPads_, inputShapes_, afterPadStrides_);
                break;
            case FIVE_DIM:
                CopyOut2MultiPos5Dim<T, GetDimSymPadIdx>(
                    counts, outBuffer, copyParams, offset, yGm_, frontPads_, backPads_, inputShapes_, afterPadStrides_);
                break;
            case SIX_DIM:
                CopyOut2MultiPos6Dim<T, GetDimSymPadIdx>(
                    counts, outBuffer, copyParams, offset, yGm_, frontPads_, backPads_, inputShapes_, afterPadStrides_);
                break;
            default:
                break;
    ...
    __aicore__ inline void FrontPadAndDataAndBackPadCaseSym(
    ...
    __aicore__ inline void FrontPadAndDataCaseSym(
        const uint32_t counts[], uint64_t& srcStart, uint64_t& dstStart, uint64_t& frontPadSrcStart)
    {
        uint32_t lastAxisFrontpad = frontPads_[inputDims_ - 1];
        uint32_t lastAxisBackpad = backPads_[inputDims_ - 1];
    ...
    __aicore__ inline void DataAndBackPadCaseSym(
    ...
    __aicore__ inline void SubProcessCutHOpt()
    {
        ...
        for (uint32_t i = 0; i < maxLoopInCore_; ++i) {
            ...
            if (innerIdx < frontPaddingLoop_) {
                ...
                FrontPadAndDataAndBackPadCaseSym(
