...
template <typename P> __aicore__ inline void CopyTilingArray(P curArray[8], P tilingArray[8], uint32_t dim)
{
...
template <typename T> __aicore__ inline MultiCopyParams<T, NDDMA_DIM> InitPadCopyParams(uint32_t loopSize, T value)
{
    return { { { UINT_32_ONE, UINT_32_ONE, UINT_32_ONE, UINT_32_ONE, UINT_32_ONE }, // srcstride
...
template <typename T>
__aicore__ inline MultiCopyParams<T, NDDMA_DIM> InitFirstDataPieceParams(uint32_t srcStride, uint32_t dstStride,
    uint32_t loopSize, T value)
{
    return { { { UINT_32_ONE, UINT_32_ONE, UINT_32_ONE, srcStride, UINT_32_ONE },
...
template <typename T>
__aicore__ inline MultiCopyParams<T, NDDMA_DIM> InitMainDataPieceParams(uint32_t srcStride, uint32_t dstStride,
    uint32_t loopSize, T value)
{
    return { { { UINT_32_ONE, UINT_32_ONE, UINT_32_ONE, srcStride, UINT_32_ONE },
        { UINT_32_ONE, UINT_32_ONE, UINT_32_ONE, dstStride, UINT_32_ONE },
...
template <typename T>
__aicore__ inline MultiCopyParams<T, NDDMA_DIM> InitLastDataPieceParams(uint32_t srcStride, uint32_t dstStride,
    uint32_t loopSize, T value)
{
    return { { { UINT_32_ONE, UINT_32_ONE, UINT_32_ONE, srcStride, UINT_32_ONE },
        { UINT_32_ONE, UINT_32_ONE, UINT_32_ONE, dstStride, UINT_32_ONE },
...
template <typename T>
__aicore__ inline void OneRowMoveAlignCopy(uint32_t frontPad, uint32_t rowLength, uint64_t srcOffset,
    const LocalTensor<T> &outBuffer, const AscendC::GlobalTensor<T> inputGm)
{
...
template <typename T>
__aicore__ inline void CopyOut2MultiPos0Dim(const LocalTensor<T> &outBuffer, DataCopyExtParams &copyParams,
    const uint64_t offset, const AscendC::GlobalTensor<T> yGm)
{
    DataCopyPad(yGm[offset], outBuffer, copyParams);
}

template <typename T, typename F>
__aicore__ inline void CopyOut2MultiPos1Dim(const uint32_t counts[], const LocalTensor<T> &outBuffer,
    DataCopyExtParams &copyParams, const uint64_t offset, const AscendC::GlobalTensor<T> yGm,
    const uint32_t frontPads[], const uint32_t backPads[], const uint32_t inputShape[],
    const uint64_t afterPadStrides[])
{
...
template <typename T>
__simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_DIM) inline void SimtRefModeCompute(const uint32_t rowNum, const uint32_t frontPad, const uint32_t backPad,
    __ubuf__ T* outBuffer, uint32_t lastLength, uint32_t lastLengthWithPad)
{
...
template <typename T>
__simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_DIM) inline void SimtSymModeCompute(const uint32_t rowNum, const uint32_t frontPad, const uint32_t backPad,
   __ubuf__ T* outBuffer, uint32_t lastLength, uint32_t lastLengthWithPad)
{
    uint32_t lastTotalPad = frontPad + backPad;
    for (uint32_t ubIdx = Simt::GetThreadIdx(); ubIdx < rowNum * lastTotalPad; ubIdx += Simt::GetThreadNum()) {
...
template <typename T, typename F>
__aicore__ inline void CopyOut2MultiPos2Dim(const uint32_t counts[], const LocalTensor<T> &outBuffer,
    DataCopyExtParams &copyParams, const uint64_t offset, const AscendC::GlobalTensor<T> yGm,
    const uint32_t frontPads[], const uint32_t backPads[], const uint32_t inputShape[],
    const uint64_t afterPadStrides[])
{
...
template <typename T, typename F>
__aicore__ inline void CopyOut2MultiPos3Dim(const uint32_t counts[], const LocalTensor<T> &outBuffer,
    DataCopyExtParams &copyParams, const uint64_t offset, const AscendC::GlobalTensor<T> yGm,
    const uint32_t frontPads[], const uint32_t backPads[], const uint32_t inputShape[],
    const uint64_t afterPadStrides[])
{
...
template <typename T, typename F>
__aicore__ inline void CopyOut2MultiPos4Dim(const uint32_t counts[], const LocalTensor<T> &outBuffer,
    DataCopyExtParams &copyParams, const uint64_t offset, const AscendC::GlobalTensor<T> yGm,
    const uint32_t frontPads[], const uint32_t backPads[], const uint32_t inputShape[],
    const uint64_t afterPadStrides[])
{
...
template <typename T, typename F>
__aicore__ inline void CopyOut2MultiPos5Dim(const uint32_t counts[], const LocalTensor<T> &outBuffer,
    DataCopyExtParams &copyParams, const uint64_t offset, const AscendC::GlobalTensor<T> yGm,
    const uint32_t frontPads[], const uint32_t backPads[], const uint32_t inputShape[],
    const uint64_t afterPadStrides[])
{
            ...
            for (uint16_t c = 0; c < loop3Num; ++c) {
                ...
                for (uint16_t d = 0; d < loop4Num; ++d) {
                    ...
                    for (uint16_t e = 0; e < loop5Num; ++e) {
                        ...
                        DataCopyPad(yGm[Dim1Offset + Dim2Offset + Dim3Offset + Dim4Offset + Dim5Offset + offset],
...
template <typename T, typename F>
__aicore__ inline void CopyOut2MultiPos6Dim(const uint32_t counts[], const LocalTensor<T> &outBuffer,
    DataCopyExtParams &copyParams, const uint64_t offset, const AscendC::GlobalTensor<T> yGm,
    const uint32_t frontPads[], const uint32_t backPads[], const uint32_t inputShape[],
    const uint64_t afterPadStrides[])
{
                ...
                for (uint16_t d = 0; d < loop4Num; ++d) {
                    ...
                    for (uint16_t e = 0; e < loop5Num; ++e) {
                        ...
                        for (uint16_t f = 0; f < loop6Num; ++f) {
                            ...
                            DataCopyPad(yGm[Dim1Offset + Dim2Offset + Dim3Offset + Dim4Offset + Dim5Offset +
                                Dim6Offset + offset],
                                outBuffer, copyParams);
                        }
                    }
