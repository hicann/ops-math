...
#include "nn_other.h"
#include "../op_graph/batch_to_space_proto.h"
...
#define ADD_INPUT_ATTR(attrName, attrValue) node.set_attr_##attrName(attrValue)
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    ...
    auto node = op::BatchToSpace("node");

    // Input: [N * bs * bs, H_in, W_in, C] = [8, 28, 28, 128], bs=2
    // Output: [N, H_out, W_out, C] = [2, 56, 56, 128]
    std::vector<int64_t> xShape = {8, 28, 28, 128};
    std::vector<int64_t> yShape = {2, 56, 56, 128};
