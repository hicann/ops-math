...
#include "nn_other.h"
#include "../op_graph/batch_to_space_nd_proto.h"
...
int32_t GenSequenceData(vector<int64_t> shapes, Tensor& input_tensor, TensorDesc& input_tensor_desc, DataType data_type)
{
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto node = op::BatchToSpaceND("node");
    std::vector<int64_t> xShape = {4, 2, 3};
    std::vector<int64_t> yShape = {2, 3, 3};
    std::vector<int64_t> blockShapeShape = {1};
    std::vector<int32_t> blockShapeValue = {2};
    std::vector<int64_t> cropsShape = {1, 2};
    std::vector<int32_t> cropsValue = {1, 0};

    ADD_SEQUENCE_INPUT(1, x, inDtype, xShape);

    ADD_INT_INPUT(2, block_shape, DT_INT32, blockShapeShape, blockShapeValue);
    ADD_INT_INPUT(3, crops, DT_INT32, cropsShape, cropsValue);
