...
uint32_t InvertPermutation::Compute(CpuKernelContext& ctx)
{
    ...
    KERNEL_CHECK_FALSE((dims == 1), KERNEL_STATUS_PARAM_INVALID, "InvertPermutation input dim need 1D, but get %d.",
                       dims);
    int64_t num = x->NumElements();
    KERNEL_CHECK_FALSE((num >= 0), KERNEL_STATUS_INNER_ERROR, "InvertPermutation get the num of input elements faild.");
    ...
    switch (data_type) {
        ...
        default:
            KERNEL_LOG_ERROR("InvertPermutation kernel data type [%s] not support.", DTypeStr(data_type).c_str());
...
uint32_t InvertPermutation::InvertPermutationCompute(Tensor* x, int64_t num, CpuKernelContext& ctx)
{
    ...
    if (total < kParallelDataNums) {
        for (size_t i = 0; i < total; i++) {
            ...
            KERNEL_CHECK_FALSE((data_ith >= 0 && data_ith < static_cast<T>(total)), KERNEL_STATUS_PARAM_INVALID,
                               "%ld is not between 0 and %zu.", static_cast<int64_t>(data_ith), total);
            KERNEL_CHECK_FALSE((y_addrs[data_ith] == -1), KERNEL_STATUS_PARAM_INVALID,
                               "%ld is duplicated in the input.", static_cast<int64_t>(data_ith));
            y_addrs[data_ith] = i;
        }
    } else {
        ...
        auto shard_invert_permutation = [&x_addrs, &y_addrs, &num](size_t start, size_t end) {
            for (size_t i = start; i < end; i++) {
                const T data_ith = x_addrs[i];
                KERNEL_CHECK_FALSE((data_ith >= 0 && data_ith < static_cast<T>(num)), KERNEL_STATUS_PARAM_INVALID,
                                   "%ld is not between 0 and %ld.", static_cast<int64_t>(data_ith), num);
                KERNEL_CHECK_FALSE((y_addrs[data_ith] == -1), KERNEL_STATUS_PARAM_INVALID,
                                   "%ld is duplicated in the input.", static_cast<int64_t>(data_ith));
                y_addrs[data_ith] = i;
            }
        ...
        KERNEL_HANDLE_ERROR(CpuKernelUtils::ParallelFor(ctx, num, num / max_core_num, shard_invert_permutation),
...
OPS_MATH_REGISTER_CPU_KERNELV2(kInvertPermutation, InvertPermutation);
} // namespace aicpu
