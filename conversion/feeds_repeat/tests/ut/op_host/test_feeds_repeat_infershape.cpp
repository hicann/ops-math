...
protected:
    static void SetUpTestCase() { std::cout << "FeedsRepeat Proto Test SetUp" << std::endl; }

    static void TearDownTestCase() { std::cout << "FeedsRepeat Proto Test TearDown" << std::endl; }
};
...
static void ExeTestCaseForFeedsRepeat(
    ...
    gert::StorageShape& outStorageShape, ge::graphStatus testCaseResult = ge::GRAPH_SUCCESS, int64_t attr = 0)
{
    ...
    std::vector<gert::Tensor*> inputTensors = {(gert::Tensor*)&selfStorageShape, (gert::Tensor*)&feedsStorageShape};
...
TEST_F(FeedsRepeat, FeedsRepeat_infershape_null_attr_repro)
{
    gert::StorageShape feedsShape = {{4, 5, 6, 7}, {4, 5, 6, 7}};
    gert::StorageShape repeatShape = {{4}, {4}};
    gert::StorageShape outShape = {};
    std::vector<gert::Tensor*> inputTensors = {(gert::Tensor*)&feedsShape, (gert::Tensor*)&repeatShape};
    std::vector<gert::StorageShape*> outputShapes = {&outShape};
    auto contextHolder = gert::InferShapeContextFaker()
                             .SetOpType("FeedsRepeat")
                             .NodeIoNum(2, 1)
                             ...
                             .InputTensors(inputTensors)
                             .OutputShapes(outputShapes)
                             // intentionally NOT calling .Attr("output_feeds_size", ...)
                             .Build();

    auto* attrs = contextHolder.GetContext()->GetAttrs();
    ASSERT_NE(attrs, nullptr);
    const int64_t* outputFeedsSize = attrs->GetAttrPointer<int64_t>(0);
    std::cout << "GetAttrPointer(0)=" << static_cast<const void*>(outputFeedsSize)
              << " GetAttrNum=" << attrs->GetAttrNum() << std::endl;
    ASSERT_EQ(outputFeedsSize, nullptr);
