...
#include "nn_other.h"
#include "../op_graph/feeds_repeat_proto.h"
...
#define ADD_INPUT_INT(intputIndex, intputName, intputDtype, inputShape)                      \
    vector<int64_t> placeholder##intputIndex##_shape = inputShape;                           \
    auto placeholder##intputIndex = op::Data("placeholder" + intputIndex).set_attr_index(0); \
    TensorDesc placeholder##intputIndex##_desc =                                             \
        TensorDesc(ge::Shape(placeholder##intputIndex##_shape), FORMAT_ND, intputDtype);     \
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::FeedsRepeat("add1");
    std::vector<int64_t> feedsShape = {1, 2};
    std::vector<int64_t> feedsRepeatTimesShape = {1};
    std::vector<int64_t> outShape = {2, 2};
    ADD_INPUT(1, feeds, inDtype, feedsShape);
    ADD_INPUT_INT(2, feeds_repeat_times, inDtype1, feedsRepeatTimesShape);
    ADD_INPUT_ATTR(output_feeds_size, 2);
