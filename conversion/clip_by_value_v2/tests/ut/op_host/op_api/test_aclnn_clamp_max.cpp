/**
 * Copyindex (c) Huawei Technologies Co., Ltd. 2023. All indexs reserved.
 *
...
TEST_F(l2_clamp_max_test, aclnnClampMax_base_case_1) {
  ...
  auto selfTensorDesc =
      TensorDesc(selfShape, selfDtype, selfFormat).Value(vector<int32_t>{0,1,2,3,4,5,6,7});
  auto maxScalar = ScalarDesc(3);
  auto outTensorDesc =
      TensorDesc(outShape, outDtype, outFormat).Value(vector<int32_t>{0,0,0,0,0,0,0,0});
...
TEST_F(l2_clamp_max_test, aclnnClampMax_base_case_2) {
  ...
  auto selfTensorDesc =
      TensorDesc(selfShape, selfDtype, selfFormat).Value(vector<float>{0,1,2,3,4,5,6,7});
  auto maxScalar = ScalarDesc(3);
  auto outTensorDesc =
      TensorDesc(outShape, outDtype, outFormat).Value(vector<float>{0,0,0,0,0,0,0,0});
...
TEST_F(l2_clamp_max_test, aclnnClampMax_5_4_float_nd_not_contiguous) {
  ...
  auto outTensorDesc =
      TensorDesc(outShape, outDtype, outFormat, outViewDim, outOffset, outStorageDim);
...
TEST_F(l2_clamp_max_test, aclnnClampMax_input_nullptr) {
  ...
  auto ut_self_nullptr =
      OP_API_UT(aclnnClampMax, INPUT((aclTensor*)nullptr, maxScalar), OUTPUT(tensor_desc));
  ...
  auto ut_max_nullptr =
      OP_API_UT(aclnnClampMax, INPUT(tensor_desc, (aclScalar*)nullptr), OUTPUT(tensor_desc));

  aclRet = ut_max_nullptr.TestGetWorkspaceSize(&workspaceSize);
  EXPECT_EQ(aclRet, ACLNN_ERR_PARAM_NULLPTR);

  auto ut_out_nullptr =
      OP_API_UT(aclnnClampMax, INPUT(tensor_desc,maxScalar), OUTPUT((aclTensor*)nullptr));
...
TEST_F(l2_clamp_max_test, ascend910_9589_aclnnClampMax_input_diff) {
  ...
  auto ut_normal =
      OP_API_UT(aclnnClampMax, INPUT(tensor_desc, maxScalar), OUTPUT(tensor_desc));
