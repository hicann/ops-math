...
TEST_F(l2_clamp_test, case_null)
{
    ...
    auto ut_1 =
        OP_API_UT(aclnnClamp, INPUT(nullptr, min_scalar_desc, max_scalar_desc), OUTPUT(out_tensor_desc_1));

    aclnnStatus aclRet_1 = ut_1.TestGetWorkspaceSize(&workspace_size);
    EXPECT_EQ(aclRet_1, ACLNN_ERR_PARAM_NULLPTR);

    auto ut_2 =
        OP_API_UT(aclnnClamp, INPUT(tensor_desc_1, min_scalar_desc, max_scalar_desc), OUTPUT(nullptr));

    aclnnStatus aclRet_2 = ut_2.TestGetWorkspaceSize(&workspace_size);
    EXPECT_EQ(aclRet_2, ACLNN_ERR_PARAM_NULLPTR);

    auto ut_3 =
        OP_API_UT(aclnnClamp, INPUT(tensor_desc_1, nullptr, nullptr), OUTPUT(out_tensor_desc_1));
