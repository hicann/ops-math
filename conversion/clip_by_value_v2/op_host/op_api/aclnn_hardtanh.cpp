...
static bool CheckNotNull(const aclTensor *self, const aclScalar *clipValueMin, const aclScalar *clipValueMax,
  ...
  OP_CHECK_NULL(clipValueMin, return false);
  OP_CHECK_NULL(clipValueMax, return false);
  return true;
}
...
static bool CheckMinMaxValue(const aclScalar *min, const aclScalar *max) {
  ...
  if (IsIntegralType(min->GetDataType())) {
    ...
    if (imin > imax) {
      OP_LOGE(ACLNN_ERR_PARAM_INVALID, "min must less than max or equal to max, but min is %ld and max is %ld.", imin, imax);
      return false;
    }
    ...
    if (fmin > fmax) {
      OP_LOGE(ACLNN_ERR_PARAM_INVALID, "min must less than max or equal to max, but min is %f and max is %f.", fmin, fmax);
      return false;
    }
  }
...
static aclnnStatus GetWorkspaceSizeCommon(const aclTensor *self, const aclScalar *clipValueMin,
...
aclnnStatus aclnnHardtanhGetWorkspaceSize(const aclTensor *self, const aclScalar *clipValueMin,
                                          const aclScalar *clipValueMax, aclTensor *out, uint64_t *workspaceSize,
                                          aclOpExecutor **executor) {
...
aclnnStatus aclnnInplaceHardtanhGetWorkspaceSize(aclTensor *selfRef, const aclScalar *clipValueMin,
