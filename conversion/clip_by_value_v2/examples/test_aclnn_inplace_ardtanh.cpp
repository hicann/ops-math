...
int main() {
  ...
  ret = PrepareInputAndOutput(selfShape, outShape, &selfDeviceAddr, &self, &clipValueMin, &clipValueMax, &outDeviceAddr, &out);
  CHECK_RET(ret == ACL_SUCCESS, return ret);  
  ...
  ret = aclrtMemcpy(resultData.data(), resultData.size() * sizeof(resultData[0]), selfDeviceAddr, size * sizeof(double),
