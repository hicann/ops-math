...
#include "nn_other.h"
#include "../op_graph/broadcast_to_proto.h"
...
int32_t GenOnesData(
    vector<int64_t> shapes, Tensor& input_tensor, TensorDesc& input_tensor_desc, DataType data_type, vector<int32_t> value)
{
    input_tensor_desc.SetRealDimCnt(shapes.size());
    size_t size = 1;
    for (uint32_t i = 0; i < shapes.size(); i++) {
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto broadcast_to_1 = op::BroadcastTo("broadcastTo1");
    std::vector<int64_t> xShape = {1, 4};
    std::vector<int64_t> bshape = {2};
    std::vector<int32_t> brcShapes = {2, 4};
    ADD_INPUT(1, x, inDtype, xShape);
    ADD_CONST_INPUT(2, shape, DT_INT32, bshape, brcShapes);

    outputs.push_back(broadcast_to_1);
    // 添加完毕
    return SUCCESS;
}
