 ...
 */
/*!
 * \file clip_by_norm_no_div_sum_tiling_arch35.cpp
 * \brief ClipByNormNoDivSum Tiling — arch35 实现
 * 4 输入 (x, greater_zeros, select_ones, maximum_ones), 1 输出 (y)
 * TilingKey: RANK_4 (rank <= 4), RANK_8 (4 < rank <= 8)
 */
#include "clip_by_norm_no_div_sum_tiling_arch35.h"
#include <algorithm>
#include <sstream>
...
// ============================================================
// Tiling 函数模块 — namespace clip_by_norm_no_div_sum
// ============================================================

namespace clip_by_norm_no_div_sum {

bool PadAndSqueeze(
    const std::vector<std::vector<int64_t>>& input_shapes, const std::vector<std::vector<int64_t>>& output_shapes,
...
{
    int64_t num_inputs = (int64_t)input_shapes.size();   // 4: x, greater_zeros, select_ones, maximum_ones
    int64_t num_outputs = (int64_t)output_shapes.size(); // 1: y
    int64_t max_rank = 0;
...
bool FindSplitAxis(
    const std::vector<int64_t>& max_bro_shape, int64_t dtype_size, int64_t ub_per_core, int64_t phys_nodes,
    SplitResult& out)
{
...
bool MultiCoreSplit(
    const std::vector<int64_t>& max_bro_shape, const SplitResult& ub_split, int64_t max_cores, MultiCoreResult& out)
{
...
// ============================================================
// ClipByNormNoDivSumTiling — CANN 主线
// ============================================================

namespace optiling {

using namespace clip_by_norm_no_div_sum;
...
ClipByNormNoDivSumTiling::ClipByNormNoDivSumTiling(gert::TilingContext* ctx) : ctx_(ctx) {}

ge::graphStatus ClipByNormNoDivSumTiling::GetShapeInfo()
{
    auto compileInfo = static_cast<const ClipByNormNoDivSumCompileInfo*>(ctx_->GetCompileInfo());
    ...
    OP_LOGI(
        ctx_->GetNodeName(), "GetShapeInfo done rank %ld dtype %ld ub %lu core %lu", rank_, dtype_size_,
...
template <int64_t R>
ge::graphStatus ClipByNormNoDivSumTiling::DoTilingAndSet()
{
    auto* tiling = ctx_->GetTilingData<ClipByNormNoDivSumTilingData<R>>();
...
ge::graphStatus ClipByNormNoDivSumTiling::RunTiling()
{
    ...
    if (mapped == 4) {
        ...
        ctx_->SetTilingKey(GET_TPL_TILING_KEY(CLIP_BY_NORM_NO_DIV_SUM_RANK_4));
    } else {
        ret = DoTilingAndSet<8>();
        ctx_->SetTilingKey(GET_TPL_TILING_KEY(CLIP_BY_NORM_NO_DIV_SUM_RANK_8));
    }
    return ret;
}

static ge::graphStatus TilingFuncClipByNormNoDivSum(gert::TilingContext* context)
{
    ClipByNormNoDivSumTiling tiling(context);
...
ge::graphStatus TilingPrepareForClipByNormNoDivSum(gert::TilingParseContext* context)
{
    fe::PlatFormInfos* platformInfo = context->GetPlatformInfo();
    auto compileInfo = context->GetCompiledInfo<ClipByNormNoDivSumCompileInfo>();
...
IMPL_OP_OPTILING(ClipByNormNoDivSum)
    .Tiling(TilingFuncClipByNormNoDivSum)
    .TilingParse<ClipByNormNoDivSumCompileInfo>(TilingPrepareForClipByNormNoDivSum);
