...
int main()
{
    ...
    // 创建other aclTensor
    ret = CreateWeightAclTensor(weightHostData, weightShape, &weightDeviceAddr, aclDataType::ACL_FLOAT16, &weight,
                                transWeightSize);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建bias aclTensor
    ...
    aclTensor* transWeight = aclCreateTensor(shape.data(), shape.size(), aclDataType::ACL_FLOAT16, s.data(), 0,
                                             aclFormat::ACL_FORMAT_NCHW, shape.data(), shape.size(),
                                             transWeightDeviceAddr);
    ...
    uint64_t groups = 1;
    // 调用TransWeight
    ret = aclnnTransConvolutionWeightGetWorkspaceSize(weight, transposed, groups, transWeight, &workspaceSize,
                                                      &executor);
