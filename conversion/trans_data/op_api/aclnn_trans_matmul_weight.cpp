...
static const std::initializer_list<DataType> ASCEND910B_DTYPE_DTYPE_SUPPORT_LIST = {
    ...
    DataType::DT_FLOAT8_E4M3FN, DataType::DT_HIFLOAT8, DataType::DT_UINT8};
    ...
    if (curArch == NpuArch::DAV_2201) {
        ...
        if (dimSize < MIN_FLOAT16_DIM_NUM_ND || dimSize > MAX_FLOAT16_DIM_NUM_ND) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                    "It is expected that tensorShape has 2-6 dimensions, but got dimensions %lu.", dimSize);
            return false;
        }
    } else {
        ...
        if (dimSize != MIN_FLOAT16_DIM_NUM_ND) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                    "It is expected that tensorShape has 2 dimensions, but got dimensions %lu.", dimSize);
            return false;
        }
    }
    return true;
...
static inline bool CheckShapeDimV2(const aclIntArray* tensorShape)
{
    ...
    if (dimSize < MIN_INT8_DIM_NUM_ND || dimSize > MAX_INT8_DIM_NUM_ND) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "It is expected that tensorShape has 2-6 dimensions, but got dimensions %lu.",
                dimSize);
        return false;
    }
    return true;
...
static aclnnStatus CheckSocValid()
{
    ...
    if (curArch == NpuArch::DAV_2201 || curArch == NpuArch::DAV_2002 || IsRegBase(curArch)) {
    ...
    for (size_t idx = 0; idx < dimSize; idx++) {
        ...
        if (idx < dimSize - 2) {
            ...
            if (dataType == aclDataType::ACL_FLOAT16 || dataType == aclDataType::ACL_BF16) {
                ...
                shapeSize *= Ops::Base::CeilAlign(static_cast<int64_t>((*tensorShape)[idx]),
                                                  MIN_SIZE_PER_BLOCK_FLOAT16);
            } else {
                // Minimum block size in INT8 is 32.
                shapeSize *= (idx == dimSize - 2) ? Ops::Base::CeilAlign(static_cast<int64_t>((*tensorShape)[idx]),
                                                                         MIN_SIZE_PER_BLOCK_FLOAT16) :
                                                    Ops::Base::CeilAlign(static_cast<int64_t>((*tensorShape)[idx]),
                                                                         MIN_SIZE_PER_BLOCK_INT8);
            }
        }
    }
                                                   ...
                                                   aclOpExecutor** executor)
{
    ...
    if (weightContiguous == mmWeightRef) {
        weightContiguous = uniqueExecutor.get()->CreateView(mmWeightRef, mmWeightRef->GetViewShape(),
                                                            mmWeightRef->GetViewOffset());
    }
