...
static aclnnStatus CheckParams(uint64_t tensorDim, bool transposed, int64_t groups)
{
    ...
    if (groups == 0) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "only support groups larger than 0, now groups is %lu.",
...
static aclnnStatus CheckTensor(const aclTensor* weightIn, const aclTensor* weightOut)
{
    if (weightIn->GetDataType() != op::DataType::DT_FLOAT16 && weightIn->GetDataType() != op::DataType::DT_FLOAT) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "only support weightIn datatype is float16/float32, now datatype is %s.",
                op::ToString(weightIn->GetDataType()).GetString());
        return ACLNN_ERR_PARAM_INVALID;
    }

    if (weightOut->GetDataType() != op::DataType::DT_FLOAT16) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "only support weightOut datatype is float16, now datatype is %s.",
                op::ToString(weightOut->GetDataType()).GetString());
        return ACLNN_ERR_PARAM_INVALID;
    }

    if (weightIn->GetViewFormat() != op::Format::FORMAT_NCHW) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "only support weightIn format is NCHW, now format is %s.",
                op::ToString(weightIn->GetViewFormat()).GetString());
        return ACLNN_ERR_PARAM_INVALID;
    }

    if (weightOut->GetViewFormat() != op::Format::FORMAT_NCHW) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "only support weightOut format is NCHW, now format is %s.",
    ...
    if (weightInEmpty || weightOutEmpty) {
        if (weightInEmpty != weightOutEmpty) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                    "weightIn and weightOut should both be empty or non-empty, but got weightIn empty=%d, weightOut "
                    "empty=%d.",
...
aclnnStatus aclnnTransConvolutionWeight(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
