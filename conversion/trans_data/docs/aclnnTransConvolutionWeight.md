...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnTransConvolutionWeightGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnTransConvolutionWeight”接口执行计算。
...
- 确定性计算：aclnnTransConvolutionWeight默认确定性实现。
- 仅支持正向Conv2D场景。
- 不支持转置卷积。
- 不支持cache缓存能力。
