...
ge::graphStatus TransDataTilingAscendC::GetHardwareInfo()
{
    ...
    OP_CHECK_IF((coreNum_ <= 0U || ubSize_ <= 0L),
                OP_LOGE(context_->GetNodeName(), "TransData GetHardwareInfo failed, core num: %u, ub size: %ld",
                        coreNum_, ubSize_),
...
bool TransDataTilingAscendC::GetShapeInfo()
{
    ...
    OP_CHECK_IF(inShape_.GetShapeSize() == 0 || outShape_.GetShapeSize() == 0,
                OP_LOGE(context_->GetNodeName(), "The input or output shape is empty!"), return false);
...
bool TransDataTilingAscendC::GetTransFormatAndDType()
{
    ...
    OP_CHECK_IF((std::find(supportSrcFormat.begin(), supportSrcFormat.end(), srcFormat) == supportSrcFormat.end() ||
                 std::find(supportDstFormat.begin(), supportDstFormat.end(), dstFormat_) == supportDstFormat.end()),
                OP_LOGE(context_->GetNodeName(), "The input or output format is invalid!"), return false);
    return true;
}
...
bool TransDataTilingAscendC::GetTransNz2NdFormatAndDType()
{
    ...
    OP_CHECK_IF((dtypeSize_ <= 0), OP_LOGE(context_->GetNodeName(), "The dtypeSize is invalid!"), return false);
    ...
    std::vector<ge::Format> supportSrcFormat = {ge::FORMAT_FRACTAL_NZ, ge::FORMAT_FRACTAL_NZ_C0_2,
                                                ...
                                                ge::FORMAT_FRACTAL_NZ_C0_32};
    std::map<ge::Format, int64_t> formatMap = {{ge::FORMAT_FRACTAL_NZ, blockSize_ / dtypeSize_},
    ...
    OP_CHECK_IF((it == formatMap.end()), OP_LOGE(context_->GetNodeName(), "The input format is invalid!"),
                return false);
...
bool TransDataTilingAscendC::CalcNzToNdShapeSize()
{
    ...
    OP_CHECK_IF(inShape_.GetDimNum() < nFour, OP_LOGE(context_->GetNodeName(), "The input shape dim is less than 4!"),
                return false);
    OP_CHECK_IF(inShape_.GetDimNum() != outShape_.GetDimNum() + nTwo,
    ...
    OP_CHECK_IF((c1_ * n1_ * n0_ * c0_ == 0), OP_LOGE(context_->GetNodeName(), "All input dim must not be zero"),
                return false);
    ...
    OP_CHECK_IF((c0_ != expectC0_ || n0_ != N0_16),
...
bool TransDataTilingAscendC::CalcC0Size()
{
    ...
    OP_CHECK_IF(dstFormat_ == ge::FORMAT_FRACTAL_NZ_C0_16 && tmpC0 != C0_16,
                OP_LOGE(context_->GetNodeName(), "The c0 should be 16 when dst format is FRACTAL_NZ_C0_16!"),
                return false);
    OP_CHECK_IF(dstFormat_ == ge::FORMAT_FRACTAL_NZ_C0_32 && tmpC0 != C0_32,
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE(context_->GetNodeName(),
                "The c0 should be 8 or 16 when dst format is FRACTAL_NZ and dtype size is %zu!", dtypeSize_),
        return false);
    OP_CHECK_IF(
    ...
    c0_ = tmpC0;
    if (srcDtype_ == ge::DT_FLOAT4_E2M1 || srcDtype_ == ge::DT_FLOAT4_E1M2 || srcDtype_ == ge::DT_INT4) {
        // 当作u8处理，c0缩小一半
        c0_ = c0_ >> 1;
    }
    return true;
...
{
    OP_CHECK_IF(!GetShapeInfo(), OP_LOGE(context_->GetNodeName(), "Failed to get shape info!"),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(!GetTransFormatAndDType(), OP_LOGE(context_->GetNodeName(), "Failed to get format and dtype info!"),
                return ge::GRAPH_FAILED);
...
void TransDataTilingAscendC::WriteNzToNdTilingData()
{
    ...
    tilingNzToNdData_.SaveToBuffer(context_->GetRawTilingData()->GetData(),
...
{
    OP_CHECK_IF(!GetTransNz2NdFormatAndDType(),
                OP_LOGE(context_->GetNodeName(), "Failed to get format and dtype info!"), return ge::GRAPH_FAILED);

    // 设置h n1 n0 c1 c0
    OP_CHECK_IF(!CalcNzToNdShapeSize(), OP_LOGE(context_->GetNodeName(), "The input shape is invalid!"),
                return ge::GRAPH_FAILED);
...
ge::graphStatus Tiling4TransDataAscendC(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF((tdTiling.GetHardwareInfo() != ge::GRAPH_SUCCESS),
...
static ge::graphStatus TilingPrepare4TransData(gert::TilingParseContext* context)
{
    ...
    OP_CHECK_IF((compileInfo->blockSize < 1),
                OP_LOGE(context->GetNodeName(), "The block size is invalid, %ld.", compileInfo->blockSize),
                return ge::GRAPH_FAILED);
