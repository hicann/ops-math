...
@register("execute_tril")
class trilApi(BaseApi):
    def __init__(self, task_result: TaskResult):
        super(trilApi, self).__init__(task_result)

    def __call__(self, input_data: InputDataset, with_output: bool = False):
        # 标杆计算
        self_mat = input_data.kwargs["self"]
        diagnoal = input_data.kwargs["diagnoal"]
        output = torch.tril(self_mat, diagnoal)
        return output


@register("execute_aclnn_tril")
class aclnnTrilApi(AclnnBaseApi):
    def __init__(self, task_result: TaskResult, backend):
        super(aclnnTrilApi, self).__init__(task_result, backend)

    def init_by_input_data(self, input_data: InputDataset):
        input_args, output_packages = super().init_by_input_data(input_data)
        # delete the attached out tensor
        input_args.pop(-1)
