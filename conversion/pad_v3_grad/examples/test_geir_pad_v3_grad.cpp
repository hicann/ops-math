...
/*!
 * \file test_geir_pad_v3_grad.cpp
 * \brief
 */
...
#include "nn_other.h"
#include "../op_graph/pad_v3_grad_proto.h"
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto padV3Grad1 = op::PadV3Grad("padV3Grad1");
    std::vector<int64_t> xShape = {4, 5};
    ...
    ADD_INPUT_ATTR(paddings_contiguous, true);

    ADD_OUTPUT(1, y, inDtype, yShape);
    outputs.push_back(padV3Grad1);
    // 添加完毕
    return SUCCESS;
}
