...
static const std::initializer_list<op::DataType> AICORE_DTYPE_SUPPORT_LIST = {op::DataType::DT_FLOAT16,
...
inline static bool IsPadV3GradReplicateAicoreRegBaseSupport(const aclTensor* gradOutput, const std::string& mode)
{
    ...
    return mode == REPLICATION_PAD_MODE && CheckType(gradOutput->GetDataType(), AICORE_REGBASE_DTYPE_SUPPORT_LIST) &&
           IsRegBase();
}
...
{
    if (gradOutput->GetViewShape().GetDimNum() == AI_CORE_DIM_3D && mode == REPLICATION_PAD_MODE &&
        CheckType(gradOutput->GetDataType(), REPLICATION_3D_AICORE_DTYPE_SUPPORT_LIST) && IsRegBase()) {
...
inline const aclTensor* PadV3GradAiCore(const aclTensor* gradOutput, const aclTensor* paddings, const std::string& mode,
                                        const bool paddingsContiguous, aclTensor* padV3GradOut, aclOpExecutor* executor)
{
    ...
    OP_CHECK(ret == ACLNN_SUCCESS,
             OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "PadV3GradAiCore ADD_TO_LAUNCHER_LIST_AICORE failed."), return nullptr);
    return padV3GradOut;
}

inline const aclTensor* PadV4GradAiCore(const aclTensor* gradOutput, const aclTensor* paddings, const std::string& mode,
                                        const bool paddingsContiguous, aclTensor* padV3GradOut, aclOpExecutor* executor)
{
                                                    ...
                                                    aclTensor* padV3GradOut, aclOpExecutor* executor)
{
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICORE(ReflectionPad3dGrad, OP_INPUT(gradOutput, paddings),
                                                   ...
                                                   aclTensor* padV3GradOut, aclOpExecutor* executor)
{
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICORE(PadV3GradReplication, OP_INPUT(gradOutput, paddings),
                                           OP_OUTPUT(padV3GradOut));
    OP_CHECK(ret == ACLNN_SUCCESS,
                                              ...
                                              aclTensor* padV3GradOut, aclOpExecutor* executor)
{
    ...
    OP_CHECK(ret == ACLNN_SUCCESS,
             OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "CircularPadGradAiCore ADD_TO_LAUNCHER_LIST_AICORE failed."),
             return nullptr);
    return padV3GradOut;
}
                                       ...
                                       const bool paddingsContiguous, aclTensor* padV3GradOut, aclOpExecutor* executor)
{
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICPU(PadV3Grad, OP_ATTR_NAMES({"mode", "paddingsContiguous"}),
                                          OP_INPUT(gradOutput, paddings), OP_OUTPUT(padV3GradOut),
                                          OP_ATTR(mode, paddingsContiguous));
