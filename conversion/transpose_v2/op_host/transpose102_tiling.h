...
public:
        ...
        if (params.dim3Len < params.block * 4U && permShape->GetStorageShape().GetDim(0) == 3) {
            ...
            if (params.subMode == 0U) {
                ...
                params.dim1OnceMax = params.dim2OnceMax / GetAlign(params.dim2Len, TRANS_BLOCK) < (TRANS_BLOCK * 2) ?
                                         TRANS_BLOCK :
                                         (params.dim2OnceMax / GetAlign(params.dim2Len, TRANS_BLOCK)) / TRANS_BLOCK *
                                             TRANS_BLOCK;
                params.dim2OnceMax = (params.dim2OnceMax / params.dim1OnceMax) / TRANS_BLOCK * TRANS_BLOCK;
            } else if (params.subMode == 1U) {
                // 2 is double
                params.dim1OnceMax = params.dim2OnceMax / GetAlign(params.tasksPerCoreLarge, TRANS_BLOCK) <
                                             (TRANS_BLOCK * 2) ?
                                         TRANS_BLOCK :
                                         (params.dim2OnceMax / GetAlign(params.tasksPerCoreLarge, TRANS_BLOCK)) /
            ...
            if (params.subMode == 0U) {
                // 2 is bufferNum
                params.dim2OnceMax = params.ubSize / 2U / params.doubleBuffer / params.dim3LenAlign / params.typeSize;
                params.dim1OnceMax = params.dim2OnceMax / params.dim2Len == 0U ? 1 :
                                                                                 params.dim2OnceMax / params.dim2Len;
                ...
                params.dim2OnceMax = params.dim1OnceMax / params.dim1Len == 0U ? 1 :
                                                                                 params.dim1OnceMax / params.dim1Len;
    ...
    uint64_t GetAlign(uint64_t len, uint64_t size) { return size == 0U ? 0U : (len + size - 1U) / size * size; }
