...
public:
    ...
    virtual rtError_t rtAicpuKernelLaunchExWithArgs(const uint32_t kernelType, const char* const opName,
    ...
    virtual rtError_t rtsLaunchCpuKernel(const rtFuncHandle funcHandle, const uint32_t numBlocks, rtStream_t st,
...
public:
    ...
    void SetAscendPath(const char* p) { ascend_latest_.assign(RealPath(string(p))); }
    void SetSocVersion(const string& full_soc_version) { full_soc_version_.assign(full_soc_version); }
    string GetSocVersion() const { return full_soc_version_; }
    bool Delegated() { return delegated_; }

    void DelegateToFake()
    {
        EXPECT_CALL(*this, rtGetSocVersion).WillRepeatedly([this](char* version, const uint32_t maxLen) {
            const errno_t ret = memcpy_s(version, maxLen, full_soc_version_.c_str(), full_soc_version_.size() + 1);
            if (ret != EOK) {
                return static_cast<rtError_t>(ret);
            }
        ...
        EXPECT_CALL(*this, rtAicpuKernelLaunchExWithArgs)
            .WillRepeatedly([this]([[maybe_unused]] const uint32_t kernelType,
                                   [[maybe_unused]] const char* const opName, [[maybe_unused]] const uint32_t numBlocks,
        ...
        EXPECT_CALL(*this, rtsBinaryLoadFromFile)
            .WillRepeatedly([this](const char* const binPath, const rtLoadBinaryConfig_t* const optionalCfg,
                                   rtBinHandle* binHandle) {
        ...
        EXPECT_CALL(*this, rtsLaunchCpuKernel)
            .WillRepeatedly([this](const rtFuncHandle funcHandle, const uint32_t numBlocks, rtStream_t st,
                                   const rtKernelLaunchCfg_t* cfg,
                                   rtCpuKernelArgs_t* argsInfo) { return RT_ERROR_NONE; });
        delegated_ = true;
    }
    ...
    MOCK_METHOD(rtError_t, rtAicpuKernelLaunchExWithArgs,
                (const uint32_t, const char* const, const uint32_t, const rtAicpuArgsEx_t*, rtSmDesc_t* const,
                 const rtStream_t, const uint32_t));
    MOCK_METHOD(rtError_t, rtsBinaryLoadFromFile,
                (const char* const binPath, const rtLoadBinaryConfig_t* const optionalCfg, rtBinHandle* binHandle));
    MOCK_METHOD(rtError_t, rtsFuncGetByName,
                (const rtBinHandle binHandle, const char* kernelName, rtFuncHandle* funcHandle));
    MOCK_METHOD(rtError_t, rtsLaunchCpuKernel,
