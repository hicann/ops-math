...
完成伪量化Matmul（包括QuantBatchMatmulV5、GroupedMatmul-伪量化、WeightQuantBatchMatmulV2）的参数预处理，支持的数据流如下：

- MM_MX_A8W4数据流：QuantBatchMatmulV4算子的MX量化场景（x为FLOAT8_E4M3FN，weight为FLOAT4_E2M1紧凑4-bit），weight由ND转换为FRACTAL_NZ格式；
- GMM_MX_A8W4数据流：GroupedMatmul算子的MX量化场景（x为FLOAT8_E4M3FN，weight为FLOAT4_E2M1紧凑4-bit），weight由ND转换为FRACTAL_NZ格式；
- MM_A16S4数据流：WeightQuantBatchMatmulV2算子的pertensor/perchannel/pergroup量化场景（x为FLOAT16/BFLOAT16，weight为INT4紧凑4-bit）；pertensor（转置/非转置）与perchannel/pergroup转置场景为ND直拷透传，perchannel/pergroup非转置场景由ND转换为FRACTAL_NZ格式；
- MM_A16F4数据流：WeightQuantBatchMatmulV2算子的pergroup量化场景（x为FLOAT16/BFLOAT16，weight为FLOAT4_E2M1紧凑4-bit）；仅支持非转置，weight由ND转换为FRACTAL_NZ格式；
- MM_A16MXF4数据流：WeightQuantBatchMatmulV2算子的MX量化场景（x为FLOAT16/BFLOAT16，weight为FLOAT4_E2M1紧凑4-bit）；转置场景为ND直拷透传，非转置场景由ND转换为FRACTAL_NZ格式。

转换过程中在需要时对weightScale、weightOffsetOptional、biasOptional进行同步处理。
...
- **参数说明**
  ...
  <table style="table-layout: fixed; width: 1195px"><colgroup>
...
- **返回值**
  ...
  <tbody>
    ...
    <tr>
      ...
      <td class="tg-0pky">输入的数据类型组合不支持，无法匹配当前支持的任何数据流（MM_MX_A8W4/GMM_MX_A8W4/MM_A16S4/MM_A16F4/MM_A16MXF4）。</td>
    </tr>
    <tr>
    ...
    <tr>
      <td class="tg-0lax">weightOffsetOptional或outWeightOffsetOptional非空（MM_MX_A8W4/GMM_MX_A8W4/MM_A16F4/MM_A16MXF4数据流；MM_A16S4数据流支持透传，要求与weightScale同viewShape、同数据类型），或kGroupSize不等于32（MM_MX_A8W4/GMM_MX_A8W4/MM_A16MXF4数据流）；MM_A16S4/MM_A16F4数据流还要求kGroupSize与weightScale的量化粒度匹配。</td>
    </tr>
    <tr>
...
<!-- npu="950" id9 -->
<details>
<summary><strong>MM_A16F4数据流</strong>（MM表示Matmul；A16F4表示x的数据类型为FLOAT16/BFLOAT16，weight的数据类型为FLOAT4_E2M1（紧凑4-bit，每字节打包2个值），仅支持pergroup量化模式）</summary>

- **weight**
  - 数据类型：FLOAT4_E2M1
  - 格式：ND
  - view shape：2-D `{K, N}`（K为reduction维，N为输出维）
  - 仅支持连续（非转置，stride `[N, 1]`）；转置weight一律返回ACLNN_ERR_PARAM_INVALID
  - 紧凑4-bit打包维（连续维）必须为偶数：非转置时N为偶数
  - 不支持空tensor

- **weightScale**
  - 数据类型：FLOAT16/BFLOAT16
  - 格式：ND
  - view shape：pergroup 2-D `{ceildiv(K, kGroupSize), N}`，且分组数大于1
  - 不支持空tensor
...
- **kGroupSize**
  - 必须大于 0，且weightScale第0维等于ceildiv(K, kGroupSize)
...
- **outWeight**
  - 数据类型：与weight相同
  - 格式：FRACTAL_NZ_C0_16（ND→NZ分形转换）
  - view shape：与weight view shape相同 `{K, N}`
  - storage shape：4-D `{ceildiv(N, 16), ceildiv(K, 16), 16, 16}`（N块在前，区别于A8W4 NZ_C0_32的K块在前）
...
<!-- npu="950" id10 -->
<details>
<summary><strong>MM_A16MXF4数据流</strong>（MM表示Matmul；A16MXF4表示x的数据类型为FLOAT16/BFLOAT16，weight的数据类型为FLOAT4_E2M1（紧凑4-bit，每字节打包2个值），MX（MicroScaling）量化模式）</summary>
...
- **weightScale**
  - 数据类型：FLOAT8_E8M0
  - 格式：ND
  - view shape：2-D `{ceildiv(K, 32), N}`；weight转置输入时，weightScale需与weight布局一致（同为转置视图，stride `[1, ceildiv(K, 32)]`）
  - 不支持空tensor
...
- **outWeight**
  - 数据类型：与weight相同
  - 格式：非转置场景为FRACTAL_NZ_C0_16（ND→NZ分形转换）；转置场景为ND直拷（物理透传）
  - view shape：与weight view shape相同 `{K, N}`
  - storage shape：FRACTAL_NZ_C0_16时为4-D `{ceildiv(N, 16), ceildiv(K, 16), 16, 16}`（N块在前，区别于A8W4 NZ_C0_32的K块在前）；ND直拷场景与输入weight保持相同的view shape/strides（属纯透传）
...
<!-- npu="950" id11 -->
MM_A16F4/MM_A16MXF4数据流调用示例如下（x为FLOAT16，weight为FLOAT4_E2M1紧凑排布；包含A16F4 pergroup非转置NZ_C0_16输出、A16MXF4非转置NZ_C0_16输出与A16MXF4转置ND直拷三个场景）：

- outWeight viewShape：与weight viewShape相同
- outWeight storageShape：FRACTAL_NZ_C0_16输出场景为`{CeilDiv(N, 16), CeilDiv(K, 16), 16, 16}`（N块在前，区别于MM_MX_A8W4的`{CeilDiv(K, 32), CeilDiv(N, 16), 16, 32}`）；ND直拷场景与输入weight保持相同的view shape/strides
- outWeight format：非转置场景为`ACL_FORMAT_FRACTAL_NZ_C0_16`，A16MXF4转置场景为`ACL_FORMAT_ND`
