...
#include "nn_other.h"
#include "../op_graph/depth_to_space_proto.h"
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                        \
    TensorDesc outputName##outputIndex##_desc =                                              \
        TensorDesc(ge::Shape(outputShape), FORMAT_NHWC, outputDtype);                        \
...
{
    Status ret = SUCCESS;
    // DepthToSpace operator definition
    auto add1 = op::DepthToSpace("add1");
    // Input shape: [N, H, W, C] = [1, 2, 2, 8] in NHWC format
    // With block_size=2, output shape will be [1, 4, 4, 2]
    // Output: H_out = H * block_size = 2 * 2 = 4
    //         W_out = W * block_size = 2 * 2 = 4
    //         C_out = C / (block_size * block_size) = 8 / 4 = 2
    std::vector<int64_t> xShape = {1, 2, 2, 8};
    std::vector<int64_t> yShape = {1, 4, 4, 2};
    ADD_INPUT(1, x, inDtype, xShape);
    ADD_INPUT_ATTR(block_size, 2);
    ADD_INPUT_ATTR(mode, "DCR");
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        for (int64_t j = 0; j < output_shape; j++) {
            LOG_PRINT("depth_to_space result[%ld] is: %f\n", j, resultData[j]);
        }
    }
