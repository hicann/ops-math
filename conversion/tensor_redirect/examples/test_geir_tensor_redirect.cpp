...
#include "nn_other.h"
#include "../op_graph/tensor_redirect_proto.h"
                    ...
                    int value)
{
    ...
    if (data_type == ge::DT_INT64 || data_type == ge::DT_UINT64) {
        int64_t* data = reinterpret_cast<int64_t*>(pData);
        for (size_t i = 0; i < size; ++i) {
            data[i] = static_cast<int64_t>(value);
        }
    } else if (data_type == ge::DT_INT32 || data_type == ge::DT_UINT32 || data_type == ge::DT_FLOAT) {
        int32_t* data = reinterpret_cast<int32_t*>(pData);
        for (size_t i = 0; i < size; ++i) {
            data[i] = static_cast<int32_t>(value);
        }
    } else if (data_type == ge::DT_INT16 || data_type == ge::DT_UINT16 || data_type == ge::DT_FLOAT16 ||
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::TensorRedirect("tensor_redirect");
    vector<int64_t> shape = {3, 4};

    ADD_INPUT(1, x, inDtype, shape);
    ADD_OUTPUT(1, output_x, inDtype, shape);
