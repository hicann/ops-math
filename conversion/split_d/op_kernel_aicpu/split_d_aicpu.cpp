...
uint32_t SplitDCpuKernel::CheckAndInitParams(const CpuKernelContext& ctx)
{
    ...
    KERNEL_CHECK_NULLPTR(split_dim_ptr, KERNEL_STATUS_PARAM_INVALID, "Get attr split_dim failed.");
...
template <typename T>
uint32_t SplitDCpuKernel::SplitDWithOneOutput(const T* input_data_ptr, std::vector<T*> output_data_vec)
{
...
template <typename T>
uint32_t SplitDCpuKernel::SplitDWithDimZero(T* input_data_ptr, std::vector<T*> output_data_vec)
{
...
template <typename T>
uint32_t SplitDCpuKernel::SplitDCompute(T* input_data_ptr, std::vector<T*> output_data_vec)
{
    ...
    while (out_idx < num_split_) {
        ...
        while (iter_count < pre_dim_size) {
            ...
            KERNEL_CHECK_FALSE(mem_ret, KERNEL_STATUS_PARAM_INVALID,
                               "Memcpy size[%ld] from input value to output[%ld] failed.", bytes_per_chunk, out_idx);
...
uint32_t SplitDCpuKernel::DoCompute()
{
    ...
    KERNEL_CHECK_FALSE((SplitDCompute<T>(input_data_ptr, output_data_vec) == KERNEL_STATUS_OK),
                       KERNEL_STATUS_PARAM_INVALID, "SplitD Compute failed.");
    return KERNEL_STATUS_OK;
}
...
uint32_t SplitDCpuKernel::Compute(CpuKernelContext& ctx)
{
    ...
    if (data_type_ == DT_FLOAT16) {
        ...
        KERNEL_LOG_WARN("Unsupported datatype[%s]", DTypeStr(data_type_).c_str());
