...
private:
    ...
    __aicore__ inline void ProcessPerLoop(int64_t globalLoopIdx, const NdDmaParams<T, DIM4>& dmaParams,
...
template <typename T>
__aicore__ inline void ViewCopyDim4<T>::Init(GM_ADDR dst, GM_ADDR dstSize, GM_ADDR dstStride, GM_ADDR dstStorageOffset,
...
template <typename T>
__aicore__ inline void ViewCopyDim4<T>::CopyIn(const GlobalTensor<T>& src, const NdDmaParams<T, DIM4>& dmaParams)
{
...
template <typename T>
__aicore__ inline void ViewCopyDim4<T>::ProcessPerLoop(int64_t globalLoopIdx, const NdDmaParams<T, DIM4>& dmaParams,
                                                       const DataCopyExtParams& copyParams, const int32_t* outLoopSize)
{
...
__aicore__ inline void ViewCopyDim4<T>::InitCopyParams()
{
    ...
    copyParams_ = {static_cast<uint16_t>(tilingData_->ubDstSize[DIM1_INDEX]),
                   static_cast<uint32_t>(tilingData_->ubDstSize[DIM0_INDEX] * sizeof(T)), static_cast<uint32_t>(0),
                   copyDstStride, 0};
    tailCopyParams_ = {static_cast<uint16_t>(this->tailUbDstSize_[DIM1_INDEX]),
