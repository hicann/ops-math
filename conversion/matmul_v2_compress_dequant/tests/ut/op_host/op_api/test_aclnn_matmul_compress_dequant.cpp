...
class l2_matmul_compress_dequant_test : public testing::Test {
 protected:
  ...
  static void MatMulCompressDequantCommonTest(TensorDesc a_desc, TensorDesc b_desc, TensorDesc index_desc, TensorDesc bias_desc,
                               TensorDesc deqScale_desc, IntArrayDesc compress_info_desc, TensorDesc out_desc,
                               aclnnStatus expect_status) {
    auto ut = OP_API_UT(aclnnMatmulCompressDequant,
                        INPUT(a_desc, b_desc, index_desc, bias_desc, deqScale_desc, nullptr, 1,
                            compress_info_desc),
                        OUTPUT(out_desc));
...
TEST_F(l2_matmul_compress_dequant_test, ascend310P_test_normal_input) {
  ...
  MatMulCompressDequantCommonTest(a_desc, b_desc, index_desc, bias_desc, deqScale_desc,
              compress_info_desc, out_desc, ACLNN_SUCCESS);
}
