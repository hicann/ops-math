...
#include "nn_other.h"
#include "../op_graph/matrix_diag_part_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    auto placeholder##intputIndex = op::Data("placeholder" + intputIndex);                                          \
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::MatrixDiagPart("add1");
    std::vector<int64_t> xShape = {32, 4, 4};
    ADD_INPUT(1, x, inDtype, xShape);

    std::vector<int64_t> yShape = {32, 4};
...
int main(int argc, char* argv[])
{
    ...
    std::map<AscendString, AscendString> global_options = {
        {"ge.exec.deviceId", "0"}, {"ge.graphRunMode", "1"}, {"ge.exec.fusionMode", "NO_FUSION"}};
    ...
    if (argc < 2) {
        printf("Usage: %s <mode>, using default mode: graph\n", argv[0]);
    }
    std::cout << (argc >= 2 ? argv[1] : "graph") << std::endl;
    char* endptr;
