...
private:
    TPipe* pipe_ = nullptr;
    const PadACTilingData* tdPtrMir_ = nullptr;
...
public:
    __aicore__ inline PadMirrorGather(TPipe* pipe) { pipe_ = pipe; }

    __aicore__ inline void Init(GM_ADDR x, GM_ADDR paddings, GM_ADDR y, const PadACTilingData* tilingData)
    {
        blockIdx_ = GetBlockIdx();
        tdPtrMir_ = tilingData;
        dimNum_ = tdPtrMir_->dimNum;
        ubAxis_ = tdPtrMir_->ubAxis;
        ubFactor_ = tdPtrMir_->ubFactor;

        // 一次VL需要处理后面3根轴，当前支持切-3轴、-4轴
        if (dimNum_ >= CONST3 && tdPtrMir_->outStride[dimNum_ - CONST3] <= VL_CNT / CONST2) {
            lastThirdDimInVL_ = true;
            vlSplitIn_ = Std::min(uint64_t(VL_CNT / tdPtrMir_->outStride[dimNum_ - CONST3]),
                                  uint64_t(tdPtrMir_->outShape[dimNum_ - CONST3]));
        } else {
            vlSplitIn_ = Std::min(uint64_t(VL_CNT / tdPtrMir_->outStride[dimNum_ - CONST2]),
                                  uint64_t(tdPtrMir_->outShape[dimNum_ - CONST2]));
        }
        ...
        pipe_->InitBuffer(inQue_, BUF_NUM, tdPtrMir_->outTileSize);
        // 正/反向各一份outTileSize; 反向上/下pad各一个blocksize的临时空间
        pipe_->InitBuffer(outQueFw_, BUF_NUM, tdPtrMir_->outTileSize);
        pipe_->InitBuffer(outQueBw_, BUF_NUM, tdPtrMir_->outTileSize + UB_BLOCK * CONST2);
        pipe_->InitBuffer(idxBuf_, VL_SIZE * CONST2); // 正向索引占前VL; 反向索引占后VL
    }

    __aicore__ inline void Process()
    {
        uint32_t startIdx = blockIdx_ * tdPtrMir_->ubPerCount;
        if (startIdx >= tdPtrMir_->ubTotalCount) {
            return;
        }

        uint32_t endIdx = (blockIdx_ + 1L) * tdPtrMir_->ubPerCount;
        endIdx = endIdx < tdPtrMir_->ubTotalCount ? endIdx : tdPtrMir_->ubTotalCount;
        ...
        uint64_t ubTailFactor = tdPtrMir_->inShape[ubAxis_] % ubFactor_;
        ...
        for (uint32_t idx = startIdx; idx < endIdx; idx++) {
            ...
            ubAxisInCopyNum = (inIndex[ubAxis_] + ubFactor_ > tdPtrMir_->inShape[ubAxis_]) ? ubTailFactor : ubFactor_;
...
private:
    __aicore__ inline void CalcDimIdx(uint32_t curIdx, uint64_t* inIndex)
    {
        for (int32_t i = ubAxis_; i >= 0; i--) {
            uint64_t factor = tdPtrMir_->inShape[i];
            if (i == ubAxis_) {
    ...
    __aicore__ inline void ProcessOneStep(const uint64_t* inIndex, uint16_t ubAxisInCopyNum,
    ...
    {
        uint32_t copyInNum = ubAxisInCopyNum * tdPtrMir_->inStride[ubAxis_];
        uint64_t inAddr = 0;
        for (uint32_t i = 0; i < dimNum_; i++) {
            inAddr += inIndex[i] * tdPtrMir_->inStride[i];
        }
    ...
    __aicore__ inline void GatherAndCopyOut(uint16_t ubAxisInCopyNum, const uint64_t* inIndex,
                                            LocalTensor<RangeType>& idxTensor)
    {
        ...
        if constexpr (UB_AXES == CONST2) {
            if (tdPtrMir_->inShape[dimNum_ - CONST2] != tdPtrMir_->outShape[dimNum_ - CONST2]) {
                GatherProcessUb2DBw(idxTensor, inLocal, outLocalBw, ubAxisInCopyNum);
            }
        } else if constexpr (UB_AXES == CONST3) {
            if (tdPtrMir_->inShape[dimNum_ - CONST3] != tdPtrMir_->outShape[dimNum_ - CONST3]) {
                GatherProcessUb3DBw(idxTensor, inLocal, outLocalBw, ubAxisInCopyNum);
            }
        } else if constexpr (UB_AXES == CONST4) {
            if (tdPtrMir_->inShape[dimNum_ - CONST4] != tdPtrMir_->outShape[dimNum_ - CONST4]) {
    ...
    __aicore__ inline bool IsInLeftPad(int64_t inIdx, int32_t inAxis)
    {
        if constexpr (IS_REFLECT) {
            return inIdx > 0 && inIdx <= (int64_t)tdPtrMir_->leftPad[inAxis];
        } else {
            return inIdx >= 0 && inIdx < (int64_t)tdPtrMir_->leftPad[inAxis];
        }
    }
    ...
    __aicore__ inline bool IsInRightPad(int64_t inIdx, int32_t inAxis)
    {
        ...
        if constexpr (IS_REFLECT) {
            return (inIdx < (int64_t)tdPtrMir_->inShape[inAxis] - 1 &&
                    inIdx > (int64_t)tdPtrMir_->leftPad[inAxis] + CONST2 * ((int64_t)tdPtrMir_->inShape[inAxis] - 1) -
                                (int64_t)tdPtrMir_->outShape[inAxis]);
        } else {
            return (inIdx < (int64_t)tdPtrMir_->inShape[inAxis] &&
                    inIdx > (int64_t)tdPtrMir_->leftPad[inAxis] + CONST2 * (int64_t)tdPtrMir_->inShape[inAxis] - 1 -
                                (int64_t)tdPtrMir_->outShape[inAxis]);
        }
    }
        ...
        for (int32_t i = 0; i < ubAxis_; i++) {
            ...
            if constexpr (IS_REFLECT) {
                ...
                if (IsInLeftPad(iDimInIdx, i)) {
                    totalOutIdx[i].outIdx[totalOutIdx[i].count] = tdPtrMir_->leftPad[i] - iDimInIdx;
                    totalOutIdx[i].count++;
                }
                // 原始
                totalOutIdx[i].outIdx[totalOutIdx[i].count] = tdPtrMir_->leftPad[i] + iDimInIdx;
                ...
                if (IsInRightPad(iDimInIdx, i)) {
                    totalOutIdx[i].outIdx[totalOutIdx[i].count] = tdPtrMir_->leftPad[i] +
                                                                  CONST2 * (tdPtrMir_->inShape[i] - 1) - iDimInIdx;
                    totalOutIdx[i].count++;
                }
            } else {
                // 左pad
                if (IsInLeftPad(iDimInIdx, i)) {
                    totalOutIdx[i].outIdx[totalOutIdx[i].count] = tdPtrMir_->leftPad[i] - 1 - iDimInIdx;
                    totalOutIdx[i].count++;
                }
                // 原始
                totalOutIdx[i].outIdx[totalOutIdx[i].count] = tdPtrMir_->leftPad[i] + iDimInIdx;
    ...
    __aicore__ inline void CopyOutFw(uint16_t ubAxisInCopyNum, const uint64_t* inIndex, OutIndicesSet* totalOutIdx)
    {
        ...
        uint32_t copyOutNum = ubAxisInCopyNum * tdPtrMir_->outStride[ubAxis_];
        DataCopyExtParams copyOutParams = {1u, static_cast<uint32_t>(copyOutNum * sizeof(T)), 0, 0, 0};

        // ub切分轴，正向
        totalOutIdx[ubAxis_].outIdx[0] = tdPtrMir_->leftPad[ubAxis_] + inIndex[ubAxis_];
        totalOutIdx[ubAxis_].count = 1;

        for (int32_t o0 = 0; o0 < totalOutIdx[0].count; o0++) {
            uint64_t o0Offset = totalOutIdx[0].outIdx[o0] * tdPtrMir_->outStride[0];
            for (int32_t o1 = 0; o1 < totalOutIdx[1].count; o1++) {
                uint64_t o1Offset = totalOutIdx[1].outIdx[o1] * tdPtrMir_->outStride[1];
                for (int32_t o2 = 0; o2 < totalOutIdx[CONST2].count; o2++) {
                    uint64_t o2Offset = totalOutIdx[CONST2].outIdx[o2] * tdPtrMir_->outStride[CONST2];
                    for (int32_t o3 = 0; o3 < totalOutIdx[CONST3].count; o3++) {
                        uint64_t o3Offset = totalOutIdx[CONST3].outIdx[o3] * tdPtrMir_->outStride[CONST3];
    ...
    __aicore__ inline void CalcUbAxisPadBwNum(uint16_t ubAxisInCopyNum, const uint64_t* inIndex, uint32_t& inLeftPadNum,
                                              uint32_t& inLeftPadStart, uint32_t& inRightPadNum,
                                              uint32_t& inRightPadStart)
    {
        if constexpr (IS_REFLECT) {
            if ((inIndex[ubAxis_] == 0 && ubAxisInCopyNum > 1 && tdPtrMir_->leftPad[ubAxis_] > 0)) {
                inLeftPadNum = Std::min((uint64_t)ubAxisInCopyNum - 1, (uint64_t)tdPtrMir_->leftPad[ubAxis_]);
                inLeftPadStart = ubAxisInCopyNum - 1 - inLeftPadNum;
            } else if (IsInLeftPad(inIndex[ubAxis_], ubAxis_)) {
                inLeftPadNum = Std::min((uint64_t)ubAxisInCopyNum,
                                        (uint64_t)tdPtrMir_->leftPad[ubAxis_] - inIndex[ubAxis_] + 1);
                inLeftPadStart = ubAxisInCopyNum - inLeftPadNum;
            }
        } else {
            if (IsInLeftPad(inIndex[ubAxis_], ubAxis_)) {
                inLeftPadNum = Std::min((uint64_t)ubAxisInCopyNum,
                                        (uint64_t)tdPtrMir_->leftPad[ubAxis_] - inIndex[ubAxis_]);
                inLeftPadStart = ubAxisInCopyNum - inLeftPadNum;
            }
        }

        uint64_t rightIdx = inIndex[ubAxis_] + ubAxisInCopyNum - 1;
        int64_t rightPad = tdPtrMir_->outShape[ubAxis_] - tdPtrMir_->inShape[ubAxis_] - tdPtrMir_->leftPad[ubAxis_];
        ...
        if constexpr (IS_REFLECT) {
            uint64_t minInIdx = tdPtrMir_->leftPad[ubAxis_] + 2 * (tdPtrMir_->inShape[ubAxis_] - 1) -
                                (tdPtrMir_->outShape[ubAxis_] - 1);
            if (rightIdx == tdPtrMir_->inShape[ubAxis_] - 1 && ubAxisInCopyNum > 1 && rightPad > 0) {
            ...
            if (IsInRightPad(rightIdx, ubAxis_)) {
                uint64_t minInIdx = tdPtrMir_->leftPad[ubAxis_] + 2 * tdPtrMir_->inShape[ubAxis_] -
                                    tdPtrMir_->outShape[ubAxis_];
                inRightPadNum = Std::min((uint64_t)ubAxisInCopyNum, (uint64_t)rightIdx - minInIdx + 1);
                inRightPadStart = 0;
            }
        }
    ...
    __aicore__ inline void CopyOutPadBwInnerLoop(LocalTensor<T>& outLocalBwReal, LocalTensor<T>& outLocalBwTmp,
    ...
    {
        for (int32_t o3 = 0; o3 < totalOutIdx[CONST3].count; o3++) {
            uint64_t outAddr = baseOffset + totalOutIdx[CONST3].outIdx[o3] * tdPtrMir_->outStride[CONST3];
            if (copyOutNum > 0) {
    ...
    __aicore__ inline void CopyOutPadBwLoopImpl(LocalTensor<T>& outLocalBwReal, LocalTensor<T>& outLocalBwTmp,
                                                uint32_t copyStartOffset, uint32_t copyOutNum, uint32_t alignOffset,
                                                DataCopyExtParams& copyOutParams, DataCopyExtParams& outParamAlign,
            ...
            for (int32_t o1 = 0; o1 < totalOutIdx[1].count; o1++) {
                ...
                for (int32_t o2 = 0; o2 < totalOutIdx[CONST2].count; o2++) {
                    uint64_t baseOffset = o0Offset + o1Offset +
                                          totalOutIdx[CONST2].outIdx[o2] * tdPtrMir_->outStride[CONST2];
                    CopyOutPadBwInnerLoop(outLocalBwReal, outLocalBwTmp, copyStartOffset, copyOutNum, alignOffset,
                                          copyOutParams, outParamAlign, totalOutIdx, baseOffset);
                }
            }
        }
    }

    __aicore__ inline void CopyOutPadBwImpl(LocalTensor<T>& outLocalBwReal, LocalTensor<T>& outLocalBwTmp,
                                            uint32_t copyStartOffset, uint32_t copyOutNum, OutIndicesSet* totalOutIdx)
    {
        ...
        if (alignRed != 0) {
            __ubuf__ T* inAddrTmp = (__ubuf__ T*)outLocalBwReal.GetPhyAddr() + copyStartOffset;
            __ubuf__ T* outAddrTmp = (__ubuf__ T*)outLocalBwTmp.GetPhyAddr();
        ...
        CopyOutPadBwLoopImpl(outLocalBwReal, outLocalBwTmp, copyStartOffset, copyOutNum, alignOffset, copyOutParams,
                             outParamAlign, totalOutIdx);
    }

    __aicore__ inline void CopyOutLeftPadBw(uint16_t ubAxisInCopyNum, const uint64_t* inIndex,
                                            LocalTensor<T>& outLocalBw, uint32_t inLeftPadNum, uint32_t inLeftPadStart,
                                            OutIndicesSet* totalOutIdx)
    {
        // reflect:
        //     左pad:   outIdx=leftNum-inIdx
        ...
        if constexpr (IS_REFLECT) {
            totalOutIdx[ubAxis_].outIdx[0] = tdPtrMir_->leftPad[ubAxis_] - inIdx;
        } else {
            totalOutIdx[ubAxis_].outIdx[0] = tdPtrMir_->leftPad[ubAxis_] - 1 - inIdx;
        }
        totalOutIdx[ubAxis_].count = 1;

        LocalTensor<T> outLocalBwReal = outLocalBw[BLOCK_NUM * CONST2];
        LocalTensor<T> outLocalBwTmp = outLocalBw[0];
        uint32_t copyOutNum = inLeftPadNum * tdPtrMir_->outStride[ubAxis_];
        uint32_t copyStartOffset = inLeftPadStart * tdPtrMir_->outStride[ubAxis_];

        CopyOutPadBwImpl(outLocalBwReal, outLocalBwTmp, copyStartOffset, copyOutNum, totalOutIdx);
    }

    __aicore__ inline void CopyOutRightPadBw(uint16_t ubAxisInCopyNum, const uint64_t* inIndex,
                                             LocalTensor<T>& outLocalBw, uint32_t inRightPadNum,
                                             uint32_t inRightPadStart, OutIndicesSet* totalOutIdx)
    {
        ...
        if constexpr (IS_REFLECT) {
            totalOutIdx[ubAxis_].outIdx[0] = tdPtrMir_->leftPad[ubAxis_] + CONST2 * (tdPtrMir_->inShape[ubAxis_] - 1) -
                                             inIdx;
        } else {
            totalOutIdx[ubAxis_].outIdx[0] = tdPtrMir_->leftPad[ubAxis_] + CONST2 * tdPtrMir_->inShape[ubAxis_] - 1 -
                                             inIdx;
        ...
        uint32_t copyOutNum = inRightPadNum * tdPtrMir_->outStride[ubAxis_];
        uint32_t copyStartOffset = inRightPadStart * tdPtrMir_->outStride[ubAxis_];

        CopyOutPadBwImpl(outLocalBwReal, outLocalBwTmp, copyStartOffset, copyOutNum, totalOutIdx);
    }

    __aicore__ inline void CopyTmpUnAlign(__ubuf__ T* inAddrTmp, __ubuf__ T* outAddrTmp, uint32_t alignOffset)
    {
        uint32_t newCnt = (sizeof(T) != sizeof(OutType)) ? CONST2 * alignOffset : alignOffset;
        auto newInAddr = reinterpret_cast<__ubuf__ OutType*>(inAddrTmp);
        auto newOutAddr = reinterpret_cast<__ubuf__ OutType*>(outAddrTmp);

        __VEC_SCOPE__
        {
            Reg::RegTensor<OutType> regData;
            Reg::UnalignRegForLoad uReg;
            Reg::MaskReg maskIdx = Reg::UpdateMask<OutType>(newCnt);

            Reg::LoadUnAlignPre(uReg, newInAddr);
            Reg::LoadUnAlign(regData, uReg, newInAddr, newCnt);
            Reg::StoreAlign(newOutAddr, regData, maskIdx);
        }
    }
    ...
    __aicore__ inline void GenGatherIndexThreeDim(LocalTensor<RangeType>& idxTensor)
    {
        ...
        uint32_t lastInDimSize = tdPtrMir_->inShape[dimNum_ - 1];
        uint16_t lastSecInDimSize = tdPtrMir_->inShape[dimNum_ - CONST2];
        int32_t outStride1 = tdPtrMir_->outStride[dimNum_ - CONST3];
        int32_t outStride2 = tdPtrMir_->outStride[dimNum_ - CONST2];
        int32_t inStride1 = tdPtrMir_->inStride[dimNum_ - CONST3];
        uint16_t lastTwoDimLoops = vlSplitIn_;
        // 切在-3轴上，-2轴上的数据都是从gather中获取到的，包含pad
        int32_t lastLeftPadNum = tdPtrMir_->leftPad[dimNum_ - 1];
        uint16_t last2LeftPadNum = tdPtrMir_->leftPad[dimNum_ - CONST2];
        uint16_t last2RightPadNum = tdPtrMir_->outShape[dimNum_ - CONST2] - lastSecInDimSize - last2LeftPadNum;
        ...
        __ubuf__ RangeType* idxAddr2 = (__ubuf__ RangeType*)idxTensor.GetPhyAddr() + VL_RANGE_CNT;
        ...
        __VEC_SCOPE__
        {
            ...
            Reg::Compares<RangeType, CMPMODE::LT>(leftMask, lineRange, 0, maskIdx);
            Reg::Arange<RangeType, Reg::IndexOrder::DECREASE_ORDER>(leftPadIdxReg,
                                                                    lastLeftPadNum - decreaseOffset - modeOffset);
            Reg::Move<RangeType, Reg::MaskMergeMode::MERGING>(lineRange, leftPadIdxReg, leftMask);

            Reg::Arange<RangeType, Reg::IndexOrder::DECREASE_ORDER>(
                rightPadIdxReg, lastRightPadArangeStart - decreaseOffset + modeOffset);
            Reg::Compares<RangeType, CMPMODE::LT>(rightMask, rightPadIdxReg, lastDimIdx + modeOffset, maskIdx);
            Reg::Move<RangeType, Reg::MaskMergeMode::MERGING>(lineRange, rightPadIdxReg, rightMask);

            // -3轴纯pad的索引, 包含末尾两根轴
            for (uint16_t i = 0; i < lastTwoDimLoops; i++) {
                Reg::Adds(lineRangeBk, lineRange, (RangeType)((lastTwoDimLoops - i - 1) * inStride1), maskIdx);
                // -2轴leftpad行数, 索引都一样
                __ubuf__ RangeType* idxAddrTmp = idxAddr + i * outStride1;
                for (uint16_t j = 0; j < last2LeftPadNum; j++) {
                    Reg::Adds(lineRangeNew, lineRangeBk,
                              (RangeType)((last2LeftPadNum - j - modeOffset) * lastInDimSize), maskIdx);
                    Reg::StoreUnAlign(idxAddrTmp, lineRangeNew, uReg, outStride2);
                }
                Reg::StoreUnAlignPost(idxAddrTmp, uReg, 0);

                // -2 轴inputshape, 索引递增
                __ubuf__ RangeType* idxAddrTmp1 = idxAddr + i * outStride1 + last2LeftPadNum * outStride2;
                for (uint16_t j = 0; j < lastSecInDimSize; j++) {
                    Reg::Adds(lineRangeNew, lineRangeBk, (RangeType)(j * lastInDimSize), maskIdx);
                    Reg::StoreUnAlign(idxAddrTmp1, lineRangeNew, uReg, outStride2);
                }
                Reg::StoreUnAlignPost(idxAddrTmp1, uReg, 0);

                // -2轴rightpad行数, 索引都一样
                Reg::Adds(lineRangeNew, lineRangeBk, (RangeType)(last2DimIdx * lastInDimSize), maskIdx);
                __ubuf__ RangeType* idxAddrTmp2 = idxAddr + i * outStride1 + last2LeftPadNum * outStride2 +
                                                  lastSecInDimSize * outStride2;
                for (uint16_t j = 0; j < last2RightPadNum; j++) {
                    Reg::Adds(lineRangeBk, lineRangeNew, (RangeType)((-1) * (j + 1 - modeOffset) * lastInDimSize),
                              maskIdx);
                    Reg::StoreUnAlign(idxAddrTmp2, lineRangeBk, uReg, outStride2);
                }
                Reg::StoreUnAlignPost(idxAddrTmp2, uReg, 0);
            }
            ...
            for (uint16_t i = 0; i < lastTwoDimLoops; i++) {
                ...
                // -2轴leftpad行数, 索引都一样
                __ubuf__ RangeType* idxAddrTmp = idxAddr2 + i * outStride1;
                for (uint16_t j = 0; j < last2LeftPadNum; j++) {
                    Reg::Adds(lineRangeNew, lineRangeBk,
                              (RangeType)((last2LeftPadNum - j - modeOffset) * lastInDimSize), maskIdx);
                    Reg::StoreUnAlign(idxAddrTmp, lineRangeNew, uReg, outStride2);
                ...
                // -2 轴inputshape, 索引递增
                __ubuf__ RangeType* idxAddrTmp1 = idxAddr2 + i * outStride1 + last2LeftPadNum * outStride2;
                for (uint16_t j = 0; j < lastSecInDimSize; j++) {
                    Reg::Adds(lineRangeNew, lineRangeBk, (RangeType)(j * lastInDimSize), maskIdx);
                    Reg::StoreUnAlign(idxAddrTmp1, lineRangeNew, uReg, outStride2);
                }
                ...
                // -2轴rightpad行数, 索引都一样
                Reg::Adds(lineRangeNew, lineRangeBk, (RangeType)(last2DimIdx * lastInDimSize), maskIdx);
                __ubuf__ RangeType* idxAddrTmp2 = idxAddr2 + i * outStride1 + last2LeftPadNum * outStride2 +
                                                  lastSecInDimSize * outStride2;
                for (uint16_t j = 0; j < last2RightPadNum; j++) {
                    Reg::Adds(lineRangeBk, lineRangeNew, (RangeType)((-1) * (j + 1 - modeOffset) * lastInDimSize),
                              maskIdx);
    ...
    __aicore__ inline void GenGatherIndex(LocalTensor<RangeType>& idxTensor)
    {
        ...
        uint32_t lastInDimSize = tdPtrMir_->inShape[dimNum_ - 1];
        int32_t lastLeftPadNum = tdPtrMir_->leftPad[dimNum_ - 1];
        uint32_t lastOutDimSize = tdPtrMir_->outShape[dimNum_ - 1];
        ...
        __VEC_SCOPE__
        {
            ...
            Reg::RegTensor<RangeType> rightPadIdxReg;
            Reg::UnalignRegForStore uReg;
            Reg::UnalignRegForStore uRegIn;

            // 先拼好-1轴的索引
            Reg::Arange(lineRange, 0);
            // 先拷出去，防止索引尾部脏数据
            Reg::StoreAlign(idxAddr, lineRange, maskIdx);
            Reg::StoreAlign(idxAddr2, lineRange, maskIdx);
            ...
            // -2轴leftpad行数, 索引都一样
            __ubuf__ RangeType* idxAddrTmp = idxAddr;
            for (uint16_t i = 0; i < lastDimsLeft; i++) {
                RangeType loopStride = (RangeType)lastInDimSize *
                                       (lastDimsLeft - i - 1); // copyout需要带边界，故这里不区分mode
                Reg::Adds(lineRangeNew, lineRange, loopStride, maskIdx);
                Reg::StoreUnAlign(idxAddrTmp, lineRangeNew, uReg, lastOutDimSize);
            }
            Reg::StoreUnAlignPost(idxAddrTmp, uReg, 0);

            // -2轴有效输入的索引
            __ubuf__ RangeType* idxAddrTmp2 = idxAddr2;
            for (uint16_t i = 0; i < lastDimsLeft; i++) {
                RangeType loopStride = (RangeType)lastInDimSize * i;
                Reg::Adds(lineRangeNew, lineRange, loopStride, maskIdx);
                Reg::StoreUnAlign(idxAddrTmp2, lineRangeNew, uRegIn, lastOutDimSize);
            }
            Reg::StoreUnAlignPost(idxAddrTmp2, uRegIn, 0);
        }
    }

    __aicore__ inline void VlInCopyProc(uint16_t inLoops, uint16_t lastInLoops, RangeType idxOffset, uint32_t maskValue,
                                        uint32_t lastInMaskValue, __ubuf__ T* curInAddr, __ubuf__ T* curOutAddr,
                                        Reg::RegTensor<RangeType>& regIdx, uint32_t idxPadOffset)
    {
        ...
        __ubuf__ T* outAddrTmp = curOutAddr;
        ...
        for (uint16_t cpIdx = 0; cpIdx < inLoops; cpIdx++) {
            ...
            Reg::Gather((Reg::RegTensor<CastType>&)regData, curInAddr, (Reg::RegTensor<IdxType>&)regNewIdx, maskIdx);
            if constexpr (sizeof(T) != 1) {
        ...
        for (uint16_t cpTailIdx = 0; cpTailIdx < lastInLoops; cpTailIdx++) {
            ...
            Reg::Gather((Reg::RegTensor<CastType>&)regData, curInAddr, (Reg::RegTensor<IdxType>&)regNewIdx, maskIdx);
            if constexpr (sizeof(T) != 1) {
                Reg::StoreUnAlign(outAddrTmp, regData, uReg, lastInMaskValue);
            } else {
                Reg::Pack(regDataT, (Reg::RegTensor<CastType>&)regData);
                Reg::StoreUnAlign(outAddrTmp, regDataT, uReg, lastInMaskValue);
            }
            Reg::StoreUnAlignPost(outAddrTmp, uReg, 0);
        }
    }
    ...
    __aicore__ inline void VlPaddingCopyProc(uint16_t gatherLoops, uint16_t padLoops, uint16_t lastPadLoops,
                                             uint32_t maskValue, uint32_t lastPadMaskValue, __ubuf__ T* curPadInAddr,
                                             __ubuf__ T* curPadOutAddr, Reg::RegTensor<RangeType>& regIdxPad,
                                             uint32_t idxPadOffset, RangeType idxOffset, uint16_t lastPadExcessIdx = 0)
    {
        ...
        Reg::RegTensor<RangeType> regIdxBk;
        Reg::RegTensor<RangeType> regNewIdx;
        Reg::RegTensor<RangeType> zeroIdxReg;
        Reg::Duplicate(zeroIdxReg, (RangeType)0);
        ...
        Reg::MaskReg maskIdx = Reg::UpdateMask<RangeType>(validMask);
        Reg::MaskReg maskLastIdx = Reg::UpdateMask<RangeType>(validLastMask);

        for (uint16_t gIdx = 0; gIdx < gatherLoops; gIdx++) {
            Reg::Adds(regIdxBk, regIdxPad, idxPadOffset, maskIdx);
            // 完整vl循环
            __ubuf__ T* outAddrTmp = curPadOutAddr + lastPadLoops * lastPadMaskValue;
            for (uint16_t pIdx = 0; pIdx < padLoops; pIdx++) {
                Reg::Adds(regNewIdx, regIdxBk, (padLoops - pIdx - 1) * idxOffset, maskIdx);
                // gather一次，copy多次
                Reg::Gather((Reg::RegTensor<CastType>&)regData, curPadInAddr, (Reg::RegTensor<IdxType>&)regNewIdx,
                            maskIdx);
                if constexpr (sizeof(T) != 1) {
                    Reg::StoreUnAlign(outAddrTmp, regData, uReg, maskValue);
                } else {
            ...
            for (uint16_t pTaiIdx = 0; pTaiIdx < lastPadLoops; pTaiIdx++) {
                ...
                Reg::Adds(regNewIdx, regIdxBk, padLoops * idxOffset - lastPadExcessIdx, maskLastIdx);
                Reg::Gather((Reg::RegTensor<CastType>&)regData, curPadInAddr, (Reg::RegTensor<IdxType>&)regNewIdx,
                            maskLastIdx);
                if constexpr (sizeof(T) != 1) {
                    Reg::StoreUnAlign(outAddrTmp, regData, uReg, lastPadMaskValue);
                } else {
                    Reg::Pack(regDataT, (Reg::RegTensor<CastType>&)regData);
                    Reg::StoreUnAlign(outAddrTmp, regDataT, uReg, lastPadMaskValue);
                }
                Reg::StoreUnAlignPost(outAddrTmp, uReg, 0);
            }
        }
    ...
    */
    __aicore__ inline void GatherProcessUb2DFw(const LocalTensor<RangeType>& idxTensor, LocalTensor<T>& inTensor,
                                               LocalTensor<T>& outTensor, uint16_t ubAxisInCopyNum)
    {
        __ubuf__ RangeType* idxAddrBw = (__ubuf__ RangeType*)idxTensor.GetPhyAddr();
        __ubuf__ RangeType* idxAddrFw = idxAddrBw + VL_RANGE_CNT;
        ...
        RangeType idxOffset = tdPtrMir_->inStride[dimNum_ - CONST2] * vlSplitLoopIn;
        uint32_t maskValue = tdPtrMir_->outStride[dimNum_ - CONST2] * vlSplitLoopIn;
        ...
        uint32_t lastCopyInMaskValue = lastCopyInPadNum * tdPtrMir_->outStride[dimNum_ - CONST2];

        __VEC_SCOPE__
        {
            Reg::RegTensor<RangeType> regIdx;
            Reg::LoadAlign(regIdx, idxAddrFw);
            VlInCopyProc(copyInPadLoops, lastCopyInPadLoops, idxOffset, maskValue, lastCopyInMaskValue, inAddr, outAddr,
                         regIdx, 0);
        }
    }
    ...
    2、-2轴倒序
    */
    __aicore__ inline void GatherProcessUb2DBw(const LocalTensor<RangeType>& idxTensor, LocalTensor<T>& inTensor,
                                               LocalTensor<T>& outTensor, uint16_t ubAxisInCopyNum)
    {
        __ubuf__ RangeType* idxAddrBw = (__ubuf__ RangeType*)idxTensor.GetPhyAddr();
        __ubuf__ RangeType* idxAddrFw = idxAddrBw + VL_RANGE_CNT;
        __ubuf__ T* inAddr = (__ubuf__ T*)inTensor.GetPhyAddr();
        __ubuf__ T* outAddr = (__ubuf__ T*)outTensor.GetPhyAddr() + BLOCK_NUM * CONST2;
        ...
        uint16_t lastPadExcessIdx = lastCopyInPadNum == 0 ?
                                        0 :
                                        (maskValue - lastCopyInMaskValue) * tdPtrMir_->inStride[dimNum_ - CONST2] /
                                            tdPtrMir_->outStride[dimNum_ - CONST2];

        __VEC_SCOPE__
        {
            Reg::RegTensor<RangeType> regIdxPad;
            Reg::LoadAlign(regIdxPad, idxAddrBw);

            VlPaddingCopyProc(1, copyInPadLoops, lastCopyInPadLoops, maskValue, lastCopyInMaskValue, inAddr, outAddr,
                              regIdxPad, 0, idxOffset, lastPadExcessIdx);
        }
    }

    template <bool IS_FW>
    __aicore__ inline void GatherProcessUb3D(const LocalTensor<RangeType>& idxTensor, LocalTensor<T>& inTensor,
                                             LocalTensor<T>& outTensor, uint16_t ubAxisInCopyNum)
    {
        bool isFw = false;
        if constexpr (IS_FW) {
        ...
        __ubuf__ RangeType* idxAddrFw = idxAddrBw + VL_RANGE_CNT;
        __ubuf__ T* inAddr = (__ubuf__ T*)inTensor.GetPhyAddr();
        __ubuf__ T* outAddr = (__ubuf__ T*)outTensor.GetPhyAddr() + (isFw ? 0 : BLOCK_NUM * CONST2);

        uint16_t vlSplitLoopIn = vlSplitIn_;
        uint32_t strideInVl = tdPtrMir_->inStride[dimNum_ - CONST2];
        uint32_t strideInVlO1 = tdPtrMir_->inStride[dimNum_ - CONST3];
        uint32_t strideOutVl = tdPtrMir_->outStride[dimNum_ - CONST2];
        uint32_t strideOutVlO1 = tdPtrMir_->outStride[dimNum_ - CONST3];

        // 该次Ub内C轴左pad, VL切3维时退化为1
        uint16_t ubAxisInCopyLoops = ubAxisInCopyNum;

        // ub切-3，这个值不会很大，uint32_t足够
        uint32_t vlLeftPadNum = tdPtrMir_->leftPad[dimNum_ - CONST2];
        uint32_t vlInNum = tdPtrMir_->inShape[dimNum_ - CONST2];
        uint32_t vlRightPadNum = tdPtrMir_->outShape[dimNum_ - CONST2] - tdPtrMir_->leftPad[dimNum_ - CONST2] -
                                 tdPtrMir_->inShape[dimNum_ - CONST2];
        uint32_t rightPadInVlOffset = vlInNum == 0 ? 0 : (vlInNum - vlRightPadNum - 1) * strideInVl;

        if (lastThirdDimInVL_) {
            strideInVl = tdPtrMir_->inStride[dimNum_ - CONST3];
            strideInVlO1 = 1;
            strideOutVl = tdPtrMir_->outStride[dimNum_ - CONST3];
            strideOutVlO1 = 1;
            ubAxisInCopyLoops = 1;
        ...
        uint16_t lastLeftPadExcessIdx = lastLeftPadNum == 0 ?
                                            0 :
                                            (maskValue - lastLeftPadMaskValue) * tdPtrMir_->inStride[dimNum_ - CONST2] /
                                                tdPtrMir_->outStride[dimNum_ - CONST2];
        ...
        uint16_t lastRightPadExcessIdx = lastrightPadNum == 0 ? 0 :
                                                                (maskValue - lastrightPadMaskValue) *
                                                                    tdPtrMir_->inStride[dimNum_ - CONST2] /
                                                                    tdPtrMir_->outStride[dimNum_ - CONST2];
        ...
        if (lastThirdDimInVL_) {
            ...
            lastLeftPadExcessIdx = lastLeftPadNum == 0 ?
                                       0 :
                                       (maskValue - lastLeftPadMaskValue) * tdPtrMir_->inStride[dimNum_ - CONST3] /
                                           tdPtrMir_->outStride[dimNum_ - CONST3];
            lastRightPadExcessIdx = lastrightPadNum == 0 ?
                                        0 :
                                        (maskValue - lastrightPadMaskValue) * tdPtrMir_->inStride[dimNum_ - CONST3] /
                                            tdPtrMir_->outStride[dimNum_ - CONST3];
        }
        ...
        __VEC_SCOPE__
        {
            ...
            Reg::LoadAlign(regIdxPad, idxAddrBw);
            Reg::LoadAlign(regIdx, idxAddrFw);
            Reg::RegTensor<RangeType> regIdxPadFixed;

            Reg::MaskReg maskIdx = Reg::CreateMask<RangeType, Reg::MaskPattern::ALL>();
            Reg::Adds(regIdxPadFixed, regIdxPad, modeOffset, maskIdx);
            ...
            for (uint16_t uiIdx = 0; uiIdx < ubAxisInCopyLoops; uiIdx++) {
                ...
                __ubuf__ T* curInOutAddr = outAddr + fixedIdx * strideOutVlO1;
                VlPaddingCopyProc(leftGatherLoops, leftPadLoops, lastLeftPadLoops, maskValue, lastLeftPadMaskValue,
                                  inAddr, curInOutAddr, regIdxPadFixed, idxPadOffset, idxOffset, lastLeftPadExcessIdx);

                // copyInPad -- gather多次 copy多次
                curInOutAddr = outAddr + fixedIdx * strideOutVlO1 + vlLeftPadNum * strideOutVl;
                VlInCopyProc(copyInPadLoops, lastCopyInPadLoops, idxOffset, maskValue, lastCopyInMaskValue, inAddr,
                             curInOutAddr, regIdx, idxPadOffset);
                ...
                VlPaddingCopyProc(rightGatherLoops, rightPadLoops, lastrightPadLoops, maskValue, lastrightPadMaskValue,
                                  inAddr, curInOutAddr, regIdxPad, idxPadOffset, idxOffset, lastRightPadExcessIdx);
            }
        }
    }
    ...
    2、补-2轴的pad
    */
    __aicore__ inline void GatherProcessUb3DFw(const LocalTensor<RangeType>& idxTensor, LocalTensor<T>& inTensor,
                                               LocalTensor<T>& outTensor, uint16_t ubAxisInCopyNum)
    {
    ...
    3、-3轴倒序
    */
    __aicore__ inline void GatherProcessUb3DBw(const LocalTensor<RangeType>& idxTensor, LocalTensor<T>& inTensor,
                                               LocalTensor<T>& outTensor, uint16_t ubAxisInCopyNum)
    {
    ...
    template <bool IS_FW>
    __aicore__ inline void GatherProcessUb4D(const LocalTensor<RangeType>& idxTensor, LocalTensor<T>& inTensor,
                                             LocalTensor<T>& outTensor, uint16_t ubAxisInCopyNum)
    {
        bool isFw = false;
        if constexpr (IS_FW) {
        ...
        __ubuf__ RangeType* idxPadAddr = (__ubuf__ RangeType*)idxTensor.GetPhyAddr();
        __ubuf__ RangeType* idxAddr = idxPadAddr + VL_RANGE_CNT;
        __ubuf__ T* inAddr = (__ubuf__ T*)inTensor.GetPhyAddr();
        __ubuf__ T* outAddr = (__ubuf__ T*)outTensor.GetPhyAddr() + (isFw ? 0 : BLOCK_NUM * CONST2);
        ...
        uint16_t vlO1LeftPadNum = tdPtrMir_->leftPad[dimNum_ - CONST3];
        uint16_t vlO1InNum = tdPtrMir_->inShape[dimNum_ - CONST3];
        uint16_t vlO1RightPadNum = tdPtrMir_->outShape[dimNum_ - CONST3] - tdPtrMir_->leftPad[dimNum_ - CONST3] -
                                   tdPtrMir_->inShape[dimNum_ - CONST3];
        ...
        if (lastThirdDimInVL_) {
            strideInVl = tdPtrMir_->inStride[dimNum_ - CONST3];
            strideInVlO1 = tdPtrMir_->inStride[dimNum_ - CONST4];
            strideOutVl = tdPtrMir_->outStride[dimNum_ - CONST3];
            strideOutVlO1 = tdPtrMir_->outStride[dimNum_ - CONST4];

            vlLeftPadNum = tdPtrMir_->leftPad[dimNum_ - CONST3];
            vlInNum = tdPtrMir_->inShape[dimNum_ - CONST3];
            vlRightPadNum = tdPtrMir_->outShape[dimNum_ - CONST3] - tdPtrMir_->leftPad[dimNum_ - CONST3] -
                            tdPtrMir_->inShape[dimNum_ - CONST3];
        ...
        uint32_t strideInN = tdPtrMir_->inStride[dimNum_ - CONST4];
        uint32_t strideOutN = tdPtrMir_->outStride[dimNum_ - CONST4];
        ...
        __VEC_SCOPE__
        {
            Reg::RegTensor<RangeType> regIdxPad;
            Reg::RegTensor<RangeType> regIdx;
            Reg::LoadAlign(regIdxPad, idxPadAddr);
            Reg::LoadAlign(regIdx, idxAddr);
            Reg::RegTensor<RangeType> regIdxPadFixed;

            Reg::MaskReg maskIdx = Reg::CreateMask<RangeType, Reg::MaskPattern::ALL>();
            Reg::Adds(regIdxPadFixed, regIdxPad, modeHOffset, maskIdx);
            ...
            for (uint16_t nIdx = 0; nIdx < ubAxisInCopyLoops; nIdx++) {
                ...
                for (uint16_t i = 0; i < vlO1LeftPadNum; i++) {
                    ...
                    __ubuf__ T* curPadOutAddr = outAddr + curOutOffset + (vlO1LeftPadNum - 1 - i) * strideOutVlO1;
                    VlPaddingCopyProc(leftGatherLoops, leftPadLoops, lastLeftPadLoops, maskValue, lastLeftPadMaskValue,
                                      inAddr, curPadOutAddr, regIdxPadFixed, idxPadOffset, idxOffset,
                                      lastLeftPadExcessIdx);

                    // H轴输入个数
                    __ubuf__ T* curOutAddr = outAddr + curOutOffset + (vlO1LeftPadNum - 1 - i) * strideOutVlO1 +
                                             vlLeftPadNum * strideOutVl;
                    VlInCopyProc(copyInPadLoops, lastCopyInPadLoops, idxOffset, maskValue, lastCopyInMaskValue, inAddr,
                    ...
                    VlPaddingCopyProc(rightGatherLoops, rightPadLoops, lastrightPadLoops, maskValue,
                                      lastrightPadMaskValue, inAddr, curPadOutAddr, regIdxPad, idxPadOffset, idxOffset,
                                      lastRightPadExcessIdx);
                }
                // C轴上的输入个数，VL切3维时退化为1
                ...
                for (uint16_t i = 0; i < vlO1InNum; i++) {
                    ...
                    __ubuf__ T* curPadOutAddr = outAddr + curOutOffset + i * strideOutVlO1;
                    VlPaddingCopyProc(leftGatherLoops, leftPadLoops, lastLeftPadLoops, maskValue, lastLeftPadMaskValue,
                                      inAddr, curPadOutAddr, regIdxPadFixed, idxPadOffset, idxOffset,
                                      lastLeftPadExcessIdx);

                    // H轴输入有效个数, VL切3维时退化为C轴输入个数
                    __ubuf__ T* curOutAddr = outAddr + curOutOffset + i * strideOutVlO1 + vlLeftPadNum * strideOutVl;
                    ...
                    idxPadOffset = curInOffset + i * strideInVlO1 + rightPadInVlOffset;
                    curPadOutAddr = outAddr + curOutOffset + i * strideOutVlO1 + vlLeftPadNum * strideOutVl +
                                    vlInNum * strideOutVl;
                    VlPaddingCopyProc(rightGatherLoops, rightPadLoops, lastrightPadLoops, maskValue,
                                      lastrightPadMaskValue, inAddr, curPadOutAddr, regIdxPad, idxPadOffset, idxOffset,
                                      lastRightPadExcessIdx);
                ...
                for (uint16_t i = 0; i < vlO1RightPadNum; i++) {
                    uint32_t idxPadOffset = curInOffset + i * strideInVlO1;
                    __ubuf__ T* curPadOutAddr = outAddr + curOutOffset + (vlO1RightPadNum - 1 - i) * strideOutVlO1;
                    VlPaddingCopyProc(leftGatherLoops, leftPadLoops, lastLeftPadLoops, maskValue, lastLeftPadMaskValue,
                                      inAddr, curPadOutAddr, regIdxPadFixed, idxPadOffset, idxOffset,
                                      lastLeftPadExcessIdx);

                    __ubuf__ T* curOutAddr = outAddr + curOutOffset + (vlO1RightPadNum - 1 - i) * strideOutVlO1 +
                                             vlLeftPadNum * strideOutVl;
                    VlInCopyProc(copyInPadLoops, lastCopyInPadLoops, idxOffset, maskValue, lastCopyInMaskValue, inAddr,
                                 curOutAddr, regIdx, idxPadOffset);
    ...
    3、补-3轴的pad
    */
    __aicore__ inline void GatherProcessUb4DFw(const LocalTensor<RangeType>& idxTensor, LocalTensor<T>& inTensor,
                                               LocalTensor<T>& outTensor, uint16_t ubAxisInCopyNum)
    {
        GatherProcessUb4D<true>(idxTensor, inTensor, outTensor, ubAxisInCopyNum);
    }

    __aicore__ inline void GatherProcessUb4DBw(const LocalTensor<RangeType>& idxTensor, LocalTensor<T>& inTensor,
                                               LocalTensor<T>& outTensor, uint16_t ubAxisInCopyNum)
    {
