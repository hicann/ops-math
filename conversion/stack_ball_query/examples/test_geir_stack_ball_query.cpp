...
#include "nn_other.h"
#include "../op_graph/stack_ball_query_proto.h"
...
#define ADD_INPUT_XYZ(intputIndex, intputName, intputDtype, inputShape)                      \
    vector<int64_t> placeholder##intputIndex##_shape = inputShape;                           \
    auto placeholder##intputIndex = op::Data("placeholder" + intputIndex).set_attr_index(0); \
    TensorDesc placeholder##intputIndex##_desc =                                             \
        TensorDesc(ge::Shape(placeholder##intputIndex##_shape), FORMAT_ND, intputDtype);     \
...
#define ADD_INPUT_CENTER_XYZ(intputIndex, intputName, intputDtype, inputShape)               \
    vector<int64_t> placeholder##intputIndex##_shape = inputShape;                           \
    auto placeholder##intputIndex = op::Data("placeholder" + intputIndex).set_attr_index(0); \
    TensorDesc placeholder##intputIndex##_desc =                                             \
        TensorDesc(ge::Shape(placeholder##intputIndex##_shape), FORMAT_ND, intputDtype);     \
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::StackBallQuery("add1");
    ...
    std::vector<int64_t> idxShape = {10, 5};

    ADD_INPUT(1, xyz, DT_FLOAT, xyzShape);
    ADD_INPUT(2, center_xyz, DT_FLOAT, centerXyzShape);
    ADD_INPUT_XYZ(3, xyz_batch_cnt, DT_INT32, xyzBatchCntShape);
    ADD_INPUT_CENTER_XYZ(4, center_xyz_batch_cnt, DT_INT32, centerXyzBatchCntShape);
    ADD_OUTPUT(1, idx, DT_INT32, idxShape);
    ADD_INPUT_ATTR(max_radius, 0.2f);
    ADD_INPUT_ATTR(sample_num, 1);
