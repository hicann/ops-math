...
#include "../op_graph/matrix_diag_v3_proto.h"
...
{
    auto matrix_diag_v3 = op::MatrixDiagV3("matrix_diag_v3");
    matrix_diag_v3.set_attr_align("RIGHT_LEFT");

    const vector<int64_t> x_shape = {2};
    const vector<std::complex<double>> x_data = {{1.0, 2.0}, {3.0, 4.0}};
    if (AddInputTensor(graph, input, inputs, "x", x_shape, DT_COMPLEX128, x_data,
                       [&](ge::Operator &data, const TensorDesc &desc) {
                           matrix_diag_v3.set_input_x(data);
                           matrix_diag_v3.update_input_desc_x(desc);
                       }) != SUCCESS) {
        return FAILED;
    }

    const vector<int64_t> k_shape = {2};
    const vector<int32_t> k_data = {-1, -1};
    if (AddConstInputTensor(graph, inputs, "k", k_shape, DT_INT32, k_data,
                            [&](ge::Operator &data, const TensorDesc &desc) {
                                matrix_diag_v3.set_input_k(data);
                                matrix_diag_v3.update_input_desc_k(desc);
                            }) != SUCCESS) {
        return FAILED;
    }

    const vector<int64_t> scalar_shape = {};
    const vector<int32_t> num_rows_data = {3};
    if (AddConstInputTensor(graph, inputs, "num_rows", scalar_shape, DT_INT32, num_rows_data,
                            [&](ge::Operator &data, const TensorDesc &desc) {
                                matrix_diag_v3.set_input_num_rows(data);
                                matrix_diag_v3.update_input_desc_num_rows(desc);
                            }) != SUCCESS) {
        return FAILED;
    }

    const vector<int32_t> num_cols_data = {2};
    if (AddConstInputTensor(graph, inputs, "num_cols", scalar_shape, DT_INT32, num_cols_data,
                            [&](ge::Operator &data, const TensorDesc &desc) {
                                matrix_diag_v3.set_input_num_cols(data);
                                matrix_diag_v3.update_input_desc_num_cols(desc);
                            }) != SUCCESS) {
        return FAILED;
    }

    const vector<std::complex<double>> padding_value_data = {{9.0, -1.0}};
    if (AddInputTensor(graph, input, inputs, "padding_value", scalar_shape, DT_COMPLEX128, padding_value_data,
                       [&](ge::Operator &data, const TensorDesc &desc) {
                           matrix_diag_v3.set_input_padding_value(data);
                           matrix_diag_v3.update_input_desc_padding_value(desc);
                       }) != SUCCESS) {
        return FAILED;
    }

    TensorDesc output_desc(ge::Shape({3, 2}), FORMAT_ND, DT_COMPLEX128);
    matrix_diag_v3.update_output_desc_y(output_desc);
    outputs.push_back(matrix_diag_v3);
    return SUCCESS;
}
} // namespace
...
int main(int argc, char *argv[])
{
    ...
    const vector<std::complex<double>> expected = {
    ...
    printf("%s - INFO - [XIR]: MatrixDiagV3 example passed\n", GetTime().c_str());
    GEFinalize();
    return SUCCESS;
}
