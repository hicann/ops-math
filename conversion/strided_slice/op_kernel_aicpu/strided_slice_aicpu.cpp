                      ...
                      std::vector<int64_t>& end_res, std::vector<int64_t>& strides_res)
{
    ...
    if (j < strides.size()) {
        if (ProcessEllipsisMask(begin, end, strides, x_shape, ellipsis_mask, new_axis_mask, i, j, bit_mask,
                                has_ellipsis, begin_j, end_j, strides_j, begin_res, end_res,
                                strides_res) == KERNEL_STATUS_INNER_ERROR) {
            return KERNEL_STATUS_INNER_ERROR;
        }
        ...
        ProcessEndMask(strides, x_shape, end_mask, shrink_axis_mask, i, j, bit_mask, end_j);
        if (ProcessNewAxisMask(new_axis_mask, i, bit_mask)) {
            return KERNEL_STATUS_OK;
        }
        if (ProcessShrinkAxisMask(x_shape, shrink_axis_mask, i, bit_mask, begin_j, strides_j, end_j) ==
...
uint32_t StridedSliceCpuKernel::InitParamsWithMasks(const std::vector<int64_t>& x_shape, int64_t begin_mask,
                                                    int64_t end_mask, int64_t ellipsis_mask, int64_t new_axis_mask,
                                                    int64_t shrink_axis_mask, std::vector<int64_t>& begin,
                                                    std::vector<int64_t>& end, std::vector<int64_t>& strides)
{
    ...
    while (i < x_shape.size()) {
        KERNEL_HANDLE_ERROR(
            ProcessMasks(begin, end, strides, x_shape, begin_mask, end_mask, ellipsis_mask, new_axis_mask,
                         shrink_axis_mask, i, j, bit_mask, has_ellipsis, begin_res, end_res, strides_res),
    ...
    strides = strides_res;
    KERNEL_LOG_INFO("[%s] begin with masks: [%s].", kStridedSlice, VectorToString(begin).c_str());
    KERNEL_LOG_INFO("[%s] end with masks: [%s].", kStridedSlice, VectorToString(end).c_str());
    KERNEL_LOG_INFO("[%s] strides with masks: [%s].", kStridedSlice, VectorToString(strides).c_str());
    return KERNEL_STATUS_OK;
}
...
{
    // check params
    KERNEL_HANDLE_ERROR(NormalCheck(ctx, kStridedSliceInputNum, kStridedSliceOutputNum), "[%s] check params failed.",
                        kStridedSlice);

    // parse params
    KERNEL_HANDLE_ERROR(ParseKernelParams(ctx), "[%s] parse kernel params failed.", kStridedSlice);

    // init params with masks
    KERNEL_HANDLE_ERROR(InitParamsWithMasks(x_shape_, begin_mask_, end_mask_, ellipsis_mask_, new_axis_mask_,
                                            shrink_axis_mask_, begin_, end_, strides_),
...
#define STRIDED_SLICE_CASE(dtype, T) \
    case dtype:                      \
        return CalStridedSlice<T>(ctx, begin_, end_, strides_, x_tensor, y_tensor)
    ...
    switch (data_type) {
        ...
        default:
            KERNEL_LOG_ERROR("[%s] doesn't support input[0] data_type [%s].", kStridedSlice,
...
uint32_t StridedSliceCpuKernel::ParseKernelParams(const CpuKernelContext& ctx)
{
    ...
    KERNEL_HANDLE_ERROR(GetMaskAttr(ctx, "end_mask", end_mask_), "[%s] get mask attr failed.", kStridedSlice);
    KERNEL_HANDLE_ERROR(GetMaskAttr(ctx, "ellipsis_mask", ellipsis_mask_), "[%s] get mask attr failed.", kStridedSlice);
    KERNEL_HANDLE_ERROR(GetMaskAttr(ctx, "new_axis_mask", new_axis_mask_), "[%s] get mask attr failed.", kStridedSlice);
    KERNEL_HANDLE_ERROR(GetMaskAttr(ctx, "shrink_axis_mask", shrink_axis_mask_), "[%s] get mask attr failed.",
                        kStridedSlice);
...
uint32_t StridedSliceCpuKernel::ParseIndexInput(const CpuKernelContext& ctx, uint32_t index, std::vector<int64_t>& vec)
{
    ...
    KERNEL_LOG_INFO("[%s] get input[%u]: [%s].", kStridedSlice, index, VectorToString(vec).c_str());
    return KERNEL_STATUS_OK;
}

uint32_t StridedSliceCpuKernel::GetMaskAttr(const CpuKernelContext& ctx, const std::string attr, int64_t& mask) const
{
    ...
    KERNEL_LOG_INFO("[%s] get attr [%s]: [%ld].", kStridedSlice, attr.c_str(), mask);
    return KERNEL_STATUS_OK;
}

OPS_MATH_REGISTER_CPU_KERNELV2(kStridedSlice, StridedSliceCpuKernel);
} // namespace aicpu
