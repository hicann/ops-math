...
/*!
 * \file test_geir_sparse_reshape.cpp
 * \brief GE IR graph mode test example for SparseReshape operator
 */
...
#include "nn_other.h"
#include "../op_graph/sparse_reshape_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                         \
    ...
    sparse_reshape1.set_input_##intputName(placeholder##intputIndex);                                       \
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                                   \
    ...
    graph.AddOp(placeholder##intputIndex);                                                                  \
    sparse_reshape1.set_input_##intputName(placeholder##intputIndex);                                       \
    sparse_reshape1.update_input_desc_##intputName(placeholder##intputIndex##_desc);                        \
    inputs.push_back(placeholder##intputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    TensorDesc outputName##outputIndex##_desc = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    sparse_reshape1.update_output_desc_##outputName(outputName##outputIndex##_desc);
...
{
    Status ret = SUCCESS;
    // SparseReshape: 3 inputs (indices, shape, new_shape), 2 outputs (y_indices, y_shape)
    auto sparse_reshape1 = op::SparseReshape("sparse_reshape1");

    // indices: (nnz=4, input_rank=2), int64
    std::vector<int64_t> indicesShape = {4, 2};
    ADD_INPUT(1, indices, inDtype, indicesShape);

    // shape: (input_rank=2,), int64 - use Const for shape values
    std::vector<int64_t> shapeShape = {2};
    ADD_CONST_INPUT(2, shape, inDtype, shapeShape);

    // new_shape: (output_rank=2,), int64 - use Const for new_shape values
    std::vector<int64_t> newShapeShape = {2};
    ADD_CONST_INPUT(3, new_shape, inDtype, newShapeShape);

    // y_indices: (nnz=4, output_rank=2)
    std::vector<int64_t> yIndicesShape = {4, 2};
    ADD_OUTPUT(1, y_indices, inDtype, yIndicesShape);

    // y_shape: (output_rank=2,)
    std::vector<int64_t> yShapeShape = {2};
    ADD_OUTPUT(2, y_shape, inDtype, yShapeShape);

    outputs.push_back(sparse_reshape1);
    return SUCCESS;
}
