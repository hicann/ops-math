...
private:
    ...
    __aicore__ inline void ProcessTopWidth(size_t loop, size_t i, int hPad2Mask, int32_t calH, bool isAtomicAdd)
    {
        ...
        CopyOutYgm(gmYOffset, hPad1 * MAX_LINE, isAtomicAdd, calH - hPad1 - hPad2 * (1 - hPad2Mask), MAX_LINE - wPad1,
        ...
        for (size_t j = 0; j < gmLoop_Width; j++) {
            ...
            gmYOffset = (loop * curOutDepth * outHeight * outWidth + cur_D * outHeight * outWidth + (MAX_LINE - wPad1) +
                         j * blockWidth);
            ...
            CopyOutYgm(gmYOffset, hPad1 * CeilAlign(calWidth, MAX_LINE), isAtomicAdd,
                       calH - hPad1 - hPad2 * (1 - hPad2Mask), calWidth, CeilAlign(calWidth, MAX_LINE),
        ...
        gmYOffset = (loop * curOutDepth * outHeight * outWidth + cur_D * outHeight * outWidth + outWidth -
                     (MAX_LINE - wPad2));

        CopyInXgm(gmXOffset, calH, MAX_LINE, (width - MAX_LINE) * sizeof(T));
        ComputeToy(0, hPad2Mask, 1, 0, calH, MAX_LINE);
        CopyOutYgm(gmYOffset, hPad1 * MAX_LINE, isAtomicAdd, calH - hPad1 - hPad2 * (1 - hPad2Mask), MAX_LINE - wPad2,
    ...
    __aicore__ inline void ProcessTopWidthToWgm(size_t loop, size_t i, int hPad2Mask, int32_t calH, bool isAtomicAdd)
    {
        ...
        CopyOutWgm(gmWOffset, hPad1 * MAX_LINE, isAtomicAdd, calH - hPad1 - hPad2 * (1 - hPad2Mask), MAX_LINE - wPad1,
        ...
        for (size_t j = 0; j < gmLoop_Width; j++) {
            ...
            CopyOutWgm(gmWOffset, hPad1 * CeilAlign(calWidth, MAX_LINE), isAtomicAdd,
                       calH - hPad1 - hPad2 * (1 - hPad2Mask), calWidth, CeilAlign(calWidth, MAX_LINE),
        ...
        CopyOutWgm(gmWOffset, hPad1 * MAX_LINE, isAtomicAdd, calH - hPad1 - hPad2 * (1 - hPad2Mask), MAX_LINE - wPad2,
    ...
    __aicore__ inline void ProcessBottomWidth(size_t loop, size_t i, int32_t calH, bool isAtomicAdd)
    {
        ...
        CopyOutYgm(gmYOffset, 0, isAtomicAdd, calH - hPad2, MAX_LINE - wPad1, CeilAlign(MAX_LINE, MAX_LINE),
        ...
        for (size_t j = 0; j < gmLoop_Width; j++) {
            ...
            CopyOutYgm(gmYOffset, 0, isAtomicAdd, calH - hPad2, calWidth, CeilAlign(calWidth, MAX_LINE),
                       (outWidth - calWidth) * sizeof(T));
            SetFlag<HardEvent::MTE3_MTE2>(eventIDMTE3ToMTE2);
            WaitFlag<HardEvent::MTE3_MTE2>(eventIDMTE3ToMTE2);
        }
        ...
        gmXOffset = (loop * curDepth * height * width + i * height * width + (height - MAX_LINE) * width + width -
                     MAX_LINE);
        gmYOffset = (loop * curOutDepth * outHeight * outWidth + cur_D * outHeight * outWidth +
        ...
        CopyOutYgm(gmYOffset, 0, isAtomicAdd, calH - hPad2, MAX_LINE - wPad2, CeilAlign(MAX_LINE, MAX_LINE),
    ...
    __aicore__ inline void ProcessBottomWidthToWgm(size_t loop, size_t i, int32_t calH, bool isAtomicAdd)
    {
        ...
        CopyOutWgm(gmWOffset, 0, isAtomicAdd, calH - hPad2, MAX_LINE - wPad1, CeilAlign(MAX_LINE, MAX_LINE),
        ...
        for (size_t j = 0; j < gmLoop_Width; j++) {
            ...
            CopyOutWgm(gmWOffset, 0, isAtomicAdd, calH - hPad2, calWidth, CeilAlign(calWidth, MAX_LINE),
                       (outWidth - calWidth) * sizeof(float));
            SetFlag<HardEvent::MTE3_MTE2>(eventIDMTE3ToMTE2);
            WaitFlag<HardEvent::MTE3_MTE2>(eventIDMTE3ToMTE2);
        }
        ...
        CopyOutWgm(gmWOffset, 0, isAtomicAdd, calH - hPad2, MAX_LINE - wPad2, CeilAlign(MAX_LINE, MAX_LINE),
    ...
    __aicore__ inline void FlatProcessHeight(size_t loop, size_t i, int wPad2Mask, int32_t calW, bool isAtomicAdd)
    {
        ...
        CopyOutYgm(gmYOffset, hPad1 * CeilAlign(width, MAX_LINE), isAtomicAdd, MAX_LINE - hPad1, outWidth,
    ...
    __aicore__ inline void FlatProcessHeighToWgm(size_t loop, size_t i, int wPad2Mask, int32_t calW, bool isAtomicAdd)
    {
        ...
        CopyOutWgm(gmWOffset, hPad1 * CeilAlign(width, MAX_LINE), isAtomicAdd, MAX_LINE - hPad1, outWidth,
                   CeilAlign(width, MAX_LINE), 0);
        SetFlag<HardEvent::MTE3_MTE2>(eventIDMTE3ToMTE2);
        WaitFlag<HardEvent::MTE3_MTE2>(eventIDMTE3ToMTE2);
    ...
    __aicore__ inline void ProcessLeftHeightMid(size_t loop, size_t i, int wPad2Mask, int32_t calW, bool isAtomicAdd)
    {
        ...
        for (size_t j = 0; j < gmLoop_Height; j++) {
            ...
            CopyOutYgm(gmYOffset, 0, isAtomicAdd, calHeight, calW - wPad1 - wPad2 * (1 - wPad2Mask),
                       CeilAlign(calW, MAX_LINE), (outWidth - (calW - wPad1 - wPad2 * (1 - wPad2Mask))) * sizeof(T));
            SetFlag<HardEvent::MTE3_MTE2>(eventIDMTE3ToMTE2);
            WaitFlag<HardEvent::MTE3_MTE2>(eventIDMTE3ToMTE2);
        }
    }

    __aicore__ inline void ProcessLeftHeightMidToWgm(size_t loop, size_t i, int wPad2Mask, int32_t calW,
                                                     bool isAtomicAdd)
    {
        size_t cur_D = GetCurD(i);
        event_t eventIDMTE3ToMTE2 = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::MTE3_MTE2));
        ...
        for (size_t j = 0; j < gmLoop_Height; j++) {
            ...
            CopyOutWgm(gmWOffset, 0, isAtomicAdd, calHeight, calW - wPad1 - wPad2 * (1 - wPad2Mask),
                       CeilAlign(calW, MAX_LINE),
    ...
    __aicore__ inline void ProcessRightHeightMid(size_t loop, size_t i, int32_t calW, bool isAtomicAdd)
    {
        ...
        for (size_t j = 0; j < gmLoop_Height; j++) {
            int64_t gmXOffset = (loop * curDepth * height * width + i * height * width + MAX_LINE * width + width -
                                 MAX_LINE + j * blockHeight * width);
            int64_t gmYOffset = (loop * curOutDepth * outHeight * outWidth + cur_D * outHeight * outWidth +
                                 (MAX_LINE - hPad1) * outWidth + outWidth - (MAX_LINE - wPad2) +
            ...
            CopyOutYgm(gmYOffset, 0, isAtomicAdd, calHeight, calW - wPad2, CeilAlign(calW, MAX_LINE),
    ...
    __aicore__ inline void ProcessRightHeightMidToWgm(size_t loop, size_t i, int32_t calW, bool isAtomicAdd)
    {
        ...
        for (size_t j = 0; j < gmLoop_Height; j++) {
            int64_t gmXOffset = (loop * curDepth * height * width + i * height * width + MAX_LINE * width + width -
                                 MAX_LINE + j * blockHeight * width);
            int64_t gmWOffset = ((MAX_LINE - hPad1) * outWidth + outWidth - (MAX_LINE - wPad2) +
                                 j * blockHeight * outWidth);
            int64_t calHeight = blockHeight;
            if (j == gmLoop_Height - 1) {
                calHeight = height - MAX_LINE - MAX_LINE - j * blockHeight;
            ...
            CopyOutWgm(gmWOffset, 0, isAtomicAdd, calHeight, calW - wPad2, CeilAlign(calW, MAX_LINE),
    ...
    __aicore__ inline void ProcessMid(size_t loop, size_t i, bool isAtomicAdd)
    {
        ...
        for (size_t j = 0; j < gmLoop_Height; j++) {
            ...
            for (size_t k = 0; k < gmLoop_Width; k++) {
                int64_t gmXOffset = (loop * curDepth * height * width + i * height * width + MAX_LINE * width +
                                     MAX_LINE + j * blockHeight * width + k * blockWidth);
                int64_t gmYOffset = (loop * curOutDepth * outHeight * outWidth + cur_D * outHeight * outWidth +
                ...
                CopyOutYgm(gmYOffset, 0, isAtomicAdd, calHeight, calWidth, CeilAlign(calWidth, MAX_LINE),
                           (outWidth - calWidth) * sizeof(T));
            }
        }
    }
    ...
    __aicore__ inline void ProcessMidToWgm(size_t loop, size_t i, bool isAtomicAdd)
    {
        ...
        for (size_t j = 0; j < gmLoop_Height; j++) {
            ...
            for (size_t k = 0; k < gmLoop_Width; k++) {
                int64_t gmXOffset = (loop * curDepth * height * width + i * height * width + MAX_LINE * width +
                                     MAX_LINE + j * blockHeight * width + k * blockWidth);
                int64_t gmWOffset = ((MAX_LINE - hPad1) * outWidth + MAX_LINE - wPad1 + j * blockHeight * outWidth +
                                     k * blockWidth);
                int64_t calWidth = blockWidth;
                if (k == gmLoop_Width - 1) {
                    calWidth = width - MAX_LINE - MAX_LINE - k * blockWidth;
                ...
                CopyOutWgm(gmWOffset, 0, isAtomicAdd, calHeight, calWidth, CeilAlign(calWidth, MAX_LINE),
                           (outWidth - calWidth) * sizeof(float));
            }
        }
    }
    ...
    __aicore__ inline void CopyInXgm(const int64_t offset, const int64_t calH, const int64_t calW,
    ...
    __aicore__ inline void CopyInWgm(const int64_t offset, const int64_t calH, const int64_t calW,
                                     const int64_t srcStride)
    {
        ...
        copyParams.dstStride = ((alignTransCalW - alignCalW)) * sizeof(float) /
                               32; //((alignWidth - alignCalW)) *sizeof(T) / 32;
    ...
    __aicore__ inline void CopyWgmToYgmFlat(size_t loop, size_t i)
    {
        ...
        CopyWgmToYgmLoopCommon(loop, i, blockHeight, blockWidth);
    }
    ...
    __aicore__ inline void CopyWgmToYgmLoopCommon(size_t loop, size_t i, int32_t blockHeight, int32_t blockWidth)
    {
        event_t eventIDMTE2ToV = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::MTE2_V));
        event_t eventIDMTE3ToMTE2 = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::MTE3_MTE2));
        size_t cur_D = GetCurD(i);
        ...
        for (size_t j = 0; j < gmLoop_Height; j++) {
            ...
            for (size_t k = 0; k < gmLoop_Width; k++) {
                ...
                outQueueY.EnQue(yLocal);
                float32Buf.FreeTensor(float32Tensor);
                CopyOutYgm(gmYOffset, 0, false, calHeight, calWidth, CeilAlign(calWidth, MAX_LINE),
                           (outWidth - calWidth) * sizeof(T));
                SetFlag<HardEvent::MTE3_MTE2>(eventIDMTE3ToMTE2);
                WaitFlag<HardEvent::MTE3_MTE2>(eventIDMTE3ToMTE2);
            }
    ...
    __aicore__ inline void CopyOutYgm(const int64_t offset, const int64_t srcOffset, const bool isAtomicAdd,
    ...
    __aicore__ inline void CopyOutWgm(const int64_t offset, const int64_t srcOffset, const bool isAtomicAdd,
                                      const int32_t calH, const int32_t calW, const int32_t alignTransCalW,
                                      const int32_t dstStride)
    {
    ...
    __aicore__ inline void ComputeToy(size_t hPad1Mask, size_t hPad2Mask, size_t wPad1Mask, size_t wPad2Mask,
    ...
    __aicore__ inline void ComputeTow(size_t hPad1Mask, size_t hPad2Mask, size_t wPad1Mask, size_t wPad2Mask,
                                      const int32_t calH, const int32_t calW)
    {
        LocalTensor<T> xLocal = inQueueX.DeQue<T>();
        LocalTensor<float> float32Tensor = float32Buf.AllocTensor<float>();
    ...
    __aicore__ inline void ComputeBasic(LocalTensor<T1>& tLocal, LocalTensor<T1>& xLocal, size_t hPad1Mask,
    ...
    template <typename T1>
    __aicore__ inline void Transose(LocalTensor<T1>& dstLocal, LocalTensor<T1>& srcLocal, const int32_t calH,
                                    const int32_t calW)
    {
