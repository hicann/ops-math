...
#include "nn_other.h"
#include "../op_graph/matrix_set_diag_v2_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape, value)                                          \
    ...
    matrixSetDiag1.set_input_##intputName(placeholder##intputIndex);                                                \
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    ...
    matrixSetDiag1.update_output_desc_##outputName(outputName##outputIndex##_desc)
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto matrixSetDiag1 = op::MatrixSetDiagV2("matrixSetDiag1");
    std::vector<int64_t> xShape = {2, 3, 4};
    std::vector<int64_t> diagonalShape = {2, 3};
    std::vector<int64_t> kShape = {1};
    ADD_INPUT(1, input, inDtype, xShape, 2.0);
    ADD_INPUT(2, diagonal, inDtype, diagonalShape, 5.0);
    ADD_INPUT(3, k, ge::DT_INT32, kShape, 0);
    ADD_OUTPUT(1, output, inDtype, xShape);
    outputs.push_back(matrixSetDiag1);
    // 添加完毕
    return SUCCESS;
}
