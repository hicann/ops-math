...
#include "nn_other.h"
#include "../op_graph/dynamic_stitch_proto.h"
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape)                          \
    ...
    dynamicStitch1.set_input_##inputName(placeholder##inputIndex);                                   \
    inputs.push_back(placeholder##inputIndex)

#define ADD_INPUT_ATTR(attrName, attrValue)     \
    do {                                        \
        dynamicStitch1.set_attr_##attrName(attrValue); \
    } while (0)
    ...
    Graph& graph)
{
    ...
    std::vector<int64_t> x0_shape = {4, 2};
    std::vector<int64_t> x1_shape = {4, 2};

    auto placeholder_indices0 = op::Data("placeholder_indices0").set_attr_index(0);
    TensorDesc placeholder_indices0_desc = TensorDesc(ge::Shape(indices0_shape), FORMAT_ND, DT_INT32);
    placeholder_indices0_desc.SetPlacement(ge::kPlacementHost);
    placeholder_indices0_desc.SetFormat(FORMAT_ND);
    Tensor tensor_indices0;
    ret = GenOnesData(indices0_shape, tensor_indices0, placeholder_indices0_desc, DT_INT32, 0);
    if (ret != SUCCESS) {
        printf("%s - ERROR - [XIR]: Generate indices0 data failed\n", GetTime().c_str());
        return FAILED;
    }
    ...
    TensorDesc placeholder_indices1_desc = TensorDesc(ge::Shape(indices1_shape), FORMAT_ND, DT_INT32);
    placeholder_indices1_desc.SetPlacement(ge::kPlacementHost);
    placeholder_indices1_desc.SetFormat(FORMAT_ND);
    Tensor tensor_indices1;
    ret = GenOnesData(indices1_shape, tensor_indices1, placeholder_indices1_desc, DT_INT32, 1);
    if (ret != SUCCESS) {
        printf("%s - ERROR - [XIR]: Generate indices1 data failed\n", GetTime().c_str());
        return FAILED;
    }
    ...
    TensorDesc placeholder_x0_desc = TensorDesc(ge::Shape(x0_shape), FORMAT_ND, inDtype);
    placeholder_x0_desc.SetPlacement(ge::kPlacementHost);
    placeholder_x0_desc.SetFormat(FORMAT_ND);
    Tensor tensor_x0;
    ret = GenOnesDataFloat32(x0_shape, tensor_x0, placeholder_x0_desc, 1.0f);
    if (ret != SUCCESS) {
        printf("%s - ERROR - [XIR]: Generate x0 data failed\n", GetTime().c_str());
        return FAILED;
    }
    ...
    TensorDesc placeholder_x1_desc = TensorDesc(ge::Shape(x1_shape), FORMAT_ND, inDtype);
    placeholder_x1_desc.SetPlacement(ge::kPlacementHost);
    placeholder_x1_desc.SetFormat(FORMAT_ND);
    Tensor tensor_x1;
    ret = GenOnesDataFloat32(x1_shape, tensor_x1, placeholder_x1_desc, 2.0f);
    if (ret != SUCCESS) {
