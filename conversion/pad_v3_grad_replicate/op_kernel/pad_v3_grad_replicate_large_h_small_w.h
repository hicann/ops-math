...
class PadV3GradReplicateLargeHSmallW
    : public PadV3GradReplicateLargeHSmallWCommon<T, PadV3GradReplicateLargeHSmallW<T>> {
    using base = PadV3GradReplicateLargeHSmallWCommon<T, PadV3GradReplicateLargeHSmallW<T>>;
    friend base;
...
__aicore__ inline void PadV3GradReplicateLargeHSmallW<T>::ImplTransposeAndCompute(const int64_t transCount)
{
    ...
    if constexpr (AscendC::IsSameType<T, half>::value) {
        for (size_t time = 0; time < SMALL_WIDTH_LIMIT / HALF_BLOCK_NUM; time++) {
            for (int i = 0; i < HALF_BLOCK_NUM; i++) {
                ...
                xDstLocalList0[i] = (uint64_t)(transposeData[this->ubFactorElement * i +
                                                             time * this->ubFactorElement * TRANSDATA_BASE_H]
                                                   .GetPhyAddr());
            }
        ...
        for (size_t time = 0; time < this->ubFactorElement / FLOAT_BLOCK_NUM; time++) {
            ...
            for (size_t i = 0; i < FLOAT_BLOCK_NUM; i++) {
                xDstLocalList1[CONST_VALUE_2 * i] = (uint64_t)(xLocal[SMALL_WIDTH_LIMIT * i +
...
__aicore__ inline void PadV3GradReplicateLargeHSmallW<T>::ComputeHGrad(const int32_t calCount, const int32_t flag)
{
    ...
    if (flag == 0) {
        for (size_t i = 0; i < this->padTop; i++) {
            Add(xLocal[this->padTop * SMALL_WIDTH_LIMIT], xLocal[i * SMALL_WIDTH_LIMIT],
                xLocal[this->padTop * SMALL_WIDTH_LIMIT], calCount);
        }
        ...
        for (size_t i = 0; i < this->padBottom; i++) {
            Add(xLocal[(COPY_ROWS_AND_COLS - 1 - this->padBottom) * SMALL_WIDTH_LIMIT],
                xLocal[(COPY_ROWS_AND_COLS - 1 - i) * SMALL_WIDTH_LIMIT],
                xLocal[(COPY_ROWS_AND_COLS - 1 - this->padBottom) * SMALL_WIDTH_LIMIT], calCount);
        }
