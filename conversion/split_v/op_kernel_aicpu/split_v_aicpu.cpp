...
uint32_t SplitVCpuKernel::ValidateAndGetSizeSplits(const CpuKernelContext& ctx, int64_t real_dim)
{
    ...
    KERNEL_CHECK_FALSE(
        (size_split_num == num_split_), KERNEL_STATUS_PARAM_INVALID,
...
{
    KERNEL_CHECK_FALSE((ValidateAndGetNumSplit(ctx) == KERNEL_STATUS_OK), KERNEL_STATUS_PARAM_INVALID,
                       "ValidateAndGetNumSplit failed.");
    KERNEL_CHECK_FALSE((ValidateAndGetSplitDim(ctx) == KERNEL_STATUS_OK), KERNEL_STATUS_PARAM_INVALID,
                       "ValidateAndGetSplitDim failed.");
    int64_t real_dim = 0;
    KERNEL_CHECK_FALSE((ValidateAndGetValue(ctx, real_dim) == KERNEL_STATUS_OK), KERNEL_STATUS_PARAM_INVALID,
                       "ValidateAndGetValue failed.");
    KERNEL_CHECK_FALSE((ValidateAndGetSizeSplits(ctx, real_dim) == KERNEL_STATUS_OK), KERNEL_STATUS_PARAM_INVALID,
                       "ValidateAndGetSizeSplits failed.");
    KERNEL_CHECK_FALSE((ValidateAndGetOutputs(ctx) == KERNEL_STATUS_OK), KERNEL_STATUS_PARAM_INVALID,
                       "ValidateAndGetOutputs failed.");
    return KERNEL_STATUS_OK;
}
...
uint32_t SplitVCpuKernel::GetSizeSplits(void* size_splits_data_ptr, int64_t real_dim)
{
    ...
    for (int64_t i = 0; i < num_split_; i++) {
        ...
        if (cur_dim == -1) {
            KERNEL_CHECK_FALSE(unique_one_dim == -1, KERNEL_STATUS_PARAM_INVALID,
    ...
    KERNEL_CHECK_FALSE(
        ((unique_one_dim == -1) && (total_dim == real_dim)) || ((unique_one_dim >= 0) && (total_dim <= real_dim)),
        ...
        "fully specified. Got [%ld], real_dim is [%ld].",
        static_cast<int64_t>(total_dim), real_dim);
    if (unique_one_dim >= 0) {
...
template <typename T>
uint32_t SplitVCpuKernel::SplitVWithOneOutput(const T* input_data_ptr, std::vector<T*> output_data_vec)
{
...
template <typename T>
uint32_t SplitVCpuKernel::SplitVWithDimZero(T* input_data_ptr, std::vector<T*> output_data_vec)
{
    ...
    for (int64_t i = 0; i < num_split_; i++) {
        ...
        KERNEL_CHECK_FALSE(mem_ret, KERNEL_STATUS_PARAM_INVALID,
                           "Memcpy size[%ld] from input value to output[%ld] failed.", copy_size, i);
...
template <typename T>
uint32_t SplitVCpuKernel::SplitVCompute(T* input_data_ptr, std::vector<T*> output_data_vec)
{
    int64_t prefix = 1;
...
uint32_t SplitVCpuKernel::DoCompute()
{
    ...
    KERNEL_CHECK_FALSE((SplitVCompute<T>(input_data_ptr, output_data_vec) == KERNEL_STATUS_OK),
                       KERNEL_STATUS_PARAM_INVALID, "SplitV Compute failed.");
    return KERNEL_STATUS_OK;
}
