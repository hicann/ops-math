...
static bool CheckShape(const aclTensor* self, const aclIntArray* splitSize, int64_t dim, const aclTensorList* out)
{
    ...
    if ((dim >= 0 && dim >= selfDim) || (dim < 0 && dim < -selfDim)) {
        OP_LOGE(
            ...
            "Expected aclnnSplitWithSize dim value [%ld] to be in range [%ld, %ld) but check failed.", dim, -selfDim,
            selfDim);
        return false;
    }
...
static aclnnStatus CheckParams(
    const aclTensor* self, const aclIntArray* splitSize, int64_t dim, const aclTensorList* out)
{
    // 1. 检查参数是否为空指针
    ...
    const aclTensor* self, const aclIntArray* splitSize, int64_t dim, aclTensorList* out, aclOpExecutor* executor)
{
    ...
    const int64_t splitLoopSize = (GetCurrentPlatformInfo().GetSocVersion() != SocVersion::ASCEND910_95) ?
                                      (isSupportSplitV2 ? SPLITV2_LOOP_SIZE : SPLIT_LOOP_SIZE) :
                                      SPLIT_LOOP_SIZE_512;
    ...
    aclOpExecutor** executor)
{
    ...
    if (splitSize->Size() == 1) {
    ...
    } else if (l0op::IsSplitV2AiCoreSupport(selfContiguous, splitSize, dim) && splitSize->Size() > SPLITV2_LOOP_SIZE) {
        ...
        l0op::SplitVAiCoreSupport(selfContiguous) && splitSize->Size() > SPLIT_LOOP_SIZE &&
