...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnSplitWithSizeGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnSplitWithSize”接口执行计算。
...
- **返回值**
  ...
  <tbody>
    ...
    <tr>
      <td>dim的取值越界，不在[-dimNum, dimNum -1]范围内，dimNum为self的维度大小。</td>
    </tr>
    <tr>
