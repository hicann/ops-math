...
int main(int argc, char* argv[])
{
    ...
    LOG_PRINT("%s - INFO - [SPLIT_GE_IR]: Start to initialize ge using ge global options\n", GetTime().c_str());
    std::map<AscendString, AscendString> global_options = {{"ge.exec.deviceId", "0"}, {"ge.graphRunMode", "1"}};
    Status ret = ge::GEInitialize(global_options);
    if (ret != SUCCESS) {
        LOG_PRINT("%s - ERROR - [SPLIT_GE_IR]: Initialize ge using ge global options failed\n", GetTime().c_str());
        return FAILED;
    }
    LOG_PRINT("%s - INFO - [SPLIT_GE_IR]: Initialize ge using ge global options success\n", GetTime().c_str());
    ...
    if (ret != SUCCESS) {
        LOG_PRINT("%s - ERROR - [SPLIT_GE_IR]: Create ir session using build options failed\n", GetTime().c_str());
        return FAILED;
    }
    ...
    LOG_PRINT("%s - INFO - [SPLIT_GE_IR]: Start to create ir session using build options\n", GetTime().c_str());
    ...
    LOG_PRINT("%s - INFO - [SPLIT_GE_IR]: Create ir session using build options success\n", GetTime().c_str());
    LOG_PRINT("%s - INFO - [SPLIT_GE_IR]: Start to add compute graph to ir session\n", GetTime().c_str());
    ...
    LOG_PRINT("%s - INFO - [SPLIT_GE_IR]: Session add ir compute graph to ir session success\n", GetTime().c_str());
    LOG_PRINT("%s - INFO - [SPLIT_GE_IR]: dump graph to txt\n", GetTime().c_str());
    std::string file_path = "./dump";
    aclgrphDumpGraph(graph, file_path.c_str(), file_path.length());
    LOG_PRINT("%s - INFO - [SPLIT_GE_IR]: Start to run ir compute graph\n", GetTime().c_str());
    std::vector<ge::Tensor> output;
    ret = session->RunGraph(graph_id, input, output);
    if (ret != SUCCESS) {
        LOG_PRINT("%s - ERROR - [SPLIT_GE_IR]: Run graph failed\n", GetTime().c_str());
        delete session;
        GEFinalize();
        return FAILED;
    }
    LOG_PRINT("%s - INFO - [SPLIT_GE_IR]: Session run ir compute graph success\n", GetTime().c_str());
    ...
    LOG_PRINT("%s - INFO - [SPLIT_GE_IR]: Start to finalize ir graph session\n", GetTime().c_str());
    ret = ge::GEFinalize();
    if (ret != SUCCESS) {
        LOG_PRINT("%s - ERROR - [SPLIT_GE_IR]: Finalize ir graph session failed\n", GetTime().c_str());
        return FAILED;
    }
    LOG_PRINT("%s - INFO - [SPLIT_GE_IR]: Finalize ir graph session success\n", GetTime().c_str());
    return SUCCESS;
}
