...
ge::graphStatus UnpackTiling::GetInputParamsSameLen()
{
    ...
    OP_CHECK_IF((splitDim_ < -inputXDimNum || splitDim_ >= inputXDimNum),
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                    context_->GetNodeName(), "axis", std::to_string(splitDim_).c_str(),
    ...
    OP_CHECK_IF(numSplit_ <= 0,
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(context_->GetNodeName(), "num", std::to_string(numSplit_).c_str(),
                                                      "The value of num must be greater than 0."),
                return ge::GRAPH_FAILED);

    const int64_t dim = inputShape_.GetDim(splitDim_);
    OP_CHECK_IF(numSplit_ != dim,
                OP_LOGE_FOR_INVALID_VALUE(context_->GetNodeName(), "num", std::to_string(numSplit_).c_str(),
...
ge::graphStatus UnpackTiling::InitParamsSameLen(int32_t maxCoreNum, uint32_t ubSize)
{
    ...
    OP_CHECK_IF(xDtypeSize_ <= 0,
                OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(context_->GetNodeName(), "x", Ops::Base::ToString(xDtype).c_str(),
...
IMPL_OP_OPTILING(Unpack).Tiling(TilingForUnpack).TilingParse<UnpackCompileInfo>(TilingPrepareForUnpack);
} // namespace optiling
