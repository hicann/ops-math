...
uint32_t UnpackCpuKernel::UnpackCompute(T* input_data_ptr, std::vector<T*> output_data_vec, CpuKernelContext& ctx)
{
    ...
    auto shard_unpack = [&](size_t start, size_t end) {
        ...
        for (uint64_t i = start; i < end; i++) {
            ...
            for (int64_t j = 0; j < prefix; j++) {
                ...
                KERNEL_CHECK_FALSE_VOID((mem_ret == EOK), "Memcpy size[%zu] from input value to output[%ld] failed.",
                                        copy_size, i);
    ...
    KERNEL_HANDLE_ERROR(CpuKernelUtils::ParallelFor(ctx, unpack_num, thread_num, shard_unpack),
