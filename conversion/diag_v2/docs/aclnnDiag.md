...
## 功能说明
  ...
  如果输入是向量（一维向量），则返回二维矩阵张量，其中self元素为对角线;
  如果输入是二维张量，则输出一维向量，取值为diagonal指定的输入矩阵的对角线元素。

## 函数原型

每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnDiagGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnDiag”接口执行计算。
