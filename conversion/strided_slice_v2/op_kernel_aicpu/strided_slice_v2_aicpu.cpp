...
uint32_t StridedSliceV2CpuKernel::CheckBeginEndDataType(const Tensor* begin, const Tensor* end)
{
    ...
    KERNEL_CHECK_FALSE((begin_type == end->GetDataType()), KERNEL_STATUS_PARAM_INVALID,
                       ...
                       static_cast<int32_t>(begin_type), static_cast<int32_t>(end->GetDataType()));
...
uint32_t StridedSliceV2CpuKernel::CheckBeginEndShape(const Tensor* begin, const Tensor* end)
{
    ...
    KERNEL_CHECK_NULLPTR(end_shape, KERNEL_STATUS_PARAM_INVALID, "Get input end shape failed")
    KERNEL_CHECK_FALSE(((begin_shape->GetDims() <= 1) && (end_shape->GetDims() <= 1) &&
                                                               ...
                                                               const std::shared_ptr<TensorShape>& begin_shape)
{
    ...
    DataType begin_type = begin->GetDataType();
    KERNEL_CHECK_FALSE((begin_type == strides->GetDataType()), KERNEL_STATUS_PARAM_INVALID,
                       "Expect begin and strides to be same data type, but got begin "
                       "data type[%d], strides data type[%d]",
                       static_cast<int32_t>(begin_type), static_cast<int32_t>(strides->GetDataType()));
    auto strides_shape = strides->GetTensorShape();
    KERNEL_CHECK_NULLPTR(strides_shape, KERNEL_STATUS_PARAM_INVALID, "Get input strides shape failed")
    KERNEL_CHECK_FALSE(
        ((strides_shape->GetDims() <= 1) && (begin_shape->NumElements() == strides_shape->NumElements())),
        KERNEL_STATUS_PARAM_INVALID,
        ...
        "number[%ld].",
        begin_shape->GetDims(), begin_shape->NumElements(), strides_shape->GetDims(), strides_shape->NumElements());
    return KERNEL_STATUS_OK;
}
...
uint32_t StridedSliceV2CpuKernel::CheckAxesDataType(const Tensor* begin, const Tensor* axes)
{
    ...
    DataType begin_type = begin->GetDataType();
    KERNEL_CHECK_FALSE((begin_type == axes->GetDataType()), KERNEL_STATUS_PARAM_INVALID,
                       "Expect begin and axes to be same data type, but got begin "
                       "data type[%d], axes data type[%d]",
                       static_cast<int32_t>(begin_type), static_cast<int32_t>(axes->GetDataType()));
    return KERNEL_STATUS_OK;
}
                                                  ...
                                                  std::vector<int64_t>& begin_vec)
{
    ...
    KERNEL_CHECK_NULLPTR(begin_data, KERNEL_STATUS_PARAM_INVALID, "Get input begin data failed")
                                                ...
                                                std::vector<int64_t>& end_vec)
{
    ...
    KERNEL_CHECK_NULLPTR(end_data, KERNEL_STATUS_PARAM_INVALID, "Get input end data failed")
                                                    ...
                                                    std::vector<int64_t>& strides_vec)
{
    ...
    if (strides == nullptr) {
        ...
        KERNEL_CHECK_NULLPTR(strides_data, KERNEL_STATUS_PARAM_INVALID, "Get input strides data failed")
                                                 ...
                                                 std::vector<int64_t>& axes_vec)
{
    ...
    if (axes == nullptr) {
        ...
        KERNEL_CHECK_NULLPTR(axes_data, KERNEL_STATUS_PARAM_INVALID, "Get input axes data failed")
        for (int64_t i = 0; i < axes->NumElements(); ++i) {
            T axes_value = axes_data[i] < 0 ? axes_data[i] + x_dims : axes_data[i];
            KERNEL_CHECK_FALSE(((axes_value >= 0) && (axes_value < x_dims)), KERNEL_STATUS_PARAM_INVALID,
                               "Check axes[%ld] value[%ld] failed, must be in range [-%d, %d]", i,
                               static_cast<int64_t>(axes_value), x_dims, x_dims - 1);
            KERNEL_CHECK_FALSE((std::find(axes_vec.begin(), axes_vec.end(), axes_value) == axes_vec.end()),
                               KERNEL_STATUS_PARAM_INVALID, "Check value failed, axes[%ld] value[%ld] is repeated", i,
                                             ...
                                             std::vector<int64_t>& end_vec, std::vector<int64_t>& strides_vec)
{
    ...
    KERNEL_CHECK_FALSE((ret == KERNEL_STATUS_OK), KERNEL_STATUS_PARAM_INVALID, "Build begin parameter failed, ret[%u]",
                       ret);

    std::vector<int64_t> end_ret{};
    ret = BuildEndParam<T>(x_shape, end, end_ret);
    KERNEL_CHECK_FALSE((ret == KERNEL_STATUS_OK), KERNEL_STATUS_PARAM_INVALID, "Build end parameter failed, ret[%u]",
                       ret);
    ...
    KERNEL_CHECK_FALSE((ret == KERNEL_STATUS_OK), KERNEL_STATUS_PARAM_INVALID, "Build axes parameter failed, ret[%u]",
                       ret);
...
template <typename T>
uint32_t StridedSliceV2CpuKernel::CheckAndBuildParam(const Tensor* x, const Tensor* begin, const Tensor* end,
                                                     const Tensor* axes, const Tensor* strides,
                                                     std::vector<int64_t>& begin_vec, std::vector<int64_t>& end_vec,
                                                     std::vector<int64_t>& strides_vec)
{
    ...
    return BuildParam<T>(x, begin, end, axes, strides, begin_vec, end_vec, strides_vec);
}
...
#define STRIDED_SLICE_V2_CASE(DT, T)                                                                        \
    case (DT): {                                                                                            \
        ret = StridedSliceCpuKernel::CalStridedSlice<T>(ctx, begin_vec, end_vec, strides_vec, ctx.Input(0), \
                                                        ctx.Output(0));                                     \
        break;                                                                                              \
    }
                                                  ...
                                                  Tensor*& axes, Tensor*& strides)
{
    ...
    auto x_shape = x->GetTensorShape();
    KERNEL_CHECK_NULLPTR(x_shape, KERNEL_STATUS_PARAM_INVALID, "Get input x shape failed")
    begin = ctx.Input(kSecondInputIndex);
    KERNEL_CHECK_NULLPTR(begin, KERNEL_STATUS_PARAM_INVALID, "Get input begin failed")
...
uint32_t StridedSliceV2CpuKernel::CheckAndBuildParamsByType(const Tensor* x, const Tensor* begin, const Tensor* end,
                                                            const Tensor* axes, const Tensor* strides,
                                                            std::vector<int64_t>& begin_vec,
                                                            std::vector<int64_t>& end_vec,
                                                            std::vector<int64_t>& strides_vec)
{
    ...
    uint32_t ret = KERNEL_STATUS_OK;
    if (type == DT_INT32) {
        ret = CheckAndBuildParam<int32_t>(x, begin, end, axes, strides, begin_vec, end_vec, strides_vec);
        KERNEL_CHECK_FALSE((ret == KERNEL_STATUS_OK), ret, "Check and build parameter failed, ret[%u]", ret);
    } else if (type == DT_INT64) {
        ret = CheckAndBuildParam<int64_t>(x, begin, end, axes, strides, begin_vec, end_vec, strides_vec);
        KERNEL_CHECK_FALSE((ret == KERNEL_STATUS_OK), ret, "Check and build parameter failed, ret[%u]", ret);
    } else {
        KERNEL_LOG_ERROR("Unsupported input begin data_type[%u], only support DT_INT32 and "
                         "DT_INT64.",
...
uint32_t StridedSliceV2CpuKernel::Compute(CpuKernelContext& ctx)
{
    ...
    ret = GetMaskAttrs(ctx, begin_mask_value, end_mask_value, ellipsis_mask_value, new_axis_mask_value,
                       shrink_axis_mask_value);
    if (ret != KERNEL_STATUS_OK) {
        return ret;
    }
    ...
    ret = CheckAndBuildParamsByType(x, begin, end, axes, strides, begin_vec, end_vec, strides_vec);
    if (ret != KERNEL_STATUS_OK) {
        return ret;
    }

    ret = StridedSliceCpuKernel::InitParamsWithMasks(x_shape_value, begin_mask_value, end_mask_value,
                                                     ellipsis_mask_value, new_axis_mask_value, shrink_axis_mask_value,
                                                     begin_vec, end_vec, strides_vec);
    KERNEL_CHECK_FALSE((ret == KERNEL_STATUS_OK), ret, "Init parameters with masks failed, ret[%u]", ret);

    return DoStridedSliceV2(ctx, begin_vec, end_vec, strides_vec);
}

OPS_MATH_REGISTER_CPU_KERNELV2(kStridedSliceV2, StridedSliceV2CpuKernel);
} // namespace aicpu
