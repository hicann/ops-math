...
static bool CheckStride(ops::QuickVector& stride, const gert::TilingContext* context)
{
    ...
    for (size_t i = 0; i < stride.GetDimNum(); i++) {
        if (stride[i] == 0) {
            OP_LOGE_FOR_INVALID_VALUES_WITH_REASON(OP_NAME, "stride", "0", "The value of stride cannot be 0.");
            return false;
        }
    }
    return true;
...
static void PositiveAxisImpl(int32_t inputDims, const gert::Tensor* axesTensor, std::vector<int64_t>& newAxes)
{
    ...
    if (data == nullptr) {
        OP_LOGE(OP_NAME, "Failed to get axes tensor data in Tiling, data is null.");
        return;
    }
...
static int64_t GetConstIndexValue(const gert::Tensor* tensor, int32_t idx, int64_t defaultValue = 0)
{
    ...
    if (dataType == ge::DT_INT32) {
        ...
        if (data == nullptr) {
            OP_LOGE(OP_NAME, "Failed to get int32 tensor data in Tiling, data is null.");
            return defaultValue;
        }
        ...
        if (data == nullptr) {
            OP_LOGE(OP_NAME, "Failed to get int64 tensor data in Tiling, data is null.");
            return defaultValue;
        }
        value = data[idx];
    } else {
        OP_LOGE_FOR_INVALID_DTYPE(OP_NAME, "end, begin or stride",
                                  ge::TypeUtils::DataTypeToSerialString(dataType).c_str(), "int32 or int64");
        return defaultValue;
    }
...
static void ConstructEndList(const gert::Tensor* endTensor, const gert::Shape& xShape, const std::vector<int64_t>& axes,
...
static bool ShouldAdjustLastStride(const SliceParametersRuntime2& param, size_t lastDim)
{
    ...
    return strideLastDim > 1 && inputLastDim % strideLastDim == 0 && beginLastDim / strideLastDim == 0 &&
...
static ge::graphStatus GetMaskValue(gert::TilingContext* context, const gert::RuntimeAttrs* attrs, size_t attrIndex,
                                    uint64_t& maskValue)
{
...
ge::graphStatus Tiling4StridedSliceV2(gert::TilingContext* context)
{
    ...
    return StrideSliceV2TilingForAscendC(context, compileInfo->coreNum, compileInfo->ubSize, compileInfo->cacheLineSize,
                                         sliceParam, xDesc->GetDataType());
}
...
ge::graphStatus TilingPrepare4StridedSliceV2(gert::TilingParseContext* context)
{
    ...
    if (ci->coreNum <= 0) {
        OP_LOGE_FOR_INVALID_VALUES_WITH_REASON(context->GetNodeName(), "core number", std::to_string(ci->coreNum),
    ...
    if (ci->ubSize <= 0) {
        OP_LOGE_FOR_INVALID_VALUES_WITH_REASON(context->GetNodeName(), "ubSize", std::to_string(ci->ubSize),
                                               "The value of ubSize cannot <= 0.");
        return ge::GRAPH_FAILED;
    }

    ci->cacheLineSize = Ops::Base::GetCacheLineSize(context);
    if (ci->cacheLineSize <= 0) {
        OP_LOGE_FOR_INVALID_VALUES_WITH_REASON(context->GetNodeName(), "cacheLineSize",
                                               std::to_string(ci->cacheLineSize),
