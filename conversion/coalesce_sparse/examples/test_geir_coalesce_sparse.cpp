...
#include "nn_other.h"
#include "../op_graph/coalesce_sparse_proto.h"
...
using std::string;
using std::vector;
#define ADD_INPUT_UNIQUELEN(intputIndex, intputName, intputDtype, inputShape)                \
...
#define ADD_INPUT_UNIQUEINDICES(intputIndex, intputName, intputDtype, inputShape)                      \
    vector<int64_t> placeholder##intputIndex##_shape = inputShape;                           \
    auto placeholder##intputIndex = op::Data("placeholder" + intputIndex).set_attr_index(0); \
    TensorDesc placeholder##intputIndex##_desc =                                             \
        TensorDesc(ge::Shape(placeholder##intputIndex##_shape), FORMAT_ND, intputDtype);     \
...
#define ADD_INPUT_INDICES(intputIndex, intputName, intputDtype, inputShape)                      \
    vector<int64_t> placeholder##intputIndex##_shape = inputShape;                           \
    auto placeholder##intputIndex = op::Data("placeholder" + intputIndex).set_attr_index(0); \
    TensorDesc placeholder##intputIndex##_desc =                                             \
        TensorDesc(ge::Shape(placeholder##intputIndex##_shape), FORMAT_ND, intputDtype);     \
...
#define ADD_INPUT_VALUES(intputIndex, intputName, intputDtype, inputShape)                   \
    vector<int64_t> placeholder##intputIndex##_shape = inputShape;                           \
    auto placeholder##intputIndex = op::Data("placeholder" + intputIndex).set_attr_index(0); \
    TensorDesc placeholder##intputIndex##_desc =                                             \
        TensorDesc(ge::Shape(placeholder##intputIndex##_shape), FORMAT_ND, intputDtype);     \
...
int CreateOppInGraph(DataType inDtype, DataType inDtype1,std::vector<ge::Tensor> &input, std::vector<Operator> &inputs,
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::CoalesceSparse("add1");
    std::vector<int64_t> unique_len_shape = {1}; //[1]
    std::vector<int64_t> unique_indices_shape = {1}; //[0]
    std::vector<int64_t> indices_shape = {4}; // [0,0,0,0]
    std::vector<int64_t> values_shape = {4}; // [2,2,2,2]
    std::vector<int64_t> new_inidces_shape = {1};
    std::vector<int64_t> new_values_shape = {1};
    ADD_INPUT_UNIQUELEN(1, unique_len, inDtype1, unique_len_shape);
    ADD_INPUT_UNIQUEINDICES(2, unique_indices, inDtype1, unique_indices_shape);
    ADD_INPUT_INDICES(3, indices, inDtype1, indices_shape);
    ADD_INPUT_VALUES(4, values, inDtype1, values_shape);
    ADD_OUTPUT(1, new_inidces, inDtype1, new_inidces_shape);
    ADD_OUTPUT(2, new_values, inDtype1, new_values_shape);
...
int main(int argc, char *argv[])
{
    ...
    DataType inDtype1 = DT_INT32;
    std::cout << inDtype1 << std::endl;

    ret = CreateOppInGraph(inDtype, inDtype1, input, inputs, outputs, graph);
    if (ret != SUCCESS) {
