...
/*!
 * \file test_geir_slice_last_dim.cpp
 * \brief GE IR test for SliceLastDim operator
 *
 * SliceLastDim: input x[4,8], start=0, end=4, stride=1 -> output y[4,4]
 */
...
#include "../op_graph/slice_last_dim_proto.h"
...
{
    Status ret = SUCCESS;
    auto node = op::SliceLastDim("slice_last_dim_1");
    ...
    node.SetAttr("start", static_cast<int64_t>(0));
    node.SetAttr("end", static_cast<int64_t>(4));
    node.SetAttr("stride", static_cast<int64_t>(1));
