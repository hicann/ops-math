...
#include "nn_other.h"
#include "../op_graph/im2col_proto.h"
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape, inputFormat)                                                     \
    ...
    im2col1.set_input_##inputName(placeholder##inputIndex);                                                             \
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape, outputFormat)                                           \
    ...
    im2col1.update_output_desc_##outputName(outputName##outputIndex##_desc)
...
#define ADD_INPUT_ATTR(attrName, attrValue) im2col1.set_attr_##attrName(attrValue)
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto im2col1 = op::Im2col("im2col");
    vector<vector<int64_t>> shape = {{1, 1, 1, 1}, {1, 1, 3, 2}, {1, 1}, {1}, {1, 1, 0, 1}};

    ADD_INPUT(0, x, inDtype, shape[0], ge::FORMAT_NCHW);
    ADD_INPUT_ATTR(ksizes, shape[2]);
    ADD_INPUT_ATTR(strides, shape[3]);
    ADD_INPUT_ATTR(dilations, shape[2]);
    ADD_INPUT_ATTR(padding_mode, "CALCULATED");
    ADD_INPUT_ATTR(pads, shape[4]);
    ADD_OUTPUT(0, y, inDtype, shape[1], ge::FORMAT_NCHW);

    outputs.push_back(im2col1);
    // 添加完毕
    return SUCCESS;
}
