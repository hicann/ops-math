...
#include "nn_other.h"
#include "../op_graph/dynamic_partition_proto.h"
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape)                                                       \
    ...
    ret = GenOnesData(placeholder##inputIndex##_shape, tensor_placeholder##inputIndex, placeholder##inputIndex##_desc, \
                      inputDtype, 2);                                                                                  \
    if (ret != SUCCESS) {                                                                                              \
        LOG_PRINT("%s - ERROR - [DYNAMIC_PARTITION_GE_IR]: Generate input data failed\n", GetTime().c_str());          \
        return FAILED;                                                                                                 \
    }                                                                                                                  \
    ...
    dynamicPartition1.set_input_##inputName(placeholder##inputIndex);                                                  \
    inputs.push_back(placeholder##inputIndex)

#define ADD_INPUT_ATTR(attrName, attrValue)               \
    do {                                                  \
        dynamicPartition1.set_attr_##attrName(attrValue); \
    } while (0)

#define ADD_OUTPUT(outputName, outputShape) dynamicPartition1.create_dynamic_output_##outputName(outputShape)
...
{
    Status ret = SUCCESS;
    auto dynamicPartition1 = op::DynamicPartition("dynamicPartition1").create_dynamic_output_y(3);
    std::vector<int64_t> xShape = {1, 1, 1};
    ADD_INPUT(0, x, inDtype, xShape);
    ADD_INPUT(1, partitions, inDtype, xShape);

    ADD_INPUT_ATTR(num_partitions, 3);

    ADD_OUTPUT(y, 3);

    outputs.push_back(dynamicPartition1);
...
int main(int argc, char* argv[])
{
    ...
    LOG_PRINT("%s - INFO - [DYNAMIC_PARTITION_GE_IR]: Start to initialize ge using ge global options\n",
              GetTime().c_str());
    std::map<AscendString, AscendString> global_options = {{"ge.exec.deviceId", "0"}, {"ge.graphRunMode", "1"}};
    Status ret = ge::GEInitialize(global_options);
    if (ret != SUCCESS) {
        LOG_PRINT("%s - ERROR - [DYNAMIC_PARTITION_GE_IR]: Initialize ge using ge global options failed\n",
                  GetTime().c_str());
        return FAILED;
    }
    LOG_PRINT("%s - INFO - [DYNAMIC_PARTITION_GE_IR]: Initialize ge using ge global options success\n",
              GetTime().c_str());
    ...
    if (ret != SUCCESS) {
        LOG_PRINT("%s - ERROR - [DYNAMIC_PARTITION_GE_IR]: Create ir session using build options failed\n",
                  GetTime().c_str());
        return FAILED;
    }
    ...
    LOG_PRINT("%s - INFO - [DYNAMIC_PARTITION_GE_IR]: Start to create ir session using build options\n",
              GetTime().c_str());
    ...
    LOG_PRINT("%s - INFO - [DYNAMIC_PARTITION_GE_IR]: Create ir session using build options success\n",
              GetTime().c_str());
    LOG_PRINT("%s - INFO - [DYNAMIC_PARTITION_GE_IR]: Start to add compute graph to ir session\n", GetTime().c_str());
    ...
    LOG_PRINT("%s - INFO - [DYNAMIC_PARTITION_GE_IR]: Session add ir compute graph to ir session success\n",
              GetTime().c_str());
    LOG_PRINT("%s - INFO - [DYNAMIC_PARTITION_GE_IR]: dump graph to txt\n", GetTime().c_str());
    std::string file_path = "./dump";
    aclgrphDumpGraph(graph, file_path.c_str(), file_path.length());
    LOG_PRINT("%s - INFO - [DYNAMIC_PARTITION_GE_IR]: Start to run ir compute graph\n", GetTime().c_str());
    std::vector<ge::Tensor> output;
    ret = session->RunGraph(graph_id, input, output);
    if (ret != SUCCESS) {
        LOG_PRINT("%s - ERROR - [DYNAMIC_PARTITION_GE_IR]: Run graph failed\n", GetTime().c_str());
        delete session;
        GEFinalize();
        return FAILED;
    }
    LOG_PRINT("%s - INFO - [DYNAMIC_PARTITION_GE_IR]: Session run ir compute graph success\n", GetTime().c_str());
    ...
    LOG_PRINT("%s - INFO - [DYNAMIC_PARTITION_GE_IR]: Precision is ok\n", GetTime().c_str());
    LOG_PRINT("%s - INFO - [DYNAMIC_PARTITION_GE_IR]: Start to finalize ir graph session\n", GetTime().c_str());
    ret = ge::GEFinalize();
    if (ret != SUCCESS) {
        LOG_PRINT("%s - ERROR - [DYNAMIC_PARTITION_GE_IR]: Finalize ir graph session failed\n", GetTime().c_str());
        return FAILED;
    }
    LOG_PRINT("%s - INFO - [DYNAMIC_PARTITION_GE_IR]: Finalize ir graph session success\n", GetTime().c_str());
    return SUCCESS;
}
