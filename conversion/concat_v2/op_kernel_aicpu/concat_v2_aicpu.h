...
struct TTypes {
    ...
    using Matrix = Eigen::TensorMap<Eigen::Tensor<T, kTensorRank, Eigen::RowMajor, Eigen::DenseIndex>, Eigen::Aligned>;

    using ConstMatrix =
        Eigen::TensorMap<Eigen::Tensor<const T, kTensorRank, Eigen::RowMajor, Eigen::DenseIndex>, Eigen::Aligned>;
};
...
private:
    ...
    uint32_t PrepareInput(
        const CpuKernelContext& ctx, std::vector<std::shared_ptr<typename TTypes<T>::ConstMatrix>>& inputs)
    {
        ...
        for (int64_t i = 0; i < n_; ++i) {
            ...
            KERNEL_CHECK_NULLPTR(input_i_ptr, KERNEL_STATUS_PARAM_INVALID, "Get input x%u failed.", i);
            int64_t input_i_num = input_i_ptr->NumElements();
            if (input_i_num == 0) {
                continue;
            }
            auto input_i_shape_ptr = input_i_ptr->GetTensorShape();
            KERNEL_CHECK_NULLPTR(input_i_shape_ptr, KERNEL_STATUS_PARAM_INVALID, "Get input x%u shape failed.", i);
            ...
            for (int32_t j = 0; j < input_dims_; ++j) {
                ...
                KERNEL_CHECK_FALSE(
                    ...
                    "Dimensions of inputs should match: shape[0][%d]=%ld vs. shape[%u][%d]=%ld", j, dim_0j, i, j,
                    dim_ij);
            }
            ...
            auto input_i = std::make_shared<typename TTypes<T>::ConstMatrix>(
                static_cast<T*>(input_i_data_ptr), inputs_flat_dim0_, inputs_flat_dim1);
    ...
    uint32_t DoCompute(CpuKernelContext& ctx)
    {
        std::vector<std::shared_ptr<typename TTypes<T>::ConstMatrix>> inputs;
        KERNEL_CHECK_FALSE(
            (PrepareInput<T>(ctx, inputs) == KERNEL_STATUS_OK), KERNEL_STATUS_PARAM_INVALID, "PrepareInput failed.");
        std::shared_ptr<typename TTypes<T>::Matrix> output = nullptr;
        KERNEL_CHECK_FALSE(
            (PrepareOutput<T>(ctx, output) == KERNEL_STATUS_OK), KERNEL_STATUS_PARAM_INVALID, "PrepareOutput failed.");
        if (inputs.size() > 0) {
    ...
    uint32_t PrepareConcatV2Sizes(
        const std::vector<std::shared_ptr<typename TTypes<T>::ConstMatrix>>& inputs, std::vector<ptrdiff_t>& sizes,
        int64_t& row_size)
    {
    ...
    uint32_t CopyPartialRows(CopyRowsContext<T>& rows_ctx, uint32_t& ret)
    {
        ...
        for (size_t j = 0; j < rows_ctx.sizes.size(); ++j) {
            ...
            if (mem_ret != EOK) {
                KERNEL_LOG_ERROR("Memcpy size[%zu] from inp[%p] to out[%p] failed.", copy_size, rows_ctx.out, inp);
    ...
    template <typename T>
    uint32_t CopyRemainRows(CopyRowsContext<T>& rows_ctx, uint32_t& ret)
    {
        ...
        for (int64_t i = rows_ctx.skipped_rows; i < dim0; ++i) {
            for (size_t j = 0; j < rows_ctx.inputs.size(); ++j) {
                ...
                if (mem_ret != EOK) {
                    KERNEL_LOG_ERROR(
                        "Memcpy size[%zu] from inp[%p] to out[%p] failed.", copy_size, inp[j], rows_ctx.out);
                    ret = KERNEL_STATUS_INNER_ERROR;
                    return KERNEL_STATUS_INNER_ERROR;
                }
                rows_ctx.out += size;
                inp[j] += size;
                KERNEL_CHECK_FALSE_EXEC((rows_ctx.out != rows_ctx.out_end), return KERNEL_STATUS_OK);
            }
        }
        return KERNEL_STATUS_OK;
    }
    ...
    uint32_t ExecConcatV2Tasks(
        const CpuKernelContext& ctx, const std::vector<std::shared_ptr<typename TTypes<T>::ConstMatrix>>& inputs,
        std::shared_ptr<typename TTypes<T>::Matrix>& output, const std::vector<ptrdiff_t>& sizes, int64_t row_size)
    {
        uint32_t ret = KERNEL_STATUS_OK;
        auto work = [this, &row_size, &sizes, &inputs, &output, &ret](int64_t start, int64_t end) -> uint32_t {
        ...
        uint32_t result = CpuKernelUtils::ParallelFor(
            ctx, static_cast<int64_t>(output->size()), static_cast<int64_t>(sizeof(T)), work);
        KERNEL_CHECK_FALSE(
            (result == KERNEL_STATUS_OK), KERNEL_STATUS_INNER_ERROR, "ParallelFor exec failed, result[%u].", result);
        KERNEL_CHECK_FALSE((ret == KERNEL_STATUS_OK), KERNEL_STATUS_INNER_ERROR, "ConcatV2CpuKernel failed.");
        KERNEL_LOG_INFO("ConcatV2CpuKernel success.");
        return KERNEL_STATUS_OK;
    }
        ...
        std::shared_ptr<typename TTypes<T>::Matrix>& output)
    {
        ...
        KERNEL_CHECK_FALSE_EXEC((ret == KERNEL_STATUS_OK), return ret)
