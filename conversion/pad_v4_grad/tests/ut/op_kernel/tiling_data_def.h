...
#define INIT_TILING_DATA(tilingStruct, tilingDataPointer, tilingPointer)                              \
    ...
    copy_gm_to_ubuf(((__ubuf__ uint8_t*)(tilingUbPointer)), ((__gm__ uint8_t*)(tilingPointer)), 0, 1, \
...
#define GET_TILING_DATA(tilingData, tilingPointer)                           \
    ...
    (tilingData).ubFactorElement = tilingDataPointer->ubFactorElement;       \
                                                                             \
    (tilingData).ncPerCore = tilingDataPointer->ncPerCore;                   \
                                                                             \
    (tilingData).tailNC = tilingDataPointer->tailNC;                         \
                                                                             \
    (tilingData).tilingKey = tilingDataPointer->tilingKey;                   \
                                                                             \
    (tilingData).workspacePerCore = tilingDataPointer->workspacePerCore;
