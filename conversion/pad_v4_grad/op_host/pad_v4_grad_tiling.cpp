...
#include "../../pad_v3_grad_replicate/op_host/pad_grad_tiling_common.h"
...
class PadV3GradV2Tiling
    : public PadGradTilingBase<PadV3GradV2Tiling<TilingData, dataTypeLen>, TilingData, dataTypeLen> {
    using Base = PadGradTilingBase<PadV3GradV2Tiling<TilingData, dataTypeLen>, TilingData, dataTypeLen>;
...
{
    if (this->dtype == FLOAT_DTYPE && this->mode == REFLECTION_MODE) {
        if (this->height <= MINI_SHAPE_MAX_HEIGHT && this->width <= FLOAT_MINI_SHAPE_MAX_WIDTH) {
            this->tilingKey = FLOAT_MINI_SHAPE_TILING_KEY; // float, mini shape
            this->divideUbNum = CONST_VALUE_3;
        } else if (this->padLeft == 0 && this->padRight == 0 && (this->padTop != 0 || this->padBottom != 0) &&
                   this->height > SMALL_H_LIMIT) {
            this->tilingKey = FLOAT_NO_W_PAD_TILING_KEY; // float, w dim no pad
            this->divideUbNum = CONST_VALUE_8;
        } else if (this->padTop == 0 && this->padBottom == 0 && (this->padLeft != 0 || this->padRight != 0)) {
            this->tilingKey = FLOAT_NO_H_PAD_TILING_KEY; // float, reflect, h dim no pad
            this->divideUbNum = CONST_VALUE_4;
        } else if (this->height <= SMALL_H_LIMIT && this->width > FLOAT_MINI_SHAPE_MAX_WIDTH) {
            this->tilingKey = FLOAT_SMALL_H_LARGE_W_TILING_KEY; // float, mini h dim
            this->divideUbNum = CONST_VALUE_3;
        } else if (this->height > MINI_SHAPE_MAX_HEIGHT && this->width <= SMALL_W_LIMIT) {
            this->tilingKey = FLOAT_LARGE_H_SMALL_W_TILING_KEY; // float, mini w dim
            this->divideUbNum = CONST_VALUE_3;
        } else {
            this->tilingKey = FLOAT_H_W_PAD_TILING_KEY; // float32, big shape, impl h_dim and w_dim pad
            this->divideUbNum = CONST_VALUE_5;
        }
    } else if (this->dtype == FLOAT_DTYPE && this->mode == EDGE_MODE) {
        this->tilingKey = FLOAT_EDGE_TILING_KEY; // mode2: float, edge
    } else if (this->dtype == FLOAT16_DTYPE && this->mode == REFLECTION_MODE) {
        if (this->height <= MINI_SHAPE_MAX_HEIGHT && this->width <= FLOAT16_MINI_SHAPE_MAX_WIDTH) {
            this->tilingKey = FLOAT16_MINI_SHAPE_TILING_KEY; // float16, mini shape
            this->divideUbNum = CONST_VALUE_3;
        } else if (this->padLeft == 0 && this->padRight == 0 && (this->padTop != 0 || this->padBottom != 0) &&
                   this->height > SMALL_H_LIMIT) {
            this->tilingKey = FLOAT16_NO_W_PAD_TILING_KEY; // w dim no pad
            this->divideUbNum = CONST_VALUE_8;
        } else if (this->padTop == 0 && this->padBottom == 0 && (this->padLeft != 0 || this->padRight != 0) &&
                   this->width > SMALL_W_LIMIT) {
            this->tilingKey = FLOAT16_NO_H_PAD_TILING_KEY; // h dim no pad
            this->divideUbNum = CONST_VALUE_4;
        } else if (this->height <= SMALL_H_LIMIT && this->width > FLOAT16_MINI_SHAPE_MAX_WIDTH) {
            ...
            this->tilingKey = FLOAT16_H_W_PAD_TILING_KEY; // float16, big shape, impl h_dim and w_dim pad
            this->divideUbNum = CONST_VALUE_5;
        }
    } else if (this->dtype == FLOAT16_DTYPE && this->mode == EDGE_MODE) {
        this->tilingKey = FLOAT16_EDGE_TILING_KEY; // mode2: float16, edge
    } else if (this->dtype == BF16_DTYPE && this->mode == REFLECTION_MODE) {
        if (this->height <= MINI_SHAPE_MAX_HEIGHT && this->width <= BFLOAT16_MINI_SHAPE_MAX_WIDTH) {
            this->tilingKey = BFLOAT16_MINI_SHAPE_TILING_KEY; // bfloat16, mini shape
            this->divideUbNum = CONST_VALUE_6;
        } else if (this->padLeft == 0 && this->padRight == 0 && (this->padTop != 0 || this->padBottom != 0) &&
                   this->height > SMALL_H_LIMIT) {
            this->tilingKey = BFLOAT16_NO_W_PAD_TILING_KEY; // w dim no pad
            this->divideUbNum = CONST_VALUE_12;
        } else if (this->padTop == 0 && this->padBottom == 0 && (this->padLeft != 0 || this->padRight != 0) &&
                   this->width > SMALL_W_LIMIT) {
            this->tilingKey = BFLOAT16_NO_H_PAD_TILING_KEY; // h dim no pad
            this->divideUbNum = CONST_VALUE_6;
        } else if (this->height <= SMALL_H_LIMIT && this->width > BFLOAT16_MINI_SHAPE_MAX_WIDTH) {
            ...
            this->tilingKey = BFLOAT16_H_W_PAD_TILING_KEY; // bfloat16, big shape, impl h_dim and w_dim pad
            this->divideUbNum = CONST_VALUE_8;
        }
    }
}
...
void PadV3GradV2Tiling<TilingData, dataTypeLen>::SplitUb()
{
    ...
    if (this->tilingKey == FLOAT_H_W_PAD_TILING_KEY || this->tilingKey == FLOAT16_H_W_PAD_TILING_KEY ||
        this->tilingKey == BFLOAT16_H_W_PAD_TILING_KEY) {
        this->ubFactorElement = this->FloorAlign(
            this->FloorAlign(canUseUbSize / this->divideUbNum / TRANSPOSE_LINES, BYTE_BLOCK) / dataTypeLen, ALIGN_16);
        ...
        this->ubFactorElement = this->FloorAlign(
            this->FloorAlign(canUseUbSize / this->divideUbNum / MINI_SHAPE_MAX_HEIGHT, BYTE_BLOCK) / dataTypeLen,
            ALIGN_16);
...
void PadV3GradV2Tiling<TilingData, dataTypeLen>::FillTilingData(TilingData* tilingData)
{
    ...
    if (this->tilingKey == FLOAT_NO_W_PAD_TILING_KEY || this->tilingKey == FLOAT16_NO_W_PAD_TILING_KEY ||
        this->tilingKey == BFLOAT16_NO_W_PAD_TILING_KEY || this->tilingKey == FLOAT_MINI_SHAPE_TILING_KEY ||
        this->tilingKey == FLOAT16_MINI_SHAPE_TILING_KEY || this->tilingKey == BFLOAT16_MINI_SHAPE_TILING_KEY) {
        ...
        wPadCopyCount = this->CeilAlign(CONST_VALUE_2 * std::max(this->padLeft, this->padRight) + 1,
                                        this->elementsPerBlock);
        int32_t partWs = this->padLeft < W_PAD_LOWER_LIMIT && this->padRight < W_PAD_LOWER_LIMIT ? CAL_COUNT :
                                                                                                   wPadCopyCount;
        this->workspacePerCore = CONST_VALUE_2 * partWs * this->dataTypeSize;
    } else {
        this->workspacePerCore = this->alignWidth * WORK_SPACE_PART * this->dataTypeSize;
    }
                            ...
                            const size_t usrWorkspace)
{
    ...
    OP_LOGD(tilingContext->GetNodeName(), "padTop is %u.", tilingData.get_padTop());
    OP_LOGD(tilingContext->GetNodeName(), "padBottom is %u.", tilingData.get_padBottom());
    OP_LOGD(tilingContext->GetNodeName(), "padLeft is %u.", tilingData.get_padLeft());
    OP_LOGD(tilingContext->GetNodeName(), "padRight is %u.", tilingData.get_padRight());
